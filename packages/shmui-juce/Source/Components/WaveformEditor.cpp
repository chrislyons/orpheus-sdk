...
void WaveformEditor::removeCueMarker(const juce::String& id)
{
    ...
    m_cueMarkers.erase(
        std::remove_if(m_cueMarkers.begin(), m_cueMarkers.end(),
...
juce::Colour WaveformEditor::cueColour(const CueMarker& marker) const
{
    ...
    switch (marker.type)
    {
        case CueType::Hook:  return tokens::lab::tone();
        case CueType::Drop:  return tokens::lab::danger();
        case CueType::Outro: return tokens::lab::warning();
        case CueType::Custom:
        default:             return tokens::wave::line();
    }
}
    ...
    for (const auto& marker : m_cueMarkers)
    {
        ...
        if (marker.label.isNotEmpty())
        {
            ...
            g.drawText(marker.label,
                       juce::Rectangle<float>(mx + 10.0f, bounds.getY(), 80.0f, 12.0f),
...
void WaveformEditor::drawAuditionRegion(juce::Graphics& g, juce::Rectangle<float> bounds)
{
    ...
    juce::Rectangle<float> region(x1, bounds.getY(),
                                  juce::jmax(1.0f, x2 - x1), bounds.getHeight());
...
{
    if (m_followMode == FollowMode::Off || m_zoomLevel <= 1.0f
        || m_waveformData.totalSamples <= 0)
        return;
    ...
    const float playheadRatio = static_cast<float>(m_playheadPosition)
                              / static_cast<float>(m_waveformData.totalSamples);
