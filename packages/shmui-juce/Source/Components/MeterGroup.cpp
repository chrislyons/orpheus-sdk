...
namespace
{
    ...
    juce::Colour defaultGroupColour(int index)
    {
        switch (index % 4)
        {
            case 0:  return tokens::group::blue();
            case 1:  return tokens::group::green();
            case 2:  return tokens::group::orange();
            default: return tokens::group::red();
        }
    }
}
...
void MeterGroup::paint(juce::Graphics& g)
{
    ...
    for (int i = 0; i < getNumGroups(); ++i)
    {
        juce::Rectangle<int> lb(juce::roundToInt(x), labelRow.getY(),
                                juce::roundToInt(stripW), labelRow.getHeight());
...
void MeterGroup::drawLabel(juce::Graphics& g, juce::Rectangle<int> area,
...
void MeterGroup::resized()
{
    ...
    const int gaps = (getNumGroups() > 0 ? getNumGroups() - 1 : 0);
    const float extraMasterGap = m_master ? (MASTER_GAP - STRIP_GAP) : 0.0f;
    const float usable = static_cast<float>(area.getWidth())
                         - STRIP_GAP * static_cast<float>(gaps) - extraMasterGap;
    const float stripW = usable / static_cast<float>(total);
    ...
    for (auto& s : m_groups)
    {
        s.meter->setBounds(juce::roundToInt(x), area.getY(),
                           juce::roundToInt(stripW), area.getHeight());
        x += stripW + STRIP_GAP;
    }
    if (m_master)
    {
        x += (MASTER_GAP - STRIP_GAP);
        m_master->setBounds(juce::roundToInt(x), area.getY(),
                            juce::roundToInt(stripW), area.getHeight());
    }
}
