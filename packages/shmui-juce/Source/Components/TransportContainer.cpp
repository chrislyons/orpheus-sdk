...
std::vector<TransportContainer::Item>& TransportContainer::regionItems(Region region)
{
    switch (region)
    {
        case Region::Left:   return m_left;
        case Region::Center: return m_center;
        case Region::Right:  return m_right;
    }
    return m_left;
}
...
void TransportContainer::layoutRegion(juce::FlexBox& box, std::vector<Item>& items)
{
    for (auto& item : items)
    {
        ...
        if (item.component != nullptr)
        {
            ...
            const int w = (item.flexWidth >= 0) ? item.flexWidth
        ...
        fi = fi.withMargin(juce::FlexItem::Margin(0.0f, m_style.buttonSpacing * 0.5f,
                                                  0.0f, m_style.buttonSpacing * 0.5f));
        box.items.add(fi);
    }
}
