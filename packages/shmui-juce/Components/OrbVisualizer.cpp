...
void OrbVisualizer::newOpenGLContextCreated() {
  ...
  const GLfloat vertices[] = {-1.0f, -1.0f, 0.0f, 1.0f, 1.0f, -1.0f, 0.0f, 1.0f,
  ...
  openGLContext.extensions.glBufferData(GL_ARRAY_BUFFER, sizeof(vertices), vertices,
                                        GL_STATIC_DRAW);
  ...
  openGLContext.extensions.glBufferData(GL_ARRAY_BUFFER, sizeof(texCoords), texCoords,
                                        GL_STATIC_DRAW);
}
...
void OrbVisualizer::renderOpenGL() {
  ...
  // Set colors
  shader->setUniform("uColor1", currentColor1.getFloatRed(), currentColor1.getFloatGreen(),
                     currentColor1.getFloatBlue());
  shader->setUniform("uColor2", currentColor2.getFloatRed(), currentColor2.getFloatGreen(),
                     currentColor2.getFloatBlue());

  // Set offsets array - use glGetUniformLocation directly for array uniforms
  GLint offsetsLocation =
      openGLContext.extensions.glGetUniformLocation(shader->getProgramID(), "uOffsets");
...
void OrbVisualizer::createNoiseTexture() {
  ...
  for (int y = 0; y < size; ++y) {
    for (int x = 0; x < size; ++x) {
      ...
      for (int octave = 0; octave < 4; ++octave) {
        ...
        const float noise =
            Interpolation::seededRandom(static_cast<float>(ix * 1000 + iy + octave * 10000));
  ...
  glBindTexture(GL_TEXTURE_2D, noiseTexture);
  glTexImage2D(GL_TEXTURE_2D, 0, GL_LUMINANCE, size, size, 0, GL_LUMINANCE, GL_UNSIGNED_BYTE,
               data.data());
