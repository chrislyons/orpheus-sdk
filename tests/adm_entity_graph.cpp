...
TEST(AdmEntityGraphTest, BuildsGraphAndAppliesThinning) {
  ...
  auto &programme = graph.add_programme(
      {"APR_0001", "Immersive Programme", adm::EntityKind::kProgramme});
  auto &content =
      graph.add_content({"ACO_0001", "Narrative", adm::EntityKind::kContent});
  graph.link_programme_to_content(programme, content);

  auto &bed =
      graph.add_bed({"AB_0001", "Main 5.1", adm::EntityKind::kBed});
  ...
  auto &object =
      graph.add_object({"AO_0001", "Spot FX", adm::EntityKind::kObject});
  ...
  const std::string json_dump =
      graph.DebugDumpJson(adm::ThinningPolicy::kEnabled);
  ...
  const auto &programmes =
      json::ExpectArray(programmes_it->second, "programmes");
  ASSERT_EQ(programmes.array.size(), 1u);
  const auto &programme_json =
      json::ExpectObject(programmes.array[0], "programme element");
  const auto *envelope_field =
      json::RequireField(programme_json, "envelope");
  const auto &envelope_json =
      json::ExpectObject(*envelope_field, "programme envelope");
  ...
  const auto &object_json =
      json::ExpectObject(objects.array[0], "object element");
  const auto *trajectory_field =
      json::RequireField(object_json, "trajectory");
  const auto &trajectory_json =
      json::ExpectArray(*trajectory_field, "trajectory");
  ASSERT_EQ(trajectory_json.array.size(), 3u);
  const auto *time_field =
      json::RequireField(trajectory_json.array[1], "time");
