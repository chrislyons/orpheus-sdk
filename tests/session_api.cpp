...
public:
  SessionHandle() {
    ...
    const auto* abi = orpheus_session_abi_v1(ORPHEUS_ABI_MAJOR, &got_major, &got_minor);
  ...
  [[nodiscard]] orpheus_session_handle get() const {
    return handle_;
  }
  [[nodiscard]] const orpheus_session_api_v1* abi() const {
    return abi_;
  }
...
TEST(SessionApiTest, CanCreateSessionAndManipulateTempo) {
  ...
  ASSERT_EQ(abi->get_transport_state(session.get(), &state), ORPHEUS_STATUS_OK);
...
TEST(SessionApiTest, ClipgridOperationsSucceed) {
  ...
  ASSERT_EQ(session_abi->add_track(session.get(), &track_desc, &track), ORPHEUS_STATUS_OK);
  ...
  ASSERT_EQ(clipgrid->add_clip(session.get(), track, &clip_desc, &clip), ORPHEUS_STATUS_OK);
  ASSERT_NE(clip, nullptr);

  EXPECT_EQ(clipgrid->set_clip_start(session.get(), clip, 1.0), ORPHEUS_STATUS_OK);
  EXPECT_EQ(clipgrid->set_clip_length(session.get(), clip, 2.0), ORPHEUS_STATUS_OK);
