...
TEST(SessionGraphOwnershipTest, TracksReturnConstReferences) {
  ...
  using MarkerSetsReturn =
      decltype(std::declval<const SessionGraph &>().marker_sets());
  using PlaylistLanesReturn =
      decltype(std::declval<const SessionGraph &>().playlist_lanes());
  ...
  static_assert(std::is_same_v<MarkerSetsReturn,
                               const std::vector<std::unique_ptr<MarkerSet>> &>);
  static_assert(std::is_same_v<PlaylistLanesReturn,
                               const std::vector<std::unique_ptr<PlaylistLane>> &>);
  static_assert(std::is_same_v<MarkersReturn,
                               const std::vector<MarkerSet::Marker> &>);
...
TEST(SessionGraphOwnershipTest, MarkerSetsAndPlaylistLanesAccessible) {
  ...
  EXPECT_EQ(std::distance(marker_set->markers_begin(), marker_set->markers_end()),
            1);
  ...
  EXPECT_EQ(
      std::distance(graph.playlist_lanes_begin(), graph.playlist_lanes_end()), 1);
}
