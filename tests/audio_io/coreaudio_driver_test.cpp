...
public:
    ...
    size_t getLastNumChannels() const { return m_last_num_channels; }
    size_t getLastNumFrames() const { return m_last_num_frames; }
    ...
    double getMeasuredSampleRate() const {
        auto duration = std::chrono::duration_cast<std::chrono::microseconds>(
            m_last_callback_time - m_start_time).count();
...
TEST_F(CoreAudioDriverTest, LatencyUnder10ms) {
    ...
    EXPECT_LT(latency, max_latency_samples)
        << "Latency " << latency << " samples (" << (latency * 1000.0 / 48000) << "ms) exceeds 30ms limit";
