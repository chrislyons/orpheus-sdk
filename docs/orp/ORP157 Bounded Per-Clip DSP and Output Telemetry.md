...
first stereo pair and leaves mono/additional lanes unchanged. Dynamics link the
configured source channels so image balance does not wander independently.
...
`RealtimeTelemetrySnapshot` schema 2 carried `output_count` plus one `AudioMeter`
per configured physical output lane. Transport published the routing matrix's
post-master, post-protection lane meters in the same coherent snapshot as group
and master meters. Unconfigured and out-of-range lanes remained silent.
...
## 7. ORP255 supersession

This record is historical. Its schema-2 physical-output telemetry description
in §4 is superseded by ORP255 for schema 3. ORP255 defines routing-output
identity separately from device-physical mapping, adds canonical aggregate and
logical-group-output domains, and retains the schema-2 fields only as legacy
compatibility fields in their original order.
