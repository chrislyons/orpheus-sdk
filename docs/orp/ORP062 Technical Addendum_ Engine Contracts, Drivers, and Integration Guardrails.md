...
sn_filename: "ORP062 Technical Addendum_ Engine Contracts, Drivers, and Integration Guardrails-2c3e5fdf.txt"
prefix: orp
...
Commands are JSON-serializable objects validated via Zod schemas at runtime.
 ...
 Loads a session JSON file or object 

 `{ path?: string, data?: object }` 

 `SaveSession` 

 Serializes session to file or string 
...
Engines MUST emit structured events with timestamps and IDs.
 ...
 Emits position and beat info 

 ≤ 30 Hz 

 `RenderProgress` 

 Percentage updates for renders 
...
Errors follow a hierarchical naming scheme.
 ...
 "Session JSON malformed at track index 4" 

 Render 

 `OutOfRange` 

 "Render duration exceeds buffer limit" 

 Transport 

 `InvalidPosition` 

 "Transport set before session loaded" 

 System 

 `CrashRecovered` 

 "Engine restarted after fault" 
...
Maintain Architecture Decision Records:
 ...
 Home Repo & UI Codename 

 Establish `orpheus-sdk` canonical repo 

 002 

 Unified Engine Boundary 

 Define cross-driver contract 

 003 

 Driver Priority & Fallback 

 Specify broker selection logic 

 004 

 Determinism Guarantees 

 Formalize golden-audio testing 
