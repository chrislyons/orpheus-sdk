...
`TransportController` retains a control-allocated `SourceCommandLifetime` for each registered source. Its `unread` count pins raw source pointers carried by Start or Seek commands, while `active_voices` pins the registry entry for every admitted voice. Seek preparation checks ring capacity before reader work; primes `4 * maxBlockFrames + 2` source frames; uses `max(requestedPosition, trimIn)` for a non-segment target below trim-IN because that is the renderer's first source read; retains loop-wrap and segment-program extensions; and publishes one existing Seek command. Start and Seek page reservations remain through every source read in the consuming callback and then release. Admission retains an active voice pin before same-handle replacement releases the old pin, so replacement is monotonic and unregister cannot observe a transient zero. Natural end, stop, panic, replacement, compaction, and teardown all release the active pin exactly once. Unread lifetimes, active voices, and pending page primes block unregistration/replacement. Destruction stops the stream worker, releases unread command leases and retained render-block reservations, removes active voices, then destroys the registry.
