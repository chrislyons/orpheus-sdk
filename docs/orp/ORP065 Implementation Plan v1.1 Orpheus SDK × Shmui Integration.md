...
sn_filename: "ORP065 Implementation Plan v1_1 Orpheus SDK × Shmui Integration-e477ef78.txt"
prefix: orp
...
### P0.REPO.003: Configure Package Namespacing (AMENDED ✅)



**Description:**

Set the UI package name to `**@orpheus/shmui**` (preserving the *Shmui* codename) and establish naming conventions for all Orpheus packages, including the native driver name.
...
- Internal codename **“Shmui”** explicitly preserved in docs with a one-line explanation (e.g., “Shmui = Orpheus UI layer”)



**ORP References:**

ORP061 §Phase 0 (Namespacing & Package Names), ORP063 §III.D



**Tooling Requirements:**

NPM account with `@orpheus` scope access (only if publishing)
...
```
jq '.name' packages/shmui/package.json         # => "@orpheus/shmui"
# If native driver pkg exists now or later:
jq '.name' packages/engine-native/package.json # => "@orpheus/engine-native"

# Quick doc check (grep for codename note)
grep -i "Shmui" docs/PACKAGE_NAMING.md
```
...
### P1.DRIV.004 — Create Native Driver Package (AMENDED ✅)



**Description:**

Initialize `**@orpheus/engine-native**`, an N-API addon package providing Orpheus engine bindings usable by both **Node.js** and **Electron** environments. Establish a minimal proof-of-concept build exporting a version function and verify cross-runtime compatibility.
...
- `CMakeLists.txt` configured for N-API addon build
- Minimal binding exports a working `getVersion()` function
- Builds successfully on **Ubuntu** (proof of concept)
- `package.json` includes `install` script for native build
- Documentation explicitly states **Node.js** and **Electron** compatibility



**ORP References:**

ORP062 §2.3, ORP063 §II.B
...
```
# Node.js test
node -e "const orpheus = require('@orpheus/engine-native'); console.log(orpheus.getVersion());"
# → Should print Orpheus version string

# (Optional) Electron test
npx electron -e "console.log(require('@orpheus/engine-native').getVersion());"
# → Should print same version string
```
...
## X. ESTIMATED TIMELINE
 ...
 Includes history preservation and bootstrap script 
 ...
 Dual driver (Native + Service) with contract automation 
 ...
 WASM, UI integration, comprehensive test suite 
 ...
 CI unification, security audit, release prep 
 ...
 Beta testing, stable release, cleanup 
...
1. **P3.CI.001 (CI Unification)**: Potential merge conflicts and workflow complexity
...
1. **P3.GOV.003 (Prebuilt Binaries)**: Platform-specific build issues, signing certificate delays
...
1. **P4.GOV.006 (Beta Testing)**: Low participation or critical bugs discovered late
...
1. **P2.TEST.005-007 (Advanced Testing)**: Time-consuming test development may delay Phase 2
...
1. **P1.CONT.004 (Contract Automation)**: Custom tooling may be complex to build correctly
...
1. **Multi-Driver Validation**: Repeat workflow using each driver type
...
1. **Cross-Platform Validation**: Workflow succeeds on all target platforms
...
1. **Performance Validation**: Operations complete within budget
...
1. **Documentation Validation**: New contributor onboards successfully
...
1. **Release Validation**: Fresh installation succeeds without issues
...
1. **Community Validation**: External adoption proves viability
...
1. **Monitoring Validation**: Telemetry provides actionable insights
...
1. **Support Validation**: Issue triage process functions smoothly
...
1. **Rollback Validation**: Emergency procedures tested
...
For integration with project management tools (Jira, Linear, etc.), sequential numbering:
 ...
 Upgrade Contract to v1.0.0-beta 
 ...
 Pre-Merge Contributor Validation 
...
## XV. AMENDMENT HISTORY
 ...
 Amendments per technical review:<br>• Added 20 new tasks (governance, testing, security)<br>• Updated finish-line criteria (+5 items)<br>• Revised timeline estimates (+2-3 weeks)<br>• Enhanced risk mitigation strategies<br>• Added sequential task numbering appendix 
...
- **Lead Architect**: ***___*** Date: ***_***
- **Engineering Manager**: ***__***** Date: _**____
- **Product Owner**: ***___***_ Date: ***_***
- **QA Lead**: ***______ Date: _***___
...
### Critical Milestones
 ...
 M1: Repo Setup Complete 

 Week 2 

 P0.TEST.001 passes 

 M2: Basic Integration 

 Week 6 

 P1.TEST.001 passes, two drivers functional 

 M3: WASM Integrated 

 Week 9 

 All three drivers functional 

 M4: Feature Complete 

 Week 12 

 P2.TEST.004 passes, UI features demonstrated 

 M5: Release Ready 

 Week 16 

 P3.TEST.004 passes, all quality gates met 
 ...
 P4.TEST.001 passes, all finish-line criteria met 
