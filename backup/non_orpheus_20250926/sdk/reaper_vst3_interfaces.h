...
public:
  ...
  virtual void* PLUGIN_API getReaperParent(
      uint32 w) = 0; // get parent track(=1), take(=2), project(=3), fxdsp(=4), trackchan(=5)
...
class IReaperUIEmbedInterface
    : public FUnknown // supported by REAPER v6.24+, queried from plug-in IEditController
{
public:
  // note: VST2 uses CanDo "hasCockosEmbeddedUI"==0xbeef0000, then opcode=effVendorSpecific,
  // index=effEditDraw, opt=(float)msg, value=parm2, ptr=parm3 see reaper_plugin_fx_embed.h
  virtual Steinberg::TPtrInt embed_message(int msg, Steinberg::TPtrInt parm2,
