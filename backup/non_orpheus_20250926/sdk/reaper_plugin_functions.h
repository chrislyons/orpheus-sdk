...
//   of REAPER may not succeed in loading, and also it may bloat your plug-in. If you wish to only
//   load needed functions, #define REAPERAPI_MINIMAL and various #define
//   REAPERAPI_WANT_<functionname> lines before including this file. You must put these definitions
//   where REAPERAPI_IMPLEMENT is defined and you can optionally put them elsewhere (to detect
//   needed REAPERAPI_WANT_xxx lines at compile- time rather than link-time).
//
...
REAPERAPI_DEF //==============================================
    ...
    // HOWEVER, it requires some temporary space, equal to the size of the data being sorted, so you
    // can pass it as the last parameter, or NULL and it will allocate and free space internally.

    void (*REAPERAPI_FUNCNAME(__mergesort))(void* base, size_t nmemb, size_t size,
...
REAPERAPI_DEF //==============================================
    ...
    // kbdsecname is the name of the KbdSectionInfo registered by this plugin, or NULL for the main
    // actions section

    bool (*REAPERAPI_FUNCNAME(AddCustomizableMenu))(const char* menuidstr, const char* menuname,
...
REAPERAPI_DEF //==============================================
              // AddProjectMarker
    // Returns the index of the created marker/region, or -1 on failure. Supply wantidx>=0 if you
    // want a particular index number, but you'll get a different index number a region and wantidx
    // is already in use.

    int (*REAPERAPI_FUNCNAME(AddProjectMarker))(ReaProject* proj, bool isrgn, double pos,
...
REAPERAPI_DEF //==============================================
    ...
    // Returns the index of the created marker/region, or -1 on failure. Supply wantidx>=0 if you
    // want a particular index number, but you'll get a different index number a region and wantidx
    // is already in use. color should be 0 (default color), or ColorToNative(r,g,b)|0x1000000

    int (*REAPERAPI_FUNCNAME(AddProjectMarker2))(ReaProject* proj, bool isrgn, double pos,
                                                 double rgnend, const char* name, int wantidx,
                                                 int color);
#endif
...
REAPERAPI_DEF //==============================================
              // AddRemoveReaScript
    // Add a ReaScript (return the new command ID, or 0 if failed) or remove a ReaScript (return >0
    // on success). Use commit==true when adding/removing a single script. When bulk adding/removing
    // n scripts, you can optimize the n-1 first calls with commit==false and commit==true for the
    // last call.

    int (*REAPERAPI_FUNCNAME(AddRemoveReaScript))(bool add, int sectionID, const char* scriptfn,
                                                  bool commit);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    bool (*REAPERAPI_FUNCNAME(AddTempoTimeSigMarker))(ReaProject* proj, double timepos, double bpm,
...
REAPERAPI_DEF //==============================================
              // AddTrackMarker
    // Returns the index of the created track marker/region, or -1 on failure. lane specifies the
    // marker lane.

    int (*REAPERAPI_FUNCNAME(AddTrackMarker))(MediaTrack* track, int lane, bool isrgn, double pos,
                                              double rgnend, const char* name, int wantidx);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // Steps through audio blocks for project. opos is start of current block in seconds, *npos
    // should be set to start of estimated next block, will be updated with new start. loopcnt
    // should be set to GetPlayLoopCnt initially, will be updated if discontinuity. max_spls may be
    // updated to shorter block length if discontinuity. sflags should be 0 initially and is the
    // running state used internally. returns 1 if looped sel, 2 if looped project, 4 if
    // loopendskip, 8 if smoothseek, 16 if fade audition (all during this block).  Typically called
    ...
    //     int thislen = len;
    //     int ret = AdvancePlaybackPosition(proj, old_pos, &nextpos, &lc, srate, &thislen,
    //     &asflags); oldpos = nextpos;
    //   goto loop
    //

    int (*REAPERAPI_FUNCNAME(AdvancePlaybackPosition))(ReaProject* proj, double opos, double* npos,
                                                       WDL_INT64* loopcnt, double srate,
                                                       int* max_spls, int* sflags);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    bool (*REAPERAPI_FUNCNAME(ARA_AttachProcessorToMediaItem))(MediaItem* item,
...
REAPERAPI_DEF //==============================================
    ...
    // nudgewhat: 0=position, 1=left trim, 2=left edge, 3=right edge, 4=contents, 5=duplicate,
    // 6=edit cursor nudgeunit: 0=ms, 1=seconds, 2=grid, 3=256th notes, ..., 15=whole notes,
    // 16=measures.beats (1.15 = 1 measure + 1.5 beats), 17=samples, 18=frames, 19=pixels, 20=item
    // lengths, 21=item selections value: amount to nudge by, or value to set to reverse: in nudge
    // mode, nudges left (otherwise ignored) copies: in nudge duplicate mode, number of copies
    // (otherwise ignored)

    bool (*REAPERAPI_FUNCNAME(ApplyNudge))(ReaProject* project, int nudgeflag, int nudgewhat,
...
REAPERAPI_DEF //==============================================
    ...
    // Returns true if the underlying samples (track or media item take) have changed, but does not
    // update the audio accessor, so the user can selectively call AudioAccessorValidateState only
    // when needed. See CreateTakeAudioAccessor, CreateTrackAudioAccessor, DestroyAudioAccessor,
...
REAPERAPI_DEF //==============================================
    ...
    // Validates the current state of the audio accessor -- must ONLY call this from the main
    // thread. Returns true if the state changed.
...
REAPERAPI_DEF //==============================================
    ...
    // Calculates loudness statistics of media via dry run render. Statistics will be displayed to
    // the user; call GetSetProjectInfo_String("RENDER_STATS") to retrieve via API. Returns 1 if
    // loudness was calculated successfully, -1 if user canceled the dry run render.
...
REAPERAPI_DEF //==============================================
    ...
    // Calculate normalize adjustment for source media. normalizeTo: 0=LUFS-I, 1=RMS-I, 2=peak,
    // 3=true peak, 4=LUFS-M max, 5=LUFS-S max. normalizeTarget: dBFS or LUFS value. normalizeStart,
    // normalizeEnd: time bounds within source media for normalization calculation. If
    // normalizationStart=0 and normalizationEnd=0, the full duration of the media will be used for
    // the calculation.

    double (*REAPERAPI_FUNCNAME(CalculateNormalization))(PCM_source* source, int normalizeTo,
...
REAPERAPI_DEF //==============================================
              // ColorToNative
    // Make an OS dependent color from RGB values (e.g. RGB() macro on Windows). r,g and b are in
    // [0..255]. See ColorFromNative.
...
REAPERAPI_DEF //==============================================
    ...
    // etc. For automation items, pass autoitem_idx|0x10000000 to base ptidx on the number of points
    // in one full loop iteration, even if the automation item is trimmed so that not all points are
    // visible. Otherwise, ptidx will be based on the number of visible points in the automation
    // item, including all loop iterations. See GetEnvelopePointEx, SetEnvelopePointEx,
...
REAPERAPI_DEF //==============================================
    ...
    int (*REAPERAPI_FUNCNAME(CountProjectMarkers))(ReaProject* proj, int* num_markersOut,
                                                   int* num_regionsOut);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // Discouraged, because GetSelectedMediaItem can be inefficient if media items are added,
    // rearranged, or deleted in between calls. Instead see CountMediaItems, GetMediaItem,
    // IsMediaItemSelected.
...
REAPERAPI_DEF //==============================================
              // CountSelectedTracks
    // Count the number of selected tracks in the project (proj=0 for active project). This function
    // ignores the master track, see CountSelectedTracks2.
...
REAPERAPI_DEF //==============================================
    ...
    // SetTempoTimeSigMarker, AddTempoTimeSigMarker.
...
REAPERAPI_DEF //==============================================
    ...
    // callback is a function pointer: void (*callback)(void* obj, const char* msg, int msglen),
    // which handles OSC messages sent from REAPER. The function return is a local osc handler. See
    // SendLocalOscMessage, DestroyOscHandler.
...
REAPERAPI_DEF //==============================================
              // CreateMIDIInput
    // Can only reliably create midi access for devices not already opened in prefs/MIDI, suitable
    // for control surfaces etc.
...
REAPERAPI_DEF //==============================================
              // CreateMIDIOutput
    // Can only reliably create midi access for devices not already opened in prefs/MIDI, suitable
    // for control surfaces etc. If streamMode is set, msoffset100 points to a persistent variable
    // that can change and reflects added delay to output in 100ths of a millisecond.

    midi_Output* (*REAPERAPI_FUNCNAME(CreateMIDIOutput))(int dev, bool streamMode,
                                                         int* msoffset100);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    MediaItem* (*REAPERAPI_FUNCNAME(CreateNewMIDIItemInProj))(MediaTrack* track, double starttime,
                                                              double endtime,
...
REAPERAPI_DEF //==============================================
    ...
    // CreateTrackAudioAccessor, DestroyAudioAccessor, AudioAccessorStateChanged,
...
REAPERAPI_DEF //==============================================
    ...
    // CreateTakeAudioAccessor, DestroyAudioAccessor, AudioAccessorStateChanged,
    // GetAudioAccessorStartTime, GetAudioAccessorEndTime, GetAudioAccessorSamples.
...
REAPERAPI_DEF //==============================================
              // CreateTrackSend
    // Create a send/receive (desttrInOptional!=NULL), or a hardware output (desttrInOptional==NULL)
    // with default properties, return >=0 on success (== new send/receive index). See
    // RemoveTrackSend, GetSetTrackSendInfo, GetTrackSendInfo_Value, SetTrackSendInfo_Value.
...
REAPERAPI_DEF //==============================================
    ...
    int (*REAPERAPI_FUNCNAME(CSurf_OnInputMonitorChangeEx))(MediaTrack* trackid, int monitor,
                                                            bool allowgang);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    void (*REAPERAPI_FUNCNAME(CSurf_OnOscControlMessage2))(const char* msg, const float* arg,
                                                           const char* argstr);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    double (*REAPERAPI_FUNCNAME(CSurf_OnPanChangeEx))(MediaTrack* trackid, double pan,
...
REAPERAPI_DEF //==============================================
    ...
    bool (*REAPERAPI_FUNCNAME(CSurf_OnRecArmChangeEx))(MediaTrack* trackid, int recarm,
                                                       bool allowgang);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    double (*REAPERAPI_FUNCNAME(CSurf_OnRecvPanChange))(MediaTrack* trackid, int recv_index,
...
REAPERAPI_DEF //==============================================
    ...
    double (*REAPERAPI_FUNCNAME(CSurf_OnRecvVolumeChange))(MediaTrack* trackid, int recv_index,
...
REAPERAPI_DEF //==============================================
    ...
    double (*REAPERAPI_FUNCNAME(CSurf_OnSendPanChange))(MediaTrack* trackid, int send_index,
                                                        double pan, bool relative);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    double (*REAPERAPI_FUNCNAME(CSurf_OnSendVolumeChange))(MediaTrack* trackid, int send_index,
                                                           double volume, bool relative);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    double (*REAPERAPI_FUNCNAME(CSurf_OnVolumeChange))(MediaTrack* trackid, double volume,
                                                       bool relative);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    double (*REAPERAPI_FUNCNAME(CSurf_OnVolumeChangeEx))(MediaTrack* trackid, double volume,
                                                         bool relative, bool allowGang);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    double (*REAPERAPI_FUNCNAME(CSurf_OnWidthChange))(MediaTrack* trackid, double width,
                                                      bool relative);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    double (*REAPERAPI_FUNCNAME(CSurf_OnWidthChangeEx))(MediaTrack* trackid, double width,
                                                        bool relative, bool allowGang);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    void (*REAPERAPI_FUNCNAME(CSurf_SetPlayState))(bool play, bool pause, bool rec,
...
REAPERAPI_DEF //==============================================
    ...
    void (*REAPERAPI_FUNCNAME(CSurf_SetSurfaceMute))(MediaTrack* trackid, bool mute,
                                                     IReaperControlSurface* ignoresurf);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    void (*REAPERAPI_FUNCNAME(CSurf_SetSurfacePan))(MediaTrack* trackid, double pan,
                                                    IReaperControlSurface* ignoresurf);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    void (*REAPERAPI_FUNCNAME(CSurf_SetSurfaceRecArm))(MediaTrack* trackid, bool recarm,
                                                       IReaperControlSurface* ignoresurf);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    void (*REAPERAPI_FUNCNAME(CSurf_SetSurfaceSolo))(MediaTrack* trackid, bool solo,
                                                     IReaperControlSurface* ignoresurf);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    void (*REAPERAPI_FUNCNAME(CSurf_SetSurfaceVolume))(MediaTrack* trackid, double volume,
                                                       IReaperControlSurface* ignoresurf);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    bool (*REAPERAPI_FUNCNAME(DeleteActionShortcut))(KbdSectionInfo* section, int cmdID,
                                                     int shortcutidx);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // Delete an envelope point. If setting multiple points at once, set noSort=true, and call
    // Envelope_SortPoints when done. autoitem_idx=-1 for the underlying envelope, 0 for the first
    // automation item on the envelope, etc. For automation items, pass autoitem_idx|0x10000000 to
    // base ptidx on the number of points in one full loop iteration, even if the automation item is
    // trimmed so that not all points are visible. Otherwise, ptidx will be based on the number of
    // visible points in the automation item, including all loop iterations. See
    // CountEnvelopePointsEx, GetEnvelopePointEx, SetEnvelopePointEx, InsertEnvelopePointEx.

    bool (*REAPERAPI_FUNCNAME(DeleteEnvelopePointEx))(TrackEnvelope* envelope, int autoitem_idx,
                                                      int ptidx);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    bool (*REAPERAPI_FUNCNAME(DeleteEnvelopePointRange))(TrackEnvelope* envelope, double time_start,
                                                         double time_end);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // Delete a range of envelope points. autoitem_idx=-1 for the underlying envelope, 0 for the
    // first automation item on the envelope, etc.
...
REAPERAPI_DEF //==============================================
              // DeleteExtState
    // Delete the extended state value for a specific section and key. persist=true means the value
    // should remain deleted the next time REAPER is opened. See SetExtState, GetExtState,
    // HasExtState.
...
REAPERAPI_DEF //==============================================
    ...
    bool (*REAPERAPI_FUNCNAME(DeleteProjectMarker))(ReaProject* proj, int markrgnindexnumber,
                                                    bool isrgn);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // Differs from DeleteProjectMarker only in that markrgnidx is 0 for the first marker/region, 1
    // for the next, etc (see EnumProjectMarkers3), rather than representing the displayed
...
REAPERAPI_DEF //==============================================
              ...
              // GetNumTakeMarkers, GetTakeMarker, SetTakeMarker
...
REAPERAPI_DEF //==============================================
    ...
    int (*REAPERAPI_FUNCNAME(DeleteTakeStretchMarkers))(MediaItem_Take* take, int idx,
...
REAPERAPI_DEF //==============================================
    ...
    bool (*REAPERAPI_FUNCNAME(DeleteTrackMarker))(MediaTrack* track, int lane,
                                                  int markrgnindexnumber, bool isrgn);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // CreateTrackAudioAccessor, AudioAccessorStateChanged, GetAudioAccessorStartTime,
    // GetAudioAccessorEndTime, GetAudioAccessorSamples.
...
REAPERAPI_DEF //==============================================
    ...
    // Open the action shortcut dialog to edit or add a shortcut for the given command ID. If
    // (shortcutidx >= 0 && shortcutidx < CountActionShortcuts()), that specific shortcut will be
    // replaced, otherwise a new shortcut will be added. See CountActionShortcuts,
    // GetActionShortcutDesc, DeleteActionShortcut.

    bool (*REAPERAPI_FUNCNAME(DoActionShortcutDialog))(HWND hwnd, KbdSectionInfo* section,
...
REAPERAPI_DEF //==============================================
    ...
    void (*REAPERAPI_FUNCNAME(DockWindowAddEx))(HWND hwnd, const char* name, const char* identstr,
                                                bool allowShow);
#endif
...
REAPERAPI_DEF //==============================================
              // EnumerateFiles
    // List the files in the "path" directory. Returns NULL/nil when all files have been listed. Use
    // fileindex = -1 to force re-read of directory (invalidate cache). See EnumerateSubdirectories
...
REAPERAPI_DEF //==============================================
    ...
    // List the subdirectories in the "path" directory. Use subdirindex = -1 to force re-read of
    // directory (invalidate cache). Returns NULL/nil when all subdirectories have been listed. See
    // EnumerateFiles
...
REAPERAPI_DEF //==============================================
              // EnumInstalledFX
    // Enumerates installed FX. Returns true if successful, sets nameOut and identOut to name and
    // ident of FX at index. In REAPER 7.42+, use index=-1 to re-read JSFX info.

    bool (*REAPERAPI_FUNCNAME(EnumInstalledFX))(int index, const char** nameOut,
                                                const char** identOut);
#endif
...
REAPERAPI_DEF //==============================================
              // EnumPitchShiftModes
    // Start querying modes at 0, returns FALSE when no more modes possible, sets strOut to NULL if
    // a mode is currently unsupported
...
REAPERAPI_DEF //==============================================
    ...
    int (*REAPERAPI_FUNCNAME(EnumProjectMarkers))(int idx, bool* isrgnOut, double* posOut,
...
REAPERAPI_DEF //==============================================
    ...
    int (*REAPERAPI_FUNCNAME(EnumProjectMarkers2))(ReaProject* proj, int idx, bool* isrgnOut,
...
REAPERAPI_DEF //==============================================
    ...
    int (*REAPERAPI_FUNCNAME(EnumProjectMarkers3))(ReaProject* proj, int idx, bool* isrgnOut,
                                                   double* posOut, double* rgnendOut,
...
REAPERAPI_DEF //==============================================
              // EnumProjects
    // idx=-1 for current project,projfn can be NULL if not interested in filename. use idx
    // 0x40000000 for currently rendering project, if any.

    ReaProject* (*REAPERAPI_FUNCNAME(EnumProjects))(int idx, char* projfnOutOptional,
...
REAPERAPI_DEF //==============================================
              // EnumProjExtState
    // Enumerate the data stored with the project for a specific extname. Returns false when there
    // is no more data. See SetProjExtState, GetProjExtState.

    bool (*REAPERAPI_FUNCNAME(EnumProjExtState))(ReaProject* proj, const char* extname, int idx,
...
REAPERAPI_DEF //==============================================
    ...
    // Enumerate which tracks will be rendered within this region when using the region render
    // matrix. When called with rendertrack==0, the function returns the first track that will be
    // rendered (which may be the master track); rendertrack==1 will return the next track rendered,
    // and so on. The function returns NULL when there are no more tracks that will be rendered
    // within this region.

    MediaTrack* (*REAPERAPI_FUNCNAME(EnumRegionRenderMatrix))(ReaProject* proj, int regionindex,
                                                              int rendertrack);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // returns false if there are no plugins on the track that support MIDI programs,or if all
    // programs have been enumerated

    bool (*REAPERAPI_FUNCNAME(EnumTrackMIDIProgramNames))(int track, int programNumber,
...
REAPERAPI_DEF //==============================================
    ...
    bool (*REAPERAPI_FUNCNAME(EnumTrackMIDIProgramNamesEx))(ReaProject* proj, MediaTrack* track,
...
REAPERAPI_DEF //==============================================
    ...
    int (*REAPERAPI_FUNCNAME(EnumTrackMarkers))(MediaTrack* track, int idx, int* laneOut,
                                                bool* isrgnOut, double* posOut, double* rgnendOut,
                                                const char** nameOut, int* markrgnindexnumberOut);
#endif
...
REAPERAPI_DEF //==============================================
              // Envelope_Evaluate
    // Get the effective envelope value at a given time position. samplesRequested is how long the
    // caller expects until the next call to Envelope_Evaluate (often, the buffer block size). The
    // return value is how many samples beyond that time position that the returned values are
    // valid. dVdS is the change in value per sample (first derivative), ddVdS is the second
    // derivative, dddVdS is the third derivative. See GetEnvelopeScalingMode.

    int (*REAPERAPI_FUNCNAME(Envelope_Evaluate))(TrackEnvelope* envelope, double time,
                                                 double samplerate, int samplesRequested,
                                                 double* valueOut, double* dVdSOut,
                                                 double* ddVdSOut, double* dddVdSOut);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    void (*REAPERAPI_FUNCNAME(Envelope_FormatValue))(TrackEnvelope* env, double value, char* bufOut,
                                                     int bufOut_sz);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // If take envelope, gets the take from the envelope. If FX, indexOut set to FX index, index2Out
    // set to parameter index, otherwise -1.

    MediaItem_Take* (*REAPERAPI_FUNCNAME(Envelope_GetParentTake))(TrackEnvelope* env, int* indexOut,
                                                                  int* index2Out);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // If track envelope, gets the track from the envelope. If FX, indexOut set to FX index,
    // index2Out set to parameter index, otherwise -1.

    MediaTrack* (*REAPERAPI_FUNCNAME(Envelope_GetParentTrack))(TrackEnvelope* env, int* indexOut,
                                                               int* index2Out);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // Sort envelope points by time. autoitem_idx=-1 for the underlying envelope, 0 for the first
    // automation item on the envelope, etc. See SetEnvelopePoint, InsertEnvelopePoint.
...
REAPERAPI_DEF //==============================================
              // ExecProcess
    // Executes command line, returns NULL on total failure, otherwise the return value, a newline,
    // and then the output of the command. If timeoutmsec is 0, command will be allowed to run
    // indefinitely (recommended for large amounts of returned output). timeoutmsec is -1 for no
    // wait/terminate, -2 for no wait and minimize
...
REAPERAPI_DEF //==============================================
    ...
    // Find the tempo/time signature marker that falls at or before this time position (the marker
    // that is in effect as of this time position).
...
REAPERAPI_DEF //==============================================
    ...
    void (*REAPERAPI_FUNCNAME(format_timestr_len))(double tpos, char* buf, int buf_sz,
...
REAPERAPI_DEF //==============================================
    ...
    void (*REAPERAPI_FUNCNAME(format_timestr_pos))(double tpos, char* buf, int buf_sz,
                                                   int modeoverride);
#endif
...
REAPERAPI_DEF //==============================================
              // get_config_var
    // gets ini configuration variable by name, raw, returns size of variable in szOut and pointer
    // to variable. special values queryable are also:
    //   __numcpu (int) cpu count.
    //   __fx_loadstate_ctx (char): 0 if unknown, or during FX state loading: 'u' (instantiating via
    //   undo), 'U' (updating via undo), 'P' (loading preset). __langpack_filename can query
    //   langpack filename. If empty or <> then no langpack loaded; if relative pathname, then
    //   langpack is not yet loaded.
...
REAPERAPI_DEF //==============================================
    ...
    bool (*REAPERAPI_FUNCNAME(get_config_var_string))(const char* name, char* bufOut,
                                                      int bufOut_sz);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    bool (*REAPERAPI_FUNCNAME(GetActionShortcutDesc))(KbdSectionInfo* section, int cmdID,
                                                      int shortcutidx, char* descOut,
                                                      int descOut_sz);
#endif
...
REAPERAPI_DEF //==============================================
              // GetAppVersion
    // Returns app version which may include an OS/arch signifier, such as: "6.17" (windows 32-bit),
    // "6.17/x64" (windows 64-bit), "6.17/OSX64" (macOS 64-bit Intel), "6.17/OSX" (macOS 32-bit),
    // "6.17/macOS-arm64", "6.17/linux-x86_64", "6.17/linux-i686", "6.17/linux-aarch64",
...
REAPERAPI_DEF //==============================================
              // GetArmedCommand
    // gets the currently armed command and section name (returns 0 if nothing armed). section name
    // is empty-string for main section.
...
REAPERAPI_DEF //==============================================
              // GetAudioAccessorSamples
    // Get a block of samples from the audio accessor. Samples are extracted immediately pre-FX, and
    // returned interleaved (first sample of first channel, first sample of second channel...).
    // Returns 0 if no audio, 1 if audio, -1 on error. See CreateTakeAudioAccessor,
    // CreateTrackAudioAccessor, DestroyAudioAccessor, AudioAccessorStateChanged,
    // GetAudioAccessorStartTime, GetAudioAccessorEndTime.//
    //
    // This function has special handling in Python, and only returns two objects, the API function
    // return value, and the sample buffer. Example usage:
    //
    ...
    // # typically GetAudioAccessorSamples() would be called within a loop, increasing pos each
    // time.
    // </code>

    int (*REAPERAPI_FUNCNAME(GetAudioAccessorSamples))(AudioAccessor* accessor, int samplerate,
...
REAPERAPI_DEF //==============================================
              // GetAudioDeviceInfo
    // get information about the currently open audio device. attribute can be MODE, IDENT_IN,
    // IDENT_OUT, BSIZE, SRATE, BPS. returns false if unknown attribute or device not open.

    bool (*REAPERAPI_FUNCNAME(GetAudioDeviceInfo))(const char* attribute, char* descOut,
                                                   int descOut_sz);
#endif
...
REAPERAPI_DEF //==============================================
              // GetCursorContext
    // return the current cursor context: 0 if track panels, 1 if items, 2 if envelopes, otherwise
    // unknown
...
REAPERAPI_DEF //==============================================
    ...
    // Returns the custom take, item, or track color that is used (according to the user preference)
    // to color the media item. The returned color is OS dependent|0x01000000 (i.e.
    // ColorToNative(r,g,b)|0x01000000), so a return of zero means "no color", not black.
...
REAPERAPI_DEF //==============================================
    ...
    // I_TCPY : int : Y offset of envelope relative to parent track (may be separate lane or overlap
    // with track contents) I_TCPH : int : visible height of envelope I_TCPY_USED : int : Y offset
    // of envelope relative to parent track, exclusive of padding I_TCPH_USED : int : visible height
    // of envelope, exclusive of padding P_TRACK : MediaTrack * : parent track pointer (if any)
...
REAPERAPI_DEF //==============================================
    ...
    bool (*REAPERAPI_FUNCNAME(GetEnvelopePoint))(TrackEnvelope* envelope, int ptidx,
                                                 double* timeOut, double* valueOut, int* shapeOut,
                                                 double* tensionOut, bool* selectedOut);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    int (*REAPERAPI_FUNCNAME(GetEnvelopePointByTimeEx))(TrackEnvelope* envelope, int autoitem_idx,
                                                        double time);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // in one full loop iteration, even if the automation item is trimmed so that not all points are
    // visible. Otherwise, ptidx will be based on the number of visible points in the automation
    // item, including all loop iterations. See CountEnvelopePointsEx, SetEnvelopePointEx,
    // InsertEnvelopePointEx, DeleteEnvelopePointEx.

    bool (*REAPERAPI_FUNCNAME(GetEnvelopePointEx))(TrackEnvelope* envelope, int autoitem_idx,
                                                   int ptidx, double* timeOut, double* valueOut,
                                                   int* shapeOut, double* tensionOut,
                                                   bool* selectedOut);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // Returns the envelope scaling mode: 0=no scaling, 1=fader scaling. All API functions deal with
    // raw envelope point values, to convert raw from/to scaled values see ScaleFromEnvelopeMode,
    // ScaleToEnvelopeMode.
...
REAPERAPI_DEF //==============================================
    ...
    bool (*REAPERAPI_FUNCNAME(GetEnvelopeStateChunk))(TrackEnvelope* env, char* strNeedBig,
...
REAPERAPI_DEF //==============================================
              // GetEnvelopeUIState
    // gets information on the UI state of an envelope: returns &1 if automation/modulation is
    // playing back, &2 if automation is being actively written, &4 if the envelope recently had an
    // effective automation mode change
...
REAPERAPI_DEF //==============================================
              // GetExtState
    // Get the extended state value for a specific section and key. See SetExtState, DeleteExtState,
    // HasExtState.
...
REAPERAPI_DEF //==============================================
    ...
    int (*REAPERAPI_FUNCNAME(GetFocusedFX))(int* tracknumberOut, int* itemnumberOut,
                                            int* fxnumberOut);
#endif
...
REAPERAPI_DEF //==============================================
              // GetFocusedFX2
    // Return value has 1 set if track FX, 2 if take/item FX, 4 set if FX is no longer focused but
    // still open. tracknumber==0 means the master track, 1 means track 1, etc. itemnumber is
    // zero-based (or -1 if not an item). For interpretation of fxnumber, see GetLastTouchedFX.
    // Deprecated, see GetTouchedOrFocusedFX

    int (*REAPERAPI_FUNCNAME(GetFocusedFX2))(int* tracknumberOut, int* itemnumberOut,
                                             int* fxnumberOut);
#endif
...
REAPERAPI_DEF //==============================================
              // GetFXEnvelope
    // Returns the FX parameter envelope. If the envelope does not exist and create=true, the
    // envelope will be created. If the envelope already exists and is bypassed and create=true,
    // then the envelope will be unbypassed.

    TrackEnvelope* (*REAPERAPI_FUNCNAME(GetFXEnvelope))(MediaTrack* track, int fxindex,
...
REAPERAPI_DEF //==============================================
    ...
    // returns a named icontheme entry for a given DPI-scaling (256=1:1). Note: the return value
    // should not be stored, it should be queried at each paint! Querying name=NULL returns the
...
REAPERAPI_DEF //==============================================
    ...
    // returns approximate input level if available, 0-511 mono inputs, |1024 for stereo pairs,
    // 4096+devidx*32 for MIDI devices
...
REAPERAPI_DEF //==============================================
              // GetItemEditingTime2
    // returns time of relevant edit, set which_item to the pcm_source (if applicable), flags (if
    // specified) will be set to 1 for edge resizing, 2 for fade change, 4 for item move, 8 for item
    // slip edit (edit cursor time or start of item)
...
REAPERAPI_DEF //==============================================
              // GetItemFromPoint
    // Returns the first item at the screen coordinates specified. If allow_locked is false, locked
    // items are ignored. If takeOutOptional specified, returns the take hit. See GetThingFromPoint.

    MediaItem* (*REAPERAPI_FUNCNAME(GetItemFromPoint))(int screen_x, int screen_y,
                                                       bool allow_locked,
...
REAPERAPI_DEF //==============================================
    ...
    bool (*REAPERAPI_FUNCNAME(GetItemStateChunk))(MediaItem* item, char* strNeedBig,
                                                  int strNeedBig_sz, bool isundoOptional);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // markeridx and regionidx are returned not necessarily as the displayed marker/region index,
    // but as the index that can be passed to EnumProjectMarkers. Either or both of markeridx and
    // regionidx may be NULL. See EnumProjectMarkers.

    void (*REAPERAPI_FUNCNAME(GetLastMarkerAndCurRegion))(ReaProject* proj, double time,
...
REAPERAPI_DEF //==============================================
              // GetLastTouchedFX
    // Returns true if the last touched FX parameter is valid, false otherwise. The low word of
    // tracknumber is the 1-based track index -- 0 means the master track, 1 means track 1, etc. If
    // the high word of tracknumber is nonzero, it refers to the 1-based item index (1 is the first
    // item on the track, etc). For track FX, the low 24 bits of fxnumber refer to the FX index in
    // the chain, and if the next 8 bits are 01, then the FX is record FX. For item FX, the low word
    // defines the FX index in the chain, and the high word defines the take number. Deprecated, see
    // GetTouchedOrFocusedFX.

    bool (*REAPERAPI_FUNCNAME(GetLastTouchedFX))(int* tracknumberOut, int* fxnumberOut,
                                                 int* paramnumberOut);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // &1=master mute,&2=master solo. This is deprecated as you can just query the master track as
    // well.
...
REAPERAPI_DEF //==============================================
    ...
    // Get text-based metadata from a media file for a given identifier. Call with identifier="" to
    // list all identifiers contained in the file, separated by newlines. May return "[Binary data]"
    // for metadata that REAPER doesn't handle.

    int (*REAPERAPI_FUNCNAME(GetMediaFileMetadata))(PCM_source* mediaSource, const char* identifier,
...
REAPERAPI_DEF //==============================================
    ...
    // B_MUTE_ACTUAL : bool * : muted (ignores solo). setting this value will not affect
    // C_MUTE_SOLO. C_LANEPLAYS : char * : on fixed lane tracks, 0=this item lane does not play,
    // 1=this item lane plays exclusively, 2=this item lane plays and other lanes also play, -1=this
    // item is on a non-visible, non-playing lane on a formerly fixed-lane track (read-only)
    // C_MUTE_SOLO : char * : solo override (-1=soloed, 0=no override, 1=unsoloed). note that this
    // API does not automatically unsolo other items when soloing (nor clear the unsolos when
    // clearing the last soloed item), it must be done by the caller via action or via this API.
    // B_LOOPSRC : bool * : loop source
    // B_ALLTAKESPLAY : bool * : all takes play
    // B_UISEL : bool * : selected in arrange view
    // C_BEATATTACHMODE : char * : item timebase, -1=track or project default, 1=beats (position,
    // length, rate), 2=beats (position only). for auto-stretch timebase: C_BEATATTACHMODE=1,
    // C_AUTOSTRETCH=1 C_AUTOSTRETCH: : char * : auto-stretch at project tempo changes, 1=enabled,
    // requires C_BEATATTACHMODE=1 C_LOCK : char * : locked, &1=locked D_VOL : double * : item
    // volume,  0=-inf, 0.5=-6dB, 1=+0dB, 2=+6dB, etc D_POSITION : double * : item position in
    // seconds D_LENGTH : double * : item length in seconds D_SNAPOFFSET : double * : item snap
    // offset in seconds D_FADEINLEN : double * : item manual fadein length in seconds D_FADEOUTLEN
    // : double * : item manual fadeout length in seconds D_FADEINDIR : double * : item fadein
    // curvature, -1..1 D_FADEOUTDIR : double * : item fadeout curvature, -1..1 D_FADEINLEN_AUTO :
    // double * : item auto-fadein length in seconds, -1=no auto-fadein D_FADEOUTLEN_AUTO : double *
    // : item auto-fadeout length in seconds, -1=no auto-fadeout C_FADEINSHAPE : int * : fadein
    // shape, 0..6, 0=linear C_FADEOUTSHAPE : int * : fadeout shape, 0..6, 0=linear I_GROUPID : int
    // * : group ID, 0=no group I_LASTY : int * : Y-position (relative to top of track) in pixels
    // (read-only) I_LASTH : int * : height in pixels (read-only) I_CUSTOMCOLOR : int * : custom
    // color, OS dependent color|0x1000000 (i.e. ColorToNative(r,g,b)|0x1000000). If you do not
    // |0x1000000, then it will not be used, but will store the color I_CURTAKE : int * : active
    // take number IP_ITEMNUMBER : int : item number on this track (read-only, returns the item
    // number directly) F_FREEMODE_Y : float * : free item positioning or fixed lane Y-position.
    // 0=top of track, 1.0=bottom of track F_FREEMODE_H : float * : free item positioning or fixed
    // lane height. 0.5=half the track height, 1.0=full track height I_FIXEDLANE : int * : fixed
    // lane of item (fine to call with setNewValue, but returned value is read-only)
    // B_FIXEDLANE_HIDDEN : bool * : true if displaying only one fixed lane and this item is in a
    // different lane (read-only) P_TRACK : MediaTrack * : (read-only)
    //
...
REAPERAPI_DEF //==============================================
    ...
    // Gets block of peak samples to buf. Note that the peak samples are interleaved, but in two or
    // three blocks (maximums, then minimums, then extra). Return value has 20 bits of returned
    // sample count, then 4 bits of output_mode (0xf00000), then a bit to signify whether extra_type
    // was available (0x1000000). extra_type can be 115 ('s') for spectral information, which will
    // return peak samples as integers with the low 15 bits frequency, next 14 bits tonality.

    int (*REAPERAPI_FUNCNAME(GetMediaItemTake_Peaks))(MediaItem_Take* take, double peakrate,
...
REAPERAPI_DEF //==============================================
    ...
    // D_VOL : double * : take volume, 0=-inf, 0.5=-6dB, 1=+0dB, 2=+6dB, etc, negative if take
    // polarity is flipped D_PAN : double * : take pan, -1..1 D_PANLAW : double * : take pan law,
    // -1=default, 0.5=-6dB, 1.0=+0dB, etc D_PLAYRATE : double * : take playback rate, 0.5=half
    // speed, 1=normal, 2=double speed, etc D_PITCH : double * : take pitch adjustment in semitones,
    // -12=one octave down, 0=normal, +12=one octave up, etc B_PPITCH : bool * : preserve pitch when
    // changing playback rate I_LASTY : int * : Y-position (relative to top of track) in pixels
    // (read-only) I_LASTH : int * : height in pixels (read-only) I_CHANMODE : int * : channel mode,
    // 0=normal, 1=reverse stereo, 2=downmix, 3=left, 4=right I_PITCHMODE : int * : pitch shifter
    // mode, -1=project default, otherwise high 2 bytes=shifter, low 2 bytes=parameter
    // I_STRETCHFLAGS : int * : stretch marker flags (&7 mask for mode override: 0=default,
    // 1=balanced, 2/3/6=tonal, 4=transient, 5=no pre-echo) F_STRETCHFADESIZE : float * : stretch
    // marker fade size in seconds (0.0025 default) I_RECPASSID : int * : record pass ID
    // I_TAKEFX_NCH : int * : number of internal audio channels for per-take FX to use (OK to call
    // with setNewValue, but the returned value is read-only) I_CUSTOMCOLOR : int * : custom color,
    // OS dependent color|0x1000000 (i.e. ColorToNative(r,g,b)|0x1000000). If you do not |0x1000000,
    // then it will not be used, but will store the color IP_SPECEDIT:CNT : int : spectral edit
    // count (read-only) IP_SPECEDIT:DELETE:x : int : read or write this key to remove the spectral
    // edit specified IP_SPECEDIT:ADD : int : read or write this key to add a new spectral edit
    // (returns index) IP_SPECEDIT:SORT : int : read or write this key to re-sort spectral edits (be
    // sure to do this following a position change or insert of new edit) I_SPECEDIT:FFT_SIZE : int
    // * : FFT size used by spectral edits for this take D_SPECEDIT:x:POSITION : double * : position
    // of spectral edit start (changing this requires a resort of spectral edits)
    ...
    // I_SPECEDIT:x:TOPFREQ_ADD:pos:val : int * : reading or writing will insert top frequency-point
    // with position/value pair, returns index I_SPECEDIT:x:TOPFREQ_DEL:y : int * : reading or
    // writing will delete top frequency-point y. there will always be at least one point.
    ...
    // I_SPECEDIT:x:BOTFREQ_ADD:pos:val : int * : reading or writing will insert bottom
    // frequency-point with position/value pair, returns index I_SPECEDIT:x:BOTFREQ_DEL:y : int * :
    // reading or writing will delete bottom frequency-point y. there will always be at least one
    // point. F_SPECEDIT:x:BOTFREQ_POS:y : float * : (read-only) get position of bottom
    // frequency-point y F_SPECEDIT:x:BOTFREQ_FREQ:y : float * : (read-only) get frequency of bottom
    // frequency-point y IP_TAKENUMBER : int : take number (read-only, returns the take number
    // directly) P_TRACK : pointer to MediaTrack (read-only) P_ITEM : pointer to MediaItem
    // (read-only) P_SOURCE : PCM_source *. Note that if setting this, you should first retrieve the
...
REAPERAPI_DEF //==============================================
    ...
    // Copies the media source filename to filenamebuf. Note that in-project MIDI media sources have
    // no associated filename. See GetMediaSourceParent.

    void (*REAPERAPI_FUNCNAME(GetMediaSourceFileName))(PCM_source* source, char* filenamebufOut,
...
REAPERAPI_DEF //==============================================
    ...
    // Returns the length of the source media. If the media source is beat-based, the length will be
    // in quarter notes, otherwise it will be in seconds.
...
REAPERAPI_DEF //==============================================
    ...
    // Returns the parent source, or NULL if src is the root source. This can be used to retrieve
    // the parent properties of sections or reversed sources for example.
...
REAPERAPI_DEF //==============================================
    ...
    void (*REAPERAPI_FUNCNAME(GetMediaSourceType))(PCM_source* source, char* typebufOut,
                                                   int typebufOut_sz);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // B_RECMON_IN_EFFECT : bool * : record monitoring in effect (current audio-thread playback
    // state, read-only) IP_TRACKNUMBER : int : track number 1-based, 0=not found, -1=master track
    // (read-only, returns the int directly) I_SOLO : int * : soloed, 0=not soloed, 1=soloed,
    // 2=soloed in place, 5=safe soloed, 6=safe soloed in place B_SOLO_DEFEAT : bool * : when set,
    // if anything else is soloed and this track is not muted, this track acts soloed I_FXEN : int *
    // : fx enabled, 0=bypassed, !0=fx active I_RECARM : int * : record armed, 0=not record armed,
    // 1=record armed I_RECINPUT : int * : record input, <0=no input. if 4096 set, input is MIDI and
    // low 5 bits represent channel (0=all, 1-16=only chan), next 6 bits represent physical input
    // (63=all, 62=VKB). If 4096 is not set, low 10 bits (0..1023) are input start channel
    // (ReaRoute/Loopback start at 512). If 2048 is set, input is multichannel input (using track
    // channel count), or if 1024 is set, input is stereo input, otherwise input is mono. I_RECMODE
    // : int * : record mode, 0=input, 1=stereo out, 2=none, 3=stereo out w/latency compensation,
    // 4=midi output, 5=mono out, 6=mono out w/ latency compensation, 7=midi overdub, 8=midi replace
    // I_RECMODE_FLAGS : int * : record mode flags, &3=output recording mode (0=post fader,
    // 1=pre-fx, 2=post-fx/pre-fader) I_RECMON : int * : record monitoring, 0=off, 1=normal, 2=not
    // when playing (tape style) I_RECMONITEMS : int * : monitor items while recording, 0=off, 1=on
    // B_AUTO_RECARM : bool * : automatically set record arm when selected (does not immediately
    // affect recarm state, script should set directly if desired) I_VUMODE : int * : track vu mode,
    // &1:disabled, &30==0:stereo peaks, &30==2:multichannel peaks, &30==4:stereo RMS,
    // &30==8:combined RMS, &30==12:LUFS-M, &30==16:LUFS-S (readout=max), &30==20:LUFS-S
    // (readout=current), &32:LUFS calculation on channels 1+2 only I_AUTOMODE : int * : track
    // automation mode, 0=trim/off, 1=read, 2=touch, 3=write, 4=latch I_NCHAN : int * : number of
    // track channels, 2-128, even numbers only I_SELECTED : int * : track selected, 0=unselected,
    // 1=selected I_WNDH : int * : current TCP window height in pixels including envelopes
    // (read-only) I_TCPH : int * : current TCP window height in pixels not including envelopes
    // (read-only) I_TCPY : int * : current TCP window Y-position in pixels relative to top of
    // arrange view (read-only) I_MCPX : int * : current MCP X-position in pixels relative to mixer
    // container (read-only) I_MCPY : int * : current MCP Y-position in pixels relative to mixer
    // container (read-only) I_MCPW : int * : current MCP width in pixels (read-only) I_MCPH : int *
    // : current MCP height in pixels (read-only) I_FOLDERDEPTH : int * : folder depth change,
    // 0=normal, 1=track is a folder parent, -1=track is the last in the innermost folder, -2=track
    // is the last in the innermost and next-innermost folders, etc I_FOLDERCOMPACT : int * : folder
    // collapsed state (only valid on folders), 0=normal, 1=collapsed, 2=fully collapsed I_MIDIHWOUT
    // : int * : track midi hardware output index, <0=disabled, low 5 bits are which channels
    // (0=all, 1-16), next 5 bits are output device index (0-31) I_MIDI_INPUT_CHANMAP : int * : -1
    // maps to source channel, otherwise 1-16 to map to MIDI channel I_MIDI_CTL_CHAN : int * : -1 no
    // link, 0-15 link to MIDI volume/pan on channel, 16 link to MIDI volume/pan on all channels
    // I_MIDI_TRACKSEL_FLAG : int * : MIDI editor track list options: &1=expand media items,
    // &2=exclude from list, &4=auto-pruned I_PERFFLAGS : int * : track performance flags, &1=no
    // media buffering, &2=no anticipative FX I_CUSTOMCOLOR : int * : custom color, OS dependent
    // color|0x1000000 (i.e. ColorToNative(r,g,b)|0x1000000). If you do not |0x1000000, then it will
    // not be used, but will store the color I_HEIGHTOVERRIDE : int * : custom height override for
    // TCP window, 0 for none, otherwise size in pixels I_SPACER : int * : 1=TCP track spacer above
    // this trackB_HEIGHTLOCK : bool * : track height lock (must set I_HEIGHTOVERRIDE before
    // locking) D_VOL : double * : trim volume of track, 0=-inf, 0.5=-6dB, 1=+0dB, 2=+6dB, etc D_PAN
    // : double * : trim pan of track, -1..1 D_WIDTH : double * : width of track, -1..1 D_DUALPANL :
    // double * : dualpan position 1, -1..1, only if I_PANMODE==6 D_DUALPANR : double * : dualpan
    // position 2, -1..1, only if I_PANMODE==6 I_PANMODE : int * : pan mode, 0=classic 3.x, 3=new
    // balance, 5=stereo pan, 6=dual pan D_PANLAW : double * : pan law of track, <0=project default,
    // 0.5=-6dB, 0.707..=-3dB, 1=+0dB, 1.414..=-3dB with gain compensation, 2=-6dB with gain
    // compensation, etc I_PANLAW_FLAGS : int * : pan law flags, 0=sine taper, 1=hybrid taper with
    // deprecated behavior when gain compensation enabled, 2=linear taper, 3=hybrid taper D_PLAYRATE
    // : double * : track playback rate, 0.5=half speed, 1=normal, 2=double speed, etc D_PITCH :
    // double * : track pitch adjustment in semitones, -12=one octave down, 0=normal, +12=one octave
    // up, etc B_PPITCH : bool * : preserve pitch when changing playback rate I_PITCHMODE : int * :
    // pitch shifter mode, -1=project default, otherwise high 2 bytes=shifter, low 2 bytes=parameter
    // P_ENV:<envchunkname or P_ENV:{GUID... : TrackEnvelope * : (read-only) chunkname can be
    // <VOLENV, <PANENV, etc; GUID is the stringified envelope GUID. B_SHOWINMIXER : bool * : track
    // control panel visible in mixer (do not use on master track) B_SHOWINTCP : bool * : track
    // control panel visible in arrange view (do not use on master track) B_MAINSEND : bool * :
    // track sends audio to parent C_MAINSEND_OFFS : char * : channel offset of track send to parent
    // C_MAINSEND_NCH : char * : channel count of track send to parent (0=use all child track
    // channels, 1=use one channel only) I_FREEMODE : int * : 1=track free item positioning enabled,
    // 2=track fixed lanes enabled (call UpdateTimeline() after changing) I_NUMFIXEDLANES : int * :
    // number of track fixed lanes (fine to call with setNewValue, but returned value is read-only)
    // C_LANESCOLLAPSED : char * : fixed lane collapse state (1=lanes collapsed, 2=track displays as
    // non-fixed-lanes but hidden lanes exist) C_LANESETTINGS : char * : fixed lane settings
    // (&1=auto-remove empty lanes at bottom, &2=do not auto-comp new recording, &4=newly recorded
    // lanes play exclusively (else add lanes in layers), &8=big lanes (else small lanes), &16=add
    // new recording at bottom (else record into first available lane), &32=hide lane buttons
    // C_LANEPLAYS:N : char * :  on fixed lane tracks, 0=lane N does not play, 1=lane N plays
    // exclusively, 2=lane N plays and other lanes also play (fine to call with setNewValue, but
    // returned value is read-only) C_ALLLANESPLAY : char * : on fixed lane tracks, 0=no lanes play,
    // 1=all lanes play, 2=some lanes play (fine to call with setNewValue 0 or 1, but returned value
    // is read-only) C_BEATATTACHMODE : char * : track timebase, -1=project default, 0=time, 1=beats
    // (position, length, rate), 2=beats (position only) F_MCP_FXSEND_SCALE : float * : scale of
    // fx+send area in MCP (0=minimum allowed, 1=maximum allowed) F_MCP_FXPARM_SCALE : float * :
    // scale of fx parameter area in MCP (0=minimum allowed, 1=maximum allowed) F_MCP_SENDRGN_SCALE
    // : float * : scale of send area as proportion of the fx+send total area (0=minimum allowed,
    // 1=maximum allowed) F_TCP_FXPARM_SCALE : float * : scale of TCP parameter area when TCP FX are
    // embedded (0=min allowed, default, 1=max allowed) I_PLAY_OFFSET_FLAG : int * : track media
    // playback offset state, &1=bypassed, &2=offset value is measured in samples (otherwise
    // measured in seconds) D_PLAY_OFFSET : double * : track media playback offset, units depend on
    // I_PLAY_OFFSET_FLAG P_PARTRACK : MediaTrack * : parent track (read-only) P_PROJECT :
    // ReaProject * : parent project (read-only)
    //
...
REAPERAPI_DEF //==============================================
              // GetMouseModifier
    // Get the current mouse modifier assignment for a specific modifier key assignment, in a
    // specific context. action will be filled in with the command ID number for a built-in mouse
    // modifier or built-in REAPER command ID, or the custom action ID string. Note: the action
    // string may have a space and 'c' or 'm' appended to it to specify command ID vs mouse modifier
    // ID. See SetMouseModifier for more information.
    //

    void (*REAPERAPI_FUNCNAME(GetMouseModifier))(const char* context, int modifier_flag,
                                                 char* actionOut, int actionOut_sz);
#endif
...
REAPERAPI_DEF //==============================================
              // GetPeakFileName
    // get the peak file name for a given file (can be either filename.reapeaks,or a hashed filename
    // in another path)
...
REAPERAPI_DEF //==============================================
    ...
    void (*REAPERAPI_FUNCNAME(GetPeakFileNameEx))(const char* fn, char* buf, int buf_sz,
                                                  bool forWrite);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    void (*REAPERAPI_FUNCNAME(GetPeakFileNameEx2))(const char* fn, char* buf, int buf_sz,
...
REAPERAPI_DEF //==============================================
    ...
    void* (*REAPERAPI_FUNCNAME(GetPeaksBitmap))(PCM_source_peaktransfer_t* pks, double maxamp,
...
REAPERAPI_DEF //==============================================
              // GetPlayLoopCnt
    // Queries the playback loopcnt for project. if render_skip_loopcnt is non-NULL, sets to either
    // -1 or the loopcnt for a skipped render-pass (used during 2nd-pass rendering).

    WDL_INT64 (*REAPERAPI_FUNCNAME(GetPlayLoopCnt))(ReaProject* proj,
                                                    WDL_INT64* render_skip_loopcnt);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // Gets user configured preferred disk read mode. mode/nb/bs are all parameters that should be
    // passed to WDL_FileRead, see for more information.
...
REAPERAPI_DEF //==============================================
    ...
    // Gets user configured preferred disk read mode for use when building peaks. mode/nb/bs are all
...
REAPERAPI_DEF //==============================================
    ...
    // Gets user configured preferred disk write mode. nb will receive two values, the initial and
    // maximum write buffer counts. mode/nb/bs are all parameters that should be passed to
    // WDL_FileWrite, see for more information.
...
REAPERAPI_DEF //==============================================
    ...
    // Gets project time offset in seconds (project settings - project start time). If rndframe is
    // true, the offset is rounded to a multiple of the project frame size.
...
REAPERAPI_DEF //==============================================
    ...
    void (*REAPERAPI_FUNCNAME(GetProjectTimeSignature2))(ReaProject* proj, double* bpmOut,
                                                         double* bpiOut);
#endif
...
REAPERAPI_DEF //==============================================
              // GetProjExtState
    // Get the value previously associated with this extname and key, the last time the project was
    // saved. See SetProjExtState, EnumProjExtState.

    int (*REAPERAPI_FUNCNAME(GetProjExtState))(ReaProject* proj, const char* extname,
...
REAPERAPI_DEF //==============================================
              // GetSelectedTrack
    // Get a selected track from a project (proj=0 for active project) by selected track count
    // (zero-based). This function ignores the master track, see GetSelectedTrack2.
...
REAPERAPI_DEF //==============================================
    ...
    MediaTrack* (*REAPERAPI_FUNCNAME(GetSelectedTrack2))(ReaProject* proj, int seltrackidx,
                                                         bool wantmaster);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    void (*REAPERAPI_FUNCNAME(GetSet_ArrangeView2))(ReaProject* proj, bool isSet,
                                                    int screen_x_start, int screen_x_end,
                                                    double* start_timeInOut, double* end_timeInOut);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    void (*REAPERAPI_FUNCNAME(GetSet_LoopTimeRange))(bool isSet, bool isLoop, double* startOut,
...
REAPERAPI_DEF //==============================================
    ...
    void (*REAPERAPI_FUNCNAME(GetSet_LoopTimeRange2))(ReaProject* proj, bool isSet, bool isLoop,
...
REAPERAPI_DEF //==============================================
    ...
    // Get or set automation item information. autoitem_idx=0 for the first automation item on an
    // envelope, 1 for the second item, etc. desc can be any of the following: D_POOL_ID : double *
    // : automation item pool ID (as an integer); edits are propagated to all other automation items
    // that share a pool ID D_POSITION : double * : automation item timeline position in seconds
    ...
    // D_POOL_QNLEN : double * : automation item pooled source length in quarter notes (setting will
    // affect all pooled instances)
    //

    double (*REAPERAPI_FUNCNAME(GetSetAutomationItemInfo))(TrackEnvelope* env, int autoitem_idx,
...
REAPERAPI_DEF //==============================================
    ...
    // Get or set automation item information. autoitem_idx=0 for the first automation item on an
    // envelope, 1 for the second item, etc. returns true on success. desc can be any of the
    // following: P_POOL_NAME : char * : name of the underlying automation item pool P_POOL_EXT:xyz
...
REAPERAPI_DEF //==============================================
    ...
    // Note that when writing some of these attributes you will need to manually update the arrange
    // and/or track panels, see TrackList_AdjustWindows

    bool (*REAPERAPI_FUNCNAME(GetSetEnvelopeInfo_String))(TrackEnvelope* env, const char* parmname,
...
REAPERAPI_DEF //==============================================
    ...
    bool (*REAPERAPI_FUNCNAME(GetSetEnvelopeState2))(TrackEnvelope* env, char* str, int str_sz,
                                                     bool isundo);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    bool (*REAPERAPI_FUNCNAME(GetSetItemState2))(MediaItem* item, char* str, int str_sz,
                                                 bool isundo);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // lane of item (fine to call with setNewValue, but returned value is read-only)
    // B_FIXEDLANE_HIDDEN : bool * : true if displaying only one fixed lane and this item is in a
    // different lane (read-only) P_NOTES : char * : item note text (do not write to returned
    // pointer, use setNewValue to update) P_EXT:xyz : char * : extension-specific persistent data
    // GUID : GUID * : 16-byte GUID, can query or update. If using a _String() function, GUID is a
    // string {xyz-...}.
    //

    void* (*REAPERAPI_FUNCNAME(GetSetMediaItemInfo))(MediaItem* item, const char* parmname,
                                                     void* setNewValue);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // P_NOTES : char * : item note text (do not write to returned pointer, use setNewValue to
    // update) P_EXT:xyz : char * : extension-specific persistent data GUID : GUID * : 16-byte GUID,
    // can query or update. If using a _String() function, GUID is a string {xyz-...}.
    //

    bool (*REAPERAPI_FUNCNAME(GetSetMediaItemInfo_String))(MediaItem* item, const char* parmname,
                                                           char* stringNeedBig, bool setNewValue);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // P_SOURCE : PCM_source *. Note that if setting this, you should first retrieve the old source,
    // set the new, THEN delete the old. P_NAME : char * : take name P_EXT:xyz : char * :
    // extension-specific persistent data GUID : GUID * : 16-byte GUID, can query or update. If
    // using a _String() function, GUID is a string {xyz-...}. D_STARTOFFS : double * : start offset
    // in source media, in seconds D_VOL : double * : take volume, 0=-inf, 0.5=-6dB, 1=+0dB, 2=+6dB,
    // etc, negative if take polarity is flipped D_PAN : double * : take pan, -1..1 D_PANLAW :
    // double * : take pan law, -1=default, 0.5=-6dB, 1.0=+0dB, etc D_PLAYRATE : double * : take
    // playback rate, 0.5=half speed, 1=normal, 2=double speed, etc D_PITCH : double * : take pitch
    // adjustment in semitones, -12=one octave down, 0=normal, +12=one octave up, etc B_PPITCH :
    // bool * : preserve pitch when changing playback rate I_LASTY : int * : Y-position (relative to
    // top of track) in pixels (read-only) I_LASTH : int * : height in pixels (read-only) I_CHANMODE
    // : int * : channel mode, 0=normal, 1=reverse stereo, 2=downmix, 3=left, 4=right I_PITCHMODE :
    // int * : pitch shifter mode, -1=project default, otherwise high 2 bytes=shifter, low 2
    // bytes=parameter I_STRETCHFLAGS : int * : stretch marker flags (&7 mask for mode override:
    // 0=default, 1=balanced, 2/3/6=tonal, 4=transient, 5=no pre-echo) F_STRETCHFADESIZE : float * :
    // stretch marker fade size in seconds (0.0025 default) I_RECPASSID : int * : record pass ID
    // I_TAKEFX_NCH : int * : number of internal audio channels for per-take FX to use (OK to call
    // with setNewValue, but the returned value is read-only) I_CUSTOMCOLOR : int * : custom color,
    // OS dependent color|0x1000000 (i.e. ColorToNative(r,g,b)|0x1000000). If you do not |0x1000000,
    // then it will not be used, but will store the color IP_SPECEDIT:CNT : int : spectral edit
    ...
    void* (*REAPERAPI_FUNCNAME(GetSetMediaItemTakeInfo))(MediaItem_Take* tk, const char* parmname,
                                                         void* setNewValue);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // resource_path/data/track_icons) P_LANENAME:n : char * : lane name (returns NULL for
    // non-fixed-lane-tracks) P_MCP_LAYOUT : const char * : layout name P_RAZOREDITS : const char *
    // : list of razor edit areas, as space-separated triples of start time, end time, and envelope
    // GUID string.
    //   Example: "0.0 1.0 \"\" 0.0 1.0 "{xyz-...}"
    // P_RAZOREDITS_EXT : const char * : list of razor edit areas, as comma-separated sets of
    // space-separated tuples of start time, end time, optional: envelope GUID string, fixed/fipm
    // top y-position, fixed/fipm bottom y-position.
    //   Example: "0.0 1.0,0.0 1.0 "{xyz-...}",1.0 2.0 "" 0.25 0.75"
    // P_TCP_LAYOUT : const char * : layout name
    // P_EXT:xyz : char * : extension-specific persistent data
    // P_UI_RECT:tcp.mute : char * : read-only, allows querying screen position + size of track
    // WALTER elements (tcp.size queries screen position and size of entire TCP, etc). GUID : GUID *
    // : 16-byte GUID, can query or update. If using a _String() function, GUID is a string
    // {xyz-...}. B_MUTE : bool * : muted B_PHASE : bool * : track phase inverted B_RECMON_IN_EFFECT
    // : bool * : record monitoring in effect (current audio-thread playback state, read-only)
    // IP_TRACKNUMBER : int : track number 1-based, 0=not found, -1=master track (read-only, returns
    // the int directly) I_SOLO : int * : soloed, 0=not soloed, 1=soloed, 2=soloed in place, 5=safe
    // soloed, 6=safe soloed in place B_SOLO_DEFEAT : bool * : when set, if anything else is soloed
    // and this track is not muted, this track acts soloed I_FXEN : int * : fx enabled, 0=bypassed,
    // !0=fx active I_RECARM : int * : record armed, 0=not record armed, 1=record armed I_RECINPUT :
    // int * : record input, <0=no input. if 4096 set, input is MIDI and low 5 bits represent
    // channel (0=all, 1-16=only chan), next 6 bits represent physical input (63=all, 62=VKB). If
    // 4096 is not set, low 10 bits (0..1023) are input start channel (ReaRoute/Loopback start at
    // 512). If 2048 is set, input is multichannel input (using track channel count), or if 1024 is
    // set, input is stereo input, otherwise input is mono. I_RECMODE : int * : record mode,
    // 0=input, 1=stereo out, 2=none, 3=stereo out w/latency compensation, 4=midi output, 5=mono
    // out, 6=mono out w/ latency compensation, 7=midi overdub, 8=midi replace I_RECMODE_FLAGS : int
    // * : record mode flags, &3=output recording mode (0=post fader, 1=pre-fx, 2=post-fx/pre-fader)
    ...
    void* (*REAPERAPI_FUNCNAME(GetSetMediaTrackInfo))(MediaTrack* tr, const char* parmname,
                                                      void* setNewValue);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    bool (*REAPERAPI_FUNCNAME(GetSetMediaTrackInfo_String))(MediaTrack* tr, const char* parmname,
                                                            char* stringNeedBig, bool setNewValue);
#endif
...
REAPERAPI_DEF //==============================================
              ...
              // str="" to get the chunk string returned (must call FreeHeapPtr when done)
              // supply str to set the state (returns zero)
    // set isundo if the state will be used for undo purposes (which may allow REAPER to get the
...
REAPERAPI_DEF //==============================================
    ...
    void (*REAPERAPI_FUNCNAME(GetSetProjectAuthor))(ReaProject* proj, bool set, char* author,
                                                    int author_sz);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // swingmode can be 1 for swing enabled, swingamt is -1..1. swingmode can be 3 for measure-grid.
    // Returns grid configuration flags

    int (*REAPERAPI_FUNCNAME(GetSetProjectGrid))(ReaProject* project, bool set,
...
REAPERAPI_DEF //==============================================
    ...
    // RENDER_SETTINGS : (&(1|2)==0)=master mix, &1=stems+master mix, &2=stems only, &4=multichannel
    // tracks to multichannel files, &8=use render matrix, &16=tracks with only mono media to mono
    // files, &32=selected media items, &64=selected media items via master, &128=selected tracks
    // via master, &256=embed transients if format supports, &512=embed metadata if format supports,
    // &1024=embed take markers if format supports, &2048=2nd pass render, &4096=render razor edits,
    // &8192=pre-fader stems (not if via master), &16384=only stem channels sent to parent,
    // &32768=preserve source metadata if possible, &(1<<16)=preserve source start offset if
    // possible, &(2<<16)=preserve source media sample rate if possible, &(4<<16)=if rendering
    // selected items or razor edits, render as a single file, &(8<<16)=parallel render via master,
    // &(16<<16)=delay render start to allow FX to initialize and load samples RENDER_BOUNDSFLAG :
    // 0=custom time bounds, 1=entire project, 2=time selection, 3=all project regions, 4=selected
    // media items, 5=selected project regions, 6=all project markers, 7=selected project markers
    ...
    // RENDER_TAILFLAG : apply render tail setting when rendering: &1=custom time bounds, &2=entire
    // project, &4=time selection, &8=all project markers/regions, &16=selected media items,
    // &32=selected project markers/regions RENDER_TAILMS : tail length in ms to render (only used
    // if RENDER_BOUNDSFLAG and RENDER_TAILFLAG are set) RENDER_ADDTOPROJ : &1=add rendered files to
    // project, &2=do not render files that are likely silent RENDER_DITHER : &1=dither, &2=noise
    // shaping, &4=dither stems, &8=noise shaping on stems, &16=disable all RENDER_NORMALIZE:
    // &1=enable normalization, (&14==0)=LUFS-I, (&14==2)=RMS, (&14==4)=peak, (&14==6)=true peak,
    // (&14==8)=LUFS-M max, (&14==10)=LUFS-S max, &16=adjust mono media -3dB, &(16|(8<<16))=adjust
    // mono media +3dB, (&(32|4096|(16<<16))==32)=normalize as if files play together,
    // (&(32|4096|(16<<16))==4096)=normalize to loudest file,
    // (&(32|4096|(16<<16))==(32|4096))=normalize as if files play together (common gain),
    // (&(32|4096|(16<<16))==(16<<16))=normalize to master mix, &64=enable brickwall limit,
    // &128=brickwall limit true peak, (&(256|2048)==256)=only normalize files that are too loud,
    // (&(256|2048)==2048)=only normalize files that are too quiet, &512=apply fade-in, &1024=apply
    // fade-out, &16384=trim starting silence, &32768=trim ending silence, &(1<<16)=pad start with
    // silence, &(2<<16)=pad end with silence, &(4<<16)=disable all render postprocessing,
    // (&((32<<16)|(64<<16))==(32<<16))=limit as if files play together,
    // (&((32<<16)|(64<<16))==(64<<16))=limit to master mix RENDER_NORMALIZE_TARGET: render
    // normalization target (0.5 means -6.02dB, requires RENDER_NORMALIZE&1) RENDER_BRICKWALL:
    // render brickwall limit (0.5 means -6.02dB, requires RENDER_NORMALIZE&64) RENDER_FADEIN:
    // render fade-in (0.001 means 1 ms, requires RENDER_NORMALIZE&512) RENDER_FADEOUT: render
    // fade-out (0.001 means 1 ms, requires RENDER_NORMALIZE&1024) RENDER_FADEINSHAPE: render
    // fade-in shape RENDER_FADEOUTSHAPE: render fade-out shape RENDER_PADSTART: pad render start
    // with silence (0.001 means 1ms, requires RENDER_NORMALIZE&(1<<16)) RENDER_PADEND: pad render
    // end with silence (0.001 means 1ms, requires RENDER_NORMALIZE&(2<<16)) RENDER_TRIMSTART: trim
    // render start threshold (0.5 means -6.02dB, requires RENDER_NORMALIZE&16384) RENDER_TRIMEND:
    // trim render end threshold (0.5 means -6.02dB, requires RENDER_NORMALIZE&32768) RENDER_DELAY:
    // seconds to delay start of render to allow FX to initialize and load samples (requires
    // RENDER_SETTINGS&(16<<16)) PROJECT_SRATE : sample rate (ignored unless PROJECT_SRATE_USE set)
    // PROJECT_SRATE_USE : set to 1 if project sample rate is used
    // MIDI_LIVE_MODE : 1 to stream MIDI instrument tracks directly to outputs with minimal
    // buffering, 0 otherwise (serialized in project files)
    //

    double (*REAPERAPI_FUNCNAME(GetSetProjectInfo))(ReaProject* project, const char* desc,
...
REAPERAPI_DEF //==============================================
    ...
    // MARKER_GUID:X : get the GUID (unique ID) of the marker or region with index X, where X is the
    // index passed to EnumProjectMarkers, not necessarily the displayed number (read-only)
    // MARKER_INDEX_FROM_GUID:{GUID} : get the GUID index of the marker or region with GUID {GUID}
    // (read-only) OPENCOPY_CFGIDX : integer for the configuration of format to use when creating
    // copies/applying FX. 0=wave (auto-depth), 1=APPLYFX_FORMAT, 2=RECORD_FORMAT RECORD_PATH :
    // recording directory -- may be blank or a relative path, to get the effective path see
    // GetProjectPathEx() RECORD_PATH_SECONDARY : secondary recording directory RECORD_FORMAT :
    // base64-encoded sink configuration (see project files, etc). Callers can also pass a simple
    // 4-byte string (non-base64-encoded), e.g. "evaw" or "l3pm", to use default settings for that
    // sink type. APPLYFX_FORMAT : base64-encoded sink configuration (see project files, etc). Used
    // only if RECFMT_OPENCOPY is set to 1. Callers can also pass a simple 4-byte string
    // (non-base64-encoded), e.g. "evaw" or "l3pm", to use default settings for that sink type.
    // RECTAG : project recording tag wildcard ($rectag). Can be used in Preferences/Audio/Recording
    // to auto-name recorded files. RENDER_FILE : render directory RENDER_PATTERN : render file name
    // (may contain wildcards) RENDER_METADATA : get or set the metadata saved with the project (not
    // metadata embedded in project media). Example, ID3 album name metadata: valuestr="ID3:TALB" to
    // get, valuestr="ID3:TALB|my album name" to set. Call with valuestr="" and is_set=false to get
    // a semicolon-separated list of defined project metadata identifiers. RENDER_TARGETS :
    // semicolon separated list of files that would be written if the project is rendered using the
    // most recent render settings RENDER_STATS : (read-only) semicolon separated list of statistics
    // for the most recently rendered files. call with valuestr="XXX" to run an action (for example,
    // "42437"=dry run render selected items) before returning statistics. RENDER_FORMAT :
    // base64-encoded sink configuration (see project files, etc). Callers can also pass a simple
    // 4-byte string (non-base64-encoded), e.g. "evaw" or "l3pm", to use default settings for that
    // sink type. RENDER_FORMAT2 : base64-encoded secondary sink configuration. Callers can also
    // pass a simple 4-byte string (non-base64-encoded), e.g. "evaw" or "l3pm", to use default
    // settings for that sink type, or "" to disable secondary render.
    //

    bool (*REAPERAPI_FUNCNAME(GetSetProjectInfo_String))(ReaProject* project, const char* desc,
                                                         char* valuestrNeedBig, bool is_set);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    void (*REAPERAPI_FUNCNAME(GetSetProjectNotes))(ReaProject* proj, bool set, char* notesNeedBig,
                                                   int notesNeedBig_sz);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // Gets or sets the attribute flag of a tempo/time signature marker. flag &1=sets time signature
    // and starts new measure, &2=does not set tempo, &4=allow previous partial measure if starting
    // new measure, &8=set new metronome pattern if starting new measure, &16=reset ruler grid if
    // starting new measure

    int (*REAPERAPI_FUNCNAME(GetSetTempoTimeSigMarkerFlag))(ReaProject* project, int point_index,
                                                            int flag, bool is_set);
#endif

#if defined(REAPERAPI_WANT_GetSetTrackGroupMembership) || !defined(REAPERAPI_MINIMAL)
REAPERAPI_DEF //==============================================
              // GetSetTrackGroupMembership
    // Gets or modifies the group membership for a track. Returns group state prior to call (each
    // bit represents one of the 32 group numbers). if setmask has bits set, those bits in setvalue
    // will be applied to group. Group can be one of: MEDIA_EDIT_LEAD MEDIA_EDIT_FOLLOW VOLUME_LEAD
    // VOLUME_FOLLOW
    // VOLUME_VCA_LEAD
    // VOLUME_VCA_FOLLOW
    // PAN_LEAD
    ...
    //
    // Note: REAPER v6.11 and earlier used _MASTER and _SLAVE rather than _LEAD and _FOLLOW, which
    // is deprecated but still supported (scripts that must support v6.11 and earlier can use the
    // deprecated strings).
    //
...
REAPERAPI_DEF //==============================================
    ...
    // Gets or modifies 32 bits (at offset, where 0 is the low 32 bits of the grouping) of the group
    // membership for a track. Returns group state prior to call. if setmask has bits set, those
    // bits in setvalue will be applied to group. Group can be one of: MEDIA_EDIT_LEAD
    // MEDIA_EDIT_FOLLOW
    // VOLUME_LEAD
    // VOLUME_FOLLOW
    // VOLUME_VCA_LEAD
...
REAPERAPI_DEF //==============================================
    ...
    // Gets or modifies the group membership for a track. Returns group state prior to call (each
    // bit represents one of the high 32 group numbers). if setmask has bits set, those bits in
    // setvalue will be applied to group. Group can be one of: MEDIA_EDIT_LEAD MEDIA_EDIT_FOLLOW
    // VOLUME_LEAD
    // VOLUME_FOLLOW
    // VOLUME_VCA_LEAD
    // VOLUME_VCA_FOLLOW
...
REAPERAPI_DEF //==============================================
    ...
    // Get or set the filename for storage of various track MIDI characteristics. 0=MIDI colormap
    // image file, 1 or 2=MIDI bank/program select file (2=set new default). If fn != NULL, a new
    // track MIDI storage file will be set; otherwise the existing track MIDI storage file will be
    // returned.
...
REAPERAPI_DEF //==============================================
    ...
    // P_ENV:<envchunkname : TrackEnvelope * : call with :<VOLENV, :<PANENV, etc appended
    // (read-only) P_EXT:xyz : char * : extension-specific persistent data B_MUTE : bool * B_PHASE :
    // bool * : true to flip phase B_MONO : bool * D_VOL : double * : 1.0 = +0dB etc D_PAN : double
    // * : -1..+1 D_PANLAW : double * : 1.0=+0.0db, 0.5=-6dB, -1.0 = projdef etc I_SENDMODE : int *
    // : 0=post-fader, 1=pre-fx, 2=post-fx (deprecated), 3=post-fx I_AUTOMODE : int * : automation
    // mode (-1=use track automode, 0=trim/off, 1=read, 2=touch, 3=write, 4=latch) I_SRCCHAN : int *
    // : -1 for no audio send. Low 10 bits specify channel offset, and higher bits specify channel
    // count. (srcchan>>10) == 0 for stereo, 1 for mono, 2 for 4 channel, 3 for 6 channel, etc.
    // I_DSTCHAN : int * : low 10 bits are destination index, &1024 set to mix to mono.
    // I_MIDIFLAGS : int * : low 5 bits=source channel 0=all, 1-16, 31=MIDI send disabled, next 5
    // bits=dest channel, 0=orig, 1-16=chan. &1024 for faders-send MIDI vol/pan. (>>14)&255 = src
    // bus (0 for all, 1 for normal, 2+). (>>22)&255=destination bus (0 for all, 1 for normal, 2+)
    // See CreateTrackSend, RemoveTrackSend.

    void* (*REAPERAPI_FUNCNAME(GetSetTrackSendInfo))(MediaTrack* tr, int category, int sendidx,
...
REAPERAPI_DEF //==============================================
    ...
    bool (*REAPERAPI_FUNCNAME(GetSetTrackSendInfo_String))(MediaTrack* tr, int category,
...
REAPERAPI_DEF //==============================================
    ...
    bool (*REAPERAPI_FUNCNAME(GetSetTrackState2))(MediaTrack* track, char* str, int str_sz,
                                                  bool isundo);
#endif
...
REAPERAPI_DEF //==============================================
              // GetTakeMarker
    // Get information about a take marker. Returns the position in media item source time, or -1 if
    // the take marker does not exist. See GetNumTakeMarkers, SetTakeMarker, DeleteTakeMarker

    double (*REAPERAPI_FUNCNAME(GetTakeMarker))(MediaItem_Take* take, int idx, char* nameOut,
...
REAPERAPI_DEF //==============================================
    ...
    // Gets information on a stretch marker, idx is 0..n. Returns -1 if stretch marker not valid.
    // posOut will be set to position in item, srcposOutOptional will be set to source media
    // position. Returns index. if input index is -1, the following marker is found using position
    // (or source position if position is -1). If position/source position are used to find marker
    // position, their values are not updated.

    int (*REAPERAPI_FUNCNAME(GetTakeStretchMarker))(MediaItem_Take* take, int idx, double* posOut,
...
REAPERAPI_DEF //==============================================
    ...
    bool (*REAPERAPI_FUNCNAME(GetTCPFXParm))(ReaProject* project, MediaTrack* track, int index,
...
REAPERAPI_DEF //==============================================
    ...
    // finds the playrate and target length to insert this item stretched to a round power-of-2
    // number of bars, between 1/8 and 256

    bool (*REAPERAPI_FUNCNAME(GetTempoMatchPlayRate))(PCM_source* source, double srcscale,
...
REAPERAPI_DEF //==============================================
    ...
    bool (*REAPERAPI_FUNCNAME(GetTempoTimeSigMarker))(ReaProject* proj, int ptidx,
...
REAPERAPI_DEF //==============================================
              // GetThemeColor
    // Returns the theme color specified, or -1 on failure. If the low bit of flags is set, the
    // color as originally specified by the theme (before any transformations) is returned,
    // otherwise the current (possibly transformed and modified) color is returned. See
...
REAPERAPI_DEF //==============================================
              // GetThingFromPoint
    // Hit tests a point in screen coordinates. Updates infoOut with information such as "arrange",
    // "fx_chain", "fx_0" (first FX in chain, floating), "spacer_0" (spacer before first track). If
    // a track panel is hit, string will begin with "tcp" or "mcp" or "tcp.mute" etc (future
    // versions may append additional information). May return NULL with valid info string to
    // indicate non-track thing.

    MediaTrack* (*REAPERAPI_FUNCNAME(GetThingFromPoint))(int screen_x, int screen_y, char* infoOut,
                                                         int infoOut_sz);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // For the main action context, the MIDI editor, or the media explorer, returns the toggle state
    // of the action. 0=off, 1=on, -1=NA because the action does not have on/off states. For the
...
REAPERAPI_DEF //==============================================
              // GetTouchedOrFocusedFX
    // mode can be 0 to query last touched parameter, or 1 to query currently focused FX. Returns
    // false if failed. If successful, trackIdxOut will be track index (-1 is master track, 0 is
    // first track). itemidxOut will be 0-based item index if an item, or -1 if not an item.
    // takeidxOut will be 0-based take index. fxidxOut will be FX index, potentially with 0x2000000
    // set to signify container-addressing, or with 0x1000000 set to signify record-input FX.
    // parmOut will be set to the parameter index if querying last-touched. parmOut will have 1 set
    // if querying focused state and FX is no longer focused but still open.

    bool (*REAPERAPI_FUNCNAME(GetTouchedOrFocusedFX))(int mode, int* trackidxOut, int* itemidxOut,
                                                      int* takeidxOut, int* fxidxOut, int* parmOut);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // ColorToNative(r,g,b)|0x1000000). Black is returned as 0x1000000, no color setting is returned
    // as 0.
...
REAPERAPI_DEF //==============================================
    ...
    // Gets a built-in track envelope by configuration chunk name, like "<VOLENV", or GUID string,
    // like "{B577250D-146F-B544-9B34-F24FBE488F1F}".
    //

    TrackEnvelope* (*REAPERAPI_FUNCNAME(GetTrackEnvelopeByChunkName))(
        MediaTrack* tr, const char* cfgchunkname_or_guid);
#endif
...
REAPERAPI_DEF //==============================================
              // GetTrackFromPoint
    // Returns the track from the screen coordinates specified. If the screen coordinates refer to a
    // window associated to the track (such as FX), the track will be returned. infoOutOptional will
    // be set to 1 if it is likely an envelope, 2 if it is likely a track FX. For a free item
    // positioning or fixed lane track, the second byte of infoOutOptional will be set to the
    // (approximate, for fipm tracks) item lane underneath the mouse. See GetThingFromPoint.

    MediaTrack* (*REAPERAPI_FUNCNAME(GetTrackFromPoint))(int screen_x, int screen_y,
                                                         int* infoOutOptional);
#endif
...
REAPERAPI_DEF //==============================================
              // GetTrackMIDILyrics
    // Get all MIDI lyrics on the track. Lyrics will be returned as one string with tabs between
    // each word. flag&1: double tabs at the end of each measure and triple tabs when skipping
    // measures, flag&2: each lyric is preceded by its beat position in the project (example with
    // flag=2: "1.1.2\tLyric for measure 1 beat 2\t2.1.1\tLyric for measure 2 beat 1	"). See
    // SetTrackMIDILyrics

    bool (*REAPERAPI_FUNCNAME(GetTrackMIDILyrics))(MediaTrack* track, int flag,
...
REAPERAPI_DEF //==============================================
    ...
    const char* (*REAPERAPI_FUNCNAME(GetTrackMIDINoteNameEx))(ReaProject* proj, MediaTrack* track,
                                                              int pitch, int chan);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    void (*REAPERAPI_FUNCNAME(GetTrackMIDINoteRange))(ReaProject* proj, MediaTrack* track,
...
REAPERAPI_DEF //==============================================
              ...
              // Retrieve visibility and color information for a track marker lane. color is
              // ColorToNative(r,g,b)|0x1000000 or 0 for default.

    bool (*REAPERAPI_FUNCNAME(GetTrackMarkerLaneInfo))(MediaTrack* track, int lane,
                                                       bool* visibleOut, int* colorOut);
#endif
...
REAPERAPI_DEF //==============================================
              // GetTrackNumSends
    // returns number of sends/receives/hardware outputs - category is <0 for receives, 0=sends, >0
...
REAPERAPI_DEF //==============================================
    ...
    bool (*REAPERAPI_FUNCNAME(GetTrackReceiveName))(MediaTrack* track, int recv_index, char* bufOut,
                                                    int bufOut_sz);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    bool (*REAPERAPI_FUNCNAME(GetTrackReceiveUIMute))(MediaTrack* track, int recv_index,
                                                      bool* muteOut);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    bool (*REAPERAPI_FUNCNAME(GetTrackReceiveUIVolPan))(MediaTrack* track, int recv_index,
...
REAPERAPI_DEF //==============================================
    ...
    // I_AUTOMODE : int * : automation mode (-1=use track automode, 0=trim/off, 1=read, 2=touch,
    // 3=write, 4=latch) I_SRCCHAN : int * : -1 for no audio send. Low 10 bits specify channel
    // offset, and higher bits specify channel count. (srcchan>>10) == 0 for stereo, 1 for mono, 2
    // for 4 channel, 3 for 6 channel, etc. I_DSTCHAN : int * : low 10 bits are destination index,
    // &1024 set to mix to mono. I_MIDIFLAGS : int * : low 5 bits=source channel 0=all, 1-16,
    // 31=MIDI send disabled, next 5 bits=dest channel, 0=orig, 1-16=chan. &1024 for faders-send
    // MIDI vol/pan. (>>14)&255 = src bus (0 for all, 1 for normal, 2+). (>>22)&255=destination bus
    // (0 for all, 1 for normal, 2+) P_DESTTRACK : MediaTrack * : destination track, only applies
    // for sends/recvs (read-only) P_SRCTRACK : MediaTrack * : source track, only applies for
    // sends/recvs (read-only) P_ENV:<envchunkname : TrackEnvelope * : call with :<VOLENV, :<PANENV,
    // etc appended (read-only) See CreateTrackSend, RemoveTrackSend, GetTrackNumSends.

    double (*REAPERAPI_FUNCNAME(GetTrackSendInfo_Value))(MediaTrack* tr, int category, int sendidx,
                                                         const char* parmname);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    bool (*REAPERAPI_FUNCNAME(GetTrackSendName))(MediaTrack* track, int send_index, char* bufOut,
                                                 int bufOut_sz);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    bool (*REAPERAPI_FUNCNAME(GetTrackSendUIMute))(MediaTrack* track, int send_index,
                                                   bool* muteOut);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    bool (*REAPERAPI_FUNCNAME(GetTrackSendUIVolPan))(MediaTrack* track, int send_index,
                                                     double* volumeOut, double* panOut);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    bool (*REAPERAPI_FUNCNAME(GetTrackStateChunk))(MediaTrack* track, char* strNeedBig,
                                                   int strNeedBig_sz, bool isundoOptional);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    bool (*REAPERAPI_FUNCNAME(GetTrackUIPan))(MediaTrack* track, double* pan1Out, double* pan2Out,
                                              int* panmodeOut);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    bool (*REAPERAPI_FUNCNAME(GetTrackUIVolPan))(MediaTrack* track, double* volumeOut,
                                                 double* panOut);
#endif
...
REAPERAPI_DEF //==============================================
              // GetUnderrunTime
    // retrieves the last timestamps of audio xrun (yellow-flash, if available), media xrun
    // (red-flash), and the current time stamp (all milliseconds)
...
REAPERAPI_DEF //==============================================
    ...
    bool (*REAPERAPI_FUNCNAME(GetUserFileNameForRead))(char* filenameNeed4096, const char* title,
                                                       const char* defext);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // input text. Maximum fields is 16. Values are returned as a comma-separated string. Returns
    // false if the user canceled the dialog. You can supply special extra information via
    // additional caption fields: extrawidth=XXX to increase text field width, separator=X to use a
    // different separator for returned fields.

    bool (*REAPERAPI_FUNCNAME(GetUserInputs))(const char* title, int num_inputs,
...
REAPERAPI_DEF //==============================================
              // GoToMarker
    // Go to marker. If use_timeline_order==true, marker_index 1 refers to the first marker on the
    // timeline.  If use_timeline_order==false, marker_index 1 refers to the first marker with the
    // user-editable index of 1.

    void (*REAPERAPI_FUNCNAME(GoToMarker))(ReaProject* proj, int marker_index,
...
REAPERAPI_DEF //==============================================
    ...
    // use_timeline_order==true, region_index 1 refers to the first region on the timeline.  If
    // use_timeline_order==false, region_index 1 refers to the first region with the user-editable
    // index of 1.

    void (*REAPERAPI_FUNCNAME(GoToRegion))(ReaProject* proj, int region_index,
                                           bool use_timeline_order);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // SetExtState, GetExtState, DeleteExtState.
...
REAPERAPI_DEF //==============================================
    ...
    // Insert a new automation item. pool_id < 0 collects existing envelope points into the
    // automation item; if pool_id is >= 0 the automation item will be a new instance of that pool
    // (which will be created as an empty instance if it does not exist). Returns the index of the
    // item, suitable for passing to other automation item API functions. See
    // GetSetAutomationItemInfo.

    int (*REAPERAPI_FUNCNAME(InsertAutomationItem))(TrackEnvelope* env, int pool_id,
...
REAPERAPI_DEF //==============================================
              // InsertEnvelopePoint
    // Insert an envelope point. If setting multiple points at once, set noSort=true, and call
    // Envelope_SortPoints when done. See InsertEnvelopePointEx.

    bool (*REAPERAPI_FUNCNAME(InsertEnvelopePoint))(TrackEnvelope* envelope, double time,
...
REAPERAPI_DEF //==============================================
    ...
    // trimmed so that not all points are visible. Otherwise, ptidx will be based on the number of
    // visible points in the automation item, including all loop iterations. See
    // CountEnvelopePointsEx, GetEnvelopePointEx, SetEnvelopePointEx, DeleteEnvelopePointEx.

    bool (*REAPERAPI_FUNCNAME(InsertEnvelopePointEx))(TrackEnvelope* envelope, int autoitem_idx,
...
REAPERAPI_DEF //==============================================
    ...
    // &4=stretch/loop to fit time sel, &8=try to match tempo 1x, &16=try to match tempo 0.5x,
    // &32=try to match tempo 2x, &64=don't preserve pitch when matching tempo, &128=no loop/section
    // if startpct/endpct set, &256=force loop regardless of global preference for looping imported
    // items, &512=use high word as absolute track index if mode&3==0 or mode&2048, &1024=insert
    // into reasamplomatic on a new track (add 1 to insert on last selected track), &2048=insert
    // into open reasamplomatic instance (add 512 to use high word as absolute track index),
    // &4096=move to source preferred position (BWF start offset), &8192=reverse. &16384=apply
    // ripple according to project setting
...
REAPERAPI_DEF //==============================================
    ...
    int (*REAPERAPI_FUNCNAME(InsertMediaSection))(const char* file, int mode, double startpct,
...
REAPERAPI_DEF //==============================================
              // InsertTrackAtIndex
    // inserts a track at idx,of course this will be clamped to 0..GetNumTracks(). wantDefaults=TRUE
    // for default envelopes/FX,otherwise no enabled fx/env. Superseded, see InsertTrackInProject
...
REAPERAPI_DEF //==============================================
    ...
    // inserts a track in project proj at idx, this will be clamped to 0..CountTracks(proj). flags&1
    // for default envelopes/FX, otherwise no enabled fx/envelopes will be added.
...
REAPERAPI_DEF //==============================================
              // IsProjectDirty
    // Is the project dirty (needing save)? Always returns 0 if 'undo/prompt to save' is disabled in
    // preferences.
...
REAPERAPI_DEF //==============================================
              // IsTrackVisible
    // If mixer==true, returns true if the track is visible in the mixer.  If mixer==false, returns
    // true if the track is visible in the track control panel.
...
REAPERAPI_DEF //==============================================
              // IsWindowTextField
    // Returns true if window is a text field or should behave as such (jsfx editor, hooked via
    // hwnd_info, etc)
...
REAPERAPI_DEF //==============================================
    ...
    int (*REAPERAPI_FUNCNAME(joystick_getinfo))(joystick_device* dev, int* axesOutOptional,
                                                int* povsOutOptional);
#endif
...
REAPERAPI_DEF //==============================================
              // joystick_update
    // Updates joystick state from hardware, returns true if successful (joystick_get* will not be
    // valid until joystick_update() is called successfully)
...
REAPERAPI_DEF //==============================================
    ...
    int (*REAPERAPI_FUNCNAME(kbd_enumerateActions))(KbdSectionInfo* section, int idx,
                                                    const char** nameOut);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    int (*REAPERAPI_FUNCNAME(KBD_OnMainActionEx))(int cmd, int val, int valhw, int relmode,
...
REAPERAPI_DEF //==============================================
              // kbd_reprocessMenu
    // Reprocess a menu recursively, setting key assignments to what their command IDs are mapped
    // to.
...
REAPERAPI_DEF //==============================================
    ...
    bool (*REAPERAPI_FUNCNAME(kbd_RunCommandThroughHooks))(KbdSectionInfo* section,
                                                           ...
                                                           const int* val, const int* valhw,
                                                           const int* relmode, HWND hwnd);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // and kbd_translateAccelerator() will process it looking for any keys bound to it, and send the
    // messages off. Returns 1 if processed, 0 if no key binding found.

    int (*REAPERAPI_FUNCNAME(kbd_translateAccelerator))(HWND hwnd, MSG* msg,
...
REAPERAPI_DEF //==============================================
    ...
    int (*REAPERAPI_FUNCNAME(LICE__DrawText))(LICE_IFont* font, LICE_IBitmap* bm, const char* str,
...
REAPERAPI_DEF //==============================================
    ...
    void (*REAPERAPI_FUNCNAME(LICE_Arc))(LICE_IBitmap* dest, float cx, float cy, float r,
...
REAPERAPI_DEF //==============================================
    ...
    void (*REAPERAPI_FUNCNAME(LICE_Blit))(LICE_IBitmap* dest, LICE_IBitmap* src, int dstx, int dsty,
...
REAPERAPI_DEF //==============================================
    ...
    void (*REAPERAPI_FUNCNAME(LICE_Blur))(LICE_IBitmap* dest, LICE_IBitmap* src, int dstx, int dsty,
...
REAPERAPI_DEF //==============================================
    ...
    void (*REAPERAPI_FUNCNAME(LICE_BorderedRect))(LICE_IBitmap* dest, int x, int y, int w, int h,
...
REAPERAPI_DEF //==============================================
    ...
    void (*REAPERAPI_FUNCNAME(LICE_Circle))(LICE_IBitmap* dest, float cx, float cy, float r,
...
REAPERAPI_DEF //==============================================
    ...
    void (*REAPERAPI_FUNCNAME(LICE_ClearRect))(LICE_IBitmap* dest, int x, int y, int w, int h,
...
REAPERAPI_DEF //==============================================
    ...
    bool (*REAPERAPI_FUNCNAME(LICE_ClipLine))(int* pX1Out, int* pY1Out, int* pX2Out, int* pY2Out,
...
REAPERAPI_DEF //==============================================
    ...
    LICE_pixel (*REAPERAPI_FUNCNAME(LICE_CombinePixels))(LICE_pixel dest, LICE_pixel src,
                                                         float alpha, int mode);
#endif
...
REAPERAPI_DEF //==============================================
              // LICE_CreateBitmap
    // create a new bitmap. this is like calling new LICE_MemBitmap (mode=0) or new LICE_SysBitmap
    // (mode=1).
...
REAPERAPI_DEF //==============================================
    ...
    void (*REAPERAPI_FUNCNAME(LICE_DrawCBezier))(LICE_IBitmap* dest, double xstart, double ystart,
...
REAPERAPI_DEF //==============================================
    ...
    void (*REAPERAPI_FUNCNAME(LICE_DrawChar))(LICE_IBitmap* bm, int x, int y, char c,
...
REAPERAPI_DEF //==============================================
    ...
    void (*REAPERAPI_FUNCNAME(LICE_DrawGlyph))(LICE_IBitmap* dest, int x, int y, LICE_pixel color,
...
REAPERAPI_DEF //==============================================
    ...
    void (*REAPERAPI_FUNCNAME(LICE_DrawRect))(LICE_IBitmap* dest, int x, int y, int w, int h,
                                              LICE_pixel color, float alpha, int mode);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    void (*REAPERAPI_FUNCNAME(LICE_DrawText))(LICE_IBitmap* bm, int x, int y, const char* string,
                                              LICE_pixel color, float alpha, int mode);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    void (*REAPERAPI_FUNCNAME(LICE_FillCBezier))(LICE_IBitmap* dest, double xstart, double ystart,
                                                 double xctl1, double yctl1, double xctl2,
...
REAPERAPI_DEF //==============================================
    ...
    void (*REAPERAPI_FUNCNAME(LICE_FillCircle))(LICE_IBitmap* dest, float cx, float cy, float r,
                                                LICE_pixel color, float alpha, int mode, bool aa);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    void (*REAPERAPI_FUNCNAME(LICE_FillConvexPolygon))(LICE_IBitmap* dest, int* x, int* y,
...
REAPERAPI_DEF //==============================================
    ...
    void (*REAPERAPI_FUNCNAME(LICE_FillRect))(LICE_IBitmap* dest, int x, int y, int w, int h,
                                              LICE_pixel color, float alpha, int mode);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    void (*REAPERAPI_FUNCNAME(LICE_FillTrapezoid))(LICE_IBitmap* dest, int x1a, int x1b, int y1,
...
REAPERAPI_DEF //==============================================
    ...
    void (*REAPERAPI_FUNCNAME(LICE_FillTriangle))(LICE_IBitmap* dest, int x1, int y1, int x2,
...
REAPERAPI_DEF //==============================================
    ...
    void (*REAPERAPI_FUNCNAME(LICE_GradRect))(LICE_IBitmap* dest, int dstx, int dsty, int dstw,
...
REAPERAPI_DEF //==============================================
    ...
    void (*REAPERAPI_FUNCNAME(LICE_Line))(LICE_IBitmap* dest, float x1, float y1, float x2,
...
REAPERAPI_DEF //==============================================
    ...
    void (*REAPERAPI_FUNCNAME(LICE_LineInt))(LICE_IBitmap* dest, int x1, int y1, int x2, int y2,
                                             LICE_pixel color, float alpha, int mode, bool aa);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    void (*REAPERAPI_FUNCNAME(LICE_MultiplyAddRect))(LICE_IBitmap* dest, int x, int y, int w, int h,
...
REAPERAPI_DEF //==============================================
    ...
    void (*REAPERAPI_FUNCNAME(LICE_PutPixel))(LICE_IBitmap* bm, int x, int y, LICE_pixel color,
                                              float alpha, int mode);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    void (*REAPERAPI_FUNCNAME(LICE_RotatedBlit))(LICE_IBitmap* dest, LICE_IBitmap* src, int dstx,
...
REAPERAPI_DEF //==============================================
    ...
    void (*REAPERAPI_FUNCNAME(LICE_RoundRect))(LICE_IBitmap* drawbm, float xpos, float ypos,
...
REAPERAPI_DEF //==============================================
    ...
    void (*REAPERAPI_FUNCNAME(LICE_ScaledBlit))(LICE_IBitmap* dest, LICE_IBitmap* src, int dstx,
                                                int dsty, int dstw, int dsth, float srcx,
...
REAPERAPI_DEF //==============================================
    ...
    void (*REAPERAPI_FUNCNAME(LICE_SimpleFill))(LICE_IBitmap* dest, int x, int y,
...
REAPERAPI_DEF //==============================================
              // LICE_ThickFLine
    // always AA. wid is not affected by scaling (1 is always normal line, 2 is always 2 physical
    // pixels, etc)

    void (*REAPERAPI_FUNCNAME(LICE_ThickFLine))(LICE_IBitmap* dest, double x1, double y1, double x2,
                                                double y2, LICE_pixel color, float alpha, int mode,
                                                int wid);
#endif
...
REAPERAPI_DEF //==============================================
              // LocalizeString
    // Returns a localized version of src_string, in section section. flags can have 1 set to only
    // localize if sprintf-style formatting matches the original.

    const char* (*REAPERAPI_FUNCNAME(LocalizeString))(const char* src_string, const char* section,
                                                      int flagsOptional);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // (ReaScripts, custom or extension plugins' actions) safely, see NamedCommandLookup().
...
REAPERAPI_DEF //==============================================
              // Main_openProject
    // opens a project. will prompt the user to save unless name is prefixed with 'noprompt:'. If
    // name is prefixed with 'template:', project file will be loaded as a template. If passed a
    // .RTrackTemplate file, adds the template to the existing project.
...
REAPERAPI_DEF //==============================================
              // Main_SaveProjectEx
    // Save the project. options: &1=save selected tracks as track template, &2=include media with
    // track template, &4=include envelopes with track template, &8=if not saving template, set as
    // the new project filename for this ReaProject. See Main_openProject, Main_SaveProject.

    void (*REAPERAPI_FUNCNAME(Main_SaveProjectEx))(ReaProject* proj, const char* filename,
                                                   int options);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // Convert play rate to/from a value between 0 and 1, representing the position on the project
    // playrate slider.
...
REAPERAPI_DEF //==============================================
    ...
    // Convert the tempo to/from a value between 0 and 1, representing bpm in the range of 40-296
    // bpm.
...
REAPERAPI_DEF //==============================================
    ...
    // Get information about the most recently previewed Media Explorer file. filename: last played
    // file name. filemode: &1:insert on new track, &2:insert into sampler, &8:tempo sync 1x,
    // &16:tempo sync 0.5x, &32:tempo sync 2x, &64:do not preserve pitch when changing playrate,
    // &128:loop selection exists, &256:time selection exists, &512:apply pitch/rate adjustment on
    // insert, &1024:apply volume adjustment on insert, &2048:apply normalization on insert,
    // &8192:reverse preview. startpct/endpct: time selection in [0.0, 1.0].
    // pitchshift/voladj/rateadj: current pitch/volume/playrate preview adjustments. srcbpm: source
    // media tempo. extrainfo: currently unused.

    bool (*REAPERAPI_FUNCNAME(MediaExplorerGetLastPlayedFileInfo))(
        char* filenameOut, int filenameOut_sz, int* filemodeOut, double* selstartOut,
        double* selendOut, double* pitchshiftOut, double* voladjOut, double* rateadjOut,
        double* sourcebpmOut, char* extrainfoOut, int extrainfoOut_sz);
#endif

#if defined(REAPERAPI_WANT_MediaItemDescendsFromTrack) || !defined(REAPERAPI_MINIMAL)
REAPERAPI_DEF //==============================================
              // MediaItemDescendsFromTrack
    // Returns 1 if the track holds the item, 2 if the track is a folder containing the track that
    // holds the item, etc.
...
REAPERAPI_DEF //==============================================
              // Menu_GetHash
              // Get a string that only changes when menu/toolbar entries are added or removed (not
    // re-ordered). Can be used to determine if a customized menu/toolbar differs from the default,
    // or if the default changed after a menu/toolbar was customized. flag==0: current default
    // menu/toolbar; flag==1: current customized menu/toolbar; flag==2: default menu/toolbar at the
    // time the current menu/toolbar was most recently customized, if it was customized in REAPER
    // v7.08 or later.

    bool (*REAPERAPI_FUNCNAME(Menu_GetHash))(const char* menuname, int flag, char* hashOut,
                                             int hashOut_sz);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    int (*REAPERAPI_FUNCNAME(MIDI_CountEvts))(MediaItem_Take* take, int* notecntOut,
...
REAPERAPI_DEF //==============================================
              // MIDI_DisableSort
    // Disable sorting for all MIDI insert, delete, get and set functions, until MIDI_Sort is
    // called.
...
REAPERAPI_DEF //==============================================
              // MIDI_EnumSelCC
    // Returns the index of the next selected MIDI CC event after ccidx (-1 if there are no more
    // selected events).
...
REAPERAPI_DEF //==============================================
              // MIDI_EnumSelEvts
    // Returns the index of the next selected MIDI event after evtidx (-1 if there are no more
    // selected events).
...
REAPERAPI_DEF //==============================================
              // MIDI_EnumSelNotes
    // Returns the index of the next selected MIDI note after noteidx (-1 if there are no more
    // selected events).
...
REAPERAPI_DEF //==============================================
    ...
    // Returns the index of the next selected MIDI text/sysex event after textsyxidx (-1 if there
...
REAPERAPI_DEF //==============================================
              // MIDI_GetAllEvts
    // Get all MIDI data. MIDI buffer is returned as a list of { int offset, char flag, int msglen,
    // unsigned char msg[] }. offset: MIDI ticks from previous event flag: &1=selected &2=muted flag
    // high 4 bits for CC shape: &16=linear, &32=slow start/end, &16|32=fast start, &64=fast end,
    // &64|16=bezier msg: the MIDI message. A meta-event of type 0xF followed by 'CCBZ ' and 5 more
    // bytes represents bezier curve data for the previous MIDI event: 1 byte for the bezier type
    // (usually 0) and 4 bytes for the bezier tension as a float. For tick intervals longer than a
    // 32 bit word can represent, zero-length meta events may be placed between valid events. See
    // MIDI_SetAllEvts.

    bool (*REAPERAPI_FUNCNAME(MIDI_GetAllEvts))(MediaItem_Take* take, char* bufOutNeedBig,
...
REAPERAPI_DEF //==============================================
    ...
    bool (*REAPERAPI_FUNCNAME(MIDI_GetCC))(MediaItem_Take* take, int ccidx, bool* selectedOut,
...
REAPERAPI_DEF //==============================================
    ...
    bool (*REAPERAPI_FUNCNAME(MIDI_GetCCShape))(MediaItem_Take* take, int ccidx, int* shapeOut,
...
REAPERAPI_DEF //==============================================
    ...
    bool (*REAPERAPI_FUNCNAME(MIDI_GetEvt))(MediaItem_Take* take, int evtidx, bool* selectedOut,
                                            bool* mutedOut, double* ppqposOut, char* msgOut,
                                            int* msgOut_sz);
#endif
...
REAPERAPI_DEF //==============================================
              // MIDI_GetGrid
    // Returns the most recent MIDI editor grid size for this MIDI take, in QN. Swing is between 0
    // and 1. Note length is 0 if it follows the grid size.

    double (*REAPERAPI_FUNCNAME(MIDI_GetGrid))(MediaItem_Take* take, double* swingOutOptional,
...
REAPERAPI_DEF //==============================================
              // MIDI_GetHash
    // Get a string that only changes when the MIDI data changes. If notesonly==true, then the
    // string changes only when the MIDI notes change. See MIDI_GetTrackHash

    bool (*REAPERAPI_FUNCNAME(MIDI_GetHash))(MediaItem_Take* take, bool notesonly, char* hashOut,
                                             int hashOut_sz);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    bool (*REAPERAPI_FUNCNAME(MIDI_GetNote))(MediaItem_Take* take, int noteidx, bool* selectedOut,
...
REAPERAPI_DEF //==============================================
    ...
    double (*REAPERAPI_FUNCNAME(MIDI_GetPPQPos_StartOfMeasure))(MediaItem_Take* take,
                                                                double ppqpos);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // Returns the project time in quarter notes corresponding to a specific MIDI tick (ppq)
    // position.
...
REAPERAPI_DEF //==============================================
    ...
    // Gets a recent MIDI input event from the global history. idx=0 for the most recent event,
    // which also latches to the latest MIDI event state (to get a more recent list, calling with
    // idx=0 is necessary). idx=1 next most recent event, returns a non-zero sequence number for the
    // event, or zero if no more events. tsOut will be set to the timestamp in samples relative to
    // the current position (0 is current, -48000 is one second ago, etc). devIdxOut will have the
    // low 16 bits set to the input device index, and 0x10000 will be set if device was enabled only
    // for control. projPosOut will be set to project position in seconds if project was playing
    // back at time of event, otherwise -1. Large SysEx events will not be included in this event
    // list.

    int (*REAPERAPI_FUNCNAME(MIDI_GetRecentInputEvent))(int idx, char* bufOut, int* bufOut_sz,
                                                        int* tsOut, int* devIdxOut,
                                                        double* projPosOut, int* projLoopCntOut);
#endif

#if defined(REAPERAPI_WANT_MIDI_GetScale) || !defined(REAPERAPI_MINIMAL)
REAPERAPI_DEF //==============================================
              // MIDI_GetScale
    // Get the active scale in the media source, if any. root 0=C, 1=C#, etc. scale &0x1=root,
    // &0x2=minor 2nd, &0x4=major 2nd, &0x8=minor 3rd, &0xF=fourth, etc.

    bool (*REAPERAPI_FUNCNAME(MIDI_GetScale))(MediaItem_Take* take, int* rootOut, int* scaleOut,
...
REAPERAPI_DEF //==============================================
    ...
    // Get MIDI meta-event properties. Allowable types are -1:sysex (msg should not include bounding
    // F0..F7), 1-14:MIDI text event types, 15=REAPER notation event. For all other meta-messages,
    // type is returned as -2 and msg returned as all zeroes. See MIDI_GetEvt.

    bool (*REAPERAPI_FUNCNAME(MIDI_GetTextSysexEvt))(
        MediaItem_Take* take, int textsyxevtidx, bool* selectedOutOptional, bool* mutedOutOptional,
        double* ppqposOutOptional, int* typeOutOptional, char* msgOptional, int* msgOptional_sz);
#endif
...
REAPERAPI_DEF //==============================================
              // MIDI_GetTrackHash
    // Get a string that only changes when the MIDI data changes. If notesonly==true, then the
    // string changes only when the MIDI notes change. See MIDI_GetHash

    bool (*REAPERAPI_FUNCNAME(MIDI_GetTrackHash))(MediaTrack* track, bool notesonly, char* hashOut,
                                                  int hashOut_sz);
#endif
...
REAPERAPI_DEF //==============================================
              // midi_init
    // Opens MIDI devices as configured in preferences. force_reinit_input and force_reinit_output
    // force a particular device index to close/re-open (pass -1 to not force any devices to
    // reopen).
...
REAPERAPI_DEF //==============================================
    ...
    bool (*REAPERAPI_FUNCNAME(MIDI_InsertCC))(MediaItem_Take* take, bool selected, bool muted,
...
REAPERAPI_DEF //==============================================
    ...
    bool (*REAPERAPI_FUNCNAME(MIDI_InsertEvt))(MediaItem_Take* take, bool selected, bool muted,
...
REAPERAPI_DEF //==============================================
              // MIDI_InsertNote
    // Insert a new MIDI note. Set noSort if inserting multiple events, then call MIDI_Sort when
    // done.

    bool (*REAPERAPI_FUNCNAME(MIDI_InsertNote))(MediaItem_Take* take, bool selected, bool muted,
...
REAPERAPI_DEF //==============================================
    ...
    // Insert a new MIDI text or sysex event. Allowable types are -1:sysex (msg should not include
    // bounding F0..F7), 1-14:MIDI text event types, 15=REAPER notation event.

    bool (*REAPERAPI_FUNCNAME(MIDI_InsertTextSysexEvt))(MediaItem_Take* take, bool selected,
...
REAPERAPI_DEF //==============================================
              // MIDI_SetAllEvts
    // Set all MIDI data. MIDI buffer is passed in as a list of { int offset, char flag, int msglen,
    // unsigned char msg[] }. offset: MIDI ticks from previous event flag: &1=selected &2=muted flag
    // high 4 bits for CC shape: &16=linear, &32=slow start/end, &16|32=fast start, &64=fast end,
    // &64|16=bezier msg: the MIDI message. A meta-event of type 0xF followed by 'CCBZ ' and 5 more
    // bytes represents bezier curve data for the previous MIDI event: 1 byte for the bezier type
...
REAPERAPI_DEF //==============================================
              // MIDI_SetCC
    // Set MIDI CC event properties. Properties passed as NULL will not be set. set noSort if
    // setting multiple events, then call MIDI_Sort when done.

    bool (*REAPERAPI_FUNCNAME(MIDI_SetCC))(MediaItem_Take* take, int ccidx,
...
REAPERAPI_DEF //==============================================
              // MIDI_SetCCShape
    // Set CC shape and bezier tension. set noSort if setting multiple events, then call MIDI_Sort
    // when done. See MIDI_SetCC, MIDI_GetCCShape

    bool (*REAPERAPI_FUNCNAME(MIDI_SetCCShape))(MediaItem_Take* take, int ccidx, int shape,
...
REAPERAPI_DEF //==============================================
              // MIDI_SetEvt
    // Set MIDI event properties. Properties passed as NULL will not be set.  set noSort if setting
    // multiple events, then call MIDI_Sort when done.

    bool (*REAPERAPI_FUNCNAME(MIDI_SetEvt))(MediaItem_Take* take, int evtidx,
                                            const bool* selectedInOptional,
                                            const bool* mutedInOptional,
...
REAPERAPI_DEF //==============================================
              // MIDI_SetNote
    // Set MIDI note properties. Properties passed as NULL (or negative values) will not be set. Set
    // noSort if setting multiple events, then call MIDI_Sort when done. Setting multiple note start
    // positions at once is done more safely by deleting and re-inserting the notes.

    bool (*REAPERAPI_FUNCNAME(MIDI_SetNote))(
        MediaItem_Take* take, int noteidx, const bool* selectedInOptional,
        const bool* mutedInOptional, const double* startppqposInOptional,
        const double* endppqposInOptional, const int* chanInOptional, const int* pitchInOptional,
...
REAPERAPI_DEF //==============================================
    ...
    // Set MIDI text or sysex event properties. Properties passed as NULL will not be set. Allowable
    // types are -1:sysex (msg should not include bounding F0..F7), 1-14:MIDI text event types,
    // 15=REAPER notation event. set noSort if setting multiple events, then call MIDI_Sort when
    // done.

    bool (*REAPERAPI_FUNCNAME(MIDI_SetTextSysexEvt))(
        MediaItem_Take* take, int textsyxevtidx, const bool* selectedInOptional,
        const bool* mutedInOptional, const double* ppqposInOptional, const int* typeInOptional,
        const char* msgOptional, int msgOptional_sz, const bool* noSortInOptional);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // list the takes that are currently being edited in this MIDI editor, starting with the active
    // take. See MIDIEditor_GetTake

    MediaItem_Take* (*REAPERAPI_FUNCNAME(MIDIEditor_EnumTakes))(HWND midieditor, int takeindex,
                                                                bool editable_only);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // last_clicked_cc_lane: returns 0-127=CC, 0x100|(0-31)=14-bit CC, 0x200=velocity, 0x201=pitch,
    // 0x202=program, 0x203=channel pressure, 0x204=bank/program select, 0x205=text, 0x206=sysex,
    // 0x207=off velocity, 0x208=notation events, 0x209=aftertouch, 0x210=media item lane
...
REAPERAPI_DEF //==============================================
    ...
    // list_X: if viewing list view, returns string describing event at row X (0-based). String will
    // have a list of key=value pairs, e.g. 'pos=4.0 len=4.0 offvel=127 msg=90317F'. pos/len times
    // are in QN, len/offvel may not be present if event is not a note. other keys which may be
    // present include pos_pq/len_pq, sel, mute, ccval14, ccshape, ccbeztension. if setting_desc is
    // unsupported, the function returns false. See MIDIEditor_GetActive, MIDIEditor_GetSetting_int
    //

    bool (*REAPERAPI_FUNCNAME(MIDIEditor_GetSetting_str))(HWND midieditor, const char* setting_desc,
                                                          char* bufOut, int bufOut_sz);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // Send an action command to the last focused MIDI editor. Returns false if there is no MIDI
    // editor open, or if the view mode (piano roll or event list) does not match the input. see
...
REAPERAPI_DEF //==============================================
              // MIDIEditor_OnCommand
    // Send an action command to a MIDI editor. Returns false if the supplied MIDI editor pointer is
    // not valid (not an open MIDI editor). see MIDIEditor_GetActive,
    // MIDIEditor_LastFocused_OnCommand
...
REAPERAPI_DEF //==============================================
    ...
    bool (*REAPERAPI_FUNCNAME(MIDIEditor_SetSetting_int))(HWND midieditor, const char* setting_desc,
                                                          int setting);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // Get or set MIDI editor settings for this track. pitchwheelrange: semitones up or down. flags
    // &1: snap pitch lane edits to semitones if pitchwheel range is defined.

    void (*REAPERAPI_FUNCNAME(MIDIEditorFlagsForTrack))(MediaTrack* track,
                                                        int* pitchwheelrangeInOut, int* flagsInOut,
                                                        bool is_set);
#endif
...
REAPERAPI_DEF //==============================================
              // OpenMediaExplorer
    // Opens mediafn in the Media Explorer, play=true will play the file immediately (or toggle
    // playback if mediafn was already open), =false will just select it.
...
REAPERAPI_DEF //==============================================
    ...
    // Send an OSC message directly to REAPER. The value argument may be NULL. The message will be
    // matched against the default OSC patterns.
...
REAPERAPI_DEF //==============================================
    ...
    double (*REAPERAPI_FUNCNAME(parse_timestr_len))(const char* buf, double offset,
                                                    int modeoverride);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    PCM_sink* (*REAPERAPI_FUNCNAME(PCM_Sink_Create))(const char* filename, const char* cfg,
...
REAPERAPI_DEF //==============================================
    ...
    PCM_sink* (*REAPERAPI_FUNCNAME(PCM_Sink_CreateEx))(ReaProject* proj, const char* filename,
...
REAPERAPI_DEF //==============================================
    ...
    PCM_sink* (*REAPERAPI_FUNCNAME(PCM_Sink_CreateMIDIFile))(const char* filename, const char* cfg,
...
REAPERAPI_DEF //==============================================
    ...
    // Calls and returns PCM_source::PeaksBuild_Begin() if mode=0, PeaksBuild_Run() if mode=1, and
    // PeaksBuild_Finish() if mode=2. Normal use is to call PCM_Source_BuildPeaks(src,0), and if
    // that returns nonzero, call PCM_Source_BuildPeaks(src,1) periodically until it returns zero
    // (it returns the percentage of the file remaining), then call PCM_Source_BuildPeaks(src,2) to
    // finalize. If PCM_Source_BuildPeaks(src,0) returns zero, then no further action is necessary.
...
REAPERAPI_DEF //==============================================
    ...
    // Create a PCM_source from filename, and override pref of MIDI files being imported as
    // in-project MIDI events.
...
REAPERAPI_DEF //==============================================
              ...
              // Create a PCM_source from a "type" (use this if you're going to load its state via
    // LoadState/ProjectStateContext). Valid types include "WAVE", "MIDI", or whatever plug-ins
    // define as well.
...
REAPERAPI_DEF //==============================================
              // PCM_Source_Destroy
    // Deletes a PCM_source -- be sure that you remove any project reference before deleting a
    // source
...
REAPERAPI_DEF //==============================================
    ...
    int (*REAPERAPI_FUNCNAME(PCM_Source_GetPeaks))(PCM_source* src, double peakrate,
                                                   double starttime, int numchannels,
                                                   int numsamplesperchannel, int want_extra_type,
                                                   double* buf);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    bool (*REAPERAPI_FUNCNAME(PCM_Source_GetSectionInfo))(PCM_source* src, double* offsOut,
...
REAPERAPI_DEF //==============================================
    ...
    REAPER_PeakGet_Interface* (*REAPERAPI_FUNCNAME(PeakGet_Create))(const char* fn, int srate,
                                                                    int nch);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // menu to select/modify pitch shifter submode, returns new value (or old value if no item
    // selected)

    int (*REAPERAPI_FUNCNAME(PitchShiftSubModeMenu))(HWND hwnd, int x, int y, int mode,
                                                     int submode_sel);
#endif
...
REAPERAPI_DEF //==============================================
              // PlayPreviewEx
    // return nonzero on success. bufflags &1=buffer source, &2=treat length changes in source as
    // varispeed and adjust internal state accordingly if buffering. measure_align<0=play
    // immediately, >0=align playback with measure start

    int (*REAPERAPI_FUNCNAME(PlayPreviewEx))(preview_register_t* preview, int bufflags,
...
REAPERAPI_DEF //==============================================
    ...
    int (*REAPERAPI_FUNCNAME(PlayTrackPreview2Ex))(ReaProject* proj, preview_register_t* preview,
...
REAPERAPI_DEF //==============================================
    ...
    // Returns a double-NULL terminated list of importable media files, suitable for passing to
    // GetOpenFileName() etc. Includes *.* (All files).
...
REAPERAPI_DEF //==============================================
    ...
    // Returns a double-NULL terminated list of importable project files, suitable for passing to
    // GetOpenFileName() etc. Includes *.* (All files).
...
REAPERAPI_DEF //==============================================
              // PreventUIRefresh
    // adds prevent_count to the UI refresh prevention state; always add then remove the same
    // amount, or major disfunction will occur
...
REAPERAPI_DEF //==============================================
    ...
    // returns offset to pass to projectconfig_var_addr() to get project-config var of name. szout
    // gets size of object. can also query "__metronome_ptr" query project metronome PCM_source**
    // offset
...
REAPERAPI_DEF //==============================================
              // PromptForAction
    // Uses the action list to choose an action. Call with session_mode=1 to create a session
    // (init_id will be the initial action to select, or 0), then poll with session_mode=0, checking
    // return value for user-selected action (will return 0 if no action selected yet, or -1 if the
    // action window is no longer available). When finished, call with session_mode=-1.
...
REAPERAPI_DEF //==============================================
              // realloc_cmd_clear
    // clears a buffer/buffer-size registration added with realloc_cmd_register_buf, and clears any
    // later registrations, frees any allocated buffers. call after values are read from any
    // registered pointers etc.
...
REAPERAPI_DEF //==============================================
              // realloc_cmd_ptr
    // special use for NeedBig script API functions - reallocates a NeedBig buffer and updates its
    // size, returns false on error
...
REAPERAPI_DEF //==============================================
    ...
    // registers a buffer/buffer-size which may be reallocated by an API (ptr/ptr_size will be
    // updated to the new values). returns a token which should be passed to realloc_cmd_clear after
    // API call and values are read.
...
REAPERAPI_DEF //==============================================
              // ReaScriptError
    // Causes REAPER to display the error message after the current ReaScript finishes. If called
    // within a Lua context and errmsg has a ! prefix, script execution will be terminated.
...
REAPERAPI_DEF //==============================================
              // reduce_open_files
              // garbage-collects extra open files and closes them. if flags has 1 set, this is done
    // incrementally (call this from a regular timer, if desired). if flags has 2 set, files are
    // aggressively closed (they may need to be re-opened very soon). returns number of files closed
    // by this call.
...
REAPERAPI_DEF //==============================================
              // RemoveTrackSend
    // Remove a send/receive/hardware output, return true on success. category is <0 for receives,
    // 0=sends, >0 for hardware outputs. See CreateTrackSend, GetSetTrackSendInfo,
    // GetTrackSendInfo_Value, SetTrackSendInfo_Value, GetTrackNumSends.
...
REAPERAPI_DEF //==============================================
    ...
    // returns false if no tracks were selected. makePrevFolder=0 for normal, 1 = as child of track
    // preceding track specified by beforeTrackIdx, 2 = if track preceding track specified by
...
REAPERAPI_DEF //==============================================
              // resolve_fn2
    // Resolves a filename "in" by using project settings etc. If no file found, out will be a copy
    // of in.

    void (*REAPERAPI_FUNCNAME(resolve_fn2))(const char* in, char* out, int out_sz,
...
REAPERAPI_DEF //==============================================
    ...
    // Resolve a render wildcard pattern into a set of nul-separated, double-nul terminated render
    // target filenames. Returns the length of the string buffer needed for the returned file list.
    // Call with path=NULL to suppress filtering out illegal pathnames, call with targets=NULL to
    // get just the string buffer length.

    int (*REAPERAPI_FUNCNAME(ResolveRenderPattern))(ReaProject* project, const char* path,
                                                    const char* pattern, char* targets,
                                                    int targets_sz);
#endif
...
REAPERAPI_DEF //==============================================
              // ResolveWildcards
    // Resolve a wildcard string. Any wildcards that are valid in the Big Clock can be resolved
    // using this function. Pass in timePosition=-1 to use the current project playback position.

    void (*REAPERAPI_FUNCNAME(ResolveWildcards))(ReaProject* project, double timePosition,
                                                 const char* wildcards, char* resolvedStringNeedBig,
                                                 int resolvedStringNeedBig_sz);
#endif

#if defined(REAPERAPI_WANT_ReverseNamedCommandLookup) || !defined(REAPERAPI_MINIMAL)
REAPERAPI_DEF //==============================================
              // ReverseNamedCommandLookup
    // Get the named command for the given command ID. The returned string will not start with '_'
    // (e.g. it will return "SWS_ABOUT"), it will be NULL if command_id is a native action.
...
REAPERAPI_DEF //==============================================
    ...
    void (*REAPERAPI_FUNCNAME(SendLocalOscMessage))(void* local_osc_handler, const char* msg,
                                                    int msglen);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // Lua example of how to pack the message string: sysex = { 0xF0, 0x00, 0xF7 } msg = "" for i=1,
    // #sysex do msg = msg .. string.char(sysex[i]) end
...
REAPERAPI_DEF //==============================================
              // SetCursorContext
    // You must use this to change the focus programmatically. mode=0 to focus track panels, 1 to
    // focus the arrange window, 2 to focus the arrange window and select env (or env==NULL to clear
    // the current track/take envelope selection)
...
REAPERAPI_DEF //==============================================
    ...
    void (*REAPERAPI_FUNCNAME(SetEditCurPos2))(ReaProject* proj, double time, bool moveview,
                                               bool seekplay);
#endif
...
REAPERAPI_DEF //==============================================
              // SetEnvelopePoint
    // Set attributes of an envelope point. Values that are not supplied will be ignored. If setting
    // multiple points at once, set noSort=true, and call Envelope_SortPoints when done. See
    // SetEnvelopePointEx.

    bool (*REAPERAPI_FUNCNAME(SetEnvelopePoint))(TrackEnvelope* envelope, int ptidx,
...
REAPERAPI_DEF //==============================================
              // SetEnvelopePointEx
    // Set attributes of an envelope point. Values that are not supplied will be ignored. If setting
    // multiple points at once, set noSort=true, and call Envelope_SortPoints when done.
    // autoitem_idx=-1 for the underlying envelope, 0 for the first automation item on the envelope,
    // etc. For automation items, pass autoitem_idx|0x10000000 to base ptidx on the number of points
    // in one full loop iteration, even if the automation item is trimmed so that not all points are
    // visible. Otherwise, ptidx will be based on the number of visible points in the automation
    // item, including all loop iterations. See CountEnvelopePointsEx, GetEnvelopePointEx,
    // InsertEnvelopePointEx, DeleteEnvelopePointEx.

    bool (*REAPERAPI_FUNCNAME(SetEnvelopePointEx))(TrackEnvelope* envelope, int autoitem_idx,
...
REAPERAPI_DEF //==============================================
    ...
    bool (*REAPERAPI_FUNCNAME(SetEnvelopeStateChunk))(TrackEnvelope* env, const char* str,
                                                      bool isundoOptional);
#endif
...
REAPERAPI_DEF //==============================================
              // SetExtState
    // Set the extended state value for a specific section and key. persist=true means the value
    // should be stored and reloaded the next time REAPER is opened. Note that with persist=true
    // set, the value will be encoded as a text line in an .ini file and may behaved in unexpected
    // ways if it contains any newlines: do not pass a string with newlines to this function. To
    // save arbitrary data persistently, use base64 encoding or similar. See GetExtState,
    // DeleteExtState, HasExtState.

    void (*REAPERAPI_FUNCNAME(SetExtState))(const char* section, const char* key, const char* value,
                                            bool persist);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    bool (*REAPERAPI_FUNCNAME(SetItemStateChunk))(MediaItem* item, const char* str,
                                                  bool isundoOptional);
#endif
...
REAPERAPI_DEF //==============================================
              // SetMasterTrackVisibility
    // set &1 to show the master track in the TCP, &2 to HIDE in the mixer. Returns the previous
    // visibility state. See GetMasterTrackVisibility.
...
REAPERAPI_DEF //==============================================
    ...
    // lane of item (fine to call with setNewValue, but returned value is read-only)
    // B_FIXEDLANE_HIDDEN : bool * : true if displaying only one fixed lane and this item is in a
    // different lane (read-only)
    //

    bool (*REAPERAPI_FUNCNAME(SetMediaItemInfo_Value))(MediaItem* item, const char* parmname,
                                                       double newvalue);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    bool (*REAPERAPI_FUNCNAME(SetMediaItemPosition))(MediaItem* item, double position,
                                                     bool refreshUI);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // Set media source of media item take. The old source will not be destroyed, it is the caller's
    // responsibility to retrieve it and destroy it after. If source already exists in any project,
    // it will be duplicated before being set. C/C++ code should not use this and instead use
...
REAPERAPI_DEF //==============================================
    ...
    // 0.5=-6dB, 0.707..=-3dB, 1=+0dB, 1.414..=-3dB with gain compensation, 2=-6dB with gain
    // compensation, etc I_PANLAW_FLAGS : int * : pan law flags, 0=sine taper, 1=hybrid taper with
    // deprecated behavior when gain compensation enabled, 2=linear taper, 3=hybrid taper
    // P_ENV:<envchunkname or P_ENV:{GUID... : TrackEnvelope * : (read-only) chunkname can be
    // <VOLENV, <PANENV, etc; GUID is the stringified envelope GUID. B_SHOWINMIXER : bool * : track
    // control panel visible in mixer (do not use on master track) B_SHOWINTCP : bool * : track
    // control panel visible in arrange view (do not use on master track) B_MAINSEND : bool * :
    ...
    bool (*REAPERAPI_FUNCNAME(SetMediaTrackInfo_Value))(MediaTrack* tr, const char* parmname,
                                                        double newvalue);
#endif
...
REAPERAPI_DEF //==============================================
              // SetMIDIEditorGrid
    // Set the MIDI editor grid division. 0.25=quarter note, 1.0/3.0=half note tripet, etc. Sets the
    // swing enabled/strength from the arrange settings.
...
REAPERAPI_DEF //==============================================
              // SetMixerScroll
    // Scroll the mixer so that leftmosttrack is the leftmost visible track. Returns the leftmost
    // track after scrolling, which may be different from the passed-in track if there are not
    // enough tracks to its right.
...
REAPERAPI_DEF //==============================================
              // SetMouseModifier
    // Set the mouse modifier assignment for a specific modifier key assignment, in a specific
    // context. Context is a string like "MM_CTX_ITEM" (see reaper-mouse.ini) or "Media item left
    // drag" (unlocalized). Modifier flag is a number from 0 to 15: add 1 for shift, 2 for control,
    // 4 for alt, 8 for win. (macOS: add 1 for shift, 2 for command, 4 for opt, 8 for control.) For
    // left-click and double-click contexts, the action can be any built-in command ID number or any
    // custom action ID string. Find built-in command IDs in the REAPER actions window (enable "show
    // command IDs" in the context menu), and find custom action ID strings in reaper-kb.ini. The
    // action string may be a mouse modifier ID (see reaper-mouse.ini) with " m" appended to it, or
    // (for click/double-click contexts) a command ID with " c" appended to it, or the text that
    // appears in the mouse modifiers preferences dialog, like "Move item" (unlocalized). For
    // example, SetMouseModifier("MM_CTX_ITEM", 0, "1 m") and SetMouseModifier("Media item left
    // drag", 0, "Move item") are equivalent. SetMouseModifier(context, modifier_flag, -1) will
    // reset that mouse modifier to default. SetMouseModifier(context, -1, -1) will reset the entire
    // context to default. SetMouseModifier(-1, -1, -1) will reset all contexts to default. See
    // GetMouseModifier.
    //

    void (*REAPERAPI_FUNCNAME(SetMouseModifier))(const char* context, int modifier_flag,
                                                 const char* action);
#endif
...
REAPERAPI_DEF //==============================================
              // SetProjectMarker
    // Note: this function can't clear a marker's name (an empty string will leave the name
    // unchanged), see SetProjectMarker4.

    bool (*REAPERAPI_FUNCNAME(SetProjectMarker))(int markrgnindexnumber, bool isrgn, double pos,
...
REAPERAPI_DEF //==============================================
    ...
    bool (*REAPERAPI_FUNCNAME(SetProjectMarker2))(ReaProject* proj, int markrgnindexnumber,
...
REAPERAPI_DEF //==============================================
    ...
    bool (*REAPERAPI_FUNCNAME(SetProjectMarker3))(ReaProject* proj, int markrgnindexnumber,
                                                  bool isrgn, double pos, double rgnend,
...
REAPERAPI_DEF //==============================================
    ...
    bool (*REAPERAPI_FUNCNAME(SetProjectMarker4))(ReaProject* proj, int markrgnindexnumber,
                                                  bool isrgn, double pos, double rgnend,
...
REAPERAPI_DEF //==============================================
    ...
    bool (*REAPERAPI_FUNCNAME(SetProjectMarkerByIndex))(ReaProject* proj, int markrgnidx,
                                                        bool isrgn, double pos, double rgnend,
...
REAPERAPI_DEF //==============================================
    ...
    // Differs from SetProjectMarker4 in that markrgnidx is 0 for the first marker/region, 1 for the
    // next, etc (see EnumProjectMarkers3), rather than representing the displayed marker/region ID
    // number (see SetProjectMarker3). IDnumber < 0 to ignore. Function will fail if attempting to
    // set a duplicate ID number for a region (duplicate ID numbers for markers are OK). flags&1 to
    // clear name. If flags&2, markers will not be re-sorted, and after making updates, you MUST
    // call SetProjectMarkerByIndex2 with markrgnidx=-1 and flags&2 to force re-sort/UI updates.

    bool (*REAPERAPI_FUNCNAME(SetProjectMarkerByIndex2))(ReaProject* proj, int markrgnidx,
                                                         bool isrgn, double pos, double rgnend,
                                                         int IDnumber, const char* name, int color,
                                                         int flags);
#endif
...
REAPERAPI_DEF //==============================================
              // SetProjExtState
    // Save a key/value pair for a specific extension, to be restored the next time this specific
    // project is loaded. Typically extname will be the name of a reascript or extension section. If
    // key is NULL or "", all extended data for that extname will be deleted.  If val is NULL or "",
    // the data previously associated with that key will be deleted. Returns the size of the state
    // for this extname. See GetProjExtState, EnumProjExtState.

    int (*REAPERAPI_FUNCNAME(SetProjExtState))(ReaProject* proj, const char* extname,
...
REAPERAPI_DEF //==============================================
    ...
    // Add (flag > 0) or remove (flag < 0) a track from this region when using the region render
    // matrix. If adding, flag==2 means force mono, flag==4 means force stereo, flag==N means force
    // N/2 channels.

    void (*REAPERAPI_FUNCNAME(SetRegionRenderMatrix))(ReaProject* proj, int regionindex,
                                                      MediaTrack* track, int flag);
#endif
...
REAPERAPI_DEF //==============================================
              // SetTakeMarker
    // Inserts or updates a take marker. If idx<0, a take marker will be added, otherwise an
    // existing take marker will be updated. Returns the index of the new or updated take marker
    // (which may change if srcPos is updated). See GetNumTakeMarkers, GetTakeMarker,
    // DeleteTakeMarker

    int (*REAPERAPI_FUNCNAME(SetTakeMarker))(MediaItem_Take* take, int idx, const char* nameIn,
...
REAPERAPI_DEF //==============================================
    ...
    // Adds or updates a stretch marker. If idx<0, stretch marker will be added. If idx>=0, stretch
    // marker will be updated. When adding, if srcposInOptional is omitted, source position will be
    // auto-calculated. When updating a stretch marker, if srcposInOptional is omitted, srcpos will
    // not be modified. Position/srcposition values will be constrained to nearby stretch markers.
    // Returns index of stretch marker, or -1 if did not insert (or marker already existed at time).

    int (*REAPERAPI_FUNCNAME(SetTakeStretchMarker))(MediaItem_Take* take, int idx, double pos,
...
REAPERAPI_DEF //==============================================
    ...
    bool (*REAPERAPI_FUNCNAME(SetTakeStretchMarkerSlope))(MediaItem_Take* take, int idx,
                                                          double slope);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // Set parameters of a tempo/time signature marker. Provide either timepos (with measurepos=-1,
    // beatpos=-1), or measurepos and beatpos (with timepos=-1). If timesig_num and timesig_denom
    // are zero, the previous time signature will be used. ptidx=-1 will insert a new tempo/time
    // signature marker. See CountTempoTimeSigMarkers, GetTempoTimeSigMarker, AddTempoTimeSigMarker.

    bool (*REAPERAPI_FUNCNAME(SetTempoTimeSigMarker))(ReaProject* proj, int ptidx, double timepos,
...
REAPERAPI_DEF //==============================================
              // SetThemeColor
    // Temporarily updates the theme color to the color specified (or the theme default color if -1
    // is specified). Returns -1 on failure, otherwise returns the color (or transformed-color).
    // Note that the UI is not updated by this, the caller should call UpdateArrange() etc as
    // necessary. If the low bit of flags is set, any color transformations are bypassed. To read a
...
REAPERAPI_DEF //==============================================
    ...
    // Updates the toggle state of an action, returns true if succeeded. Only ReaScripts can have
    // their toggle states changed programmatically. See RefreshToolbar2.
...
REAPERAPI_DEF //==============================================
              // SetTrackColor
    // Set the custom track color, color is OS dependent (i.e. ColorToNative(r,g,b). To unset the
    // track color, see SetMediaTrackInfo_Value I_CUSTOMCOLOR
...
REAPERAPI_DEF //==============================================
    ...
    // Update visibility and color for a track marker lane. color is ColorToNative(r,g,b)|0x1000000
    // or 0 for default.

    bool (*REAPERAPI_FUNCNAME(SetTrackMarkerLaneInfo))(MediaTrack* track, int lane, bool visible,
                                                       int color);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // Flag is unused at present. str is passed in as beat position, tab, text, tab (example with
    // flag=2: "1.1.2\tLyric for measure 1 beat 2\t2.1.1\tLyric for measure 2 beat 1	"). See
    // GetTrackMIDILyrics
...
REAPERAPI_DEF //==============================================
    ...
    bool (*REAPERAPI_FUNCNAME(SetTrackMIDINoteName))(int track, int pitch, int chan,
                                                     const char* name);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // channel < 0 assigns note name to all channels. pitch 128 assigns name for CC0, pitch 129 for
    // CC1, etc.

    bool (*REAPERAPI_FUNCNAME(SetTrackMIDINoteNameEx))(ReaProject* proj, MediaTrack* track,
...
REAPERAPI_DEF //==============================================
    ...
    // 31=MIDI send disabled, next 5 bits=dest channel, 0=orig, 1-16=chan. &1024 for faders-send
    // MIDI vol/pan. (>>14)&255 = src bus (0 for all, 1 for normal, 2+). (>>22)&255=destination bus
    // (0 for all, 1 for normal, 2+) See CreateTrackSend, RemoveTrackSend, GetTrackNumSends.

    bool (*REAPERAPI_FUNCNAME(SetTrackSendInfo_Value))(MediaTrack* tr, int category, int sendidx,
                                                       const char* parmname, double newvalue);
#endif
...
REAPERAPI_DEF //==============================================
              // SetTrackSendUIPan
    // send_idx<0 for receives, >=0 for hw ouputs, >=nb_of_hw_ouputs for sends. isend=1 for end of
    // edit, -1 for an instant edit (such as reset), 0 for normal tweak.

    bool (*REAPERAPI_FUNCNAME(SetTrackSendUIPan))(MediaTrack* track, int send_idx, double pan,
                                                  int isend);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    bool (*REAPERAPI_FUNCNAME(SetTrackSendUIVol))(MediaTrack* track, int send_idx, double vol,
                                                  int isend);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    bool (*REAPERAPI_FUNCNAME(SetTrackStateChunk))(MediaTrack* track, const char* str,
                                                   bool isundoOptional);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // monitor: 0=no monitoring, 1=monitoring, 2=auto-monitoring. returns new value or -1 if error.
    // igngroupflags: &1 to prevent track grouping, &2 to prevent selection ganging

    int (*REAPERAPI_FUNCNAME(SetTrackUIInputMonitor))(MediaTrack* track, int monitor,
                                                      int igngroupflags);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    double (*REAPERAPI_FUNCNAME(SetTrackUIPan))(MediaTrack* track, double pan, bool relative,
                                                bool done, int igngroupflags);
#endif

#if defined(REAPERAPI_WANT_SetTrackUIPolarity) || !defined(REAPERAPI_MINIMAL)
REAPERAPI_DEF //==============================================
              // SetTrackUIPolarity
              // polarity (AKA phase): <0 toggles, 0=normal, >0=inverted. returns new value or -1 if
              // error.igngroupflags: &1 to prevent track grouping, &2 to prevent selection ganging

    int (*REAPERAPI_FUNCNAME(SetTrackUIPolarity))(MediaTrack* track, int polarity,
                                                  int igngroupflags);
#endif
...
REAPERAPI_DEF //==============================================
              // SetTrackUIRecArm
    // recarm: <0 toggles, >0 sets recarm, 0=unsets recarm. returns new value or -1 if error.
    // igngroupflags: &1 to prevent track grouping, &2 to prevent selection ganging
...
REAPERAPI_DEF //==============================================
              // SetTrackUISolo
    // solo: <0 toggles, 1 sets solo (default mode), 0=unsets solo, 2 sets solo (non-SIP), 4 sets
    // solo (SIP). returns new value or -1 if error. igngroupflags: &1 to prevent track grouping, &2
...
REAPERAPI_DEF //==============================================
    ...
    double (*REAPERAPI_FUNCNAME(SetTrackUIVolume))(MediaTrack* track, double volume, bool relative,
                                                   bool done, int igngroupflags);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    double (*REAPERAPI_FUNCNAME(SetTrackUIWidth))(MediaTrack* track, double width, bool relative,
                                                  bool done, int igngroupflags);
#endif
...
REAPERAPI_DEF //==============================================
              // ShowConsoleMsg
    // Show a message to the user (also useful for debugging). Send "\n" for newline, "" to clear
    // the console. Prefix string with "!SHOW:" and text will be added to console without opening
    // the window. See ClearConsole
...
REAPERAPI_DEF //==============================================
    ...
    // track_routing, item, ruler, envelope, envelope_point, envelope_item. ctxOptional can be a
    // track pointer for track_*, item pointer for item* (but is optional). for envelope_point,
    // ctx2Optional has point index, ctx3Optional has item index (0=main envelope, 1=first AI). for
    // envelope_item, ctx2Optional has AI index (1=first AI)

    void (*REAPERAPI_FUNCNAME(ShowPopupMenu))(const char* name, int x, int y,
...
REAPERAPI_DEF //==============================================
              // Splash_GetWnd
    // gets the splash window, in case you want to display a message over it. Returns NULL when the
    // splash window is not displayed.
...
REAPERAPI_DEF //==============================================
              // SplitMediaItem
    // the original item becomes the left-hand split, the function returns the right-hand split (or
    // NULL if the split failed)
...
REAPERAPI_DEF //==============================================
    ...
    // MIDI-as-control input queue, or sends to a MIDI hardware output. mode=0 for VKB, 1 for
    // control (actions map etc), 2 for VKB-on-current-channel; 16 for external MIDI device 0, 17
    // for external MIDI device 1, etc; see GetNumMIDIOutputs, GetMIDIOutputName.
...
REAPERAPI_DEF //==============================================
              // TakeFX_AddByName
    // Adds or queries the position of a named FX in a take. See TrackFX_AddByName() for information
    // on fxname and instantiate. FX indices can have 0x2000000 added to them, in which case they
    // will be used to address FX in containers. To address a container, the 1-based subitem is
    // multiplied by one plus the count of the FX chain and added to the 1-based container item
    // index. e.g. to address the third item in the container at the second position of the track FX
    // chain for tr, the index would be 0x2000000 + 3*(TrackFX_GetCount(tr)+1) + 2. This can be
    // extended to sub-containers using TrackFX_GetNamedConfigParm with container_count and similar
    // logic. In REAPER v7.06+, you can use the much more convenient method to navigate hierarchies,
    // see TrackFX_GetNamedConfigParm with parent_container and container_item.X.

    int (*REAPERAPI_FUNCNAME(TakeFX_AddByName))(MediaItem_Take* take, const char* fxname,
                                                int instantiate);
#endif
...
REAPERAPI_DEF //==============================================
              // TakeFX_CopyToTake
    // Copies (or moves) FX from src_take to dest_take. Can be used with src_take=dest_take to
    // reorder. FX indices can have 0x2000000 added to them, in which case they will be used to
    // address FX in containers. To address a container, the 1-based subitem is multiplied by one
    // plus the count of the FX chain and added to the 1-based container item index. e.g. to address
    // the third item in the container at the second position of the track FX chain for tr, the
    // index would be 0x2000000 + 3*(TrackFX_GetCount(tr)+1) + 2. This can be extended to
    // sub-containers using TrackFX_GetNamedConfigParm with container_count and similar logic. In
    // REAPER v7.06+, you can use the much more convenient method to navigate hierarchies, see
    // TrackFX_GetNamedConfigParm with parent_container and container_item.X.

    void (*REAPERAPI_FUNCNAME(TakeFX_CopyToTake))(MediaItem_Take* src_take, int src_fx,
...
REAPERAPI_DEF //==============================================
              // TakeFX_CopyToTrack
    // Copies (or moves) FX from src_take to dest_track. dest_fx can have 0x1000000 set to reference
    // input FX. FX indices for tracks can have 0x1000000 added to them in order to reference record
    // input FX (normal tracks) or hardware output FX (master track). FX indices can have 0x2000000
    // added to them, in which case they will be used to address FX in containers. To address a
    // container, the 1-based subitem is multiplied by one plus the count of the FX chain and added
    // to the 1-based container item index. e.g. to address the third item in the container at the
    ...
    // TrackFX_GetNamedConfigParm with container_count and similar logic. In REAPER v7.06+, you can
    // use the much more convenient method to navigate hierarchies, see TrackFX_GetNamedConfigParm
    // with parent_container and container_item.X.

    void (*REAPERAPI_FUNCNAME(TakeFX_CopyToTrack))(MediaItem_Take* src_take, int src_fx,
...
REAPERAPI_DEF //==============================================
              // TakeFX_Delete
    // Remove a FX from take chain (returns true on success) FX indices can have 0x2000000 added to
    // them, in which case they will be used to address FX in containers. To address a container,
    // the 1-based subitem is multiplied by one plus the count of the FX chain and added to the
    // 1-based container item index. e.g. to address the third item in the container at the second
    // position of the track FX chain for tr, the index would be 0x2000000 +
    // 3*(TrackFX_GetCount(tr)+1) + 2. This can be extended to sub-containers using
    // TrackFX_GetNamedConfigParm with container_count and similar logic. In REAPER v7.06+, you can
    // use the much more convenient method to navigate hierarchies, see TrackFX_GetNamedConfigParm
    // with parent_container and container_item.X.
...
REAPERAPI_DEF //==============================================
              // TakeFX_EndParamEdit
    //  FX indices can have 0x2000000 added to them, in which case they will be used to address FX
    //  in containers. To address a container, the 1-based subitem is multiplied by one plus the
    //  count of the FX chain and added to the 1-based container item index. e.g. to address the
    //  third item in the container at the second position of the track FX chain for tr, the index
    //  would be 0x2000000 + 3*(TrackFX_GetCount(tr)+1) + 2. This can be extended to sub-containers
    //  using TrackFX_GetNamedConfigParm with container_count and similar logic. In REAPER v7.06+,
    //  you can use the much more convenient method to navigate hierarchies, see
    //  TrackFX_GetNamedConfigParm with parent_container and container_item.X.
...
REAPERAPI_DEF //==============================================
    ...
    // Note: only works with FX that support Cockos VST extensions. FX indices can have 0x2000000
    // added to them, in which case they will be used to address FX in containers. To address a
    // container, the 1-based subitem is multiplied by one plus the count of the FX chain and added
    // to the 1-based container item index. e.g. to address the third item in the container at the
    // second position of the track FX chain for tr, the index would be 0x2000000 +
    ...
    bool (*REAPERAPI_FUNCNAME(TakeFX_FormatParamValue))(MediaItem_Take* take, int fx, int param,
                                                        double val, char* bufOut, int bufOut_sz);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    bool (*REAPERAPI_FUNCNAME(TakeFX_FormatParamValueNormalized))(MediaItem_Take* take, int fx,
...
REAPERAPI_DEF //==============================================
    ...
    // returns index of effect visible in chain, or -1 for chain hidden, or -2 for chain visible but
    // no effect selected
...
REAPERAPI_DEF //==============================================
              // TakeFX_GetEnabled
    // See TakeFX_SetEnabled FX indices can have 0x2000000 added to them, in which case they will be
    // used to address FX in containers. To address a container, the 1-based subitem is multiplied
    // by one plus the count of the FX chain and added to the 1-based container item index. e.g. to
    // address the third item in the container at the second position of the track FX chain for tr,
    // the index would be 0x2000000 + 3*(TrackFX_GetCount(tr)+1) + 2. This can be extended to
    // sub-containers using TrackFX_GetNamedConfigParm with container_count and similar logic. In
    // REAPER v7.06+, you can use the much more convenient method to navigate hierarchies, see
    // TrackFX_GetNamedConfigParm with parent_container and container_item.X.
...
REAPERAPI_DEF //==============================================
    ...
    // Returns the FX parameter envelope. If the envelope does not exist and create=true, the
    // envelope will be created. If the envelope already exists and is bypassed and create=true,
    // then the envelope will be unbypassed. FX indices can have 0x2000000 added to them, in which
    // case they will be used to address FX in containers. To address a container, the 1-based
    // subitem is multiplied by one plus the count of the FX chain and added to the 1-based
    // container item index. e.g. to address the third item in the container at the second position
    // of the track FX chain for tr, the index would be 0x2000000 + 3*(TrackFX_GetCount(tr)+1) + 2.
    // This can be extended to sub-containers using TrackFX_GetNamedConfigParm with container_count
    // and similar logic. In REAPER v7.06+, you can use the much more convenient method to navigate
    // hierarchies, see TrackFX_GetNamedConfigParm with parent_container and container_item.X.

    TrackEnvelope* (*REAPERAPI_FUNCNAME(TakeFX_GetEnvelope))(MediaItem_Take* take, int fxindex,
                                                             int parameterindex, bool create);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // returns HWND of floating window for effect index, if any FX indices can have 0x2000000 added
    // to them, in which case they will be used to address FX in containers. To address a container,
    // the 1-based subitem is multiplied by one plus the count of the FX chain and added to the
    // 1-based container item index. e.g. to address the third item in the container at the second
    // position of the track FX chain for tr, the index would be 0x2000000 +
    // 3*(TrackFX_GetCount(tr)+1) + 2. This can be extended to sub-containers using
...
REAPERAPI_DEF //==============================================
    ...
    bool (*REAPERAPI_FUNCNAME(TakeFX_GetFormattedParamValue))(MediaItem_Take* take, int fx,
                                                              int param, char* bufOut,
                                                              int bufOut_sz);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    bool (*REAPERAPI_FUNCNAME(TakeFX_GetFXName))(MediaItem_Take* take, int fx, char* bufOut,
                                                 int bufOut_sz);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // FX indices can have 0x2000000 added to them, in which case they will be used to address FX in
    // containers. To address a container, the 1-based subitem is multiplied by one plus the count
    // of the FX chain and added to the 1-based container item index. e.g. to address the third item
    // in the container at the second position of the track FX chain for tr, the index would be
    // 0x2000000 + 3*(TrackFX_GetCount(tr)+1) + 2. This can be extended to sub-containers using
    // TrackFX_GetNamedConfigParm with container_count and similar logic. In REAPER v7.06+, you can
    // use the much more convenient method to navigate hierarchies, see TrackFX_GetNamedConfigParm
    // with parent_container and container_item.X.

    int (*REAPERAPI_FUNCNAME(TakeFX_GetIOSize))(MediaItem_Take* take, int fx, int* inputPinsOut,
                                                int* outputPinsOut);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // TrackFX_GetNamedConfigParm FX indices can have 0x2000000 added to them, in which case they
    // will be used to address FX in containers. To address a container, the 1-based subitem is
    // multiplied by one plus the count of the FX chain and added to the 1-based container item
    // index. e.g. to address the third item in the container at the second position of the track FX
    // chain for tr, the index would be 0x2000000 + 3*(TrackFX_GetCount(tr)+1) + 2. This can be
    ...
    bool (*REAPERAPI_FUNCNAME(TakeFX_GetNamedConfigParm))(MediaItem_Take* take, int fx,
                                                          const char* parmname, char* bufOutNeedBig,
                                                          int bufOutNeedBig_sz);
#endif
...
REAPERAPI_DEF //==============================================
              // TakeFX_GetOffline
    // See TakeFX_SetOffline FX indices can have 0x2000000 added to them, in which case they will be
    // used to address FX in containers. To address a container, the 1-based subitem is multiplied
    // by one plus the count of the FX chain and added to the 1-based container item index. e.g. to
    // address the third item in the container at the second position of the track FX chain for tr,
    // the index would be 0x2000000 + 3*(TrackFX_GetCount(tr)+1) + 2. This can be extended to
...
REAPERAPI_DEF //==============================================
    ...
    // TakeFX_SetOpen FX indices can have 0x2000000 added to them, in which case they will be used
    // to address FX in containers. To address a container, the 1-based subitem is multiplied by one
    // plus the count of the FX chain and added to the 1-based container item index. e.g. to address
    // the third item in the container at the second position of the track FX chain for tr, the
    // index would be 0x2000000 + 3*(TrackFX_GetCount(tr)+1) + 2. This can be extended to
    // sub-containers using TrackFX_GetNamedConfigParm with container_count and similar logic. In
...
REAPERAPI_DEF //==============================================
    ...
    double (*REAPERAPI_FUNCNAME(TakeFX_GetParam))(MediaItem_Take* take, int fx, int param,
...
REAPERAPI_DEF //==============================================
    ...
    bool (*REAPERAPI_FUNCNAME(TakeFX_GetParameterStepSizes))(MediaItem_Take* take, int fx,
...
REAPERAPI_DEF //==============================================
    ...
    double (*REAPERAPI_FUNCNAME(TakeFX_GetParamEx))(MediaItem_Take* take, int fx, int param,
                                                    double* minvalOut, double* maxvalOut,
                                                    double* midvalOut);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // gets the parameter index from an identifying string (:wet, :bypass, or a string returned from
    // GetParamIdent), or -1 if unknown. FX indices can have 0x2000000 added to them, in which case
    // they will be used to address FX in containers. To address a container, the 1-based subitem is
    // multiplied by one plus the count of the FX chain and added to the 1-based container item
    // index. e.g. to address the third item in the container at the second position of the track FX
    // chain for tr, the index would be 0x2000000 + 3*(TrackFX_GetCount(tr)+1) + 2. This can be
    // extended to sub-containers using TrackFX_GetNamedConfigParm with container_count and similar
    ...
    int (*REAPERAPI_FUNCNAME(TakeFX_GetParamFromIdent))(MediaItem_Take* take, int fx,
...
REAPERAPI_DEF //==============================================
    ...
    // gets an identifying string for the parameter FX indices can have 0x2000000 added to them, in
    // which case they will be used to address FX in containers. To address a container, the 1-based
    // subitem is multiplied by one plus the count of the FX chain and added to the 1-based
    // container item index. e.g. to address the third item in the container at the second position
    // of the track FX chain for tr, the index would be 0x2000000 + 3*(TrackFX_GetCount(tr)+1) + 2.
    // This can be extended to sub-containers using TrackFX_GetNamedConfigParm with container_count
    ...
    bool (*REAPERAPI_FUNCNAME(TakeFX_GetParamIdent))(MediaItem_Take* take, int fx, int param,
                                                     char* bufOut, int bufOut_sz);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    bool (*REAPERAPI_FUNCNAME(TakeFX_GetParamName))(MediaItem_Take* take, int fx, int param,
                                                    char* bufOut, int bufOut_sz);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    double (*REAPERAPI_FUNCNAME(TakeFX_GetParamNormalized))(MediaItem_Take* take, int fx,
                                                            int param);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // gets the effective channel mapping bitmask for a particular pin. high32Out will be set to the
    // high 32 bits. Add 0x1000000 to pin index in order to access the second 64 bits of mappings
    // independent of the first 64 bits. FX indices can have 0x2000000 added to them, in which case
    // they will be used to address FX in containers. To address a container, the 1-based subitem is
    // multiplied by one plus the count of the FX chain and added to the 1-based container item
    // index. e.g. to address the third item in the container at the second position of the track FX
    // chain for tr, the index would be 0x2000000 + 3*(TrackFX_GetCount(tr)+1) + 2. This can be
    ...
    int (*REAPERAPI_FUNCNAME(TakeFX_GetPinMappings))(MediaItem_Take* take, int fx, int isoutput,
                                                     int pin, int* high32Out);
#endif

#if defined(REAPERAPI_WANT_TakeFX_GetPreset) || !defined(REAPERAPI_MINIMAL)
REAPERAPI_DEF //==============================================
              // TakeFX_GetPreset
    // Get the name of the preset currently showing in the REAPER dropdown, or the full path to a
    // factory preset file for VST3 plug-ins (.vstpreset). See TakeFX_SetPreset. FX indices can have
    // 0x2000000 added to them, in which case they will be used to address FX in containers. To
    // address a container, the 1-based subitem is multiplied by one plus the count of the FX chain
    // and added to the 1-based container item index. e.g. to address the third item in the
    // container at the second position of the track FX chain for tr, the index would be 0x2000000 +
    // 3*(TrackFX_GetCount(tr)+1) + 2. This can be extended to sub-containers using
    // TrackFX_GetNamedConfigParm with container_count and similar logic. In REAPER v7.06+, you can
    // use the much more convenient method to navigate hierarchies, see TrackFX_GetNamedConfigParm
    // with parent_container and container_item.X.

    bool (*REAPERAPI_FUNCNAME(TakeFX_GetPreset))(MediaItem_Take* take, int fx, char* presetnameOut,
...
REAPERAPI_DEF //==============================================
    ...
    // Returns current preset index, or -1 if error. numberOfPresetsOut will be set to total number
    // of presets available. See TakeFX_SetPresetByIndex FX indices can have 0x2000000 added to
    // them, in which case they will be used to address FX in containers. To address a container,
    // the 1-based subitem is multiplied by one plus the count of the FX chain and added to the
    // 1-based container item index. e.g. to address the third item in the container at the second
    // position of the track FX chain for tr, the index would be 0x2000000 +
    ...
    int (*REAPERAPI_FUNCNAME(TakeFX_GetPresetIndex))(MediaItem_Take* take, int fx,
...
REAPERAPI_DEF //==============================================
    ...
    void (*REAPERAPI_FUNCNAME(TakeFX_GetUserPresetFilename))(MediaItem_Take* take, int fx,
                                                             char* fnOut, int fnOut_sz);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    bool (*REAPERAPI_FUNCNAME(TakeFX_NavigatePresets))(MediaItem_Take* take, int fx,
                                                       int presetmove);
#endif
...
REAPERAPI_DEF //==============================================
              // TakeFX_SetEnabled
    // See TakeFX_GetEnabled FX indices can have 0x2000000 added to them, in which case they will be
    // used to address FX in containers. To address a container, the 1-based subitem is multiplied
    // by one plus the count of the FX chain and added to the 1-based container item index. e.g. to
    // address the third item in the container at the second position of the track FX chain for tr,
    // the index would be 0x2000000 + 3*(TrackFX_GetCount(tr)+1) + 2. This can be extended to
...
REAPERAPI_DEF //==============================================
              ...
              // gets plug-in specific named configuration value (returns true on success). see
    // TrackFX_SetNamedConfigParm FX indices can have 0x2000000 added to them, in which case they
    // will be used to address FX in containers. To address a container, the 1-based subitem is
    // multiplied by one plus the count of the FX chain and added to the 1-based container item
    // index. e.g. to address the third item in the container at the second position of the track FX
    // chain for tr, the index would be 0x2000000 + 3*(TrackFX_GetCount(tr)+1) + 2. This can be
    ...
    bool (*REAPERAPI_FUNCNAME(TakeFX_SetNamedConfigParm))(MediaItem_Take* take, int fx,
...
REAPERAPI_DEF //==============================================
              // TakeFX_SetOffline
    // See TakeFX_GetOffline FX indices can have 0x2000000 added to them, in which case they will be
    // used to address FX in containers. To address a container, the 1-based subitem is multiplied
    // by one plus the count of the FX chain and added to the 1-based container item index. e.g. to
    // address the third item in the container at the second position of the track FX chain for tr,
    // the index would be 0x2000000 + 3*(TrackFX_GetCount(tr)+1) + 2. This can be extended to
...
REAPERAPI_DEF //==============================================
              // TakeFX_SetOpen
    // Open this FX UI. See TakeFX_GetOpen FX indices can have 0x2000000 added to them, in which
    // case they will be used to address FX in containers. To address a container, the 1-based
    // subitem is multiplied by one plus the count of the FX chain and added to the 1-based
    // container item index. e.g. to address the third item in the container at the second position
    // of the track FX chain for tr, the index would be 0x2000000 + 3*(TrackFX_GetCount(tr)+1) + 2.
...
REAPERAPI_DEF //==============================================
    ...
    bool (*REAPERAPI_FUNCNAME(TakeFX_SetParam))(MediaItem_Take* take, int fx, int param,
                                                double val);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    bool (*REAPERAPI_FUNCNAME(TakeFX_SetParamNormalized))(MediaItem_Take* take, int fx, int param,
                                                          double value);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // sets the channel mapping bitmask for a particular pin. returns false if unsupported (not all
    // types of plug-ins support this capability). Add 0x1000000 to pin index in order to access the
    // second 64 bits of mappings independent of the first 64 bits. FX indices can have 0x2000000
    // added to them, in which case they will be used to address FX in containers. To address a
    // container, the 1-based subitem is multiplied by one plus the count of the FX chain and added
    // to the 1-based container item index. e.g. to address the third item in the container at the
    // second position of the track FX chain for tr, the index would be 0x2000000 +
    ...
    bool (*REAPERAPI_FUNCNAME(TakeFX_SetPinMappings))(MediaItem_Take* take, int fx, int isoutput,
...
REAPERAPI_DEF //==============================================
              // TakeFX_SetPreset
    // Activate a preset with the name shown in the REAPER dropdown. Full paths to .vstpreset files
    // are also supported for VST3 plug-ins. See TakeFX_GetPreset. FX indices can have 0x2000000
    // added to them, in which case they will be used to address FX in containers. To address a
    // container, the 1-based subitem is multiplied by one plus the count of the FX chain and added
    // to the 1-based container item index. e.g. to address the third item in the container at the
    // second position of the track FX chain for tr, the index would be 0x2000000 +
    ...
    bool (*REAPERAPI_FUNCNAME(TakeFX_SetPreset))(MediaItem_Take* take, int fx,
...
REAPERAPI_DEF //==============================================
    ...
    // Sets the preset idx, or the factory preset (idx==-2), or the default user preset (idx==-1).
    // Returns true on success. See TakeFX_GetPresetIndex. FX indices can have 0x2000000 added to
    // them, in which case they will be used to address FX in containers. To address a container,
    // the 1-based subitem is multiplied by one plus the count of the FX chain and added to the
    // 1-based container item index. e.g. to address the third item in the container at the second
    // position of the track FX chain for tr, the index would be 0x2000000 +
...
REAPERAPI_DEF //==============================================
              // TakeFX_Show
    // showflag=0 for hidechain, =1 for show chain(index valid), =2 for hide floating window(index
    // valid), =3 for show floating window (index valid) FX indices can have 0x2000000 added to
    // them, in which case they will be used to address FX in containers. To address a container,
    // the 1-based subitem is multiplied by one plus the count of the FX chain and added to the
    // 1-based container item index. e.g. to address the third item in the container at the second
    // position of the track FX chain for tr, the index would be 0x2000000 +
...
REAPERAPI_DEF //==============================================
    ...
    // Gets theme layout information. section can be 'global' for global layout override, 'seclist'
    // to enumerate a list of layout sections, otherwise a layout section such as 'mcp', 'tcp',
    // 'trans', etc. idx can be -1 to query the current value, -2 to get the description of the
    // section (if not global), -3 will return the current context DPI-scaling (256=normal,
    // 512=retina, etc), or 0..x. returns false if failed.

    bool (*REAPERAPI_FUNCNAME(ThemeLayout_GetLayout))(const char* section, int idx, char* nameOut,
                                                      int nameOut_sz);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    const char* (*REAPERAPI_FUNCNAME(ThemeLayout_GetParameter))(
        int wp, const char** descOutOptional, int* valueOutOptional, int* defValueOutOptional,
        int* minValueOutOptional, int* maxValueOutOptional);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // Sets theme layout override for a particular section -- section can be 'global' or 'mcp' etc.
...
REAPERAPI_DEF //==============================================
    ...
    // sets theme layout parameter to value. persist=true in order to have change loaded on next
    // theme load. note that the caller should update layouts via ??? to make changes visible.
...
REAPERAPI_DEF //==============================================
    ...
    double (*REAPERAPI_FUNCNAME(TimeMap2_beatsToTime))(ReaProject* proj, double tpos,
...
REAPERAPI_DEF //==============================================
    ...
    // if measures is non-NULL, measures will be set to the measure count, return value will be
    // beats since measure. if cml is non-NULL, will be set to current measure length in beats (i.e.
    // time signature numerator) if fullbeats is non-NULL, and measures is non-NULL, fullbeats will
    // get the full beat count (same value returned if measures is NULL). if cdenom is non-NULL,
    // will be set to the current time signature denominator.

    double (*REAPERAPI_FUNCNAME(TimeMap2_timeToBeats))(ReaProject* proj, double tpos,
...
REAPERAPI_DEF //==============================================
    ...
    // Get the QN position and time signature information for the start of a measure. Return the
    // time in seconds of the measure start.

    double (*REAPERAPI_FUNCNAME(TimeMap_GetMeasureInfo))(ReaProject* proj, int measure,
...
REAPERAPI_DEF //==============================================
    ...
    // Fills in a string representing the active metronome pattern. For example, in a 7/8 measure
    // divided 3+4, the pattern might be "ABCABCD". For backwards compatibility, by default the
    // function will return 1 for each primary beat and 2 for each non-primary beat, so "1221222" in
    // this example, and does not support triplets. If buf is set to "EXTENDED", the function will
    // return the full string as displayed in the pattern editor, including all beat types and
    // triplet representations. Pass in "SET:string" with a correctly formed pattern string matching
    // the current time signature numerator to set the click pattern. The time signature numerator
    // can be deduced from the returned string, and the function returns the time signature
    // denominator.

    int (*REAPERAPI_FUNCNAME(TimeMap_GetMetronomePattern))(ReaProject* proj, double time,
...
REAPERAPI_DEF //==============================================
    ...
    void (*REAPERAPI_FUNCNAME(TimeMap_GetTimeSigAtTime))(ReaProject* proj, double time,
                                                         int* timesig_numOut, int* timesig_denomOut,
                                                         double* tempoOut);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    int (*REAPERAPI_FUNCNAME(TimeMap_QNToMeasures))(ReaProject* proj, double qn,
...
REAPERAPI_DEF //==============================================
    ...
    // Converts project quarter note count (QN) to time. QN is counted from the start of the
    // project, regardless of any partial measures. See TimeMap2_QNToTime
...
REAPERAPI_DEF //==============================================
    ...
    // Converts project time position to quarter note count (QN). QN is counted from the start of
...
REAPERAPI_DEF //==============================================
              // Track_GetPeakHoldDB
    // Returns meter hold state, in dB*0.01 (0 = +0dB, -0.01 = -1dB, 0.02 = +2dB, etc). If clear is
    // set, clears the meter hold. If channel==1024 or channel==1025, returns loudness values if
    // this is the master track or this track's VU meters are set to display loudness.
...
REAPERAPI_DEF //==============================================
              // Track_GetPeakInfo
    // Returns peak meter value (1.0=+0dB, 0.0=-inf) for channel. If channel==1024 or channel==1025,
    // returns loudness values if this is the master track or this track's VU meters are set to
    // display loudness.
...
REAPERAPI_DEF //==============================================
    ...
    void (*REAPERAPI_FUNCNAME(TrackCtl_SetToolTip))(const char* fmt, int xpos, int ypos,
                                                    bool topmost);
#endif
...
REAPERAPI_DEF //==============================================
              // TrackFX_AddByName
    // Adds or queries the position of a named FX from the track FX chain (recFX=false) or record
    // input FX/monitoring FX (recFX=true, monitoring FX are on master track). Specify a negative
    // value for instantiate to always create a new effect, 0 to only query the first instance of an
    // effect, or a positive value to add an instance if one is not found. If instantiate is <=
    // -1000, it is used for the insertion position (-1000 is first item in chain, -1001 is second,
    // etc). fxname can have prefix to specify type: VST3:,VST2:,VST:,AU:,JS:, or DX:, or FXADD:
    // which adds selected items from the currently-open FX browser, FXADD:2 to limit to 2 FX added,
    // or FXADD:2e to only succeed if exactly 2 FX are selected. Returns -1 on failure or the new
    // position in chain on success. FX indices can have 0x2000000 added to them, in which case they
    // will be used to address FX in containers. To address a container, the 1-based subitem is
    // multiplied by one plus the count of the FX chain and added to the 1-based container item
    // index. e.g. to address the third item in the container at the second position of the track FX
    // chain for tr, the index would be 0x2000000 + 3*(TrackFX_GetCount(tr)+1) + 2. This can be
    ...
    int (*REAPERAPI_FUNCNAME(TrackFX_AddByName))(MediaTrack* track, const char* fxname, bool recFX,
                                                 int instantiate);
#endif
...
REAPERAPI_DEF //==============================================
              // TrackFX_CopyToTake
    // Copies (or moves) FX from src_track to dest_take. src_fx can have 0x1000000 set to reference
    // input FX. FX indices for tracks can have 0x1000000 added to them in order to reference record
    // input FX (normal tracks) or hardware output FX (master track). FX indices can have 0x2000000
    // added to them, in which case they will be used to address FX in containers. To address a
    // container, the 1-based subitem is multiplied by one plus the count of the FX chain and added
    ...
    void (*REAPERAPI_FUNCNAME(TrackFX_CopyToTake))(MediaTrack* src_track, int src_fx,
                                                   MediaItem_Take* dest_take, int dest_fx,
                                                   bool is_move);
#endif
...
REAPERAPI_DEF //==============================================
              // TrackFX_CopyToTrack
    // Copies (or moves) FX from src_track to dest_track. Can be used with src_track=dest_track to
    // reorder, FX indices have 0x1000000 set to reference input FX. FX indices for tracks can have
    // 0x1000000 added to them in order to reference record input FX (normal tracks) or hardware
    // output FX (master track). FX indices can have 0x2000000 added to them, in which case they
    // will be used to address FX in containers. To address a container, the 1-based subitem is
    // multiplied by one plus the count of the FX chain and added to the 1-based container item
    // index. e.g. to address the third item in the container at the second position of the track FX
    // chain for tr, the index would be 0x2000000 + 3*(TrackFX_GetCount(tr)+1) + 2. This can be
    ...
    void (*REAPERAPI_FUNCNAME(TrackFX_CopyToTrack))(MediaTrack* src_track, int src_fx,
                                                    MediaTrack* dest_track, int dest_fx,
                                                    bool is_move);
#endif
...
REAPERAPI_DEF //==============================================
              // TrackFX_Delete
    // Remove a FX from track chain (returns true on success) FX indices for tracks can have
    // 0x1000000 added to them in order to reference record input FX (normal tracks) or hardware
    // output FX (master track). FX indices can have 0x2000000 added to them, in which case they
    // will be used to address FX in containers. To address a container, the 1-based subitem is
    // multiplied by one plus the count of the FX chain and added to the 1-based container item
...
REAPERAPI_DEF //==============================================
    ...
    //  FX indices for tracks can have 0x1000000 added to them in order to reference record input FX
    //  (normal tracks) or hardware output FX (master track). FX indices can have 0x2000000 added to
    //  them, in which case they will be used to address FX in containers. To address a container,
    //  the 1-based subitem is multiplied by one plus the count of the FX chain and added to the
    //  1-based container item index. e.g. to address the third item in the container at the second
    //  position of the track FX chain for tr, the index would be 0x2000000 +
...
REAPERAPI_DEF //==============================================
    ...
    // Note: only works with FX that support Cockos VST extensions. FX indices for tracks can have
    // 0x1000000 added to them in order to reference record input FX (normal tracks) or hardware
    // output FX (master track). FX indices can have 0x2000000 added to them, in which case they
    // will be used to address FX in containers. To address a container, the 1-based subitem is
    // multiplied by one plus the count of the FX chain and added to the 1-based container item
    ...
    bool (*REAPERAPI_FUNCNAME(TrackFX_FormatParamValue))(MediaTrack* track, int fx, int param,
                                                         double val, char* bufOut, int bufOut_sz);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    bool (*REAPERAPI_FUNCNAME(TrackFX_FormatParamValueNormalized))(MediaTrack* track, int fx,
                                                                   int param, double value,
                                                                   char* buf, int buf_sz);
#endif
...
REAPERAPI_DEF //==============================================
              // TrackFX_GetByName
    // Get the index of the first track FX insert that matches fxname. If the FX is not in the chain
    // and instantiate is true, it will be inserted. See TrackFX_GetInstrument, TrackFX_GetEQ.
    // Deprecated in favor of TrackFX_AddByName.

    int (*REAPERAPI_FUNCNAME(TrackFX_GetByName))(MediaTrack* track, const char* fxname,
                                                 bool instantiate);
#endif
...
REAPERAPI_DEF //==============================================
              // TrackFX_GetEnabled
    // See TrackFX_SetEnabled FX indices for tracks can have 0x1000000 added to them in order to
    // reference record input FX (normal tracks) or hardware output FX (master track). FX indices
    // can have 0x2000000 added to them, in which case they will be used to address FX in
    // containers. To address a container, the 1-based subitem is multiplied by one plus the count
    // of the FX chain and added to the 1-based container item index. e.g. to address the third item
    // in the container at the second position of the track FX chain for tr, the index would be
    // 0x2000000 + 3*(TrackFX_GetCount(tr)+1) + 2. This can be extended to sub-containers using
...
REAPERAPI_DEF //==============================================
              // TrackFX_GetEQ
    // Get the index of ReaEQ in the track FX chain. If ReaEQ is not in the chain and instantiate is
    // true, it will be inserted. See TrackFX_GetInstrument, TrackFX_GetByName.
...
REAPERAPI_DEF //==============================================
    ...
    // 6=bandpass, 7=parallel bandpass. Bandidx (ignored for master gain): 0=target first band
    // matching bandtype, 1=target 2nd band matching bandtype, etc.
    //
    // See TrackFX_GetEQ, TrackFX_GetEQParam, TrackFX_SetEQParam, TrackFX_SetEQBandEnabled. FX
    // indices for tracks can have 0x1000000 added to them in order to reference record input FX
    // (normal tracks) or hardware output FX (master track). FX indices can have 0x2000000 added to
    // them, in which case they will be used to address FX in containers. To address a container,
    // the 1-based subitem is multiplied by one plus the count of the FX chain and added to the
    // 1-based container item index. e.g. to address the third item in the container at the second
    ...
    bool (*REAPERAPI_FUNCNAME(TrackFX_GetEQBandEnabled))(MediaTrack* track, int fxidx, int bandtype,
                                                         int bandidx);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // Bandtype: -1=master gain, 0=hipass, 1=loshelf, 2=band, 3=notch, 4=hishelf, 5=lopass,
    // 6=bandpass, 7=parallel bandpass. Bandidx (ignored for master gain): 0=target first band
    // matching bandtype, 1=target 2nd band matching bandtype, etc. Paramtype (ignored for master
    // gain): 0=freq, 1=gain, 2=Q. See TrackFX_GetEQ, TrackFX_SetEQParam, TrackFX_GetEQBandEnabled,
    // TrackFX_SetEQBandEnabled. FX indices for tracks can have 0x1000000 added to them in order to
    // reference record input FX (normal tracks) or hardware output FX (master track). FX indices
    // can have 0x2000000 added to them, in which case they will be used to address FX in
    // containers. To address a container, the 1-based subitem is multiplied by one plus the count
    // of the FX chain and added to the 1-based container item index. e.g. to address the third item
    ...
    bool (*REAPERAPI_FUNCNAME(TrackFX_GetEQParam))(MediaTrack* track, int fxidx, int paramidx,
...
REAPERAPI_DEF //==============================================
    ...
    // returns HWND of floating window for effect index, if any FX indices for tracks can have
    // 0x1000000 added to them in order to reference record input FX (normal tracks) or hardware
    // output FX (master track). FX indices can have 0x2000000 added to them, in which case they
    // will be used to address FX in containers. To address a container, the 1-based subitem is
    // multiplied by one plus the count of the FX chain and added to the 1-based container item
...
REAPERAPI_DEF //==============================================
    ...
    bool (*REAPERAPI_FUNCNAME(TrackFX_GetFormattedParamValue))(MediaTrack* track, int fx, int param,
                                                               char* bufOut, int bufOut_sz);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    bool (*REAPERAPI_FUNCNAME(TrackFX_GetFXName))(MediaTrack* track, int fx, char* bufOut,
                                                  int bufOut_sz);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // TrackFX_GetEQ, TrackFX_GetByName.
...
REAPERAPI_DEF //==============================================
    ...
    int (*REAPERAPI_FUNCNAME(TrackFX_GetIOSize))(MediaTrack* track, int fx, int* inputPinsOut,
                                                 int* outputPinsOut);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // param.X.container_map.delete : read this value in order to remove the mapping for this
    // parameter container_map.add : read from this value to add a new container parameter mapping
    // -- will return new parameter index (accessed via param.X.container_map.*)
    // container_map.add.FXID.PARMIDX : read from this value to add/get container parameter mapping
    // for FXID/PARMIDX -- will return the parameter index (accessed via param.X.container_map.*).
    // FXID can be a full address (must be a child of the container) or a 0-based sub-index
    // (v7.06+). container_map.get.FXID.PARMIDX : read from this value to get container parameter
    // mapping for FXID/PARMIDX -- will return the parameter index (accessed via
    // param.X.container_map.*). FXID can be a full address (must be a child of the container) or a
    // 0-based sub-index (v7.06+). chain_pdc_actual : returns the actual chain latency in samples,
    // only valid after playback has commenced, may be rounded up to block size. chain_pdc_reporting
    // : returns the reported chain latency, always valid, not rounded to block size.
    //
    //
    ...
    // param.X.lfo.[active,dir,phase,speed,strength,temposync,free,shape] : parameter moduation LFO
    // state param.X.acs.[active,dir,strength,attack,release,dblo,dbhi,chan,stereo,x2,y2] :
    ...
    // param.X.learn.mode : absolution/relative mode flag (0: Absolute, 1: 127=-1,1=+1, 2: 63=-1,
    // 65=+1, 3: 65=-1, 1=+1, 4: toggle if nonzero) param.X.learn.flags : &1=selected track only,
    // &2=soft takeover, &4=focused FX only, &8=LFO retrigger, &16=visible FX only
    ...
    // ITEMx : [ReaVerb] state configuration line, when writing should be followed by a write of
    // DONE FILE, FILEx, -FILEx, +FILEx, -FILE* : [RS5k] file list, -/+ prefixes are write-only,
    // when writing any, should be followed by a write of DONE MODE, RSMODE : [RS5k] general mode,
    // resample mode VIDEO_CODE : [video processor] code force_auto_bypass : 0 or 1 - force
    // auto-bypass plug-in on silence parallel : 0, 1 or 2 - 1=process plug-in in parallel with
    // previous, 2=process plug-in parallel and merge MIDI instance_oversample_shift : instance
    // oversampling shift amount, 0=none, 1=~96k, 2=~192k, etc. When setting requires playback
    // stop/start to take effect chain_oversample_shift : chain oversampling shift amount, 0=none,
    // 1=~96k, 2=~192k, etc. When setting requires playback stop/start to take effect chain_pdc_mode
    // : chain PDC mode (0=classic, 1=new-default, 2=ignore PDC, 3=hwcomp-master) chain_sel :
    // selected/visible FX in chain renamed_name : renamed FX instance name (empty string = not
    // renamed) container_nch : number of internal channels for container container_nch_in : number
    // of input pins for container container_nch_out : number of output pints for container
    // container_nch_feedback : number of internal feedback channels enabled in container
    // focused : reading returns 1 if focused. Writing a positive value to this sets the FX UI as
    // "last focused." last_touched : reading returns two integers, one indicates whether FX is the
    // last-touched FX, the second indicates which parameter was last touched. Writing a negative
    // value ensures this plug-in is not set as last touched, otherwise the FX is set "last
    // touched," and last touched parameter index is set to the value in the string (if valid).
    //  FX indices for tracks can have 0x1000000 added to them in order to reference record input FX
    //  (normal tracks) or hardware output FX (master track). FX indices can have 0x2000000 added to
    //  them, in which case they will be used to address FX in containers. To address a container,
    //  the 1-based subitem is multiplied by one plus the count of the FX chain and added to the
    ...
    bool (*REAPERAPI_FUNCNAME(TrackFX_GetNamedConfigParm))(MediaTrack* track, int fx,
                                                           const char* parmname,
                                                           char* bufOutNeedBig,
                                                           int bufOutNeedBig_sz);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // Adds or queries the position of a named MIDI effect in the track MIDI FX chain, which
    // processes events before instrument FX. Specify instantiate semantics as with
    // TrackFX_AddByName. Returns -1 on failure or the new position in chain on success.

    int (*REAPERAPI_FUNCNAME(TrackMIDI_FX_AddByName))(MediaTrack* track, const char* fxname,
                                                      int instantiate);
#endif
...
REAPERAPI_DEF //==============================================
              // TrackFX_GetOffline
    // See TrackFX_SetOffline FX indices for tracks can have 0x1000000 added to them in order to
    // reference record input FX (normal tracks) or hardware output FX (master track). FX indices
    // can have 0x2000000 added to them, in which case they will be used to address FX in
    // containers. To address a container, the 1-based subitem is multiplied by one plus the count
    // of the FX chain and added to the 1-based container item index. e.g. to address the third item
...
REAPERAPI_DEF //==============================================
              // TrackFX_GetOpen
              // Returns true if this FX UI is open in the FX chain window or a floating window. See
    // TrackFX_SetOpen FX indices for tracks can have 0x1000000 added to them in order to reference
    // record input FX (normal tracks) or hardware output FX (master track). FX indices can have
    // 0x2000000 added to them, in which case they will be used to address FX in containers. To
    // address a container, the 1-based subitem is multiplied by one plus the count of the FX chain
    // and added to the 1-based container item index. e.g. to address the third item in the
    // container at the second position of the track FX chain for tr, the index would be 0x2000000 +
...
REAPERAPI_DEF //==============================================
    ...
    double (*REAPERAPI_FUNCNAME(TrackFX_GetParam))(MediaTrack* track, int fx, int param,
                                                   double* minvalOut, double* maxvalOut);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    bool (*REAPERAPI_FUNCNAME(TrackFX_GetParameterStepSizes))(MediaTrack* track, int fx, int param,
...
REAPERAPI_DEF //==============================================
    ...
    double (*REAPERAPI_FUNCNAME(TrackFX_GetParamEx))(MediaTrack* track, int fx, int param,
                                                     double* minvalOut, double* maxvalOut,
                                                     double* midvalOut);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // gets the parameter index from an identifying string (:wet, :bypass, :delta, or a string
    // returned from GetParamIdent), or -1 if unknown. FX indices for tracks can have 0x1000000
    // added to them in order to reference record input FX (normal tracks) or hardware output FX
    // (master track). FX indices can have 0x2000000 added to them, in which case they will be used
    // to address FX in containers. To address a container, the 1-based subitem is multiplied by one
    // plus the count of the FX chain and added to the 1-based container item index. e.g. to address
    // the third item in the container at the second position of the track FX chain for tr, the
    // index would be 0x2000000 + 3*(TrackFX_GetCount(tr)+1) + 2. This can be extended to
    ...
    int (*REAPERAPI_FUNCNAME(TrackFX_GetParamFromIdent))(MediaTrack* track, int fx,
                                                         const char* ident_str);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // gets an identifying string for the parameter FX indices for tracks can have 0x1000000 added
    // to them in order to reference record input FX (normal tracks) or hardware output FX (master
    // track). FX indices can have 0x2000000 added to them, in which case they will be used to
    // address FX in containers. To address a container, the 1-based subitem is multiplied by one
    // plus the count of the FX chain and added to the 1-based container item index. e.g. to address
    // the third item in the container at the second position of the track FX chain for tr, the
    // index would be 0x2000000 + 3*(TrackFX_GetCount(tr)+1) + 2. This can be extended to
    ...
    bool (*REAPERAPI_FUNCNAME(TrackFX_GetParamIdent))(MediaTrack* track, int fx, int param,
                                                      char* bufOut, int bufOut_sz);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    bool (*REAPERAPI_FUNCNAME(TrackFX_GetParamName))(MediaTrack* track, int fx, int param,
                                                     char* bufOut, int bufOut_sz);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // gets the effective channel mapping bitmask for a particular pin. high32Out will be set to the
    // high 32 bits. Add 0x1000000 to pin index in order to access the second 64 bits of mappings
    // independent of the first 64 bits. FX indices for tracks can have 0x1000000 added to them in
    // order to reference record input FX (normal tracks) or hardware output FX (master track). FX
    // indices can have 0x2000000 added to them, in which case they will be used to address FX in
    // containers. To address a container, the 1-based subitem is multiplied by one plus the count
    // of the FX chain and added to the 1-based container item index. e.g. to address the third item
    // in the container at the second position of the track FX chain for tr, the index would be
    // 0x2000000 + 3*(TrackFX_GetCount(tr)+1) + 2. This can be extended to sub-containers using
    ...
    int (*REAPERAPI_FUNCNAME(TrackFX_GetPinMappings))(MediaTrack* tr, int fx, int isoutput, int pin,
                                                      int* high32Out);
#endif
...
REAPERAPI_DEF //==============================================
              // TrackFX_GetPreset
    // Get the name of the preset currently showing in the REAPER dropdown, or the full path to a
    // factory preset file for VST3 plug-ins (.vstpreset). See TrackFX_SetPreset. FX indices for
    // tracks can have 0x1000000 added to them in order to reference record input FX (normal tracks)
    // or hardware output FX (master track). FX indices can have 0x2000000 added to them, in which
    // case they will be used to address FX in containers. To address a container, the 1-based
    // subitem is multiplied by one plus the count of the FX chain and added to the 1-based
    // container item index. e.g. to address the third item in the container at the second position
    // of the track FX chain for tr, the index would be 0x2000000 + 3*(TrackFX_GetCount(tr)+1) + 2.
    ...
    bool (*REAPERAPI_FUNCNAME(TrackFX_GetPreset))(MediaTrack* track, int fx, char* presetnameOut,
                                                  int presetnameOut_sz);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // Returns current preset index, or -1 if error. numberOfPresetsOut will be set to total number
    // of presets available. See TrackFX_SetPresetByIndex FX indices for tracks can have 0x1000000
    // added to them in order to reference record input FX (normal tracks) or hardware output FX
    // (master track). FX indices can have 0x2000000 added to them, in which case they will be used
    // to address FX in containers. To address a container, the 1-based subitem is multiplied by one
    // plus the count of the FX chain and added to the 1-based container item index. e.g. to address
    ...
    int (*REAPERAPI_FUNCNAME(TrackFX_GetPresetIndex))(MediaTrack* track, int fx,
                                                      int* numberOfPresetsOut);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // returns index of effect visible in record input chain, or -1 for chain hidden, or -2 for
...
REAPERAPI_DEF //==============================================
    ...
    // [0x1000000..0x1000000+n). On the master track, this accesses monitoring FX rather than record
    // input FX.
...
REAPERAPI_DEF //==============================================
    ...
    void (*REAPERAPI_FUNCNAME(TrackFX_GetUserPresetFilename))(MediaTrack* track, int fx,
                                                              char* fnOut, int fnOut_sz);
#endif
...
REAPERAPI_DEF //==============================================
              // TrackFX_SetEnabled
    // See TrackFX_GetEnabled FX indices for tracks can have 0x1000000 added to them in order to
    // reference record input FX (normal tracks) or hardware output FX (master track). FX indices
    // can have 0x2000000 added to them, in which case they will be used to address FX in
    // containers. To address a container, the 1-based subitem is multiplied by one plus the count
    // of the FX chain and added to the 1-based container item index. e.g. to address the third item
...
REAPERAPI_DEF //==============================================
    ...
    // matching bandtype, 1=target 2nd band matching bandtype, etc.
    //
    // See TrackFX_GetEQ, TrackFX_GetEQParam, TrackFX_SetEQParam, TrackFX_GetEQBandEnabled. FX
    // indices for tracks can have 0x1000000 added to them in order to reference record input FX
    // (normal tracks) or hardware output FX (master track). FX indices can have 0x2000000 added to
    // them, in which case they will be used to address FX in containers. To address a container,
    // the 1-based subitem is multiplied by one plus the count of the FX chain and added to the
    ...
    bool (*REAPERAPI_FUNCNAME(TrackFX_SetEQBandEnabled))(MediaTrack* track, int fxidx, int bandtype,
...
REAPERAPI_DEF //==============================================
    ...
    // 6=bandpass, 7=parallel bandpass. Bandidx (ignored for master gain): 0=target first band
    // matching bandtype, 1=target 2nd band matching bandtype, etc. Paramtype (ignored for master
    // gain): 0=freq, 1=gain, 2=Q. See TrackFX_GetEQ, TrackFX_GetEQParam, TrackFX_GetEQBandEnabled,
    // TrackFX_SetEQBandEnabled. FX indices for tracks can have 0x1000000 added to them in order to
    // reference record input FX (normal tracks) or hardware output FX (master track). FX indices
    // can have 0x2000000 added to them, in which case they will be used to address FX in
    // containers. To address a container, the 1-based subitem is multiplied by one plus the count
    ...
    bool (*REAPERAPI_FUNCNAME(TrackFX_SetEQParam))(MediaTrack* track, int fxidx, int bandtype,
...
REAPERAPI_DEF //==============================================
    ...
    bool (*REAPERAPI_FUNCNAME(TrackFX_SetNamedConfigParm))(MediaTrack* track, int fx,
                                                           const char* parmname, const char* value);
#endif
...
REAPERAPI_DEF //==============================================
              // TrackFX_SetOffline
    // See TrackFX_GetOffline FX indices for tracks can have 0x1000000 added to them in order to
    // reference record input FX (normal tracks) or hardware output FX (master track). FX indices
    // can have 0x2000000 added to them, in which case they will be used to address FX in
    // containers. To address a container, the 1-based subitem is multiplied by one plus the count
    // of the FX chain and added to the 1-based container item index. e.g. to address the third item
...
REAPERAPI_DEF //==============================================
              // TrackFX_SetOpen
    // Open this FX UI. See TrackFX_GetOpen FX indices for tracks can have 0x1000000 added to them
    // in order to reference record input FX (normal tracks) or hardware output FX (master track).
    // FX indices can have 0x2000000 added to them, in which case they will be used to address FX in
    // containers. To address a container, the 1-based subitem is multiplied by one plus the count
    // of the FX chain and added to the 1-based container item index. e.g. to address the third item
    // in the container at the second position of the track FX chain for tr, the index would be
...
REAPERAPI_DEF //==============================================
    ...
    bool (*REAPERAPI_FUNCNAME(TrackFX_SetParamNormalized))(MediaTrack* track, int fx, int param,
                                                           double value);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // sets the channel mapping bitmask for a particular pin. returns false if unsupported (not all
    // types of plug-ins support this capability). Add 0x1000000 to pin index in order to access the
    // second 64 bits of mappings independent of the first 64 bits. FX indices for tracks can have
    // 0x1000000 added to them in order to reference record input FX (normal tracks) or hardware
    // output FX (master track). FX indices can have 0x2000000 added to them, in which case they
    // will be used to address FX in containers. To address a container, the 1-based subitem is
    // multiplied by one plus the count of the FX chain and added to the 1-based container item
    ...
    bool (*REAPERAPI_FUNCNAME(TrackFX_SetPinMappings))(MediaTrack* tr, int fx, int isoutput,
                                                       int pin, int low32bits, int hi32bits);
#endif
...
REAPERAPI_DEF //==============================================
              // TrackFX_SetPreset
    // Activate a preset with the name shown in the REAPER dropdown. Full paths to .vstpreset files
    // are also supported for VST3 plug-ins. See TrackFX_GetPreset. FX indices for tracks can have
    // 0x1000000 added to them in order to reference record input FX (normal tracks) or hardware
    // output FX (master track). FX indices can have 0x2000000 added to them, in which case they
    // will be used to address FX in containers. To address a container, the 1-based subitem is
    // multiplied by one plus the count of the FX chain and added to the 1-based container item
    ...
    bool (*REAPERAPI_FUNCNAME(TrackFX_SetPreset))(MediaTrack* track, int fx,
                                                  const char* presetname);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // Sets the preset idx, or the factory preset (idx==-2), or the default user preset (idx==-1).
    // Returns true on success. See TrackFX_GetPresetIndex. FX indices for tracks can have 0x1000000
    // added to them in order to reference record input FX (normal tracks) or hardware output FX
    // (master track). FX indices can have 0x2000000 added to them, in which case they will be used
    // to address FX in containers. To address a container, the 1-based subitem is multiplied by one
    // plus the count of the FX chain and added to the 1-based container item index. e.g. to address
...
REAPERAPI_DEF //==============================================
              // TrackFX_Show
    // showflag=0 for hidechain, =1 for show chain(index valid), =2 for hide floating window(index
    // valid), =3 for show floating window (index valid) FX indices for tracks can have 0x1000000
    // added to them in order to reference record input FX (normal tracks) or hardware output FX
    // (master track). FX indices can have 0x2000000 added to them, in which case they will be used
    // to address FX in containers. To address a container, the 1-based subitem is multiplied by one
    // plus the count of the FX chain and added to the 1-based container item index. e.g. to address
...
REAPERAPI_DEF //==============================================
    ...
    void (*REAPERAPI_FUNCNAME(Undo_EndBlock2))(ReaProject* proj, const char* descchange,
                                               int extraflags);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    void (*REAPERAPI_FUNCNAME(Undo_OnStateChange_Item))(ReaProject* proj, const char* name,
                                                        MediaItem* item);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    void (*REAPERAPI_FUNCNAME(Undo_OnStateChangeEx))(const char* descchange, int whichStates,
                                                     int trackparm);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    void (*REAPERAPI_FUNCNAME(Undo_OnStateChangeEx2))(ReaProject* proj, const char* descchange,
...
REAPERAPI_DEF //==============================================
    ...
    void (*REAPERAPI_FUNCNAME(RegisterTimecodeInput))(void (*cb)(const char* frame,
                                                                 double seconds));
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // Updates disk I/O statistics with bytes transferred since last call. notify REAPER of a write
    // error by calling with readamt=0, writeamt=-101010110 for unknown or -101010111 for disk full
...
REAPERAPI_DEF //==============================================
              // UpdateItemLanes
    // Recalculate lane arrangement for fixed lane tracks, including auto-removing empty lanes at
...
REAPERAPI_DEF //==============================================
              // ValidatePtr2
    // Return true if the pointer is a valid object of the right type in proj (proj is ignored if
    // pointer is itself a project). Supported types are: ReaProject*, MediaTrack*, MediaItem*,
    // MediaItem_Take*, TrackEnvelope* and PCM_source*.

    bool (*REAPERAPI_FUNCNAME(ValidatePtr2))(ReaProject* proj, void* pointer,
...
#endif
#if defined(REAPERAPI_WANT_ARA_AttachProcessorToMediaItem) || !defined(REAPERAPI_MINIMAL)
      {(void**)&REAPERAPI_FUNCNAME(ARA_AttachProcessorToMediaItem),
       "ARA_AttachProcessorToMediaItem"},
#endif
#if defined(REAPERAPI_WANT_ARA_DetachProcessorFromMediaItem) || !defined(REAPERAPI_MINIMAL)
      {(void**)&REAPERAPI_FUNCNAME(ARA_DetachProcessorFromMediaItem),
       "ARA_DetachProcessorFromMediaItem"},
#endif
#if defined(REAPERAPI_WANT_ApplyNudge) || !defined(REAPERAPI_MINIMAL)
...
#endif
#if defined(REAPERAPI_WANT_GetSetAutomationItemInfo_String) || !defined(REAPERAPI_MINIMAL)
      {(void**)&REAPERAPI_FUNCNAME(GetSetAutomationItemInfo_String),
       "GetSetAutomationItemInfo_String"},
#endif
#if defined(REAPERAPI_WANT_GetSetEnvelopeInfo_String) || !defined(REAPERAPI_MINIMAL)
...
#endif
#if defined(REAPERAPI_WANT_GetSetMediaItemTakeInfo_String) || !defined(REAPERAPI_MINIMAL)
      {(void**)&REAPERAPI_FUNCNAME(GetSetMediaItemTakeInfo_String),
       "GetSetMediaItemTakeInfo_String"},
#endif
#if defined(REAPERAPI_WANT_GetSetMediaTrackInfo) || !defined(REAPERAPI_MINIMAL)
...
#endif
#if defined(REAPERAPI_WANT_GetSetTrackGroupMembershipHigh) || !defined(REAPERAPI_MINIMAL)
      {(void**)&REAPERAPI_FUNCNAME(GetSetTrackGroupMembershipHigh),
       "GetSetTrackGroupMembershipHigh"},
#endif
#if defined(REAPERAPI_WANT_GetSetTrackMIDISupportFile) || !defined(REAPERAPI_MINIMAL)
...
#endif
#if defined(REAPERAPI_WANT_GetToggleCommandStateThroughHooks) || !defined(REAPERAPI_MINIMAL)
      {(void**)&REAPERAPI_FUNCNAME(GetToggleCommandStateThroughHooks),
       "GetToggleCommandStateThroughHooks"},
#endif
#if defined(REAPERAPI_WANT_GetTooltipWindow) || !defined(REAPERAPI_MINIMAL)
...
#endif
#if defined(REAPERAPI_WANT_MediaExplorerGetLastPlayedFileInfo) || !defined(REAPERAPI_MINIMAL)
      {(void**)&REAPERAPI_FUNCNAME(MediaExplorerGetLastPlayedFileInfo),
       "MediaExplorerGetLastPlayedFileInfo"},
#endif
#if defined(REAPERAPI_WANT_MediaItemDescendsFromTrack) || !defined(REAPERAPI_MINIMAL)
...
#endif
#if defined(REAPERAPI_WANT_MIDIEditor_LastFocused_OnCommand) || !defined(REAPERAPI_MINIMAL)
      {(void**)&REAPERAPI_FUNCNAME(MIDIEditor_LastFocused_OnCommand),
       "MIDIEditor_LastFocused_OnCommand"},
#endif
#if defined(REAPERAPI_WANT_MIDIEditor_OnCommand) || !defined(REAPERAPI_MINIMAL)
...
#endif
#if defined(REAPERAPI_WANT_plugin_getImportableProjectFilterList) || !defined(REAPERAPI_MINIMAL)
      {(void**)&REAPERAPI_FUNCNAME(plugin_getImportableProjectFilterList),
       "plugin_getImportableProjectFilterList"},
#endif
#if defined(REAPERAPI_WANT_plugin_register) || !defined(REAPERAPI_MINIMAL)
...
#endif
#if defined(REAPERAPI_WANT_TakeFX_FormatParamValueNormalized) || !defined(REAPERAPI_MINIMAL)
      {(void**)&REAPERAPI_FUNCNAME(TakeFX_FormatParamValueNormalized),
       "TakeFX_FormatParamValueNormalized"},
#endif
#if defined(REAPERAPI_WANT_TakeFX_GetChainVisible) || !defined(REAPERAPI_MINIMAL)
...
#endif
#if defined(REAPERAPI_WANT_TrackFX_FormatParamValueNormalized) || !defined(REAPERAPI_MINIMAL)
      {(void**)&REAPERAPI_FUNCNAME(TrackFX_FormatParamValueNormalized),
       "TrackFX_FormatParamValueNormalized"},
#endif
#if defined(REAPERAPI_WANT_TrackFX_GetByName) || !defined(REAPERAPI_MINIMAL)
...
#endif
#if defined(REAPERAPI_WANT_TrackFX_GetFormattedParamValue) || !defined(REAPERAPI_MINIMAL)
      {(void**)&REAPERAPI_FUNCNAME(TrackFX_GetFormattedParamValue),
       "TrackFX_GetFormattedParamValue"},
#endif
#if defined(REAPERAPI_WANT_TrackFX_GetFXGUID) || !defined(REAPERAPI_MINIMAL)
