...
inline bool getBinary(const char* fn, const char* section, const char* key, void* data,
                      unsigned int len) {
  return GetPrivateProfileStruct(section, key, data, len, fn) != 0;
}
inline unsigned int getString(const char* fn, const char* section, const char* key, char* value,
                              unsigned int value_len, const char* def_value = nullptr) {
  return (unsigned int)GetPrivateProfileString(section, key, def_value ? def_value : "", value,
                                               value_len, fn);
}
...
inline void setBinary(const char* fn, const char* section, const char* key, const void* data,
                      unsigned int len) {
...
static inline std::string readValue(const std::filesystem::path& path, const std::string& section,
...
static inline void writeValue(const std::filesystem::path& path, const std::string& section,
...
inline bool getBinary(const char* fn, const char* section, const char* key, void* data,
                      unsigned int len) {
  std::string v = readValue(std::filesystem::path(fn), section, key);
  if (v.empty() || v.length() < len * 2)
    return false;
  unsigned char* out = (unsigned char*)data;
  for (unsigned int i = 0; i < len; ++i) {
    unsigned int byte;
    if (sscanf(v.c_str() + i * 2, "%02x", &byte) != 1)
      return false;
