 ...
 * dispatcher will be called with opcode=effVendorSpecific, index=effEditDraw, value=parm2,
 * ptr=(void*)(INT_PTR)parm3, opt=message (REAPER_FXEMBED_WM_*)
 *
 * to support via VST3: IEditController should support IReaperUIEmbedInterface, see
 * reaper_vst3_interfaces.h
 *
 * to support via ARA2: the plug-in's ARA factory should expose IReaperARAEmbedInterface (defined
 * below)
 *
...
#define REAPER_FXEMBED_DRAWINFO_FLAG_IS_RETINA                                                     \
  0x00100 /* 7.36+/macOS, implies width/height, mouse_x/mouse_y are doubled vs screen coordinates  \
           */
 ...
 *
 * if flags has REAPER_FXEMBED_DRAWINFO_FLAG_PAINT_OPTIONAL set, update is optional. if no change
 * since last draw, return 0. if flags has REAPER_FXEMBED_DRAWINFO_FLAG_LBUTTON_CAPTURED set, left
 * mouse button is down and captured if flags has REAPER_FXEMBED_DRAWINFO_FLAG_RBUTTON_CAPTURED set,
 * right mouse button is down and captured
 *
 * HiDPI:
 * if REAPER_FXEMBED_IBitmap::Extended(REAPER_FXEMBED_EXT_GET_ADVISORY_SCALING,NULL) returns
 * nonzero, then it is a 24.8 scalefactor for UI drawing this is the same value as
 * REAPER_FXEMBED_DrawInfo::dpi
 *
 * return 1 if drawing occurred, 0 otherwise.
 *
 */

#define REAPER_FXEMBED_WM_SETCURSOR                                                                \
  0x0020 // parm3: REAPER_FXEMBED_DrawInfo*. set mouse cursor and return
...
typedef struct REAPER_FXEMBED_SizeHints { // alias to MINMAXINFO
  ...
  int dpi; // DPI scaling - 0=unknown (REAPER v7.35 and earlier), otherwise 256=100%. this excludes
           // any retina drawing
 ...
 *
 */
#define REAPER_FXEMBED_RGBA(r, g, b, a)                                                            \
...
public:
  ...
  virtual int getRowSpan() = 0; // includes any off-bitmap data. this is in sizeof(unsigned int)
                                // units, not bytes.
...
#define REAPER_FXEMBED_EXT_GET_ADVISORY_SCALING                                                    \
  0x2003 // data ignored, returns .8 fixed point. returns 0 if unscaled
