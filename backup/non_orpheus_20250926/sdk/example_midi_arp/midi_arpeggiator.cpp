...
public:
  SimpleArp() {}
  MIDI_effect* Duplicate() override {
    return new SimpleArp();
  }
  ...
  int PropertiesWindow(HWND) override {
    return 0;
  }
  void Process(MIDI_effect_transfer_t* block) override {
    if (!block || !block->input_events || !block->output_events)
      return;
    int pos = 0;
    MIDI_event_t* evt;
  ...
  int LoadState(const char*, ProjectStateContext*) override {
    return 0;
  }
  int Extended(int, void*, void*, void*) override {
    return 0;
  }
};

static const char* ArpName() {
  return "Example Arpeggiator";
}
static MIDI_effect* CreateArp() {
  return new SimpleArp();
}
