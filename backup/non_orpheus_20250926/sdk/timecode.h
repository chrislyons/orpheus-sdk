...
enum class FrameRate { FPS24 = 24, FPS25 = 25, FPS30 = 30, FPS30_DROP = 29 };
...
inline double SyncToIncoming(const Frame& tc) {
  return ToSeconds(tc);
}
inline Frame SyncToOutgoing(double position, FrameRate rate) {
  return FromSeconds(position, rate);
}
