  ...
  for (int i = 0; i < itemCount; ++i) {
    ...
    for (int t = 0; t < takeCount; ++t) {
      ...
      if (buf && MIDI_GetAllEvts(take, buf, &buf_sz)) {
        ...
        int ppq = 0;
        while (pos < buf_sz) {
          int offset = *(int*)(buf + pos);
          pos += 4;
          unsigned char flags = (unsigned char)buf[pos++];
          (void)flags;
          ...
          if (msglen >= 3 && (msg[0] & 0xF0) == 0x90 && msg[2] > 0) {
            int pitch = msg[1];
            static const char* steps[12] = {"C",  "C#", "D",  "D#", "E",  "F",
                                            "F#", "G",  "G#", "A",  "A#", "B"};
...
static bool DoImportTrackMIDIFromMusicXML(MediaTrack* tr, const char* fn) {
  ...
  while (std::fgets(line, sizeof(line), f)) {
    ...
    if (inNote) {
      char* p;
      if ((p = std::strstr(line, "<step>")))
        step = p[6];
      if ((p = std::strstr(line, "<alter>")))
        alter = std::atoi(p + 7);
      if ((p = std::strstr(line, "<octave>")))
        octave = std::atoi(p + 8);
      if (std::strstr(line, "</note>")) {
        int base;
        switch (step) {
