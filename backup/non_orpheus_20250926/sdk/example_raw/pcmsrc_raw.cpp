...
public:
  ...
  void GetInfoString(char* buf, int buflen, char* title, int titlelen) {
    ...
    if (IsOpen()) {
      ...
      lstrcpyn(buf, "Media offline", buflen);
    else if (m_fh)
      lstrcpyn(buf, "Error initializing decoder", buflen);
    else
      lstrcpyn(buf, "Error opening file", buflen);
  }

  bool IsOpen() {
    return m_fh && m_bps && m_nch &&
           m_srate > 0; // todo: check to make sure decoder is initialized properly
  }
  ...
  int ReadSamples(double* buf, int length) {
    ...
    if (m_fh) {
      int blockAlign = m_nch * (m_bps / 8);
      if (blockAlign < 1)
        blockAlign = 1; // should never happen but we hate div0's
      ...
      if (length > 0) {
        ...
        if (rd > 0) {
          if (update_disk_counters)
            update_disk_counters(rd * blockAlign, 0);
          ...
          if (m_bps == 16) // we only support 16 bit
          {
            ...
            memset(buf, 0, rd * m_nch * sizeof(double));
        }
      }
    }
...
REAPER_PLUGIN_DLL_EXPORT int REAPER_PLUGIN_ENTRYPOINT(REAPER_PLUGIN_HINSTANCE hInstance,
  ...
  if (rec) {
    ...
    if (!rec->Register || !rec->Register("pcmsrc", &myRegStruct) ||
