...
static void doubletomem16(double in, unsigned char* outp) {
  ...
  if ((in) < 0.0) {
    if ((in) <= -1.0)
      ...
      (out) = (short)((int)(((in) * 32768.0) - 0.5));
  } else {
    if ((in) >= (32766.5 / 32768.0))
      (out) = 32767;
    else
      (out) = (short)(int)((in) * 32768.0 + 0.5);
  }
...
public:
  ...
  PCM_sink_raw(const char* fn, void* cfgdata, int cfgdata_l, int nch, int srate, bool buildpeaks) {
    m_peakbuild = 0;
    m_bps = 16;
    ...
    if (GetPreferredDiskWriteMode)
      GetPreferredDiskWriteMode(&wm, nb, &bs);
  ...
  void WriteDoubles(double** samples, int len, int nch, int offset, int spacing) {
    ...
    {
      double* in =
          ch < nch ? samples[ch] : samples[nch - 1]; // if the input chancount is less, make sure we
                                                     // feed it valid inputs for the higher channels

      int x;

      if (ch < nch || (ch == 1 && nch == 1))
        for (x = 0; x < len; x++)
          doubletomem16(in[x], bout + (x * m_nch + ch) * (m_bps / 8));
      else
        for (x = 0; x < len; x++)
          memset(bout + (x * m_nch + ch) * (m_bps / 8), 0, 2);
    }
...
static HWND ShowConfig(const void* cfg, int cfg_l, HWND parent) {
  if (cfg_l >= 4 && *((int*)cfg) == SINK_FOURCC) {
    ...
    return CreateDialogParam(g_hInst, MAKEINTRESOURCE(IDD_RAWSINK_CFG), parent, wavecfgDlgProc,
                             (LPARAM)x);
  }
  return 0;
}
...
SWELL_DEFINE_DIALOG_RESOURCE_BEGIN(IDD_RAWSINK_CFG,
