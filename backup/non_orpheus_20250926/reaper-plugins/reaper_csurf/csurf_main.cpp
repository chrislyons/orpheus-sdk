...
extern reaper_csurf_reg_t csurf_bcf_reg, csurf_faderport_reg, csurf_faderport2_reg, csurf_hui_reg,
    csurf_mcu_reg, csurf_mcuex_reg, csurf_tranzport_reg, csurf_alphatrack_reg, csurf_01X_reg,
    csurf_osc_reg, csurf_www_reg;
...
int (*EnumProjectMarkers3)(ReaProject*, int idx, bool* isrgn, double* pos, double* rgnend,
...
double (*TimeMap2_timeToBeats)(void* proj, double tpos, int* measures, int* cml, double* fullbeats,
                               int* cdenom);
double (*Track_GetPeakInfo)(MediaTrack* tr, int chidx);
double (*Track_GetPeakHoldDB)(MediaTrack* tr, int chidx, bool clear);

bool (*SetProjectMarkerByIndex2)(ReaProject* __proj, int idx, bool isrgn, double pos, double rgnend,
                                 int ID, const char* name, int color, int flags);
int (*AddProjectMarker2)(ReaProject* __proj, bool isrgn, double pos, double rgnend,
...
void (*adjustZoom)(double amt, int forceset, bool doupd,
...
int (*TrackFX_SetEQParam)(MediaTrack* tr, int fxidx, int bandtype, int bandidx, int paramtype,
                          double val, bool isnorm);
bool (*TrackFX_GetEQParam)(MediaTrack* tr, int fxidx, int paramidx, int* bandtype, int* bandidx,
                           int* parmtype, double* normval);
...
double (*TrackFX_GetParamEx)(MediaTrack* tr, int fx, int param, double* minval, double* maxval,
                             double* midval);
...
bool (*WDL_ChooseDirectory)(HWND parent, const char* text, const char* initialdir, char* fn,
                            int fnsize, bool preservecwd);
char* (*WDL_ChooseFileForOpen)(HWND, const char*, const char*, const char*, const char*,
...
void* (*GetSetTrackSendInfo)(MediaTrack* tr, int category, int sendidx, const char* parmname,
                             void* setNewValue);
...
bool (*GetProjExtState)(ReaProject* proj, const char* extname, const char* key, char* val,
                        int vallen);
...
void (*GetSet_LoopTimeRange2)(ReaProject* __proj, bool isSet, bool isLoop, double* start,
...
REAPER_PLUGIN_DLL_EXPORT int REAPER_PLUGIN_ENTRYPOINT(REAPER_PLUGIN_HINSTANCE hInstance,
  ...
  if (!init_csurf_api(hInstance, rec))
    return 0;
  int errcnt = 0;
#define IMPAPI(x)                                                                                  \
