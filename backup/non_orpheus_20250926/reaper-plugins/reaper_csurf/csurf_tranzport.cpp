        ...
        if (v) {
          if (m_tranz_shiftstate) {
            ...
            if (m_tranz_curmode == 0) {
              ...
              if (tr)
                CSurf_SetSurfacePan(tr, CSurf_OnPanChange(tr, v / 20.0, true), this);
            }
          }
...
public:
  ...
  const char* GetDescString() {
    descspace.SetFormatted(512, __LOCALIZE_VERFMT("Frontier TranzPort (dev %d,%d)", "csurf"),
    ...
    if ((now - m_frameupd_lastrun) >= (1000 / max(*g_config_csurf_rate, 1))) {
      ...
      if (m_midiout) {
        ...
        if (t) {
          ...
          if (fabs(dv) >= 10.0)
            snprintf(vb + 1, sizeof(vb) - 1, "%+.0f", dv);
          else
            snprintf(vb + 1, sizeof(vb) - 1, "%+.1f", dv);

          mkpanstr(pb + 1, pan);
          if (!strnicmp(pb + 1, "cent", 4))
            strcpy(pb + 1, "ctr");
          else if (strstr(pb + 1, "%")) {
            char* p = strstr(pb + 1, "%");
            p[0] = p[1];
        ...
        if (strlen(buf) >= 13)
          ...
          memcpy(p = timebuf + 20 + 20 - strlen(buf), buf, strlen(buf));
        if (m_tranz_curmode == 0)
          p[-1] = 0x7e;
