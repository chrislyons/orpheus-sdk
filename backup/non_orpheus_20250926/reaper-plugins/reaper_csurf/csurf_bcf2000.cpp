  ...
  void OnMIDIEvent(MIDI_event_t* evt) {
    if ((evt->midi_message[0] & 0xf0) == 0xB0) {
      ...
      if (evt->midi_message[1] >= 0x51 && evt->midi_message[1] <= 0x58) // volume set
      {
        ...
        if (tr)
          CSurf_SetSurfaceVolume(
              tr, CSurf_OnVolumeChange(tr, charToVol(evt->midi_message[2]), false), this);
      ...
      } else if (evt->midi_message[1] >= 0x01 && evt->midi_message[1] <= 0x08) // pan set
      {
        ...
        MediaTrack* tr = CSurf_TrackFromID(trackid, g_csurf_mcpmode);
        if (tr)
          CSurf_SetSurfacePan(tr, CSurf_OnPanChange(tr, charToPan(evt->midi_message[2]), false),
                              this);
      } else if (evt->midi_message[1] == 0x59)
        CSurf_OnPlay();
      else if (evt->midi_message[1] == 0x5a)
        CSurf_OnStop();
...
public:
  ...
  const char* GetDescString() {
    descspace.SetFormatted(512, __LOCALIZE_VERFMT("BCF 2000 (dev %d,%d)", "csurf"), m_midi_in_dev,
                           m_midi_out_dev);
