  ...
  if (sockcnt) {
    while (!s_threadquit) {
      ...
      for (i = 0; i < s_osc_handlers.GetSize(); ++i) {
        ...
        if (osc->m_recvsock != INVALID_SOCKET ||
            (osc->m_recv_enable && osc->m_recvaddr.sin_port == 0 &&
             osc->m_sendsock != INVALID_SOCKET)) {
          buf[0] = 0;
          ...
          if (osc->m_recvsock != INVALID_SOCKET) {
            ...
            len = recvfrom(osc->m_recvsock, buf, sizeof(buf), 0,
                           (struct sockaddr*)&osc->m_last_recv_addr, &socklen);
          } else {
        ...
        if ((osc->m_sendsock != INVALID_SOCKET ||
             (osc->m_recvsock != INVALID_SOCKET && osc->m_send_enable &&
              osc->m_last_recv_addr.sin_port > 0 && osc->m_sendaddr.sin_port == 0)) &&
          ...
          while (sendq.Available() >= sizeof(int)) {
            ...
            if (len < 1 || len > MAX_OSC_MSG_LEN || len > sendq.Available())
              break;
            ...
            if (packetlen > 16 && packetlen + sizeof(int) + len > maxpacket) {
              ...
              if (s_threadquit)
                break;
              if (osc->m_sendsock != INVALID_SOCKET) {
                sendto(osc->m_sendsock, packetstart, packetlen, 0,
                       (struct sockaddr*)&osc->m_sendaddr, sizeof(osc->m_sendaddr));
              } else if (osc->m_last_recv_addr.sin_port > 0) {
                sendto(osc->m_recvsock, packetstart, packetlen, 0,
                       (struct sockaddr*)&osc->m_last_recv_addr, sizeof(osc->m_last_recv_addr));
              }
              if (osc->m_sendsleep)
                Sleep(osc->m_sendsleep);
          ...
          if (packetlen > 16) {
            ...
            if (s_threadquit)
              break;
            if (osc->m_sendsock != INVALID_SOCKET) {
              sendto(osc->m_sendsock, packetstart, packetlen, 0, (struct sockaddr*)&osc->m_sendaddr,
                     sizeof(osc->m_sendaddr));
            } else if (osc->m_last_recv_addr.sin_port > 0) {
              sendto(osc->m_recvsock, packetstart, packetlen, 0,
                     (struct sockaddr*)&osc->m_last_recv_addr, sizeof(osc->m_last_recv_addr));
            }
...
int OscGetInput(OscHandler* osc) {
  ...
  if (osc->m_recvq.Available() >= sizeof(int)) {
    ...
    while (s_q.Available() >= sizeof(int)) {
      ...
      if (len <= 0 || len > MAX_PACKET_SIZE || len > s_q.Available())
        break;
...
void OscSendOutput(OscHandler* osc, OscMessageWrite* wmsg) {
  if (osc->m_sendsock != INVALID_SOCKET ||
      (osc->m_recvsock != INVALID_SOCKET && osc->m_send_enable && osc->m_sendaddr.sin_port == 0 &&
