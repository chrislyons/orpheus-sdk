...
2.Press the knob (Pan knob - below the display) on channel 1 - this is below the screen displaying
'REMOTE'. 3.Press the knob on channel 3 - below screen display 'C*BASE'. 4.If prompted (if already
in 'cubendo' mode you will not be prompted) press the knob on channel 8 - screen displays 'YES'.
5.To ensure the 01X always boots up into the above mode you need to save this setting. Hold down the
SHIFT key then press UTILITY.

Note2: 'Cubendo' mode is not as versatile as 'General' mode. In General mode all of the 01Xs buttons
are available and in any combination. In 'Cubendo' mode many of the buttons are unavailable
externally, and some of the buttons generate the same key-codes. 'EDIT' generates 90 33 7F/00 which
is the same code as SHIFT F4. All of the EQ buttons generate the same code - 90 4C 7F/00 in
'Cubendo' mode. 'GROUP' button is unavailable, as is 'DYNAMICS' (but SHIFT+DYNAMICS (labelled
PLUG-IN) is available), AUDIO,INST,MIDI,BUS/AUX,OTHER,SELECTED CHANNEL, PAGE SHIFT,MARKER, are all
unavailable. SHIFT only generates a key-code with certain combinations of other keys.

Note3: At the time of writing I have yet to Address the 01X display in General mode (it is not the
same as 'Cubendo' mode). This is why this mapping uses 'Cubendo' mode. I have placed two separate
support requests with YAMAHA Global and YAMAHA UK support but have heard nothing as yet.

Note4: Many of the keys generate different key-codes in General mode compared to 'Cubendo' mode -
editing of the some of the values of this file if General mode is used will be required.

Note5: General mode is preferred as all of the keys are available, in any combination, and all have
discrete values.

Note6: This is a hacked-about version of the original Cockos MCU file CSurf_MCU.cpp and there are
probably still many enries surplus to requirements.

Note7: The REAPER functionality provided herein greatly surpasses that of the 01X as factory
supplied with any DAW, with the exception that, currently, no FX control is currently possible.

Note8: The following lists the key-codes generated in both 'Cubendo' and Genereal modes of 01X
operation:
...
Volume/Level:	ex 00-70 00-7f	where	'x'=0 for track-1,
                                                                                ...
                                                                                'x'=8 for master.
                                                                                ex 00 00 = -oo
(minimum) ex 70 7f = full-scale (maximum)
...
Note9:	To achieve the level of functionality available here many of the keys have multiple
functions. Whilst effort has been made to make this as intuitive as possible - do yourself a favour
and watch the accompanying demo/manual - I've produced a very detailed, and useful,video demo and
reference 'manual'
- you WILL miss out on a lot of great time-saving features otherwise.
Of course if you can just read the code, with sufficient understanding of every mapped-function and
combination, and remember it all then forget the video :)

Note10: The Jog wheel has been utilised extensively - use it for fastest-ever project navigation,
project zooming (both in and out, both horizontally and vertically), scrolling both up and down, and
left and right. You can also use it to create loops and time selections - and adjust existing loops.

Note11:	Rew and FFW (<< & >>) can be used for item navigation, transient navigation, as well as loop
and project navigation, etc.

Note12: If you manage to successfully write to the 01X screen in General mode please drop a note
into the REAPER forum in order for others to be able to make use this, similarly if you come up with
any useful mods, or suggestions.
...
MCU=>PC:
  The MCU seems to send, when it boots (or is reset) F0 00 00 66 14 01 58 59 5A 57 18 61 05 57 18 61
05 F7
  ...
  to the extent the buttons below have LEDs, you can set them by sending these messages, with 7f for
on, 1 for blink, 0 for off. 90 0x vv    :   rec arm push x=0..7 (vv:..) 90 0x vv    :   solo push
x=8..F (vv:..) 90 1x vv    :   mute push x=0..7 (vv:..) 90 1x vv    :   selected push x=8..F (vv:..)
...
PC=>MCU:
  ...
  F0 00 00 66 14 12 xx <data> F7   : update LCD. xx=offset (0-112), string. display is 55 chars
wide, second line begins at 56, though. F0 00 00 66 14 08 00 F7          : reset MCU F0 00 00 66 14
20 0x 03 F7       : put track in VU meter mode, x=track
...
class CSurf_01X : public IReaperControlSurface {
  ...
  void MCUReset() {
    ...
    if (m_midiout)
      m_midiout->Send(0x90, 0x5e, 0, -1);
    if (m_midiout)
      m_midiout->Send(0x90, 0x5f, 0, -1);
  ...
  void OnMIDIEvent(MIDI_event_t* evt) {
    ...
    if ((evt->midi_message[0] & 0xf0) == 0xe0) // volume fader move
    {
      ...
      if (tid == 8)
        ...
        tid += 1 + m_offset + m_allmcus_bank_offset;
      ...
      if (tr) {
        if (m_flipmode) {
          CSurf_SetSurfacePan(
              tr,
              CSurf_OnPanChange(tr, int14ToPan(evt->midi_message[2], evt->midi_message[1]), false),
              NULL);
        } else
          CSurf_SetSurfaceVolume(
              tr,
              CSurf_OnVolumeChange(tr, int14ToVol(evt->midi_message[2], evt->midi_message[1]),
                                   false),
              NULL);
      }
      ...
      } else if (evt->midi_message[1] == 0x3c) // jog wheel
      {
        if (m_01x_jogstate == 1) {
          if (evt->midi_message[2] == 0x41)
            SendMessage(g_hwnd, WM_COMMAND, 40112, 0); // V-Out Zoom
          else if (evt->midi_message[2] == 0x01)
            SendMessage(g_hwnd, WM_COMMAND, 40111, 0); // V-In Zoom
        } else if (m_01x_jogstate == 2) {
          if (evt->midi_message[2] == 0x41)
            SendMessage(g_hwnd, WM_COMMAND, 40138, 0); // scroll-UP
          else if (evt->midi_message[2] == 0x01)
            SendMessage(g_hwnd, WM_COMMAND, 40139, 0); // scroll-DN
        } else if (m_01x_jogstate == 3) {
          if (evt->midi_message[2] == 0x41)
            SendMessage(g_hwnd, WM_COMMAND, 40140, 0); // scroll-L
          else if (evt->midi_message[2] == 0x01)
            SendMessage(g_hwnd, WM_COMMAND, 40141, 0); // scroll-R
        } else if (m_01x_jogstate == 4) {
          if (evt->midi_message[2] == 0x41)
            SendMessage(g_hwnd, WM_COMMAND, 1011, 0); // H-Out Zoom
          else if (evt->midi_message[2] == 0x01)
            SendMessage(g_hwnd, WM_COMMAND, 1012, 0); // H-In Zoom
        } else if ((m_01x_jogstate == 5) && (m_01x_keystate & 2)) {
          if (evt->midi_message[2] == 0x41)
            SendMessage(g_hwnd, WM_COMMAND, 40102, 0); // set time sel. left
          else if (evt->midi_message[2] == 0x01)
            SendMessage(g_hwnd, WM_COMMAND, 40103, 0); // set time sel. right
        } else if (evt->midi_message[2] >= 0x41)
          CSurf_OnRewFwd(0, 0x40 - (int)evt->midi_message[2]);
        else if (evt->midi_message[2] > 0 && evt->midi_message[2] < 0x40)
          CSurf_OnRewFwd(0, evt->midi_message[2]);
      }
    ...
    else if ((evt->midi_message[0] & 0xf0) == 0x90) // button pushes
    {
      ...
      if (evt->midi_message[2] >= 0x40) {
        if (evt->midi_message[1] >= 0x2e && evt->midi_message[1] <= 0x31) {
          ...
          for (x = 0; x < m_01X_list.GetSize(); x++) {
            ...
            if (item && !item->m_is_mcuex && item->m_midiout) {
              item->m_midiout->Send(0xB0, 0x40 + 11,
                                    '0' + (((m_allmcus_bank_offset + 1) / 10) % 10), -1);
        ...
        if (evt->midi_message[1] == 0x2a) // PAN CH-PARAM
        {
          ...
          if (m_midiout)
            m_midiout->Send(0x90, 0x2a, (m_01x_keystate & 8) ? 0x01 : 0, -1);
        ...
        } else if (evt->midi_message[1] == 0x5b)     // <<
        {
          if (m_01x_keystate > 15) {
            (m_01x_keystate & 1) ? SendMessage(g_hwnd, WM_COMMAND, 40376, 0)
                                 : SendMessage(g_hwnd, WM_COMMAND, 40416, 0);
          } // prev.transient/select+move to prev.item
          else if (m_01x_keystate & 1) {
            (GetSetRepeat(-1) & 1) ? SendMessage(g_hwnd, WM_COMMAND, 40632, 0)
                                   : SendMessage(g_hwnd, WM_COMMAND, 40042, 0);
        ...
        } else if (evt->midi_message[1] == 0x47)     // shift-undo
        {
          ...
          if (m_01x_keystate & 16) {
            SendMessage(g_hwnd, WM_COMMAND, 40415,
                        0); // reset selected envelope points to zero/centre
          }
          SendMessage(g_hwnd, WM_COMMAND, 40030, 0); // redo
        } else if (evt->midi_message[1] == 0x48)     // save
        {
          (m_01x_keystate & 128) ? (SendMessage(g_hwnd, WM_COMMAND, 40392, 0))
                                 : (SendMessage(g_hwnd, WM_COMMAND, 40026, 0));
          // save project or save track-template
        } else if (evt->midi_message[1] == 0x49) // shift-save
        {
          (m_01x_keystate & 128) ? (SendMessage(g_hwnd, WM_COMMAND, 40394, 0))
                                 : (SendMessage(g_hwnd, WM_COMMAND, 40022, 0));
        ...
        } else if (evt->midi_message[1] == 0x36) // f1
        {
          if (m_01x_keystate & 8) {
            (m_01x_keystate |= 128);
            if (m_midiout)
              m_midiout->Send(0x90, 0x2a, 0x7f, -1);
          } else
            SendMessage(g_hwnd, WM_COMMAND, 40001, 0); // add track
        ...
        } else if (evt->midi_message[1] == 0x37)     // f2
        {
          if (m_01x_keystate & 8) {
            ...
            SendMessage(g_hwnd, WM_COMMAND, 40279, 0); // docker toggle
          if (m_01x_keystate & 1)
            (m_01x_keystate ^= 1);
        ...
        } else if (evt->midi_message[1] == 0x38)     // f3
        {
          if (m_01x_keystate & 8) {
            ...
            SendMessage(g_hwnd, WM_COMMAND, (m_01x_keystate & 1) ? 40344 : 40298,
                        0); // toggle fx bypass track/all
          }
          if (m_01x_keystate & 1)
            (m_01x_keystate ^= 1);
        } else if (evt->midi_message[1] == 0x39) // f4
        {
          if (m_01x_keystate & 8) {
            ...
            ((m_01x_keystate & 4) ? m_01x_keystate ^= 4 : m_01x_keystate |= 4);
            (m_01x_keystate & 4) ? SendMessage(g_hwnd, WM_COMMAND, 40490, 0)
                                 : SendMessage(g_hwnd, WM_COMMAND, 40491, 0);
          }
        ...
        {
          if (m_01x_keystate & 128) {
            SendMessage(g_hwnd, WM_COMMAND, (m_01x_keystate & 1) ? 40530 : 40529,
                        0); // sel./toggle items
          } else if (m_01x_keystate & 64) {
            SendMessage(g_hwnd, WM_COMMAND, (m_01x_keystate & 1) ? 40207 : 40206,
                        0); // pitch up/down cent
          } else if (m_01x_keystate & 32) {
            SendMessage(g_hwnd, WM_COMMAND, (m_01x_keystate & 1) ? 40520 : 40519,
                        0); // itemrate up/down 10cents
          } else if (m_01x_keystate & 16) {
            SendMessage(g_hwnd, WM_COMMAND, (m_01x_keystate & 1) ? 40052 : 40406,
                        0); // view vol env/toggle vol env active
          } else {
            SendMessage(g_hwnd, WM_COMMAND, (m_01x_keystate & 1) ? 40061 : 40196,
                        0); // split items at edit-play cursor/time sel.
          }
          if (m_01x_keystate & 1)
            (m_01x_keystate ^= 1);
        ...
        {
          if (m_01x_keystate & 128) {
            SendMessage(g_hwnd, WM_COMMAND, (m_01x_keystate & 1) ? 40639 : 40638,
                        0); // show item-take FX chain/duplicate item/take
          } else if (m_01x_keystate & 64) {
            SendMessage(g_hwnd, WM_COMMAND, (m_01x_keystate & 1) ? 40205 : 40204,
                        0); // pitch up/down semitone
          } else if (m_01x_keystate & 32) {
            SendMessage(g_hwnd, WM_COMMAND, (m_01x_keystate & 1) ? 40518 : 40517,
                        0); // itemrate up/down semitone
          } else if (m_01x_keystate & 16) {
            SendMessage(g_hwnd, WM_COMMAND, (m_01x_keystate & 1) ? 40053 : 40407,
                        0); // view pan env/toggle pan env. active
          } else {
            SendMessage(g_hwnd, WM_COMMAND, (m_01x_keystate & 1) ? 40202 : 40109,
                        0); // open pri./sec. editor
          }
          if (m_01x_keystate & 1)
            (m_01x_keystate ^= 1);
        ...
        {
          if (m_01x_keystate & 128) {
            SendMessage(g_hwnd, WM_COMMAND, (m_01x_keystate & 1) ? 40209 : 40361,
                        0); // apply track FX to item (mono O/P)/apply track fx to item (stereo O/P)
          } else if (m_01x_keystate & 64) {
            SendMessage(g_hwnd, WM_COMMAND, (m_01x_keystate & 1) ? 40516 : 40515,
                        0); // pitch up/down octave
          } else if (m_01x_keystate & 32) {
            SendMessage(g_hwnd, WM_COMMAND, (m_01x_keystate & 1) ? 40525 : 40524,
                        0); // playrate up/down 10cents
          } else if (m_01x_keystate & 16) {
            SendMessage(g_hwnd, WM_COMMAND, (m_01x_keystate & 1) ? 40050 : 40408,
                        0); // view pre-fx vol/pan envs
          } else {
            SendMessage(g_hwnd, WM_COMMAND, (m_01x_keystate & 1) ? 40642 : 40643,
                        0); // explode takes in order/place
          }
          if (m_01x_keystate & 1)
            (m_01x_keystate ^= 1);
        ...
        {
          if (m_01x_keystate & 128) {
            SendMessage(g_hwnd, WM_COMMAND, (m_01x_keystate & 1) ? 40011 : 40009,
                        0); // view item properties/source properties
          } else if (m_01x_keystate & 64) {
            SendMessage(g_hwnd, WM_COMMAND, (m_01x_keystate & 1) ? 40637 : 40377,
                        0); // view virt.midi-keyb./all input to vkb
          } else if (m_01x_keystate & 32) {
            SendMessage(g_hwnd, WM_COMMAND, (m_01x_keystate & 1) ? 40523 : 40522,
                        0); // transport playrate up/down semitone
          } else if (m_01x_keystate & 16) {
            SendMessage(g_hwnd, WM_COMMAND, (m_01x_keystate & 1) ? 40051 : 40409,
                        0); // view pre-fx pan env/toggle pre-fx vol.pan active
          } else {
            SendMessage(g_hwnd, WM_COMMAND, (m_01x_keystate & 1) ? 40011 : 40009,
                        0); // view item properties/source properties
          }
        ...
        } else if (evt->midi_message[1] == 0x64) // zoom
        {
          ...
          if (m_01x_keystate ^ 2)
            (m_01x_jogstate = 0);
          if (m_midiout)
            m_midiout->Send(0x90, 0x64, (m_01x_keystate & 2) ? 0x01 : 0, -1);
        } else if (evt->midi_message[1] == 0x60) // ^
        {
          if (m_01x_keystate & 2) {
            (m_01x_jogstate = 1);
            if (m_midiout)
              m_midiout->Send(0x90, 0x64, 0x7f, -1);
          } else if (m_01x_keystate & 128) // mode f1
          {
            SendMessage(g_hwnd, WM_COMMAND, 40117, 0); // Move items up one track
          } else
            SendMessage(g_hwnd, WM_COMMAND, (m_01x_keystate > 15) ? 40418 : 40286,
                        0);                      // sel.&move to item in prev track
        } else if (evt->midi_message[1] == 0x61) // v
        {
          if (m_01x_keystate & 2) {
            (m_01x_jogstate = 2);
            if (m_midiout)
              m_midiout->Send(0x90, 0x64, 0x7f, -1); // mode led
          } else if (m_01x_keystate & 128) {
            SendMessage(g_hwnd, WM_COMMAND, 40118, 0); // Move items down one track
          } else
            SendMessage(g_hwnd, WM_COMMAND, (m_01x_keystate > 16) ? 40419 : 40285, 0);
        ...
        else if (evt->midi_message[1] == 0x62) // <
        {
          if (m_01x_keystate & 2) {
            ...
            SendMessage(g_hwnd, WM_COMMAND, (m_01x_keystate & 128) ? 40120 : 40416, 0);
        ...
        else if (evt->midi_message[1] == 0x63) // >
        {
          if (m_01x_keystate & 2) {
            ...
            SendMessage(g_hwnd, WM_COMMAND, (m_01x_keystate & 128) ? 40119 : 40417, 0);
        ...
        } else if (evt->midi_message[1] == 0x65) // scrub button
        {
          ...
          if (m_midiout)
            m_midiout->Send(0x90, 0x65, (m_01x_jogstate == 5) ? 0x7f : 0, -1);
        }
...
public:
  ...
  const char* GetDescString() {
    descspace.SetFormatted(512, __LOCALIZE_VERFMT("YAMAHA 01X (dev %d,%d)", "csurf"), m_midi_in_dev,
                           m_midi_out_dev);
...
#define VU_BOTTOM 70
  ...
  void SetTrackListChange() {
    if (m_midiout) {
      ...
      for (x = 0; x < 8; x++) {
        MediaTrack* t =
            CSurf_TrackFromID(x + m_offset + m_allmcus_bank_offset + 1, g_csurf_mcpmode);
...
#define FIXID(id)                                                                                  \
  ...
  int id = oid;                                                                                    \
  if (id > 0) {                                                                                    \
    id -= m_offset + m_allmcus_bank_offset + 1;                                                    \
    if (id == 8)                                                                                   \
      id = -1;                                                                                     \
  } else if (id == 0)                                                                              \
    id = 8;
  ...
  bool GetTouchState(MediaTrack* trackid, int isPan) {
    ...
    if (!m_flipmode != !isPan) {
      if (id >= 0 && id < 8) {
        if (m_pan_lasttouch[id] == 1 || (timeGetTime() - m_pan_lasttouch[id]) <
  ...
  void OnTrackSelection(MediaTrack* trackid) {
    ...
    int newpos = tid - 1;
    if (newpos >= 0 &&
        (newpos < m_allmcus_bank_offset || newpos >= m_allmcus_bank_offset + movesize)) {
      ...
      if (no != m_allmcus_bank_offset) {
        ...
        for (x = 0; x < m_01X_list.GetSize(); x++) {
          CSurf_01X* item = m_01X_list.Get(x);
          if (item && !item->m_is_mcuex && item->m_midiout) {
            item->m_midiout->Send(0xB0, 0x40 + 11, '0' + (((m_allmcus_bank_offset + 1) / 10) % 10),
                                  -1);
            item->m_midiout->Send(0xB0, 0x40 + 10, '0' + ((m_allmcus_bank_offset + 1) % 10), -1);
          }
        }
...
static IReaperControlSurface* createFunc(const char* type_string, const char* configString,
                                         int* errStats) {
  int parms[4];
  parseParms(configString, parms);

  return new CSurf_01X(!strcmp(type_string, "MCUEX"), parms[0], parms[1], parms[2], parms[3],
                       errStats);
}
...
static HWND configFunc(const char* type_string, HWND parent, const char* initConfigString) {
  return CreateDialogParam(g_hInst, MAKEINTRESOURCE(IDD_SURFACEEDIT_MCU), parent, dlgProc,
