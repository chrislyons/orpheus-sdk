    ...
    if (evt->midi_message[0] == 0xb0) {
          ...
          if (m_alpha_fx >= 0) {
            ...
            if (evt->midi_message[1] == 0x12) {
              ...
              if (m_alpha_fxparm != np - 2) // if not bypass env
              {
                ...
                if ((fabs(ov - minval) < 0.001 || fabs(ov - maxval) < 0.001) &&
                    fabs(oov - ov) < 0.001)
      ...
      case 0x32:
        ...
        if (ispress) {
          ...
          if (m_midiout)
            m_midiout->Send(0x90, 0x32, m_alphatrack_flipmode ? 0x7f : 0, -1);
      ...
      case 0x52: // add marker
        if (ispress) {
          SendMessage(g_hwnd, WM_COMMAND,
                      m_tranz_shiftstate ? ID_INSERT_MARKERRGN : ID_INSERT_MARKER, 0);
          m_tranz_shiftstate &= 5;
        }
        break;
      ...
      case 0x5c:
        if (ispress && m_tranz_shiftstate) {
          ...
          m_button_states = ispress ? 2 : 0;
        break;
      case 0x46:
        if (ispress && (m_tranz_shiftstate & 4))
          m_tranz_shiftstate = 1; // if pressed and a stuck shift state is on, remove it
        else if (!ispress &&
                 (m_tranz_shiftstate & 2)) // end of press, shift state wasnt touched, stick it
          m_tranz_shiftstate = 4;
        else
          m_tranz_shiftstate = ispress ? 3 : 0;
        if (m_midiout)
          m_midiout->Send(0x90, 0x46, m_tranz_shiftstate ? 0x7f : 0, -1);
        break;
      case 0x68:
      ...
      case 0x22:
        if (ispress)
          m_alpha_fxcap |= (8 << (evt->midi_message[1] - 0x21));
        else
          m_alpha_fxcap &= ~(8 << (evt->midi_message[1] - 0x21));
        break;
      case 0x20:
...
public:
  ...
  const char* GetDescString() {
    descspace.SetFormatted(512, __LOCALIZE_VERFMT("Frontier AlphaTrack (dev %d,%d)", "csurf"),
  ...
  void Run() {
    ...
    {
      if (m_alphatrack_fingerpos >= 0 && m_alphatrack_fingerposcenter >= 0 &&
          m_alphatrack_nfingers && m_alphatrack_fingerposcenter != m_alphatrack_fingerpos) {
        ...
        } else if (m_alphatrack_nfingers & 1) // zoom
        {
          ...
          adjustZoom(((m_alphatrack_fingerposcenter - m_alphatrack_fingerpos) *
                      (int)(now - m_frameupd_lastrun2)) /
                         -3000.0,
                     0, true, zm);
          //
        }
    ...
    if (now >= m_frameupd_lastrun + (1000 / max(*g_config_csurf_rate, 1)) ||
      ...
      if (m_midiout) {
        ...
        if (t && (m_alpha_fxcap & 1)) {
          ...
          if (m_alpha_fx >= TrackFX_GetCount(t))
            m_alpha_fx = TrackFX_GetCount(t) - 1;
          if (m_alpha_fx < 0)
            m_alpha_fx = 0;

          int np = TrackFX_GetNumParams(t, m_alpha_fx);
          if (m_alpha_fxparm >= np)
            m_alpha_fxparm = np - 1;
          if (m_alpha_fxparm < 0)
            m_alpha_fxparm = 0;
          ...
          if (!timebuf[0] && np > 0)
            snprintf(timebuf, sizeof(timebuf), "%.3f", v);
          UpdateTranzDisplay(16, timebuf, 16, m_tranz_oldbuf);
        } else {
          if (t == CSurf_TrackFromID(0, false))
            sprintf(timebuf, "master");
          else if (t) {
          ...
          if (t) {
            {
              ...
              if (showvol)
                mkvolstr(timebuf, vol);
              else if (showpan)
                mkpanstr(timebuf, pan);
            }
          }
          UpdateTranzDisplay(16, timebuf, 16, m_tranz_oldbuf);
        }
...
#define FIXID(id)                                                                                  \
  ...
  void SetSurfaceSolo(MediaTrack* trackid, bool solo) {
    ...
    if (m_midiout) {
      if (!oid)
        m_midiout->Send(0x90, 0x73, (m_tranz_anysolo_state = !!solo) ? 0x7f : 0, -1);
