...
enum { HZOOM_EDITPLAYCUR = 0, HZOOM_EDITCUR = 1, HZOOM_VIEWCTR = 2, HZOOM_MOUSECUR = 3 };

enum { VZOOM_VIEWCTR = 0, VZOOM_TOPVIS = 1, VZOOM_LASTSEL = 2, VZOOM_MOUSECUR = 3 };
...
extern void (*CSurf_SetSurfaceVolume)(MediaTrack* trackid, double volume,
                                      IReaperControlSurface* ignoresurf);
extern void (*CSurf_SetSurfacePan)(MediaTrack* trackid, double pan,
                                   IReaperControlSurface* ignoresurf);
extern void (*CSurf_SetSurfaceMute)(MediaTrack* trackid, bool mute,
                                    IReaperControlSurface* ignoresurf);
extern void (*CSurf_SetSurfaceSelected)(MediaTrack* trackid, bool selected,
                                        IReaperControlSurface* ignoresurf);
extern void (*CSurf_SetSurfaceSolo)(MediaTrack* trackid, bool solo,
                                    IReaperControlSurface* ignoresurf);
extern void (*CSurf_SetSurfaceRecArm)(MediaTrack* trackid, bool recarm,
                                      IReaperControlSurface* ignoresurf);
...
extern void (*CSurf_SetPlayState)(bool play, bool pause, bool rec,
                                  IReaperControlSurface* ignoresurf);
extern void (*CSurf_SetRepeatState)(bool rep, IReaperControlSurface* ignoresurf);

extern bool (*GetProjExtState)(ReaProject* proj, const char* extname, const char* key, char* val,
                               int vallen);
...
extern int (*SetProjExtState)(ReaProject* proj, const char* extname, const char* key,
                              const char* val);
...
extern double (*CSurf_OnVolumeChangeEx)(MediaTrack* trackid, double volume, bool relative,
                                        bool allowgang);
extern double (*CSurf_OnPanChangeEx)(MediaTrack* trackid, double pan, bool relative,
                                     bool allowgang);
extern double (*CSurf_OnWidthChangeEx)(MediaTrack* trackid, double width, bool relative,
                                       bool allowgang);
...
extern double (*CSurf_OnSendVolumeChange)(MediaTrack* trackid, int sendidx, double volume,
                                          bool relative);
extern double (*CSurf_OnSendPanChange)(MediaTrack* trackid, int sendidx, double pan, bool relative);
extern double (*CSurf_OnRecvVolumeChange)(MediaTrack* trackid, int recvidx, double volume,
                                          bool relative);
...
extern void (*adjustZoom)(double amt, int forceset, bool doupd,
                          int centermode); // 0,true,-1 are defaults
...
extern void (*format_timestr_pos)(double tpos, char* buf, int buflen,
...
extern int (*EnumProjectMarkers3)(ReaProject*, int idx, bool* isrgn, double* pos, double* rgnend,
                                  const char** name, int* markrgnindexnumber, int* colorOut);
...
extern void (*KBD_OnMainActionEx)(int cmd, int val, int valhw, int relmode, HWND hwnd,
                                  ReaProject* __proj);
...
extern double (*TimeMap2_timeToBeats)(void* proj, double tpos, int* measures, int* cml,
...
extern int (*TrackFX_SetEQParam)(MediaTrack* tr, int fxidx, int bandtype, int bandidx,
                                 int paramtype, double val, bool isnorm);
extern bool (*TrackFX_GetEQParam)(MediaTrack* tr, int fxidx, int paramidx, int* bandtype,
                                  int* bandidx, int* parmtype, double* normval);
extern bool (*TrackFX_SetEQBandEnabled)(MediaTrack* tr, int fxidx, int bandtype, int bandidx,
                                        bool enable);
...
extern double (*TrackFX_GetParam)(MediaTrack* tr, int fx, int param, double* minval,
                                  double* maxval);
extern double (*TrackFX_GetParamEx)(MediaTrack* tr, int fx, int param, double* minval,
...
extern bool (*TrackFX_GetFormattedParamValue)(MediaTrack* tr, int fx, int param, char* buf,
                                              int buflen);
...
extern void (*GetSet_LoopTimeRange2)(ReaProject* __proj, bool isSet, bool isLoop, double* start,
                                     double* end, bool allowautoseek);

extern bool (*WDL_ChooseDirectory)(HWND parent, const char* text, const char* initialdir, char* fn,
                                   int fnsize, bool preservecwd);

extern char* (*WDL_ChooseFileForOpen)(HWND, const char*, const char*, const char*, const char*,
                                      const char*, bool, bool, const char*, void*,
#ifdef _WIN32
                                      HINSTANCE hInstance
#else
...
extern void* (*GetSetTrackSendInfo)(MediaTrack* tr, int category, int sendidx, const char* parmname,
                                    void* setNewValue);
extern int (*GetTrackNumSends)(MediaTrack* tr, int category);

extern bool (*SetProjectMarkerByIndex2)(ReaProject* __proj, int idx, bool isrgn, double pos,
                                        double rgnend, int ID, const char* name, int color,
                                        int flags);
extern int (*AddProjectMarker2)(ReaProject* __proj, bool isrgn, double pos, double rgnend,
                                const char* name, int wantidx, int color);
