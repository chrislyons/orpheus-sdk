    ...
    if ((evt->midi_message[0] & 0xf0) == 0xB0) {
      if (evt->midi_message[1] == 0x0f) // select track
      {
        ...
        if (m_babyhui_wt >= 0) {
          ...
          if (m_babyhui_wt == 0xc && evt->midi_message[2] == 0x43) {
          ...
          } else if (m_babyhui_wt == 0xa && evt->midi_message[2] >= 0x40 &&
                     evt->midi_message[2] <= 0x43) {
            if (evt->midi_message[2] > 0x41) // increase by X
            {
              ...
              if (m_allhui_bank_offset >= msize)
                m_allhui_bank_offset = msize - 1;
            } else {
              m_allhui_bank_offset -= (evt->midi_message[2] & 1) ? 8 : 1;
              if (m_allhui_bank_offset < 0)
                m_allhui_bank_offset = 0;
            }
          ...
          } else if (evt->midi_message[2] == 0x0) // un touch
          {
            ...
            if (m_babyhui_wt == 0xe) {
              ...
              MediaTrack* tr =
                  CSurf_TrackFromID(m_offset + m_allhui_bank_offset + trackid, g_csurf_mcpmode);
          ...
          } else if (evt->midi_message[2] == 0x42) // mute click (0x02 is when you let go)
          {
            if (m_babyhui_wt == 0xe) {
              ...
              if ((evt->midi_message[0] & 15) < (m_size / 8))
                trackid += (evt->midi_message[0] & 15) * 8;
              MediaTrack* tr =
                  CSurf_TrackFromID(m_allhui_bank_offset + m_offset + trackid, g_csurf_mcpmode);
              if (tr)
          ...
          } else if (evt->midi_message[2] == 0x43) // solo click (0x03 is when you let go)
          {
            ...
            if (m_babyhui_wt < 8) {
              ...
              MediaTrack* tr =
                  CSurf_TrackFromID(trackid + m_allhui_bank_offset + m_offset, g_csurf_mcpmode);
              if (tr)
          ...
          if (evt->midi_message[2] >= 0x40 && allow_passthru) {
            MIDI_event_t evt2 = {0,
                                 3,
                                 {evt->midi_message[0] ^ 0x0f,
                                  ((evt->midi_message[2] << 4) & 0xf0) | m_babyhui_wt, 0x40}};
        ...
        if (tr) {
          if (m_flipmode)
            CSurf_SetSurfacePan(
                ...
                CSurf_OnPanChange(
                    tr,
                    int14ToPan(m_babyhui_track_msb[evt->midi_message[0] & 15][trackid],
                               evt->midi_message[2]),
                    false),
                NULL);
          else
            CSurf_SetSurfaceVolume(
                ...
                CSurf_OnVolumeChange(
                    tr,
                    int14ToVol(m_babyhui_track_msb[evt->midi_message[0] & 15][trackid],
                               evt->midi_message[2]),
                    false),
                NULL);
        }
      ...
      {
        m_babyhui_track_msb[evt->midi_message[0] & 15][evt->midi_message[1] - 0x0] =
            evt->midi_message[2];
      }
    }
  }
...
public:
  ...
  const char* GetDescString() {
    descspace.SetFormatted(512, __LOCALIZE_VERFMT("HUI(partial) (dev %d,%d)", "csurf"),
  ...
  void SetTrackListChange() {
    ...
    for (trackid = 0; trackid < 256; ++trackid) {
      MediaTrack* tr =
          CSurf_TrackFromID(m_offset + m_allhui_bank_offset + trackid, g_csurf_mcpmode);
      if (!tr)
        SetTrackTitleImpl(trackid, "");
    }
  }

#define FIXID(id)                                                                                  \
  const int oid = CSurf_TrackToID(trackid, g_csurf_mcpmode);                                       \
  const int id = oid - (m_offset + m_allhui_bank_offset);
...
#define SYSEX_TRACKTITLE_LEN 13
      unsigned char sysex[SYSEX_TRACKTITLE_LEN] = {0xF0, 0x00, 0x00, 0x66, 0x05, 0x00, 0x10,
  ...
  bool GetTouchState(MediaTrack* trackid, int isPan) {
    ...
    if (!m_flipmode != !isPan) {
      if (id >= 0 && id < 256 && id < m_size) {
        if ((timeGetTime() - m_pan_lasttouch[id]) <
...
static HWND configFunc(const char* type_string, HWND parent, const char* initConfigString) {
  return CreateDialogParam(g_hInst, MAKEINTRESOURCE(IDD_SURFACEEDIT_HUI), parent, dlgProc,
