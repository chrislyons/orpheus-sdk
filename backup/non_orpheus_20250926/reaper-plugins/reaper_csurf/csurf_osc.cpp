...
bool LoadCfgContext(ProjectStateContext* ctx, WDL_PtrList<char>* cfg, char* errbuf, int errbuf_sz) {
  ...
  if (!keys.GetSize()) {
    ...
    if (rc) {
      while (!rc->GetLine(buf, sizeof(buf))) {
        LineParserInt lp;
        if (lp.parseDestroyBuffer(buf) || lp.getnumtokens() < 1)
          continue;
  ...
  while (!ctx->GetLine(buf, sizeof(buf))) {
    ++line;
    LineParser lp;
    if (lp.parse(buf) || lp.getnumtokens() < 2)
      continue;
    ...
    if (!keys.Exists(key)) {
      if (!errbuf)
        continue; // errbuf is non-NULL only when loading from UI, silently ignore unknown actions
                  // otherwise

      haderr = true;
      snprintf(errbuf, errbuf_sz,
               __LOCALIZE_VERFMT("Unknown action \"%s\" on line %d", "csurf_osc"), key, line);
      break;
    }
    ...
    for (i = 1; i < lp.getnumtokens(); ++i) {
      ...
      if (i == 1) {
        if (strstarts(key, "DEVICE_") || strstarts(key, "REAPER_")) {
          ...
          if (strends(key, "_FOLLOWS") &&
              (!strcmp(pattern, "REAPER") || !strcmp(pattern, "DEVICE") ||
               !strcmp(pattern, "LAST_TOUCHED") || !strcmp(pattern, "FOCUSED") ||
      ...
      if (!strchr("nfbtrsi", pattern[0])) {
        if (errbuf)
          snprintf(
              errbuf, errbuf_sz,
              __LOCALIZE_VERFMT(
                  "Pattern \"%s\" starts with unknown flag '%c' on line %d (allowed: [nfbtrsi])",
                  "csurf_osc"),
      ...
      if (pattern[1] != '/') {
        if (errbuf)
          snprintf(
              errbuf, errbuf_sz,
              __LOCALIZE_VERFMT("Pattern \"%s\" does not start with '/' on line %d", "csurf_osc"),
              pattern, line);
        break;
      }
      if (strlen(pattern) > 512) {
        if (errbuf)
          snprintf(errbuf, errbuf_sz,
                   __LOCALIZE_VERFMT("Pattern is too long on line %d", "csurf_osc"), line);
        break;
      }
    }
...
static int parse_number_or_command_id(const char* b) {
  ...
  // (and if it happens to match an action ID, it might change the behavior, but that seems very
  // unlikely)
  if (*b != '_')
    return atoi(b);
  char buf[1024];
  const char* p = b + 1;
  while (*p == '_' || isalnum_safe(*p))
    p++;
  if (p == b + 1)
    return 0;
...
static const char* skip_number_or_command_id(const char* b) {
  if (*b == '_') {
    b++;
    while (*b == '_' || isalnum_safe(*b))
      b++;
  } else {
    while (isdigit_safe(*b))
      ++b;
  }
  return b;
}
...
static int OscPatternMatch(const char* a, const char* b, int* wcmatches, int* numwc, int* numslots,
                           int* slotcnt, int* rptcnt, char* flag) {
  if (!a)
    return -1;
  if (!b)
    return 1;
  ...
  while (*a && *b) {
    if (*a == *b || *a == '?' || *b == '?') {
      ...
      if (*a == '*') {
        wca = true;
        if (!*++a)
          return 0; // wildcard is at end of specifier, match complete
      } else {
        wca = false;
        if (!*++b)
          return 0; // wildcard is at end of specifier, match complete
...
static void PackCfg(WDL_String* str, const char* name, int flags, int recvport, const char* sendip,
                    int sendport, int maxpacketsz, int sendsleep, const char* cfgfn) {
  if (!name)
    name = "";
  if (!sendip)
    sendip = "";
  if (!cfgfn)
    cfgfn = "";
  str->SetFormatted(1024, "\"%s\" %d %d \"%s\" %d %d %d \"%s\"", name, flags, recvport, sendip,
                    sendport, maxpacketsz, sendsleep, cfgfn);
}

static void ParseCfg(const char* cfgstr, char* name, int namelen, int* flags, int* recvport,
                     char* sendip, int* sendport, int* maxpacketsz, int* sendsleep, char* cfgfn,
                     int cfgfnlen) {
  name[0] = 0;
  *flags = 0;
  *recvport = 0;
  ...
  *sendsleep = DEF_SENDSLEEP;
  ...
  if (strstr(cfgstr, "@@@"))
    return; // reject config str from pre13

  LineParser lp;
  if (!lp.parse(cfgstr)) {
    if (lp.getnumtokens() > 0)
      lstrcpyn(name, lp.gettoken_str(0), namelen);
    if (lp.getnumtokens() > 1)
      *flags = lp.gettoken_int(1);
    if (lp.getnumtokens() > 2)
      *recvport = lp.gettoken_int(2);
    if (lp.getnumtokens() > 3)
      lstrcpyn(sendip, lp.gettoken_str(3), 64);
    if (lp.getnumtokens() > 4)
      *sendport = lp.gettoken_int(4);
    if (lp.getnumtokens() > 5)
      *maxpacketsz = lp.gettoken_int(5);
    if (lp.getnumtokens() > 6)
      *sendsleep = lp.gettoken_int(6);
    if (lp.getnumtokens() > 7)
      lstrcpyn(cfgfn, lp.gettoken_str(7), cfgfnlen);
  }
}
...
static const char* const SPECIAL_STRING_CLEARCACHE =
    "//clearcache"; // never actually dereferenced, just a unique pointer
...
#define SETSURFFLOATWC(pattern, wc, numwc, fval)                                                   \
  SetSurfaceVal(pattern, wc, numwc, NULL, &fval, NULL, NULL)
...
#define SETSURFBOOL(pattern, bval)                                                                 \
  {                                                                                                \
    double v = (double)bval;                                                                       \
    SetSurfaceVal(pattern, 0, 0, 0, 0, &v, 0);                                                     \
  }
#define SETSURFBOOLWC(pattern, wc, numwc, bval)                                                    \
  {                                                                                                \
    double v = (double)bval;                                                                       \
    SetSurfaceVal(pattern, wc, numwc, 0, 0, &v, 0);                                                \
  }
...
#define SETSURFTRACKBOOL(pattern, tidx, bval)                                                      \
  {                                                                                                \
    double v = (double)bval;                                                                       \
    SetSurfaceTrackVal(pattern, tidx, 0, 0, &v, 0);                                                \
  }
...
public:
  ...
  int m_curfx; // 0=first fx on m_curtrack, etc. TODO: handle m_curfx&(1<<24) for input FX (unsure
               // how this will be handled now)
  ...
  int m_followflag; // &1=track follows last touched, &2=FX follows last touched, &4=FX follows
                    // focused FX, &8=track bank follows mixer, &16=reaper track sel follows device,
                    // &32=insert reaeq on any FX_EQ message
  ...
  int m_wantfx; // &1=want fx parm feedback, &2=want last touched fx feedback, &4=want fx inst
                // feedback, &8=want fx parm feedback for inactive tracks, &16=want fx inst feedback
                // for inactive tracks, &32=want fxeq feedback, &64=want fxeq feedback for inactive
                // tracks
  ...
  CSurf_Osc(const char* name, int flags, int recvport, const char* sendip, int sendport,
            int maxpacketsz, int sendsleep, OscLocalHandler* osc_local, const char* cfgfn)
    ...
    if (name[0]) {
      ...
      if (senden) {
        if (!sendport && rcven)
          ...
          m_desc.AppendFormatted(512, __LOCALIZE_VERFMT("send %s:%d", "csurf_osc"), sendip,
                                 sendport);
      }
      if (rcven || senden)
        m_desc.Append(")");
    ...
    for (i = 0; i < customcfg.GetSize(); ++i) {
      LineParserInt lp;
      if (lp.parseDestroyBuffer(customcfg.Get(i)) || lp.getnumtokens() < 2)
        continue;
      ...
      if (strstarts(key, "DEVICE_") || strstarts(key, "REAPER_")) {
        if (strends(key, "_COUNT")) {
          if (isint(pattern)) {
            int a = atoi(pattern);
            if (a > 0) {
              if (strends(key, "_TRACK_COUNT"))
                m_trackbanksize = a;
              else if (strends(key, "_SEND_COUNT"))
                m_sendbanksize = a;
              else if (strends(key, "_RECEIVE_COUNT"))
                m_recvbanksize = a;
              else if (strends(key, "_FX_COUNT"))
                m_fxbanksize = a;
              else if (strends(key, "_FX_PARAM_COUNT"))
                m_fxparmbanksize = a;
              else if (strends(key, "_FX_INST_PARAM_COUNT"))
                m_fxinstparmbanksize = a;
              else if (strends(key, "_MARKER_COUNT"))
                m_markerbanksize = a;
              else if (strends(key, "_REGION_COUNT"))
                m_regionbanksize = a;
            }
            skipdef = true;
          }
        ...
        if (!strcmp(key, "TIME"))
          m_wantpos |= 1;
        else if (!strcmp(key, "BEAT"))
          m_wantpos |= 2;
        else if (!strcmp(key, "SAMPLES"))
          m_wantpos |= 4;
        else if (!strcmp(key, "FRAMES"))
          m_wantpos |= 8;
        else if (!strcmp(key, "MASTER_VU"))
          m_wantvu |= 1;
        else if (!strcmp(key, "MASTER_VU_L") || !strcmp(key, "MASTER_VU_R"))
          m_wantvu |= (1 | 4);
        else if (!strcmp(key, "TRACK_VU"))
          m_wantvu |= 2;
        else if (!strcmp(key, "TRACK_VU_L") || !strcmp(key, "TRACK_VU_R"))
          m_wantvu |= (2 | 4);
        else if (strstarts(key, "FX_PARAM_VALUE") || !strcmp(key, "FX_WETDRY"))
          m_wantfx |= 1;
        else if (strstarts(key, "LAST_TOUCHED_FX_"))
          m_wantfx |= 2;
        else if (strstarts(key, "FX_INST_"))
          m_wantfx |= 4;
        else if (strstarts(key, "FX_EQ_"))
          m_wantfx |= 32;
        else if (!strncmp(key, "MARKER_", 7))
          m_wantmarker |= 1;
        else if (!strncmp(key, "REGION_", 7))
          m_wantmarker |= 2;
        else if (!strncmp(key, "LAST_MARKER_", 12))
          m_wantmarker |= 4;
        else if (!strncmp(key, "LAST_REGION_", 12))
          m_wantmarker |= 8;
        else if (!strcmp(key, "LOOP_START_TIME") || !strcmp(key, "LOOP_END_TIME"))
          m_wantmarker |= 16;
        else if (!strcmp(key, "TRACK_VOLUME_TOUCH"))
          m_supports_touch |= 1;
        else if (!strcmp(key, "TRACK_PAN_TOUCH"))
          m_supports_touch |= 2;
        else if (!strcmp(key, "TRACK_PAN2_TOUCH"))
          m_supports_touch |= 4;

        // want inactive track FX feedback only if we have this many wildcards
        if (strstarts(key, "FX_PARAM_VALUE"))
          fx_feedback_wc = 3;
        else if (!strcmp(key, "FX_WETDRY"))
          fx_feedback_wc = 2;
        else if (strstarts(key, "FX_INST_"))
          fx_feedback_wc = 2;
        else if (strstarts(key, "FX_EQ_"))
      ...
      if (skipdef && lp.getnumtokens() < 3)
        continue;

      // check if we have already seen this key
      int pos;
      for (pos = 0; pos < m_msgtab.GetSize(); ++pos) {
        if ((!pos || !m_msgtab.Get(pos - 1)) && !strcmp(key, m_msgtab.Get(pos)))
          break;
      }
      if (pos < m_msgtab.GetSize())
        ++pos;
      else
        m_msgtab.Insert(pos++, strdup(key));
      ...
      for (j = (skipdef ? 2 : 1); j < lp.getnumtokens(); ++j) {
        ...
        if (fx_feedback_wc && CountWildcards(pattern) >= fx_feedback_wc) {
          if (strstarts(key, "FX_INST_PARAM_VALUE"))
            m_wantfx |= 16;
          else if (strstarts(key, "FX_EQ_"))
            m_wantfx |= 32;
          else
    ...
    for (i = 0; i < m_msgtab.GetSize(); ++i) {
      ...
      if (!p)
        continue;
      if (!i || !m_msgtab.Get(i - 1))
        m_msgkeyidx.Insert(p, i);
      else
        m_msgvalidx.Insert(p, i);
    }
  ...
  void Run() {
    ...
    if (!m_wantpos && !m_wantvu && !wantmarker && !wantregion && !wantloopregion)
      return;
  ...
  const char* FindOscMatch(const char* msg, int* wc, int* numwc, int* rptcnt, char* flag) {
    ...
    if (!validx)
      return 0; // message didn't match anything
    ...
    if (OscPatternMatch(msg, p, wc, numwc, &numslots, slotcnt, rptcnt, flag))
      return 0; // assert

    if (*rptcnt > 1) {
      if (*rptcnt > MAX_OSC_RPTCNT)
        return 0;
      ...
      for (i = 0; i < numslots; ++i) {
        ...
        if (slotcnt[i] != 1 && slotcnt[i] != *rptcnt)
          return 0;
        for (j = 0; j < *rptcnt; ++j) {
  ...
  static double GetFloatArg(OscMessageRead* rmsg, char flag, bool* hasarg = 0) {
    ...
    if (hasarg)
      *hasarg = false;
    if (flag == 't')
      return 1.0; // trigger messages do not need an argument
    return 0.0;
  }
  ...
  int TriggerMessage(OscMessageRead* rmsg, char flag) {
    if (strchr("ntbr", flag)) {
      ...
      if (flag == 'b' && hasarg && v == 0.0)
        return -1;
      if (flag == 'b' && hasarg && v == 1.0)
        return 1;
      if (flag == 't' && (!hasarg || v == 1.0))
        return 1;
      if (flag == 'r' && hasarg && v < m_rotarycenter)
        return -1;
      if (flag == 'r' && hasarg && v > m_rotarycenter)
        return 1;
    }
  ...
  bool ProcessGlobalAction(OscMessageRead* rmsg, const char* pattern, char flag) {
    ...
    if (!strcmp(pattern, "REPLACE")) {
      ...
      if (t) {
        ...
        if (flag == 't' || repl != (t > 0)) {
          if (repl)
            Main_OnCommand(41330, 0); // autosplit
          else
            Main_OnCommand(41186, 0);
        }
      }
      return true;
    }
    ...
    if (rew || fwd) {
      ...
      } else if (m_altnav == 1) // nav by marker
      {
        if (TriggerMessage(rmsg, flag) > 0) {
          m_nav_active = 0;
          if (rew)
            Main_OnCommand(40172, 0); // go to previous marker
          else if (fwd)
            Main_OnCommand(40173, 0); // go to next marker
        }
    ...
    if (!strcmp(pattern, "LOOP_START_TIME") || !strcmp(pattern, "LOOP_END_TIME")) {
      if (flag == 'f') {
        ...
        GetSet_LoopTimeRange2(NULL, false, true, &st, &end, false);
        if (!strcmp(pattern, "LOOP_START_TIME"))
          st = v;
        else
          end = v;
  ...
  bool ProcessStateAction(OscMessageRead* rmsg, const char* pattern, char flag, int* wc,
                          int numwc) {
    if (!strstarts(pattern, "DEVICE_") && !strstarts(pattern, "REAPER_"))
      return false;
      ...
      if (flag == 's' || TriggerMessage(rmsg, flag) > 0) {
        if (flag == 's') {
          ...
          if (s) {
            if (!strcmp(pattern, "REAPER_TRACK_FOLLOWS")) {
              if (!strcmp(s, "DEVICE"))
                pattern = "REAPER_TRACK_FOLLOWS_DEVICE";
              else
                pattern = "REAPER_TRACK_FOLLOWS_REAPER";
            } else if (!strcmp(pattern, "DEVICE_TRACK_FOLLOWS")) {
              if (!strcmp(s, "LAST_TOUCHED"))
                pattern = "DEVICE_TRACK_FOLLOWS_LAST_TOUCHED";
              else
                pattern = "DEVICE_TRACK_FOLLOWS_DEVICE";
            } else if (!strcmp(pattern, "DEVICE_TRACK_BANK_FOLLOWS")) {
              if (!strcmp(s, "MIXER"))
                pattern = "DEVICE_TRACK_BANK_FOLLOWS_MIXER";
              else
                pattern = "DEVICE_TRACK_BANK_FOLLOWS_DEVICE";
            } else if (!strcmp(pattern, "DEVICE_FX_FOLLOWS")) {
              if (!strcmp(s, "LAST_TOUCHED"))
                pattern = "DEVICE_FX_FOLLOWS_LAST_TOUCHED";
              else if (!strcmp(s, "FOCUSED"))
                pattern = "DEVICE_FX_FOLLOWS_FOCUSED";
              else
                pattern = "DEVICE_FX_FOLLOWS_DEVICE";
            }
          }
        }
        ...
        if (strstarts(pattern, "REAPER_TRACK_FOLLOWS_")) {
          ...
          if (!strcmp(pattern, "DEVICE_TRACK_FOLLOWS_LAST_TOUCHED")) {
            ...
            if (tr)
              Extended(CSURF_EXT_SETLASTTOUCHEDTRACK, tr, 0, 0);
          }
        }
          ...
          if (!strcmp(pattern, "DEVICE_FX_FOLLOWS_LAST_TOUCHED")) {
            ...
            if (GetLastTouchedFX(&tidx, &fxidx, 0)) {
              ...
              if (tr)
                Extended(CSURF_EXT_SETLASTTOUCHEDFX, tr, 0, &fxidx);
            }
            ...
            if (GetFocusedFX(&tidx, 0, &fxidx)) {
              MediaTrack* tr = CSurf_TrackFromID(tidx, false);
              if (tr)
                Extended(CSURF_EXT_SETFOCUSEDFX, tr, 0, &fxidx);
            }
          }
        }
      }
    ...
    if (strends(pattern, "_SELECT")) {
      ...
      if (ival >= 0) {
        if (strends(pattern, "_TRACK_SELECT")) {
          ...
          if (m_curtrack < 0)
            m_curtrack = 0;
          else if (m_curtrack > numtracks)
            m_curtrack = numtracks;
          if (m_followflag & 16) {
            MediaTrack* tr = CSurf_TrackFromID(m_curtrack, !!(m_followflag & 8));
            if (tr)
              SetOnlyTrackSelected(tr);
          ...
          if (m_curbankstart < 0)
            ...
            m_curbankstart = max(0, numtracks - 1);
          ...
          if (m_curfxparmbankstart < 0)
            m_curfxparmbankstart = 0;
          else if (m_curfxparmbankstart >= numparms)
            m_curfxparmbankstart = max(0, numparms - 1);
          ...
          int fxidx = -1;
          if (tr)
            fxidx = TrackFX_GetInstrument(tr);
          if (fxidx >= 0)
            numparms = TrackFX_GetNumParams(tr, fxidx);
          m_curfxinstparmbankstart = (ival - 1) * m_fxinstparmbanksize;
          if (m_curfxinstparmbankstart < 0)
            m_curfxinstparmbankstart = 0;
          else if (m_curfxinstparmbankstart >= numparms)
            m_curfxinstparmbankstart = max(0, numparms - 1);
          ...
          if (m_curmarkerbankstart < 0)
            m_curmarkerbankstart = 0;
          else if (m_curmarkerbankstart >= nm)
            m_curmarkerbankstart = max(0, nm - 1);
          ...
          if (m_curregionbankstart < 0)
            m_curregionbankstart = 0;
          else if (m_curregionbankstart >= nr)
            m_curregionbankstart = max(0, nr - 1);
    ...
    if (prevbank || nextbank) {
      if (TriggerMessage(rmsg, flag) > 0 && m_trackbanksize) {
        int numtracks = CSurf_NumTracks(!!(m_followflag & 8));
        if (prevbank)
          m_curbankstart -= m_trackbanksize;
        else
          m_curbankstart += m_trackbanksize;
        if (m_curbankstart < 0) {
          if (m_followflag & 8)
    ...
    if (prevfxparmbank || nextfxparmbank) {
      if (TriggerMessage(rmsg, flag) > 0 && m_fxparmbanksize) {
        ...
        if (tr)
          numparms = TrackFX_GetNumParams(tr, m_curfx);
        if (prevfxparmbank)
          m_curfxparmbankstart -= m_fxparmbanksize;
        else
          m_curfxparmbankstart += m_fxparmbanksize;
        if (m_curfxparmbankstart < 0)
          m_curfxparmbankstart = max(0, numparms - 1);
    ...
    if (prevfxinstparmbank || nextfxinstparmbank) {
      if (TriggerMessage(rmsg, flag) > 0 && m_fxinstparmbanksize) {
        ...
        if (fxidx >= 0)
          numparms = TrackFX_GetNumParams(tr, fxidx);
        if (prevfxinstparmbank)
          m_curfxinstparmbankstart -= m_fxinstparmbanksize;
        else
          m_curfxinstparmbankstart += m_fxinstparmbanksize;
        if (m_curfxinstparmbankstart < 0)
          m_curfxinstparmbankstart = max(0, numparms - 1);
    ...
    if (prevmarkerbank || nextmarkerbank) {
      if (TriggerMessage(rmsg, flag) > 0 && m_markerbanksize) {
        ...
        if (prevmarkerbank)
          m_curmarkerbankstart -= m_markerbanksize;
        else if (nextmarkerbank)
          m_curmarkerbankstart += m_markerbanksize;
        if (m_curmarkerbankstart < 0)
          m_curmarkerbankstart = max(0, nm - 1);
        else if (m_curmarkerbankstart >= nm)
    ...
    if (prevregionbank || nextregionbank) {
      if (TriggerMessage(rmsg, flag) > 0 && m_regionbanksize) {
        ...
        CountProjectMarkers(NULL, &nm, &nr);
        if (prevregionbank)
          m_curregionbankstart -= m_regionbanksize;
        else if (nextregionbank)
          m_curregionbankstart += m_regionbanksize;
        if (m_curregionbankstart < 0)
          m_curregionbankstart = max(0, nr - 1);
        else if (m_curregionbankstart >= nr)
  ...
  bool ProcessMarkerRegionAction(OscMessageRead* rmsg, const char* pattern, char flag, int* wc,
                                 int numwc) {
    int idx = -1;
    bool is_region = false;
    if (strstarts(pattern, "LAST_MARKER_") || (is_region = strstarts(pattern, "LAST_REGION_"))) {
      ...
      if (idx < 0)
        return true;
    } else if (strstarts(pattern, "MARKER_") || (is_region = strstarts(pattern, "REGION_"))) {
      if (!numwc)
        return true;
      idx = wc[0] - 1;
      if (is_region)
        idx += m_curregionbankstart;
      else
        idx += m_curmarkerbankstart;
      ...
      for (int i = 0;;) {
        ...
        if (!nexti)
          return true; // ran out of search items
    ...
    } else if (strstarts(pattern, "MARKERID_") || (is_region = strstarts(pattern, "REGIONID_"))) {
      if (!numwc || wc[0] < 0)
        return true;
      ...
      for (int pass = 0;; pass++) {
        ...
        if (pass || AddProjectMarker2(NULL, is_region, 0.0, 1.0, "", wc[0], 0) < 0)
          return true;
      }
    } else
      return false;
    ...
    if (strends(pattern, "_NAME")) {
      ...
      if (p && EnumProjectMarkers3(NULL, idx, &isrgn, &pos, &endpos, NULL, &ID, &color) &&
          isrgn == is_region) {
        SetProjectMarkerByIndex2(NULL, idx, is_region, pos, endpos, ID, p, color, !p[0] ? 1 : 0);
        Undo_OnStateChangeEx(isrgn ? "Set region name via OSC" : "Set marker name via OSC",
                             UNDO_STATE_MISCCFG, 0xdefe7000 | 200);
      }
    } else if (strends(pattern, "_NUMBER")) {
      double v = GetFloatArg(rmsg, flag, &hasarg);
      if (hasarg && EnumProjectMarkers3(NULL, idx, &isrgn, &pos, &endpos, NULL, &ID, &color) &&
          isrgn == is_region) {
        ID = (int)floor(v + 0.5);
        SetProjectMarkerByIndex2(NULL, idx, is_region, pos, endpos, ID, NULL, color, 0);
        Undo_OnStateChangeEx(isrgn ? "Set region number via OSC" : "Set marker number via OSC",
                             UNDO_STATE_MISCCFG, 0xdefe7000 | 200);
      }
      ...
      if (hasarg && EnumProjectMarkers3(NULL, idx, &isrgn, &pos, &endpos, NULL, &ID, &color) &&
        ...
        if (isrgn)
          endpos += pos;
        SetProjectMarkerByIndex2(NULL, idx, is_region, pos, endpos, ID, NULL, color, 0);
        Undo_OnStateChangeEx(isrgn ? "Set region start via OSC" : "Set marker position via OSC",
                             UNDO_STATE_MISCCFG, 0xdefe7000 | 200);
      }
  ...
  bool ProcessTrackAction(OscMessageRead* rmsg, const char* pattern, char flag, int* wc,
                          int numwc) {
    ...
    MediaTrack* tr = CSurf_TrackFromID(tidx, !!(m_followflag & 8));
    if (!tr)
      return true; // badly formed pattern

    if (strends(pattern, "_NAME")) {
      const char* p = rmsg->PopStringArg(false);
      if (p) {
        GetSetMediaTrackInfo((INT_PTR)tr, "P_NAME", (void*)p);
        Undo_OnStateChangeEx("Set track name via OSC", UNDO_STATE_TRACKCFG,
                             0xdefe7000 | 200); // undocumented defer-undo syntax
      }
      return true;
    }
    ...
    if (pan || pan2) {
      ...
      if (hasarg) {
        if (strchr("nbtr", flag))
          v = v * 2.0 - 1.0;
        if (pan)
          CSurf_OnPanChangeEx(tr, v, false, true);
        else if (pan2)
          CSurf_OnWidthChangeEx(tr, v, false, true);
    ...
    if (mute || solo || recarm || monitor) {
      ...
      if (t || is_direct_parm) {
        ...
        if (a >= 0) {
          if (mute)
            CSurf_OnMuteChangeEx(tr, a, true);
          else if (solo)
            CSurf_OnSoloChangeEx(tr, a, true);
          else if (recarm)
            CSurf_OnRecArmChangeEx(tr, a, true); // triggers UpdateAllExternalSurfaces
          else if (monitor)
            CSurf_OnInputMonitorChangeEx(tr, a, true);
          ...
          for (i = 0; i <= n; ++i) {
            ...
            if (tr == ttr || (sel && IsTrackSelected(ttr))) {
              if (mute)
                SetSurfaceMute(ttr, !!a);
              else if (solo)
                SetSurfaceSolo(ttr, !!a);
              else if (recarm)
                SetSurfaceRecArm(ttr, !!a);
              else if (monitor)
                Extended(CSURF_EXT_SETINPUTMONITOR, ttr, &a, 0);
            }
          }
        }
      }
    ...
    int automode = -1;
    if (!strcmp(pattern, "TRACK_AUTO_TRIM"))
      automode = AUTO_MODE_TRIM;
    else if (!strcmp(pattern, "TRACK_AUTO_READ"))
      automode = AUTO_MODE_READ;
    else if (!strcmp(pattern, "TRACK_AUTO_LATCH"))
      automode = AUTO_MODE_LATCH;
    else if (!strcmp(pattern, "TRACK_AUTO_TOUCH"))
      automode = AUTO_MODE_TOUCH;
    else if (!strcmp(pattern, "TRACK_AUTO_WRITE"))
      automode = AUTO_MODE_WRITE;
    if (automode >= 0) {
      int t = TriggerMessage(rmsg, flag);
      if (t) {
        if (t < 0)
        ...
        for (i = -1; i < n; ++i) {
          ...
          if (!ttr || (i >= 0 && !IsTrackSelected(ttr)))
            continue;
  ...
  bool ProcessFXAction(OscMessageRead* rmsg, const char* pattern, char flag, int* wc, int numwc) {
    ...
    if (fxparm || fxwet || fxeqwet || instparm || lasttouchedparm) {
      ...
      if (fArg || intArg) {
        ...
        if (fxparm) {
          if (!numwc)
            return true; // need at least parmidx
          parmidx = m_curfxparmbankstart + wc[0] - 1;
          if (numwc > 1)
            fxidx = wc[1] - 1;
          if (numwc > 2)
            tidx = m_curbankstart + wc[2];
          ...
          tr = CSurf_TrackFromID(tidx, !!(m_followflag & 8));
          if (tr)
            fxidx = TrackFX_GetEQ(tr, !!(m_followflag & 32));
          parmidx = TrackFX_GetParamFromIdent(tr, fxidx, ":wet");
        } else if (instparm) {
          if (!numwc)
            return true; // need at least parmidx
          ...
          if (!GetLastTouchedFX(&tidx, &fxidx, &parmidx))
            return true;
    ...
    if (isinst || iseq) {
      ...
      if (strends(pattern, "_BYPASS"))
        pattern = "FX_BYPASS";
      else if (strends(pattern, "_OPEN_UI"))
        pattern = "FX_OPEN_UI";
      else if (strends(pattern, "_PREV_PRESET"))
        pattern = "FX_PREV_PRESET";
      else if (strends(pattern, "_NEXT_PRESET"))
        pattern = "FX_NEXT_PRESET";
      else if (strends(pattern, "_PRESET"))
        pattern = "FX_PRESET";
    } else {
      if (numwc)
        fxidx = wc[0] - 1;
    ...
    if (strstarts(pattern, "FX_EQ_")) {
      ...
      if (strends(pattern, "_MASTER_GAIN")) {
        ...
        if (strstr(pattern, "_HIPASS_"))
          bandtype = 0;
        else if (strstr(pattern, "_LOSHELF_"))
          bandtype = 1;
        else if (strstr(pattern, "_BAND_"))
          bandtype = 2;
        else if (strstr(pattern, "_NOTCH_"))
          bandtype = 3;
        else if (strstr(pattern, "_HISHELF_"))
          bandtype = 4;
        else if (strstr(pattern, "_LOPASS_"))
          bandtype = 5;

        if (strends(pattern, "_FREQ"))
          parmtype = 0;
        else if (strends(pattern, "_GAIN"))
          parmtype = 1;
        else if (strends(pattern, "_Q"))
          parmtype = 2;
        else if (strends(pattern, "_BYPASS"))
          parmtype = -1;
      }
  ...
  static void DoOSCAction(const char* pattern, const int* cmd, const char* str, const float* val,
                          bool isRelative) {
    int cmdid = 0;
    if (cmd)
      cmdid = *cmd;
    else if (str && str[0])
      cmdid = NamedCommandLookup(str);
    if (!cmdid)
      return;
    ...
    if (!strcmp(pattern, "ACTION")) {
      ...
      if (cmd)
        MIDIEditor_LastFocused_OnCommand(cmdid, false);
  ...
  bool ProcessAction(OscMessageRead* rmsg, const char* pattern, char flag, int* wc, int numwc) {
    ...
    if (goto_marker || goto_region) {
      ...
      if (idx >= 0) {
        if (goto_marker)
          GoToMarker(NULL, idx + m_curmarkerbankstart, true);
        else if (goto_region)
          GoToRegion(NULL, idx + m_curregionbankstart, true);
      }
      return true;
    }
  ...
  bool ProcessVKBMIDI(OscMessageRead* rmsg, const char* pattern, char flag, int* wc, int numwc) {
    if (strncmp(pattern, "VKB_MIDI_", 9))
      return false;
    ...
    if (isPitch || isPC || isCP || isCC || isPA || !strcmp(pattern, "NOTE")) {
      ...
      if (isPitch) {
        ...
        if (isPC || isCP) {
          ...
          if (numwc < 1)
            return false; // CC/PA/NOTE all require a wildcard to specify the note parameter
  ...
  bool ProcessMessage(OscMessageRead* rmsg) {
    ...
    for (i = rptcnt - 1; i >= 0; --i) {
      int* twc = wc + i * numwc;
      if (!numwc && ProcessGlobalAction(rmsg, pattern, flag))
        ok = true;
      else if (ProcessStateAction(rmsg, pattern, flag, twc, numwc))
        ok = true;
      else if (ProcessTrackAction(rmsg, pattern, flag, twc, numwc))
        ok = true;
      else if (ProcessMarkerRegionAction(rmsg, pattern, flag, twc, numwc))
        ok = true;
      else if (ProcessFXAction(rmsg, pattern, flag, twc, numwc))
        ok = true;
      else if (ProcessSendAction(rmsg, pattern, flag, twc, numwc))
        ok = true;
      else if (ProcessAction(rmsg, pattern, flag, twc, numwc))
        ok = true;
      else if (ProcessVKBMIDI(rmsg, pattern, flag, twc, numwc))
        ok = true;
    }
  ...
  // nval=normalized
  void SetSurfaceVal(const char* pattern, const int* wcval, int numwcval, const int* ival,
                     const double* fval, const double* nval, const char* sval) {
    if (osc_send_inactive())
      return;
    if (!pattern)
      return;
    ...
    if (!keyidx)
      return; // code error, asking to send an undefined key string
    ...
    for (i = *keyidx + 1; i < m_msgtab.GetSize(); ++i) {
      ...
      if (flag == 'i')
        ...
        tfval = fval;
      else if (strchr("nbt", flag))
        tfval = nval;
      else if (flag == 's')
        tsval = sval;
      else
        continue; // don't send 'r' messages to the device
      ...
      char msg[2048];
      {
        ...
        if (needed_wc != numwcval)
          continue; // must have exact match of wildcard counts
...

      {
        ...
        if (tival)
          need_update = lastval->UpdateInt(*tival);
        else if (tfval)
          need_update = lastval->UpdateFloat((float)*tfval);
        else if (tsval == SPECIAL_STRING_CLEARCACHE) {
          if (lastval != &f)
            m_lastvals.Delete(msg);
          continue;
        } else if (tsval)
          need_update = lastval->UpdateString(tsval);
        else
          continue; // unknown type

        if (lastval == &f)
          m_lastvals.Insert(msg, f);
        else if (!need_update)
          continue;
      }
      ...
      if (tival)
        wmsg.PushIntArg(*tival);
      else if (tfval)
        wmsg.PushFloatArg((float)*tfval);
      else if (tsval)
        wmsg.PushStringArg(tsval);

      if (m_osc)
        OscSendOutput(m_osc, &wmsg);
  ...
  void SetSurfaceTrackVal(const char* pattern, int tidx, int* ival, double* fval, double* nval,
                          const char* sval) {
    if (tidx == m_curtrack) {
  ...
  static void GetFXName(MediaTrack* tr, int fxidx, char* buf, int buflen) {
    ...
    if (TrackFX_GetFXName(tr, fxidx, buf, buflen) && buf[0]) {
      const char* prefix[] = {
          "VST: ", "VSTi: ", "VST3: ", "VST3i: ", "JS: ", "DX: ", "DXi: ", "AU: ", "AUi: "};
      int i;
  ...
  void UpdateSurfaceTrack(int tidx) {
    ...
    bool sel = false;
    if (tr)
      sel = IsTrackSelected(tr);
    SetSurfaceSelectedImpl(tidx, sel);

    double vol = 0.0;
    if (tr)
      GetTrackUIVolPan(tr, &vol, 0);
    SetSurfaceVolumeImpl(tidx, vol);

    double pan[2] = {0.0, 0.0};
    int panmode = PAN_MODE_NEW_BALANCE;
    if (tr)
      GetTrackUIPan(tr, &pan[0], &pan[1], &panmode);
    ...
    for (i = 0; i < m_sendbanksize; ++i) {
      buf[0] = 0;
      if (tr)
        GetTrackSendName(tr, i, buf, sizeof(buf));
      if (!buf[0])
        sprintf(buf, (!tidx ? "Aux %d" : "Send %d"), i + 1);
      int wc[2] = {i + 1, tidx - m_curbankstart};
      if (!tidx)
        SETSURFSTRWC("MASTER_SEND_NAME", wc, 1, buf);
      if (iscur)
        SETSURFSTRWC("TRACK_SEND_NAME", wc, 1, buf);
      if (isbank)
        SETSURFSTRWC("TRACK_SEND_NAME", wc, 2, buf);

      double vol = 0.0;
      double pan = 0.0;
      if (tr)
        GetTrackSendUIVolPan(tr, i, &vol, &pan);
    ...
    if (tidx) {
      for (i = 0; i < m_recvbanksize; ++i) {
        buf[0] = 0;
        if (tr)
          GetTrackReceiveName(tr, i, buf, sizeof(buf));
        if (!buf[0])
          sprintf(buf, "Recv %d", i + 1);
        int wc[2] = {i + 1, tidx - m_curbankstart};
        if (iscur)
          SETSURFSTRWC("TRACK_RECV_NAME", wc, 1, buf);
        if (isbank)
          SETSURFSTRWC("TRACK_RECV_NAME", wc, 2, buf);

        double vol = 0.0;
        double pan = 0.0;
        if (tr)
          GetTrackReceiveUIVolPan(tr, i, &vol, &pan);
  ...
  void SetActiveFXEQChange() {
    ...
    static const char* bandstr[6] = {"_HIPASS", "_LOSHELF", "_BAND",
                                     "_NOTCH",  "_HISHELF", "_LOPASS"};
    ...
    if (tr && fxidx >= 0) {
      ...
      for (i = 0; i < numparms; ++i) {
        ...
        int f = (fxidx << 16) | i;
        Extended(FX_IDX_MODE(fxidx) == FX_IDX_MODE_REC ? CSURF_EXT_SETFXPARAM_RECFX
  ...
  void SetActiveFXChange() {
    ...
    if (m_curfx >= numfx)
      m_curfx = 0;
    if (m_curfxparmbankstart >= numparms)
      m_curfxparmbankstart = 0;
    ...
    int banksel = 0;
    if (m_fxparmbanksize > 0)
      banksel = m_curfxparmbankstart / m_fxparmbanksize;
  ...
  void SendUpdateFXParms(MediaTrack* tr, int curfx, int numfx, int numparms, const int* wc_lead,
                         int wc_lead_size) {
    if (osc_send_inactive())
      return;
    int i;
    for (i = 0; i < m_fxparmbanksize; ++i) {
      ...
      wc[0] = i + 1;
      if (wcsz > 1)
        memcpy(wc + 1, wc_lead, (wcsz - 1) * sizeof(int));
  ...
  void SetActiveFXInstChange() {
    ...
    if (fxidx >= 0)
      numparms = TrackFX_GetNumParams(tr, fxidx);
    if (m_curfxinstparmbankstart >= numparms)
      m_curfxinstparmbankstart = 0;
    ...
    sprintf(buf, "%d",
            m_fxinstparmbanksize > 0 ? m_curfxinstparmbankstart / m_fxinstparmbanksize + 1 : 1);
  ...
  void SetMarkerBankChange(int old_marker_size = 0, int old_region_size = 0) {
    ...
    while (0 != (i = EnumProjectMarkers3(NULL, i, &isrgn, &pos, &endpos, &name, &idx, NULL))) {
      ...
      if (mcnt >= m_markerbanksize && rcnt >= m_regionbanksize)
        break;
    }
    ...
    if (domarkers) {
      for (i = max(mcnt + 1, 1); i <= marker_top; ++i) {
        ...
        if (p == SPECIAL_STRING_CLEARCACHE)
          SETSURFSTRWC("MARKER_TIME", wc, 1, p);
      }
    }
    if (doregions) {
  ...
  void SetSurfaceSelected(MediaTrack* tr, bool selected) {
    ...
    int tidx = GetSurfaceTrackIdx(tr);
    if (tidx >= 0)
      SetSurfaceSelectedImpl(tidx, selected);
  }
  ...
  void SetSurfaceVolumeImpl(int tidx, double volume) {
    ...
    if (!tidx)
      SetSurfaceVal("MASTER_VOLUME", 0, 0, 0, &db, &v, buf);
  ...
  void SetSurfaceVolume(MediaTrack* tr, double volume) {
    ...
    int tidx = GetSurfaceTrackIdx(tr);
    if (tidx >= 0)
      SetSurfaceVolumeImpl(tidx, volume);
  }
  ...
  void SetSurfaceRecArm(MediaTrack* tr, bool recarm) {
    ...
    int tidx = GetSurfaceTrackIdx(tr);
    if (tidx >= 0)
      SetSurfaceRecArmImpl(tidx, recarm);
  }

  bool GetTouchState(MediaTrack* tr, int ispan) {
    if (ispan != 0 && ispan != 1 && ispan != 2)
      return false;

    const int tidx = GetSurfaceTrackIdx(tr);
    if (tidx < 0 || tidx >= MAX_LASTTOUCHED_TRACK)
      return false;
    ...
    DWORD lastt = (!ispan       ? m_vol_lasttouch[tidx]
                   : ispan == 1 ? m_pan_lasttouch[tidx]
                                : m_pan2_lasttouch[tidx]);
    if (!lastt)
      return false;
  ...
  int Extended(int call, void* parm1, void* parm2, void* parm3) {
    ...
    if (call == CSURF_EXT_SETPAN_EX || call == CSURF_EXT_SETINPUTMONITOR ||
        call == CSURF_EXT_SETSENDVOLUME || call == CSURF_EXT_SETSENDPAN ||
        call == CSURF_EXT_SETRECVVOLUME || call == CSURF_EXT_SETRECVPAN ||
        call == CSURF_EXT_SETFXENABLED || call == CSURF_EXT_SETFXOPEN) {
      if (osc_send_inactive())
        return 1;
      if (parm1)
        tidx = CSurf_TrackToID((MediaTrack*)parm1, !!(m_followflag & 8));
      if (tidx < 0)
        return 1;
      parm1 = &tidx;
    ...
    if (call == CSURF_EXT_SETFXENABLED_IMPL || call == CSURF_EXT_SETFXOPEN_IMPL) {
      if (parm1 && parm2) // parm3 can be index 0
      {
        ...
        if (tidx == m_curtrack) {
          if (call == CSURF_EXT_SETFXENABLED_IMPL) {
            if (fxidx == m_curfx)
              SETSURFBOOLWC("FX_BYPASS", wc, 0, en);
            SETSURFBOOLWC("FX_BYPASS", wc, 1, en);
            if (isinst)
              SETSURFBOOL("FX_INST_BYPASS", en);
            if (iseq)
              SETSURFBOOL("FX_EQ_BYPASS", en);
          } else if (call == CSURF_EXT_SETFXOPEN_IMPL) {
            if (fxidx == m_curfx)
              SETSURFBOOLWC("FX_OPEN_UI", wc, 0, en);
            SETSURFBOOLWC("FX_OPEN_UI", wc, 1, en);
            if (isinst)
              SETSURFBOOL("FX_INST_OPEN_UI", en);
            if (iseq)
              SETSURFBOOL("FX_EQ_OPEN_UI", en);
          }
        }
        if (wc[1] > 0 && wc[1] <= m_trackbanksize) {
          if (call == CSURF_EXT_SETFXENABLED_IMPL) {
            ...
            if (isinst)
              SETSURFBOOLWC("FX_INST_BYPASS", wc + 1, 1, en);
            if (iseq)
              SETSURFBOOLWC("FX_EQ_BYPASS", wc + 1, 1, en);
          } else if (call == CSURF_EXT_SETFXOPEN_IMPL) {
            SETSURFBOOLWC("FX_OPEN_UI", wc, 2, en);
            if (isinst)
              SETSURFBOOLWC("FX_INST_OPEN_UI", wc + 1, 1, en);
            if (iseq)
              SETSURFBOOLWC("FX_EQ_OPEN_UI", wc + 1, 1, en);
          }
        }
      }
      return 1;
    ...
    if (call == CSURF_EXT_SETFXPARAM) {
      ...
      if (parm1 && parm2 && parm3) {
        ...
        if ((m_wantfx & 4) && fxidx == TrackFX_GetInstrument(tr))
          isinst = true;
        else if ((m_wantfx & 32) && fxidx == TrackFX_GetEQ(tr, false))
          iseq = true;
        ...
        if (tidx == m_curtrack) {
          ...
          if ((m_wantfx & 1) && fxidx == m_curfx) {
            if (parmidx >= m_curfxparmbankstart &&
          ...
          if (isinst) {
            if (parmidx >= m_curfxinstparmbankstart &&
        ...
        if (m_wantfx & 2) {
          ...
          int lfxidx = -1;
          int lparmidx = -1;
          if (GetLastTouchedFX(&ltidx, &lfxidx, &lparmidx) && tidx == ltidx && fxidx == lfxidx &&
              parmidx == lparmidx) {
            islasttouched = true;
          }
        }

        if (iscurfx || iscurinst || iscureq || islasttouched || iscurtrackfx || isbanktrackfx ||
            isbanktrackfxinst || isbanktrackeq) {
          char buf[512];
          buf[0] = 0;
          ...
          if (iscureq || isbanktrackeq) {
            ...
            if (TrackFX_GetEQParam(tr, fxidx, parmidx, &bandtype, &bandidx, &parmtype, 0)) {
              ...
              if (iscureq)
                SetSurfaceVal(pattern, (isband ? wc : 0), (isband ? 1 : 0), 0, 0, &val, buf);
              if (isbanktrackeq)
                SetSurfaceVal(pattern, (isband ? wc : wc + 1), (isband ? 2 : 1), 0, 0, &val, buf);
            }
          }
          ...
          if (!wet_only && (iscurinst || isbanktrackfxinst)) {
            ...
            if (iscurinst)
              SetSurfaceVal("FX_INST_PARAM_VALUE", wc, 1, 0, 0, &val, buf);
            if (isbanktrackfxinst)
              SetSurfaceVal("FX_INST_PARAM_VALUE", wc, 2, 0, 0, &val, buf);
          }
    ...
    if (call == CSURF_EXT_SETFOCUSEDFX || call == CSURF_EXT_SETLASTTOUCHEDFX) {
      if (parm1 && !parm2 && parm3) {
  ...
  bool osc_send_inactive() const {
    return (!m_osc || !m_osc->m_send_enable) && !m_osc_local;
  }
};
...
void OscLocalMessageToHost(const char* msg, const double* value) {
  ...
  if (!tmposc) {
    ...
    Plugin_Register("csurf_inst",
                    (void*)tmposc); // install as a hidden surface so GetTouchState() is called, etc
  }
...
static IReaperControlSurface* createFunc(const char* typestr, const char* cfgstr, int* err) {
  ...
  char cfgfn[2048];
  ParseCfg(cfgstr, name, sizeof(name), &flags, &recvport, sendip, &sendport, &maxpacketsz,
           &sendsleep, cfgfn, sizeof(cfgfn));
...
void SendLocalOscMessage(void* csurf_osc, const char* msg, int msglen) {
  if (!csurf_osc || !msg || !msg[0] || msglen > MAX_OSC_MSG_LEN)
    return;
...
static WDL_DLGRET OscListenProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam) {
  ...
  case WM_INITDIALOG: {
    ...
    if (lParam) {
      char buf[512];
      snprintf(buf, sizeof(buf), __LOCALIZE_VERFMT("OSC: listening to port %d", "csurf_osc"),
               (int)lParam);
      ...
      if (s_osc)
        ...
        SetDlgItemText(hwndDlg, IDC_EDIT1,
                       __LOCALIZE("Error: can't create OSC listener", "csurf_osc"));
...
static WDL_DLGRET dlgProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam) {
  ...
  case WM_INITDIALOG: {
    ...
    if (lParam) {
      ParseCfg((const char*)lParam, name, sizeof(name), &flags, &recvport, sendip, &sendport,
               &maxpacketsz, &sendsleep, s_cfgfn, sizeof(s_cfgfn));
    }
    if (!sendip[0])
      strcpy(sendip, "0.0.0.0");
    WDL_UTF8_HookComboBox(GetDlgItem(hwndDlg, IDC_COMBO1));
    WDL_UTF8_HookComboBox(GetDlgItem(hwndDlg, IDC_COMBO4));

    SendDlgItemMessage(hwndDlg, IDC_COMBO4, CB_ADDSTRING, 0,
                       (LPARAM)__LOCALIZE("Device IP/port", "csurf_osc"));
    SendDlgItemMessage(hwndDlg, IDC_COMBO4, CB_ADDSTRING, 0,
                       (LPARAM)__LOCALIZE("Device IP/port [send only]", "csurf_osc"));
    SendDlgItemMessage(hwndDlg, IDC_COMBO4, CB_ADDSTRING, 0,
                       (LPARAM)__LOCALIZE("Local port", "csurf_osc"));
    SendDlgItemMessage(hwndDlg, IDC_COMBO4, CB_ADDSTRING, 0,
                       (LPARAM)__LOCALIZE("Local port [receive only]", "csurf_osc"));
    SendDlgItemMessage(hwndDlg, IDC_COMBO4, CB_ADDSTRING, 0,
                       (LPARAM)__LOCALIZE("Configure device IP+local port", "csurf_osc"));
    SendDlgItemMessage(hwndDlg, IDC_COMBO4, CB_ADDSTRING, 0,
                       (LPARAM)__LOCALIZE("Disabled", "csurf_osc"));
    ...
    case 2:
      ...
      break;
    case 3:
      cbidx = recvport == 0 ? 0 : sendport == 0 ? 2 : 4;
      break;
    }
  ...
  case WM_COMMAND:
    if (uMsg == WM_INITDIALOG ||
      ...
      if (recvport) {
        DialogBoxParam(g_hInst, MAKEINTRESOURCE(IDD_OSC_LISTEN), hwndDlg, OscListenProc,
                       (LPARAM)recvport);
      }
        ...
        if (s_cfgfn[0] && !LoadCfgFile(s_cfgfn, 0, errbuf, sizeof(errbuf))) {
          if (!errbuf[0])
            snprintf(errbuf, sizeof(errbuf),
                     __LOCALIZE_VERFMT("Warning: possible error parsing config file \"%s\"",
                                       "csurf_osc"),
                     s_cfgfn);
  ...
  case WM_USER + 100: // refresh pattern config file list
  {
    ...
    if (!ds.First(dir)) {
      do {
        ...
        if (len > extlen && !stricmp(fn + len - extlen, OSC_EXT) &&
            strnicmp(fn, "default.", strlen("default."))) {
    ...
    SendDlgItemMessage(hwndDlg, IDC_COMBO1, CB_ADDSTRING, 0,
                       (LPARAM)__LOCALIZE("Default", "csurf_osc"));
    ...
    for (i = 0; i < s_patterncfgs.GetSize(); ++i) {
      ...
      if (!stricmp(s_cfgfn, p))
        cursel = a;
    }

    SendDlgItemMessage(hwndDlg, IDC_COMBO1, CB_ADDSTRING, 0,
                       (LPARAM)__LOCALIZE("(refresh list)", "csurf_osc"));
    SendDlgItemMessage(hwndDlg, IDC_COMBO1, CB_ADDSTRING, 0,
                       (LPARAM)__LOCALIZE("(open config directory)", "csurf_osc"));
...
static HWND configFunc(const char* typestr, HWND parent, const char* cfgstr) {
  return CreateDialogParam(g_hInst, MAKEINTRESOURCE(IDD_SURFACEEDIT_OSC), parent, dlgProc,
                           (LPARAM)cfgstr);
}
