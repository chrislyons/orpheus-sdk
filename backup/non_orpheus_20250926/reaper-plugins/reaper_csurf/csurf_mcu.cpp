...
class CSurf_MCU : public IReaperControlSurface {
  ...
  unsigned int m_fader_lasttouch[256]; // m_fader_touchstate changes will clear this, moves
                                       // otherwise set it. if set to -1, then totally disabled
...
#define FADER_REPOS_WAIT 250
  ...
  int GetBankOffset() const {
    return (m_cfg_flags & CONFIG_FLAG_NOBANKOFFSET) ? (m_offset + 1)
  ...
  bool OnJogWheel(MIDI_event_t* evt) {
    if ((evt->midi_message[0] & 0xf0) == 0xb0 && evt->midi_message[1] == 0x3c) // jog wheel
    {
  ...
  bool OnMarker(MIDI_event_t* evt) {
    SendMessage(g_hwnd, WM_COMMAND, IsKeyDown(VK_SHIFT) ? ID_INSERT_MARKERRGN : ID_INSERT_MARKER,
                0);
    return true;
  }
  ...
  bool OnButtonPress(MIDI_event_t* evt) {
    ...
    if (evt->midi_message[2] >= 0x40) {
      ...
      bool double_click =
          (int)evt_code == m_button_last && now - m_button_last_time < DOUBLE_CLICK_INTERVAL;
  ...
  void OnMIDIEvent(MIDI_event_t* evt) {
    ...
    static const MidiHandlerFunc handlers[nHandlers] = {
        &CSurf_MCU::OnMCUReset, &CSurf_MCU::OnFaderMove,   &CSurf_MCU::OnRotaryEncoder,
        &CSurf_MCU::OnJogWheel, &CSurf_MCU::OnButtonPress,
    };
...
public:
  ...
  const char* GetDescString() {
    m_descspace.SetFormatted(512,
                             m_is_mcuex
                                 ? __LOCALIZE_VERFMT("Mackie Control Extended (dev %d,%d)", "csurf")
  ...
  {
    snprintf(m_configtmp, sizeof(m_configtmp), "%d %d %d %d %d", m_offset, m_size, m_midi_in_dev,
             m_midi_out_dev, m_cfg_flags);
    return m_configtmp;
  }
...
#define FIXID(id)                                                                                  \
  ...
  bool GetTouchState(MediaTrack* trackid, int isPan = 0) {
    ...
    if (id >= 0 && id < 9) {
      if (!(m_cfg_flags & CONFIG_FLAG_FADER_TOUCH_MODE) && !m_fader_touchstate[id] &&
          m_fader_lasttouch[id] && m_fader_lasttouch[id] != 0xffffffff) {
      ...
      if (no != m_allmcus_bank_offset) {
        ...
        for (x = 0; x < m_mcu_list.GetSize(); x++) {
          ...
          if (mcu && !mcu->m_is_mcuex && mcu->m_midiout) {
            mcu->m_midiout->Send(0xB0, 0x40 + 11, '0' + (((m_allmcus_bank_offset + 1) / 10) % 10),
                                 -1);
  ...
  if (!m_is_mcuex) {
    ...
    } else if (tmode == 5 || tmode == 8) // frames
    {
      ...
      *op = 0;
      ...
      int num_measures = 0;
      double beats =
          TimeMap2_timeToBeats(NULL, pp, &num_measures, NULL, NULL, NULL) + 0.000000000001;
    ...
    if (m_mackie_lasttime_mode != tmode) {
      ...
      m_midiout->Send(0x90, 0x72, m_mackie_lasttime_mode > 0 && tmode < 3 ? 0x7F : 0,
                      -1); // set beats light
    }
...
static IReaperControlSurface* createFunc(const char* type_string, const char* configString,
  ...
  return new CSurf_MCU(!strcmp(type_string, "MCUEX"), parms[0], parms[1], parms[2], parms[3],
                       parms[4], errStats);
}
...
static HWND configFunc(const char* type_string, HWND parent, const char* initConfigString) {
  return CreateDialogParam(g_hInst, MAKEINTRESOURCE(IDD_SURFACEEDIT_MCU1), parent, dlgProc,
