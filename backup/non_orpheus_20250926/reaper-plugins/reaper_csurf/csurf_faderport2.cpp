    ...
    if (evt->midi_message[0] == 0xb0 && evt->midi_message[1] == 0x10) {
        ...
        if (ishit && !m_fader_touchstate && m_vol_lastpos >= 0) {
          ...
          if (tr) {
            if (m_flipmode)
              CSurf_SetSurfacePan(
                  tr,
                  CSurf_OnPanChange(tr, int14ToPan(m_vol_lastpos >> 7, m_vol_lastpos & 127), false),
                  this);
            else
              CSurf_SetSurfaceVolume(
                  tr,
                  CSurf_OnVolumeChange(tr, int14ToVol(m_vol_lastpos >> 7, m_vol_lastpos & 127),
                                       false),
                  this);
          }
        }
      ...
      case 0x2A:
        if (ishit) {
          ...
          if (m_midiout)
            m_midiout->Send(0x90, 0x2A, m_flipmode ? 1 : 0, -1);
      ...
      if (sendback && m_midiout)
        m_midiout->Send(evt->midi_message[0], evt->midi_message[1], evt->midi_message[2], -1);
    }
  }
...
public:
  ...
  const char* GetDescString() {
    descspace.SetFormatted(512, __LOCALIZE_VERFMT("PreSonus FaderPort FP2 (dev %d,%d)", "csurf"),
...
#define FIXID(id)                                                                                  \
  ...
  bool GetTouchState(MediaTrack* trackid, int isPan) {
    ...
    if (!id) {
      if (!m_flipmode != !isPan) {
        if (m_pan_lasttouch == 1 ||
            (timeGetTime() - m_pan_lasttouch) < 3000) // fake touch, go for 3s after last movement
        {
          return true;
        }
        return false;
  ...
  void SetAutoMode(int mode) {
    if (m_midiout) {
      ...
      m_midiout->Send(0xa0, 0x10, 0, -1); // mode==0); // dont set off light, it disables the
                                          // fader?!
...
static IReaperControlSurface* createFunc_FP2(const char* type_string, const char* configString,
                                             int* errStats) {
  int parms[4];
