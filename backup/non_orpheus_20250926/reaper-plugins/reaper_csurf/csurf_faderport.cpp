...
class CSurf_FaderPort : public IReaperControlSurface {
  ...
  void OnMIDIEvent(MIDI_event_t* evt) {
    if (evt->midi_message[0] == 0xb0) {
      if (evt->midi_message[1] == 0)
        ...
        if (tr) {
          if (m_flipmode) {
            CSurf_SetSurfacePan(
                tr,
                CSurf_OnPanChange(tr, int14ToPan(m_faderport_lasthw, evt->midi_message[2]), false),
                NULL);
          } else
            CSurf_SetSurfaceVolume(
                tr,
                CSurf_OnVolumeChange(tr, int14ToVol(m_faderport_lasthw, evt->midi_message[2]),
                                     false),
                NULL);
        }
      }
      ...
      } else if (evt->midi_message[1] == 0xb || evt->midi_message[1] == 0xc ||
                 evt->midi_message[1] == 0xd) // automation read
      {
        if (evt->midi_message[2]) {
      ...
      } else if (evt->midi_message[1] == 0x0e) // undo/redo
      {
        if (evt->midi_message[2]) {
          ...
          SendMessage(g_hwnd, WM_COMMAND,
                      (m_faderport_buttonstates & 2) ? IDC_EDIT_REDO : IDC_EDIT_UNDO, 0);
        }
...
public:
  ...
  const char* GetDescString() {
    descspace.SetFormatted(512, __LOCALIZE_VERFMT("PreSonus FaderPort (dev %d,%d)", "csurf"),
...
#define FIXID(id)                                                                                  \
  ...
  void SetRepeatState(bool rep) {
    if (m_midiout)
      m_midiout->Send(0xa0, 8, rep ? 1 : 0, -1);
  }
