...
public:
  ...
  void III_get_side_info_2(struct III_sideinfo* si, int stereo, int ms_stereo, int sfreq,
                           int mpeg25);
  int do_layer3_sideinfo(struct frame* fr);
  int III_dequantize_sample(real xr[SBLIMIT][SSLIMIT], const int* scf, struct gr_info_s* gr_infos,
  ...
  int II_step_two(unsigned int* bit_alloc, real fraction[2][4][SBLIMIT], int* scale,
  ...
  int decode(unsigned int header, struct frame* fr, const unsigned char* in, int isize, sample* out,
             int osize, int* done);
