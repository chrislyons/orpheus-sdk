...
int mpglib::II_step_two(unsigned int* bit_alloc, real fraction[2][4][SBLIMIT], int* scale,
  ...
  for (i = jsbound; i < sblimit; i++, alloc1 += (size_t)(int)(1 << step)) {
    ...
    if ((ba = *bita++)) {
      ...
      fraction[0][0][i] = fraction[0][1][i] = fraction[0][2][i] = fraction[1][0][i] =
          fraction[1][1][i] = fraction[1][2][i] = 0.0;
    }
    /*
...
static void II_select_table(struct frame* fr) {
  static const int translate[3][2][16] = {{{0, 2, 2, 2, 2, 2, 2, 0, 0, 0, 1, 1, 1, 1, 1, 0},
...
int mpglib::do_layer2(sample* pcm_sample, int* pcm_point) {
  ...
  for (i = 0; i < SCALE_BLOCK; i++) {
    if (!II_step_two(bit_alloc, fraction, scale, fr, i >> 2))
      return 0;
    for (j = 0; j < 3; j++) {
      if (single >= 0) {
