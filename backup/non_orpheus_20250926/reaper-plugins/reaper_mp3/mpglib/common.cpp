...
const int freqs[9] = {44100, 48000, 32000, 22050, 24000, 16000, 11025, 12000, 8000};
...
int decode_header(struct frame* fr, unsigned int newhead) {
  ...
  if (freqs[fr->sampling_frequency] == 0)
    return 0;
  if ( // fr->bitrate_index!=0xF &&
      tabsel_123[fr->lsf][0][fr->bitrate_index] == 0)
    return 0;
...
unsigned get_frame_size_dword(
    unsigned int newhead) { // this is used really a lot, so lets put a "fast" version
#if 0
	{
		frame fr;
  ...
  if (freqs[sampling_frequency] == 0)
    return 0;
  if ( // bitrate_index!=0xF &&
      tabsel_123[lsf][0][bitrate_index] == 0)
    return 0;
...
unsigned int mpglib::get1bit(void) {
  ...
  if (mp.reservoir_ptr > 0) {
    ...
    if (bb > sizeof(mp.bitcache))
      bb = sizeof(mp.bitcache);
    ...
    if (mp.bitcache_size > sizeof(mp.bitcache) * 8)
      mp.bitcache_size = sizeof(mp.bitcache) * 8;
