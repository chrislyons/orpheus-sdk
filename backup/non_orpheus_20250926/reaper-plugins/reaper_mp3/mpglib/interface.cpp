...
int mpglib::decode(unsigned int header, struct frame* fr, const unsigned char* in, int isize,
                   sample* out, int osize, int* done) {
  ...
  if (isize > MAXFRAMESIZE - 4)
    isize = MAXFRAMESIZE - 4;
  ...
  *done = 0;
  ...
  /* buffer the ancillary data and reservoir for next frame */
  {
    ...
    if (mp.reservoir_bytes_total > sizeof(mp.reservoir))
      mp.reservoir_bytes_total = sizeof(mp.reservoir);
...
unsigned frame::get_bitrate() { // bits / second == (bytes*8) / time_seconds / 1000 == (bytes*8) /
                                // (sample_count() / srate) return (framesize+4)*8 *
                                // get_sample_rate() / get_sample_count();
