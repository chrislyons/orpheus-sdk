...
public:
  ...
  bool has_file_open() const {
    return m_framefile != NULL;
  } // only signifies whether a file is open (file could be closed but index is valid)

  bool IsCBR() const {
    return m_cbr_frame_size > 0;
  }

  int GetFrameCount() {
    return m_cbr_frame_size > 0 || m_framefile || m_numframes == m_frameposmemcache.GetSize()
               ? m_numframes
               : 0;
  }
  ...
  unsigned int GetSeekPositionForSample(INT64 splpos, int decsr, int* dumpSamples) {
    ...
    if (frame_pos < 0)
      frame_pos = 0;
    else if (frame_pos >= GetFrameCount())
      frame_pos = GetFrameCount() - 1;
...
private:
  unsigned int GetFramePos(int f) {
    ...
    } else if (m_cbr_frame_size > 0 && f >= 0 && f < m_numframes &&
               WDL_NORMALLY(m_encodingtag == 2)) {
      return m_cbr_base + m_cbr_frame_size * f;
    } else if (m_numframes == m_frameposmemcache.GetSize() && f >= 0 && f < m_numframes)
      return m_frameposmemcache.Get()[f];
  ...
  WDL_TypedQueue<unsigned int>
      m_frameposmemcache; // only used if !m_framefile, and then only if the mp3 is small enough
