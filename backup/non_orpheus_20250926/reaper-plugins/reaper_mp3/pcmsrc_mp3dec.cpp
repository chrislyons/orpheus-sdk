...
REAPER_PeakBuild_Interface* (*PeakBuild_CreateEx)(PCM_source* src, const char* fn, int srate,
                                                  int nch, int flags);
...
void (*__mergesort)(void* base, size_t nmemb, size_t size, int (*compar)(const void*, const void*),
                    void* tmpspace);
...
public:
  const char* m_err;
  PooledDecoderMediaInfo_Base() : m_metadata(false, WDL_StringKeyedArray<char*>::freecharptr) {
    m_channel_mode = -1;
    m_err = 0;
  ...
  PooledDecoderInstance* Open(const char* filename, int peaksMode, bool forceread) {
    ...
    if (peaksMode) {
      ...
      nb = 1;
      bs = 8192;
      if (GetPreferredDiskReadModePeak)
        GetPreferredDiskReadModePeak(&mode, &nb, &bs);
    } else {
      if (GetPreferredDiskReadMode)
        GetPreferredDiskReadMode(&mode, &nb, &bs);
    }
    ...
    if (!m_index || forceread || !m_srate || !m_nch || !m_syncframeinfo.framesize || m_err) // sync
    {
      m_stream_startpos = 0;
      ...
      if (!m_index) {
        m_index = mp3_index::indexFromFilename(filename, file,
                                               file->GetSize() >= g_config_reapindex_minsize);
      }
      ...
      while (!pdi->m_decoder.SyncState()) {
        if (pdi->m_decoder.queue_bytes_in.Available() < 4096) {
          ...
          pdi->m_read_pos += l;
          if (l > 0)
            pdi->m_decoder.queue_bytes_in.Add(buf, l);
        }
        if (pdi->m_decoder.Run())
          break;
      }
      ...
      if (pdi->m_decoder.SyncState()) {
        ...
        if (m_index)
          m_lengthsamples =
              (INT64)((m_index->GetFrameCount() * (double)m_srate) / pdi->m_decoder.GetFrameRate() +
                      0.5);
        else
          m_lengthsamples = 0;
...
#define POOLEDSRC_POOLEDMEDIAINFO_HAS_CLOSE
  void Close() {
    if (m_index && m_index->has_file_open()) // do not release index if it was built to memory
                                             // (regenerating would be slow)
    {
      mp3_index::release_index(m_index);
      m_index = NULL;
    }
...
int POOLED_PCMSOURCE_CLASSNAME::PoolExtended(int call, void* parm1, void* parm2, void* parm3) {
  ...
  if (call == PCM_SOURCE_EXT_GETBITRATE && m_filepool && m_filepool->extraInfo && parm1) {
    WDL_INT64 datalen =
        m_filepool->extraInfo->m_stream_endpos - m_filepool->extraInfo->m_stream_startpos;
  ...
  if (call == PCM_SOURCE_EXT_WRITE_METADATA && parm1 && parm2 && m_filepool &&
      m_filepool->extraInfo) {
    ...
    if (merge && ok) {
      ...
      if (rawtags.GetSize()) {
        ...
        for (int i = 0; i < metadata.GetSize(); ++i) {
          ...
          if (!strncmp(key, "ID3:", 4))
            DeleteID3Raw(&rawtags, key);
        }
      }
    }
    ...
    if (id3len && ok)
      ok = fw->Write(hb.Get(), id3len) == id3len;
    hb.Resize(0, false);

    if (ok)
      ok = !fr->SetPosition(spos) && CopyFileData(fr, fw, epos - spos);

    int apelen = PackApeChunk(&hb, &metadata);
    if (apelen && ok)
      ok = fw->Write(hb.Get(), apelen) == apelen;
  ...
  if (call == PCM_SOURCE_EXT_ENUMCUES_EX && parm2 && m_filepool && m_filepool->extraInfo) {
    ...
    if (cue)
      memset(cue, 0, sizeof(REAPER_cue));
    ...
    if (!m_filepool->extraInfo->m_cues.GetSize()) {
      ...
      while (chap) {
        ...
        if (sep1) {
          ...
          if (startms >= 0) {
            ...
            if (endms > startms)
              id3_cue.endpos = (double)endms * 0.001;
      ...
      __mergesort(m_filepool->extraInfo->m_cues.Get(), m_filepool->extraInfo->m_cues.GetSize(),
...
PooledDecoderInstance* POOLED_PCMSOURCE_CLASSNAME::CreateInstance() {
  if (!m_filepool->extraInfo)
    m_filepool->extraInfo = new PooledDecoderMediaInfo;
  return m_filepool->extraInfo->Open(GetFileName(), m_peaksMode, !m_filepool->HasResources());
}

void PCM_source_mp3::PooledGetSamples(PCM_source_transfer_t* block,
                                      PooledDecoderInstance* poolreadinst, void* rsModeToUse) {
  if (!poolreadinst->m_file)
    return;
  // process from file
  ...
  if (block->absolute_time_s == -100000.0)
    poolreadinst->m_lastpos = -100000;
  ...
  if (fabs(poolreadinst->m_lastpos - (block->time_s + lat) * decsr) >= 8.0) {
    ...
    if (mindex && mindex->GetFrameCount() > 0) {
      ...
      if (m_adjustLatency)
        poolreadinst->m_dump_samples += mindex->m_start_eatsamples;
      else {
        int framesize = (decsr < 32000 ? 576 : 1152);

        // ugh, old code was so inaccurate, this should make it match
        poolreadinst->m_dump_samples -= framesize; // insert an extra frame of latency  (old seek
                                                   // code had an excess +1 in the frame count)
      ...
      poolreadinst->m_dump_samples = 0;
      poolreadinst->m_read_pos =
          m_filepool->extraInfo->m_stream_startpos +
          (int)((m_filepool->extraInfo->m_stream_endpos -
                 m_filepool->extraInfo->m_stream_startpos) *
  ...
  if (poolreadinst->m_dump_samples < 0) {
    int l =
        (-poolreadinst->m_dump_samples) * sizeof(double) * poolreadinst->m_decoder.GetNumChannels();
    poolreadinst->m_dump_samples = 0;
  ...
  int tr = 0;
  int hasHadRdError = 0;
  while (poolreadinst->m_decoder.queue_samples_out.Available() <
         len * (int)sizeof(double) * poolreadinst->m_decoder.GetNumChannels()) {
    ...
    if (poolreadinst->m_decoder.Run())
      break;
    int l = poolreadinst->m_decoder.queue_samples_out.Available();

    if (l <= os && hasHadRdError)
      break;

    if (poolreadinst->m_dump_samples > 0 && l > 0) {
      l /= sizeof(double) * poolreadinst->m_decoder.GetNumChannels();
      if (l > poolreadinst->m_dump_samples)
        l = poolreadinst->m_dump_samples;
      poolreadinst->m_decoder.queue_samples_out.Advance(l * sizeof(double) *
                                                        poolreadinst->m_decoder.GetNumChannels());
  ...
  if (tr && update_disk_counters)
    update_disk_counters(tr, 0);

  int samples_read = 0;
  if (poolreadinst->m_decoder.GetNumChannels()) {
    samples_read = poolreadinst->m_decoder.queue_samples_out.Available() / sizeof(double) /
                   poolreadinst->m_decoder.GetNumChannels();

    INT64 maxs = (m_filepool->extraInfo->GetLengthSamples(m_adjustLatency) -
                  poolreadinst->m_decode_srcsplpos);

    if (maxs < 0)
      maxs = 0;
    if (samples_read > maxs)
      samples_read = (int)maxs;
    if (samples_read > len)
      samples_read = len;
  ...
  if (samples_read > 0) {
    ...
    if (poolreadinst->m_decoder.GetNumChannels() == block->nch) {
      memcpy(sampleoutptr, poolreadinst->m_decoder.queue_samples_out.Get(),
             samples_read * sizeof(double) * block->nch);
  ...
  {
    block->samples_out = poolreadinst->m_resampler->ResampleOut(block->samples, samples_read,
                                                                block->length, block->nch);
  }
...
int PCM_source_mp3::PropertiesWindow(HWND hwndParent) {
  return (int)DialogBoxParam(g_hInst, MAKEINTRESOURCE(IDD_MP3ITEMINFO), hwndParent, _dlgProc,
                             (LPARAM)this);
}
...
void PCM_source_mp3::GetPropsStr(WDL_FastString& s) {
  if (!IsAvailable()) {
    if (!m_filepool || !m_filepool->extraInfo) {
      s.Set(m_isOffline ? __LOCALIZE("File offline", "mp3dec_DLG_120")
                        : __LOCALIZE("File not opened", "mp3dec_DLG_120"));
    ...
    if (inst) {
      s.AppendFormatted(512,
                        __LOCALIZE_VERFMT("MPEG layer %d, @ %d Hz %d channels", "mp3dec_DLG_120"),
                        inst->m_decoder.GetLayer(), inst->m_decoder.GetSampleRate(),
                        inst->m_decoder.GetNumChannels());
      s.Append("\r\n");
      ReleaseFile(inst, 0);
    }

    if (m_filepool && m_filepool->extraInfo && m_filepool->extraInfo->m_index &&
        m_filepool->extraInfo->m_index->GetFrameCount()) {
      s.Append(__LOCALIZE("Length:", "mp3dec_DLG_120"));
      s.Append(" ");
      format_timestr(m_filepool->extraInfo->GetLengthSamples(m_adjustLatency) /
                         (double)m_filepool->extraInfo->m_srate,
                     buf, sizeof(buf));
      s.Append(buf);
      s.Append("\r\n");
      ...
      if (t == 0)
        encstr = __LOCALIZE("VBR", "mp3dec_DLG_120");
      else if (t == 1)
        encstr = __LOCALIZE("ABR", "mp3dec_DLG_120");
      else if (t == 2)
        encstr = __LOCALIZE("CBR", "mp3dec_DLG_120");
      s.AppendFormatted(512, __LOCALIZE_VERFMT("Encoding: %s", "mp3dec_DLG_120"), encstr);
      s.Append("\r\n");
      s.AppendFormatted(
          512, __LOCALIZE_VERFMT("Channel Mode: %s", "mp3dec_DLG_120"),
          m_filepool->extraInfo->m_channel_mode == 0 ? __LOCALIZE("Stereo", "mp3dec_DLG_120")
          : m_filepool->extraInfo->m_channel_mode == 1
              ? __LOCALIZE("Joint Stereo", "mp3dec_DLG_120")
          : m_filepool->extraInfo->m_channel_mode == 2 ? __LOCALIZE("Dual Stereo", "mp3dec_DLG_120")
          : m_filepool->extraInfo->m_channel_mode == 3
              ? __LOCALIZE("Mono", "mp3dec_DLG_120")
              : __LOCALIZE("(not available)", "mp3dec_DLG_120"));
      s.Append("\r\n");

      s.AppendFormatted(512, __LOCALIZE_VERFMT("Bitrate (average): %.0f kbps", "mp3dec_DLG_120"),
                        (double)(m_filepool->extraInfo->m_stream_endpos -
                                 m_filepool->extraInfo->m_stream_startpos) /
                            (m_filepool->extraInfo->GetLengthSamples(m_adjustLatency) /
                             (double)m_filepool->extraInfo->m_srate) *
                            8.0 / 1000.0);
      s.Append("\r\n");

      if (m_filepool->extraInfo->m_index->IsCBR())
        s.AppendFormatted(512, __LOCALIZE_VERFMT("%d frames in file [CBR]", "mp3dec_DLG_120"),
                          m_filepool->extraInfo->m_index->GetFrameCount());
      else
        s.AppendFormatted(512, __LOCALIZE_VERFMT("%d frames in file [indexed]", "mp3dec_DLG_120"),
                          m_filepool->extraInfo->m_index->GetFrameCount());
      s.Append("\r\n");
    } else {
...
WDL_DLGRET PCM_source_mp3::propsDlgProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam) {
  ...
  case WM_INITDIALOG: {
    ...
    bool has_img =
        m_filepool && m_filepool->extraInfo && m_filepool->extraInfo->m_metadata.Exists("ID3:APIC");
      ...
      if (m_filepool && m_filepool->extraInfo) {
        ...
        if (infostr) {
          ...
          if (ExportMetadataImageToTmpFile(GetFileName(), infostr, &imgfn)) {
            ...
            if (OpenImageModal)
              OpenImageModal(hwndDlg, imgfn.Get(), imgdesc);
...
void PCM_source_mp3::SaveState(ProjectStateContext* ctx) {
  ...
  if (relative_fn) {
    ...
    ctx->AddLine("FILE %p~ %d", GetFileName(), m_adjustLatency);
}
...
PCM_source* CreateFromFile(const char* filename, int priority) {
  ...
  if (priority > 4 && lfn > 4 &&
      (!stricmp(filename + lfn - 4, ".mp3") || !stricmp(filename + lfn - 4, ".mp2"))) {
...
const char*
EnumFileExtensions(int i, const char** descptr) // call increasing i until returns a string, if
                                                // descptr's output is NULL, use last description
{
  if (i == 0) {
    if (descptr)
      *descptr = __LOCALIZE("MPEG Audio files", "mp3");
    return "MP2;MP3";
  }
  if (descptr)
    *descptr = NULL;
  ...
  if (i == 4096)
    return "REAPINDEX"; // let it know this shit should be deleted, if it can be
  return NULL;
}
...
public:
  ...
  int SyncState() {
    return m_dec.SyncState();
  } // > 0 if synched
  int Run() {
    int ret = m_dec.Run();
    m_dec.queue_bytes_in.Compact();
    return ret;
  } // returns -1 on non-recoverable error
  int GetSampleRate() {
    return m_dec.GetSampleRate();
  }
  int GetNumChannels() {
    return m_dec.GetNumChannels();
  }
...
public:
  ...
  PCM_source_mp3_metadata(const char* fn)
    ...
    if (fr && fr->IsOpen()) {
      ...
      if (fend > fstart) {
        // slight extra work here, mp3_index::quickMetadataRead will re-skip the ID3v2 tag, we could
        // prime it with pos=fstart but meh that would require more changes
        mp3_index::mp3_metadata md;
        if (mp3_index::quickMetadataRead(fn, fr, &md,
                                         fr->GetSize() >= g_config_reapindex_minsize)) {
          m_samplerate = md.srate;
          m_numchan = md.nch;
          m_len = md.len;
          if (m_len > 0.0)
            m_bitrate = 8.0 * (double)(fend - fstart) / m_len;
          m_success = true;
        }
      }
    }
...
REAPER_PLUGIN_DLL_EXPORT int REAPER_PLUGIN_ENTRYPOINT(REAPER_PLUGIN_HINSTANCE hInstance,
  ...
  if (rec) {
    ...
    *(void**)&gOnMallocFailPtr = rec->GetFunc("gOnMallocFail");
    ...
    if (!PeakGet_Create || !PeakBuild_CreateEx || !Resampler_Create || !format_timestr ||
        !resolve_fn || !get_ini_file || !rec->Register)
      return 0;
