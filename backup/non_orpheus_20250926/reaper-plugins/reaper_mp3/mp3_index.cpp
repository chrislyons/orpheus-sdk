...
extern void (*GetPeakFileNameEx2)(const char* fn, char* buf, int bufmax, bool forWrite,
                                  const char* extension);

static size_t FILE_WRITE_INT_LE(unsigned int value, WDL_FileWrite* hf) {
  unsigned char buf[4] = {(unsigned char)(value & 0xff), (unsigned char)((value >> 8) & 0xff),
...
mp3_index* mp3_index::indexFromFilename(const char* fn, WDL_FileRead* fr, bool allow_index_file) {
  ...
  if (g_indexes.GetSize() &&
      (_tmp = (mp3_index**)bsearch(_tmp, g_indexes.GetList(), g_indexes.GetSize(), sizeof(void*),
                                   _sortfunc)) &&
      *_tmp) {
    t = *_tmp;
    t->m_refcnt++;
...
bool mp3_index::quickMetadataRead(const char* fn, WDL_FileRead* fr, mp3_metadata* metadata,
                                  bool allow_index_file) {
  if (WDL_NOT_NORMALLY(!metadata) || WDL_NOT_NORMALLY(!fr))
    return false;
  mp3_index tmp(fn);
...
int mp3_index::ReadFrameListFromCache() // 0 if found
{
  ...
  if (m_framefile->Read(buf, 16) == 16 && !memcmp(buf, "RIDX", 4)) {
    ...
    if (ni && ni * 4 + 16 + 8 <= l && fs == (unsigned int)st.st_size &&
        (abs((int)(ft - st.st_mtime)) < 5 || abs((int)(ft - st.st_mtime - 3600)) < 5 ||
         abs((int)(ft - st.st_mtime) + 3600) < 5)) {
...
void mp3_index::BuildFrameList(WDL_FileRead* fpsrc, mp3_metadata* quick_length_check,
  ...
  while (!ateof || m_filebuf.Available() > 32) {
    if (m_filebuf.Available() < 32768 && !ateof) {
      ...
      if (firstblock) {
        firstblock = 0;
        if (l > 10 && !memcmp(buf, "ID3", 3) && buf[3] != -1 && buf[4] != -1 && buf[6] >= 0 &&
            buf[7] >= 0 && buf[8] >= 0 && buf[9] >= 0) {
          byte_pos = 10 + (((int)buf[6]) << 21);
          byte_pos += ((int)buf[7]) << 14;
          byte_pos += ((int)buf[8]) << 7;
          byte_pos += ((int)buf[9]);
          if (buf[3] == 4 && (buf[5] & 0x10))
            byte_pos += 10; // skip id3v2.4 footer
          m_filebuf.Clear();
    ...
    if (!lasthdr) {
      if (decode_header(&fr, this_header) && m_filebuf.Available() >= 8 + fr.framesize) {
        ...
        unsigned int next_header =
            (in_ptr[0] << 24) | (in_ptr[1] << 16) | (in_ptr[2] << 8) | in_ptr[3];
    ...
    if (lasthdr && !mp3_decoder::CompareHeader(lasthdr, this_header) &&
      ...
      if (firstframe) {
        ...
        if (fr.lay == 3) {
          ...
          if (!fr.lsf)
            rdbuf += fr.mode == 3 ? 17 : 32;
          else
            rdbuf += fr.mode == 3 ? 9 : 17;

          if (fr.lsf)
            frame_len_samples = 576;
          ...
          if (!memcmp(rdbuf, "Xing", 4) || !memcmp(rdbuf, "Info", 4)) {
            ...
            {
              tag_frame_cnt =
                  1 + ((rdbuf[0] << 24) | (rdbuf[1] << 16) | (rdbuf[2] << 8) | rdbuf[3]);
              rdbuf += 4;
            }
            if (flags & 2) {
              rdbuf += 4;
            ...
            if ((unsigned int)m_end_eatsamples > 3200)
              m_end_eatsamples = 0;
            if ((unsigned int)m_start_eatsamples > 3200)
              m_end_eatsamples = m_start_eatsamples = 0; // invalid start = reset to defaults
          } else {
            // no tag -- default handling?
            if (!fr.lsf)
              m_start_eatsamples -= 576;
          }

          m_start_eatsamples += 529 + frame_len_samples;
          m_end_eatsamples -= 529;
          if (m_end_eatsamples < 0)
            m_end_eatsamples = 0;
        } else {
          m_start_eatsamples +=
              482; // approx layer 2 latency, geh. lame actually skips 240 samples less, but this
                   // seems mo betta for twolame encoded files at least
        }

        if (quick_length_check) {
          if (!tag_frame_cnt && WDL_NORMALLY(fr.framesize))
            tag_frame_cnt = (unsigned int)((fpsrc->GetSize() - byte_pos) / (fr.framesize + 4));
          quick_length_check->len =
              (frame_len_samples * (double)tag_frame_cnt - m_start_eatsamples - m_end_eatsamples) /
              (double)fr.get_sample_rate();
        ...
        if (m_encodingtag == 2 && tag_frame_cnt != 0
          ...
          const WDL_INT64 sizediff =
              (fpsrc->GetSize() - (fr.framesize + 4) * tag_frame_cnt - byte_pos);
          // if file is too small, or has more than 1k appended to end (128 bytes would be fine),
          // manually index
      ...
      if (!fpo)
        m_frameposmemcache.Add(&byte_pos, 1);
      else {
