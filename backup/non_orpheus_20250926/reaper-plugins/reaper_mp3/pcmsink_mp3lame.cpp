...
extern REAPER_PeakBuild_Interface* (*PeakBuild_CreateEx)(PCM_source* src, const char* fn, int srate,
                                                         int nch, int flags);
...
static void FixSampleRate(int& sr) {
  if (sr <= 8000)
    ...
    sr = 32000;
  else if (sr <= 44100 || sr == 88200 || sr == 176400 || sr == 352800)
    sr = 44100;
  else
    sr = 48000;
}
...
public:
  ...
  PCM_sink_mp3lame(const char* fn, void* cfgdata, int cfgdata_l, int nch, int srate,
                   bool buildpeaks) {
    m_peakbuild = 0;
    m_bitrate = 128;
    m_stereomode = 0;
    ...
    if (m_nch == 1)
      m_stereomode = 3; // BE_MP3_MODE_MONO
    else if ((m_stereomode & STEREO_JSMASK) == DISABLE_JOINT_STEREO)
      m_stereomode = 0; // force stereo
    else
      m_stereomode = 1; // BE_MP3_MODE_JSTEREO
    ...
    if (m_fh && !m_fh->IsOpen()) {
      delete m_fh;
      m_fh = NULL;
      if (SetCurrentSinkError)
        SetCurrentSinkError(__LOCALIZE("Can't create file", "mp3"), fn);
    }
    if (m_fh) {
      ...
      m_enc = new LameEncoder(srate, nch, m_bitrate, m_stereomode, m_quality, m_vbrmethod, m_vbrq,
                              m_vbrmax, m_abr, rpgain, &metadata);
    }
  ...
  double GetLength() {
    return m_lensamples / (double)m_srate;
  } // length in seconds, so far
  INT64 GetFileSize() {
    return m_filesize;
  }
  ...
  void WriteDoubles(ReaSample** samples, int len, int nch, int offset, int spacing) {
    ...
    if (m_peakbuild)
      m_peakbuild->ProcessSamples(tmpptrs, len, m_nch, 0, spacing);
  ...
  int Extended(int call, void* parm1, void* parm2, void* parm3) {
    if (call == PCM_SINK_EXT_DONE) {
      if (parm2 && WDL_NORMALLY(m_enc && m_fh)) {
        WDL_StringKeyedArray<char*> updated_metadata(true,
                                                     WDL_StringKeyedArray<char*>::freecharptr);
...
static unsigned int GetFmt(const char** desc) {
  if (desc)
    *desc = __LOCALIZE("MP3 (encoder by LAME project)", "mp3");
...
static int s_bitrates[] = {8,  16,  24,  32,  40,  48,  56,  64, 80,
...
"quality" stored in configuration file:
  ...
  saved by 5.16+: 10=max, 12=cbr, 11=abr, -1=vbr. parameter for lame_enc quality will be determined
based on bitrate/method
  ...
  *newqual = vbrq;
  ...
  if (*newmode == MODE_ABR) {
    *newbr = abr;
    ...
    if (br < s_bitrates[0])
      br = s_bitrates[0];
    else if (br > s_bitrates[s_numbitrates - 1])
      br = s_bitrates[s_numbitrates - 1];
    *newbr = br;
  }
}

static void NewToOldConfig(int newmode, int newqual, int newbr, int* br, int* vbrq, int* abr,
                           int* vbrmax, int* vbrmethod) {
  *abr = 0;
  *vbrq = 4;
  *br = 32;
  *vbrmax = 320;
...
void PCM_sink_mp3lame::GetOutputInfoString(char* buf, int buflen) {
  ...
  OldToNewConfig(m_bitrate, m_vbrq, m_abr, m_vbrmax, m_quality, m_vbrmethod, &newmode, &newqual,
                 &newbr);

  snprintf(buf, buflen, __LOCALIZE_VERFMT("MP3 (lame_enc) %dHz %dch", "mp3"), m_srate,
           m_enc ? m_enc->GetNumChannels() : m_nch);
  lstrcatn(buf, " ", buflen);
...
WDL_DLGRET wavecfgDlgProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam) {
  ...
  case WM_INITDIALOG: {
    ...
    SendDlgItemMessage(hwndDlg, IDC_MODE, CB_ADDSTRING, 0,
                       (LPARAM)__LOCALIZE("Maximum bitrate/quality", "mp3dec_DLG_155"));
    SendDlgItemMessage(hwndDlg, IDC_MODE, CB_ADDSTRING, 0,
                       (LPARAM)__LOCALIZE("Target quality  (VBR)", "mp3dec_DLG_155"));
    SendDlgItemMessage(hwndDlg, IDC_MODE, CB_ADDSTRING, 0,
                       (LPARAM)__LOCALIZE("Target bitrate  (ABR)", "mp3dec_DLG_155"));
    SendDlgItemMessage(hwndDlg, IDC_MODE, CB_ADDSTRING, 0,
                       (LPARAM)__LOCALIZE("Constant bitrate  (CBR)", "mp3dec_DLG_155"));

    WDL_UTF8_HookComboBox(GetDlgItem(hwndDlg, IDC_COMBO1));
    SendDlgItemMessage(hwndDlg, IDC_COMBO1, CB_ADDSTRING, 0,
                       (LPARAM)__LOCALIZE("Maximum q=0 (slow)", "mp3dec_DLG_155"));
    SendDlgItemMessage(hwndDlg, IDC_COMBO1, CB_ADDSTRING, 0,
                       (LPARAM)__LOCALIZE("Better q=2 (recommended)", "mp3dec_DLG_155"));
    SendDlgItemMessage(hwndDlg, IDC_COMBO1, CB_ADDSTRING, 0,
                       (LPARAM)__LOCALIZE("Normal q=3", "mp3dec_DLG_155"));
    SendDlgItemMessage(hwndDlg, IDC_COMBO1, CB_ADDSTRING, 0,
                       (LPARAM)__LOCALIZE("Fast encode q=5", "mp3dec_DLG_155"));
    SendDlgItemMessage(hwndDlg, IDC_COMBO1, CB_ADDSTRING, 0,
                       (LPARAM)__LOCALIZE("Faster encode q=7", "mp3dec_DLG_155"));
    SendDlgItemMessage(hwndDlg, IDC_COMBO1, CB_ADDSTRING, 0,
                       (LPARAM)__LOCALIZE("Fastest encode q=9", "mp3dec_DLG_155"));
    ...
    int idx = 1;
    if (quality == 10)
      idx = 1; // default to better if last was maximum bitrate/quality
    else if (quality < 2 || quality > 10)
      idx = 0;
    else if (quality < 3)
      idx = 1;
    else if (quality < 5)
  ...
  case WM_USER + 1000: // refresh
  {
    ...
    ShowWindow(GetDlgItem(hwndDlg, IDC_QUALITY_APPROX),
               (mode == MODE_MAX || en_qual ? SW_HIDE : SW_SHOW));
    ...
    ShowWindow(GetDlgItem(hwndDlg, IDC_BITRATE_APPROX),
               (mode == MODE_MAX || en_br ? SW_HIDE : SW_SHOW));
  }
    return 0;
  case WM_USER + 201: {
    const char* p = LameEncoder::GetInfo();
    if (!p)
      p = __LOCALIZE("LAME unavailable", "mp3dec_DLG_155");
  ...
  case WM_USER + 1024: // apply
    ...
    if (lParam) {
      ...
      if (newbr < 0 || newbr >= s_numbitrates)
        newbr = 128;
      else
        newbr = s_bitrates[newbr];
...
static HWND ShowConfig(const void* cfg, int cfg_l, HWND parent) {
  if (cfg_l >= 4 && *((int*)cfg) == REAPER_FOURCC('m', 'p', '3', 'l')) {
    ...
    return CreateDialogParam(g_hInst, MAKEINTRESOURCE(IDD_MP3SINK_CFG), parent, wavecfgDlgProc,
                             (LPARAM)x);
  }
  return 0;
}
...
static PCM_sink* CreateSink(const char* filename, void* cfg, int cfg_l, int nch, int srate,
                            bool buildpeaks) {
  if (cfg_l >= 4 && *((int*)cfg) == REAPER_FOURCC('m', 'p', '3', 'l')) {
...
static int ExtendedSinkInfo(int call, void* parm1, void* parm2, void* parm3) {
  if (call == PCMSINKEXT_GETFORMATDESC || call == PCMSINKEXT_GETFORMATDATARATE) {
    ...
    if (len < 4 || *((int*)cfg) != REAPER_FOURCC('m', 'p', '3', 'l'))
      return 0;
    ...
    if (call == PCMSINKEXT_GETFORMATDATARATE) {
      if (vbrm >= 0)
        return (abr * 1000 * 3 / 2) / 8;

      return (br * 1000) / 8;
    }

    if (vbrm < 0)
      sprintf(desc, __LOCALIZE_VERFMT("%dkbps MP3", "mp3"), br);
    else
      sprintf(desc, "%s", __LOCALIZE("VBR MP3", "mp3"));
    if ((stereomode & STEREO_JSMASK) == DISABLE_JOINT_STEREO)
      strcat(desc, " [JS=0]");
    return 1;
  }
...
static pcmsink_register_ext_t mySinkRegStruct = {{GetFmt, GetExtension, ShowConfig, CreateSink},
                                                 ExtendedSinkInfo};
