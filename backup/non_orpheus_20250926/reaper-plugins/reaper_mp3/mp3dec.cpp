...
bool mp3_decoder::CompareHeader(unsigned int ref, unsigned int tf) {
  ...
  if (diff & (3 << 19))
    return true; // different mpeg 1/2/2.5
  if ((diff >> 17) & 3)
    return true; // different layer
  if ((diff >> 10) & 0x3)
    return true; // different sample rate

  bool mono1 = ((ref >> 6) & 0x3) == 3;
  bool mono2 = ((tf >> 6) & 0x3) == 3;
  if (mono1 != mono2)
    return true; // must not change channels

  return false;
}

int mp3_decoder::Run() // return -1 on error that can't be recovered from, otherwise if output data
                       // doesn't change, assume more input needed
{
  ...
  if (!m_lastframe.framesize) {
    while (queue_bytes_in.Available() > 4) {
      ...
      unsigned int this_header =
          (in_ptr[0] << 24) | (in_ptr[1] << 16) | (in_ptr[2] << 8) | in_ptr[3];
      if ((m_sync_frame && CompareHeader(m_sync_frame, this_header)) ||
          !decode_header(&m_lastframe, this_header)) {
        m_sync_state = 0;
        ...
        if (queue_bytes_in.Available() < m_lastframe.framesize)
          return 0;
        break;
      }
    }
  ...
  if (m_lastframe.framesize && queue_bytes_in.Available() >= m_lastframe.framesize) {
    ...
    int ret = m_decoder.decode(m_sync_frame, &m_lastframe, (unsigned char*)queue_bytes_in.Get(),
                               m_lastframe.framesize, m_spltmp.Get(),
                               m_lastframe.get_sample_count(), &done);

    if (ret == MP3_NEED_MORE) {

      memset(m_spltmp.Get(), 0, ns * sizeof(double)); // bit resevoir empty -- zero samples instead
                                                      // -- usually this will be after a seek anyway
      //       old behavior (wrong): return 0; // try again in a bit. this should never happen since
      //       we parse our frame ourself anyway
    }
