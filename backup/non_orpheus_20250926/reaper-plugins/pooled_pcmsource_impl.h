...
// out of range peaksmode (<0 or >=NUM_SOURCE_POOLS) will result in an unpooled source, which may
// not support peaks or duplicate or other APIs (metadata retrieval only really)
...
public:
  ...
  PCM_source*
      m_hiressrc; // can be shared across all instances since it's singlethreaded and without state
  DWORD m_hiressrc_lastused;
};
#endif

static WDL_PoolList_NoFreeOnDestroy<WDL_ResourcePool<PooledDecoderInstance, PooledDecoderMediaInfo>>
    g_file_pool[NUM_SOURCE_POOLS]; // [m_peaksMode]

class FileNameListEnt {
public:
  FileNameListEnt(char* identstr) {
...
public:
...
#endif
    if (m_filename_ent)
      ns->Open(m_filename_ent->WDL_POOLLIST_identstr);
  ...
  const char* GetFileName() {
    ...
    if (!p)
      p = m_filepool ? m_filepool->WDL_POOLLIST_identstr : NULL;
    return p ? p : "";
  }
  ...
  int GetNumChannels() {
    return m_filepool && m_filepool->extraInfo ? m_filepool->extraInfo->GetNumChannels() : 1;
  } // return number of channels

  double GetPreferredPosition() {
    return m_filepool && m_filepool->extraInfo ? m_filepool->extraInfo->GetPreferredPosition()
                                               : -1.0;
  }

  double GetSampleRate() {
    return m_filepool && m_filepool->extraInfo ? m_filepool->extraInfo->GetSampleRate() : 0.0;
  }
  ...
  int GetBitsPerSample() {
    return m_filepool && m_filepool->extraInfo ? m_filepool->extraInfo->GetBitsPerSample() : 0;
  }
  ...
  void OpenPeaks() {
    ...
    if (!m_filepool->extraInfo->m_peakgetter && m_filename_ent) {
      ...
      if (sr > 0 && nch > 0)
        m_filepool->extraInfo->m_peakgetter =
            PeakGet_Create(m_filename_ent->WDL_POOLLIST_identstr, sr, nch);
    }
#endif
  }
  ...
  void PooledGetSamples(PCM_source_transfer_t* block, PooledDecoderInstance* poolreadinst,
                        void* rsModeToUse);
  void GetSamples(PCM_source_transfer_t* block) {
    block->samples_out = 0;

    PooledDecoderInstance* poolreadinst = 0;
    if (m_filepool && m_filepool->extraInfo && m_filepool->extraInfo->IsAvailable() &&
        (poolreadinst = GetFile())) {
...
#define STORE_BLOCK_TIMEINFO                                                                       \
  double _ot = block->start_time;                                                                  \
  double _oat = block->absolute_time_s;
#define RESTORE_BLOCK_TIMEINFO                                                                     \
  block->start_time = _ot;                                                                         \
  block->absolute_time_s = _oat;
#else
#define STORE_BLOCK_TIMEINFO
#define RESTORE_BLOCK_TIMEINFO
#endif
...
#else
  {
    ...
    if (pb) {
      ...
      if (block->extra_requested_data &&
          block->extra_requested_data_type == PEAKINFO_EXTRADATA_SPECTROGRAM1)
        peakres = 40.0;
      if (block->extra_requested_data2 &&
          block->extra_requested_data_type2 == PEAKINFO_EXTRADATA_SPECTROGRAM1)
        peakres = 40.0;
    ...
    if (pg && block->extra_requested_data &&
        block->extra_requested_data_type == PEAKINFO_EXTRADATA_SPECTROGRAM1)
      peakres = 40.0;
    if (pg && block->extra_requested_data2 &&
        block->extra_requested_data_type2 == PEAKINFO_EXTRADATA_SPECTROGRAM1)
      peakres = 40.0;
  ...
  int PeaksBuild_Begin() {
    if (!m_filepool || !m_filepool->extraInfo)
      return 0;
#ifndef POOLEDSRC_GETPEAKINFO_NOIMPL
    if (m_filepool->extraInfo->m_peakbuilder)
      return 1;
    OpenPeaks();

    if (m_filepool->extraInfo->m_peakgetter || !IsAvailable() || !m_filename_ent)
      return 0;
    ...
    if (sr > 0 && nch > 0) {
      ...
      m_filepool->extraInfo->m_peakbuilder = PeakBuild_CreateEx(
          ni, m_filename_ent->WDL_POOLLIST_identstr, sr, nch, (POOLEDSRC_WANTFPPEAKS) ? 1 : 0);
#else
      m_filepool->extraInfo->m_peakbuilder =
          PeakBuild_Create(ni, m_filename_ent->WDL_POOLLIST_identstr, sr, nch);
#endif
      return 1;
    }
    delete ni;
...
#define PCM_SOURCE_EXT_GETMETADATA_HANDLER(metadata)                                               \
  ...
  if (call == PCM_SOURCE_EXT_ENUMMETADATA && parm2 && parm3) {                                     \
    const char *k, *v = (metadata)->Enumerate((int)(INT_PTR)parm1, &k);                            \
    if (!v)                                                                                        \
      return 0;                                                                                    \
    *(const char**)parm2 = k;                                                                      \
    *(const char**)parm3 = v;                                                                      \
  ...
  int Extended(int call, void* parm1, void* parm2, void* parm3) {
    if (call == PCM_SOURCE_EXT_ENDPLAYNOTIFY) {
      if (m_filepool)
        m_filepool->ReleaseResources(this);
...
void POOLED_PCMSOURCE_CLASSNAME::Open(const char* filename, int peaksMode) {
  ...
  if (peaksMode >= 0 && peaksMode < NUM_SOURCE_POOLS) {
    ...
    m_filepool =
        m_filename_ent ? g_file_pool[peaksMode].Get(m_filename_ent->WDL_POOLLIST_identstr) : 0;
  } else {
    m_filepool =
        new WDL_ResourcePool<PooledDecoderInstance, PooledDecoderMediaInfo>(strdup(filename));
  }
...
void POOLED_PCMSOURCE_CLASSNAME::PooledGetSamples(PCM_source_transfer_t *block,
PooledDecoderInstance *poolreadinst)
{
}
...
static int pooled_pcm_source_garbage_collect(int flags) {
  int rv = 0;
  WDL_PtrList<WDL_ResourcePool<PooledDecoderInstance, PooledDecoderMediaInfo>>* list =
      &g_file_pool[0].pool;
...
#define POOLED_GETSAMPLES_MANAGE_RESAMPLER(msrate)                                                 \
  ...
  } else if (poolreadinst->m_resampler_state == 1)                                                 \
    do_resample = 2;                                                                               \
  if (do_resample) {                                                                               \
...
#define POOLED_GETSAMPLES_ON_SEEK(tpos, msrate)                                                    \
  if (do_resample) {                                                                               \
    ...
    if (do_resample == 2) {                                                                        \
      ...
      tpos = (INT64)floor(block->time_s *                                                          \
                          msrate); /* this should probably be +0.5, but a35f321 bleh */            \
    }                                                                                              \
  }
