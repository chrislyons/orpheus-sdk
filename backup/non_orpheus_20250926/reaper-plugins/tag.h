...
static const char* s_genres[] = {
    ...
    // Lame extensions
    "Goa", "Drum & Bass", "Club-House", "Hardcore", "Terror", "Indie", "BritPop", "Anotherpunk",
    "Polsk Punk", "Beat", "Christian Gangsta", "Heavy Metal", "Black Metal", "Crossover",
    "Contemporary Christian", "Christian Rock", "Merengue", "Salsa", "Trash Metal", "Anime", "JPop",
    "SynthPop"
...
const char* GetGenre(const char* val) {
  int g = atoi(val);
  if (!g && val[0] == '(')
    g = atoi(val + 1); // support things like "(8)Jazz"
  if (!g && strcmp(val, "0") && strncmp(val, "(0", 2))
    g = -1;
...
static const char* ID3_ETCO_TYPE[] = {"padding",         "end of initial silence",
                                      "intro start",     "main part start",
                                      "outro start",     "outro end",
                                      "verse start",     "refrain start",
                                      "interlude start", "theme start",
                                      "variation start", "key change",
                                      "time change",     "momentary unwanted noise",
                                      "sustained noise", "sustained noise end",
                                      "intro end",       "main part end",
                                      "verse end",       "refrain end",
                                      "theme end",       "profanity",
                                      "profanity end"};
int GetETCOType(const char* desc, int len) {
  for (int i = 0; i < sizeof(ID3_ETCO_TYPE) / sizeof(ID3_ETCO_TYPE[0]); ++i) {
    if (!strnicmp(desc, ID3_ETCO_TYPE[i], len))
      return i;
  }
  return -1;
}

#define _GetInt32LE(p) (((p)[0]) | ((p)[1] << 8) | ((p)[2] << 16) | ((p)[3] << 24))

bool ParseAPEv2(unsigned char* buf, int buflen, int tagcnt, WDL_StringKeyedArray<char*>* metadata) {
  bool has_tag = false;
...
int SynchSafeBytesToInt(bool issynchsafe, const char* bytes, int bytes_sz) {
  if (!bytes || bytes_sz <= 0 || bytes_sz > 4)
    return -1;
  ...
  if (issynchsafe) {
    ...
    int b = 0;
    if (bytes_sz > 3)
      i += ((unsigned char)bytes[b++]) << 24;
    if (bytes_sz > 2)
      i += ((unsigned char)bytes[b++]) << 16;
    if (bytes_sz > 1)
      i += ((unsigned char)bytes[b++]) << 8;
...
bool ParseID3v1(const char* buf, WDL_StringKeyedArray<char*>* metadata,
                bool fast) // fast: only parse common tags (parsing en masse)
{
  bool has_tag = false;
  ...
  memcpy(tag, buf + 3, 30);
  tag[30] = 0;
  if (AddID3v1Tag("ID3:TIT2", tag, metadata))
    has_tag = true;

  memcpy(tag, buf + 33, 30);
  tag[30] = 0;
  if (AddID3v1Tag("ID3:TPE1", tag, metadata))
    has_tag = true;

  memcpy(tag, buf + 63, 30);
  tag[30] = 0;
  if (AddID3v1Tag("ID3:TALB", tag, metadata))
    has_tag = true;
  ...
  tag[id3v11 ? 28 : 30] = 0;
  if (AddID3v1Tag("ID3:COMM", tag, metadata))
    has_tag = true;
...
bool ParseID3v2(char* buf, int tag_size, WDL_StringKeyedArray<char*>* metadata,
                bool fast, // fast: only parse common tags (parsing en masse)
  ...
  if (buf[5] & 0x80) // unsynchronisation at tag level
  {
    ...
    while (i < tag_size) {
      p[j] = p[i];
      if (i < tag_size - 1 && p[i] == 0xFF && p[i + 1] == 0x00)
        i++;
      i++;
      j++;
    }
  ...
  int ext_hdr_sz =
      (buf[3] != 2 && (buf[5] & 0x40)) ? SynchSafeBytesToInt(buf[3] == 4, buf + 10, 4) : 0;
  if (ext_hdr_sz < 0)
    return false;
  ...
  while (WDL_NORMALLY(id3_pos > 0) && id3_pos + chunk_hdr_size <= 10 + tag_size) {
    if (!memcmp(buf + id3_pos, "\0\0\0", 3))
      break; // we are into padding
    ...
    if (frame_sz <= 0)
      break;
    id3_pos += (buf[3] == 2 ? 3 : 4);
    if (id3_pos + frame_sz < 0 || id3_pos + frame_sz > 10 + tag_size)
      break;
    ...
    if (buf[3] != 2) {
      ...
      if ((buf[3] == 3 && (fmt_flags & 0xDF)) || (buf[3] == 4 && (fmt_flags & 0xBF))) {
        // unsupported frame format, ignore it (extended, compressed, encrypted, unsynchronised,
        // etc)
        id3_pos += frame_sz;
        continue;
        ...
        frame_sz--;
        if (WDL_NOT_NORMALLY(frame_sz <= 0))
          continue; // maybe allow this for some tag types?
      }
    }
    ...
    if (frameid.Get()[0] == 'T' && strcmp(frameid.Get(), buf[3] == 2 ? "TXX" : "TXXX")) {
      ...
      if (!strcmp(frameid.Get(), buf[3] == 2 ? "TCO" : "TCON")) {
        ...
        if (genre != frame_data.Get())
          frame_data.Set(genre);
      }
    }
    // text tags with encoding byte + short desc
      ...
      if (!strncmp(frameid.Get(), "COM", 3) || !strcmp(frameid.Get(), "ULT") ||
          !strcmp(frameid.Get(), "USLT")) {
        lang = p + skip_bytes;
        skip_bytes += 3;
      }
    ...
    // binary tags with short desc
    else if (!fast && (!strcmp(frameid.Get(), "PRIV") ||
                       !strcmp(frameid.Get(), buf[3] == 2 ? "UFI" : "UFID"))) {
      shortdesc.Set("");
      ...
      if (skip_bytes < frame_sz) {
        ...
        if (pictype >= 0 && pictype < 128 && dataoffs > 0 && skip_bytes < frame_sz) {
          if (frame_data.GetLength())
            frame_data.Append(" ");
          frame_data.AppendFormatted(512, "offset:%" WDL_PRI_UINT64 " length:%d", dataoffs,
                                     frame_sz - skip_bytes);
        }
      }
    } else if (!fast && buf[3] != 2 && !strcmp(frameid.Get(), "CTOC")) {
      int pos = 0;
      while (pos + 2 < frame_sz && p[pos])
        ++pos;
      if (pos + 2 < frame_sz && p[pos + 2]) {
        frame_data.SetFormatted(512, "%d chapters", p[pos + 2]);
      }
      if (!frame_data.Get()[0])
        frame_data.Set("[table of contents data]");
    } else if (!fast && buf[3] != 2 && !strcmp(frameid.Get(), "CHAP")) {
      frameid.AppendFormatted(512, "%03d", ++chapcnt);

      int pos = 0;
      while (pos < frame_sz && p[pos])
        ++pos; // advance through CTOC child element
      ++pos;
      ...
      if (((frame_sz - pos - 1) % 5) == 0) {
        ...
        if (p[0] == 0x02) // timestamp is ms
        {
          for (++pos; pos <= frame_sz - 5; pos += 5) {
            ...
            const char* desc = evt_type < sizeof(ID3_ETCO_TYPE) / sizeof(ID3_ETCO_TYPE[0])
                                   ? ID3_ETCO_TYPE[evt_type]
                                   : "unknown";
            frame_data.AppendFormatted(512, "%s%u:%s", frame_data.GetLength() ? "," : "", timestamp,
                                       desc);
          }
        }
      }
...
int ReadMediaTags(WDL_FileRead* fr, WDL_StringKeyedArray<char*>* metadata, WDL_INT64* _fstart,
                  WDL_INT64* _fend) {
  if (!fr || !fr->IsOpen() || !metadata)
    return 0;
