...
/*
** Endian-tools and defines (currently only __ppc__ and BIG_ENDIAN is recognized, for OS X -- all
*other platforms are assumed to be LE)
*/
...
#define ADVANCE_TIME_BY_SAMPLES(t, spls, srate)                                                    \
  ((t) += (spls) / (double)(srate)) // not completely accurate but still quite accurate.
#else
#define ADVANCE_TIME_BY_SAMPLES(t, spls, srate)                                                    \
  ((t) = floor((t) * (srate) + spls + 0.5) /                                                       \
         (double)(srate)) // good forever? may have other issues though if srate changes. disabled
                          // for now
#endif
...
//
//  CLAP plugins can also (v6.80+) get their context information by
//  getFunc("clap_get_reaper_context") void *(*clap_get_reaper_context)(const clap_host *host, int
//  sel); sel=1 for parent track if track FX sel=2 for parent take if item FX sel=3 for project
//  sel=4 for FxDsp
...
typedef struct reaper_plugin_info_t {
  ...
  /*
  Register() is the API that plug-ins register most things, be it keyboard shortcuts, project
  importers, etc. Register() is also available by using GetFunc("plugin_register")
  ...
  APIdef_*:
    To make a function registered with API_* available via ReaScript, follow the API_ registration
  with: double myfunction(char* str, int flag); const char *defstring =
  "double\0char*,int\0str,flag\0help text for myfunction"
  ...
  hookcommand:
    ...
    runCommand() should return true if it processed the command (prevent further hooks or the action
  from running) It is OK to call Main_OnCommand() from runCommand(), but it must check for and
  ...
  hookcommand2:
    ...
    onAction returns true if it processed the command (preventing further hooks or actions from
  running) val/val2 are used for actions triggered by MIDI/OSC/mousewheel
  ...
  hookpostcommand2:
     void (*hook)(KbdSectionInfo *section, int actionCommandID, int val, int valhw, int relmode,
  HWND hwnd, ReaProject *proj); rec->Register("hookpostcommand2",hook);

  command_id:
    Registers/looks up a command ID for an action. Parameter is a unique string with only A-Z, a-z,
  0-9. int command = Register("command_id","MyCommandName"); returns 0 if unsupported/out of actions
  ...
  hwnd_info:  (6.29+)
    query information about a hwnd
     ...
     -- note, for v7.23+ ( -- check with GetAppVersion() -- ), you may also use a function with this
  prototype: int (*callback)(HWND hwnd, INT_PTR info_type, const MSG *msg); // if msg is non-NULL,
  it will have information about the currently-processing event.
    ...
    info_type:
       ...
       1 = query if global hotkeys should be processed for this context  (6.60+)
           return 1 if global hotkey should be skipped (and default accelerator processing used
  instead) return -1 if global hotkey should be forced
  ...
  file_in_project_ex2:
     Extended syntax to receive rename notifications, or to have your plug-in request that the file
  go in a subdirectory:
     ...
     INT_PTR fileInProjectCallback(void *_userdata, int msg, void *parm) {
       ...
       if (msg == 0x100) return (INT_PTR)"samples"; // subdirectory name, if desired (return 0 if
  not desired)
...
struct MIDI_event_t {
  int frame_offset;
  int size; // bytes used by midi_message, can be >3, but should never be <3, even if a short 1 or 2
            // byte msg
  unsigned char midi_message[4]; // size is number of bytes valid -- can be more than 4!

  // new helpers
  bool is_note() const {
    return (midi_message[0] & 0xe0) == 0x80;
  }
  ...
  enum { CC_ALL_SOUND_OFF = 120, CC_ALL_NOTES_OFF = 123, CC_EOF_INDICATOR = CC_ALL_NOTES_OFF };
};
...
protected:
  // this is only defined in REAPER 4.60+, for 4.591 and earlier you should delete only via the
...
static inline void TrackStateCtx_WriteTrackMarker(ProjectStateContext* ctx,
                                                  const track_marker_t& tm) {
  ctx->AddLine("TM %d %d %d %.17g %.17g %s", tm.index, tm.lane, tm.isrgn ? 1 : 0, tm.pos, tm.rgnend,
               tm.name.c_str());
}
...
typedef struct _MIDI_eventprops {
  ...
  char* varmsg;
  int varmsglen;
  int setflag; // &1:selected, &2:muted, &4:ppqpos, &8:endppqpos or bez tension, &16:msg1 high bits,
               // &32:msg1 low bits, &64:msg2, &128:msg3, &256:varmsg, &1024:shape/tension fields
               // used, &16384:no sort after set
} MIDI_eventprops;
...
typedef struct _PCM_source_peaktransfer_t {
  ...
  double samplerate; // peakrate is peaks per second, samplerate is used only as a hint for what
                     // style of peaks to draw, OK to pass in zero
...
#define PEAKINFO_EXTRADATA_LOUDNESS_DEPRECATED                                                     \
  ((int)'l') // use PEAKINFO_EXTRADATA_LOUDNESS_RAW instead
...
#define PEAKINFO_EXTRADATA_LOUDNESS_INTERNAL ((int)'!')
  ...
  static inline int extra_blocksize(int extra_requested_data_type) {
    ...
    case PEAKINFO_EXTRADATA_SPECTRAL1:
      return SPECTRAL1_BYTES;
    case PEAKINFO_EXTRADATA_SPECTROGRAM1:
      return SPECTROGRAM1_BLOCKSIZE_BYTES;
    case PEAKINFO_EXTRADATA_MIDITEXT:
      return MIDITEXT_BYTES;
    case PEAKINFO_EXTRADATA_LOUDNESS_DEPRECATED:
      return LOUDNESS_DEPRECATED_BYTES;
    case PEAKINFO_EXTRADATA_LOUDNESS_RAW:
      return LOUDNESS_RAW_BYTES;
    case PEAKINFO_EXTRADATA_LOUDNESS_INTERNAL:
      return LOUDNESS_INTERNAL_BYTES;
    }
    return 0;
  }

  enum {
    SPECTROGRAM1_BLOCKSIZE_BYTES =
        128 * 3 / 2,     // 128 bins, 12 bits each (MSB1, (LSN1<<4)|LSN2, MSB2)
    SPECTRAL1_BYTES = 4, // one LE int per channel per sample spectral info: low 15 bits frequency,
                         // next 14 bits density (16383=tonal, 0=noise, 12288 = a bit noisy)
    MIDITEXT_BYTES = 1,  // at most one character per pixel
    LOUDNESS_DEPRECATED_BYTES =
        4, // 4 byte LE integer: LUFS-M low 12 bits, LUFS-S next 12 bits, 0-3000 valid: ex: 0 means
           // -150.0 LU (consider this -inf), 1500 means +0 LU - loudness values returned for each
           // channel even if the calculation is for all channels combined
    LOUDNESS_RAW_BYTES =
        8, // 4 byte LE float LUFS-M low 32 bits, 4 byte LE float LUFS-S high 32 bits:
...
typedef struct _REAPER_midi_realtime_write_struct_t {
  ...
  int overwritemode; // 0=overdub, 1=replace,
                     // -1 = literal (do nothing just add), -2 = literal, do not apply default
                     // curves 65536+(16 bit mask) = replace notes on just these channels
                     // (touch-replace)
  ...
  unsigned int* overwrite_actives; // [16(note)+16(CC)+16(poly AT)][4]; only used when overwritemode
                                   // is >0 CC: 127=pitch, 126=program, 125=channel pressure

  double do_not_quantize_past_sec; // amount in future that quantizing should never move things past
                                   // (or 0 for not used)
...
public:
  ...
  virtual bool SetFileName(
      const char* newfn) = 0; // return TRUE if supported, this will only be called when offline
  ...
  virtual double GetSampleRate() = 0; // returns preferred sample rate. if < 1.0 then it is assumed
                                      // to be silent (or MIDI)
  ...
  virtual int PeaksBuild_Begin() = 0;   // returns nonzero if building is opened, otherwise it may
                                        // mean building isn't necessary
...
typedef struct _REAPER_cue {
  ...
  int m_flags; // &1:DEPRECATED caller must call Extended(PCM_SOURCE_EXT_ENUMCUES, -1, &cue, 0) when
               // finished, &2:time is QN, &0x10000:write cue regardless of sink settings, &4:is
               // chapter, &(8|16)=8:has low confidence cue name, &(8|16)=16:has medium confidence
               // cue name, &(8|16)=24:has high confidence cue name
...
typedef struct _REAPER_inline_positioninfo {
  ...
  int width, height; // width and height of view of the item. if doing a partial update this may be
                     // larger than the bitmap passed in
...
#define PCM_SOURCE_EXT_INLINEEDITOR                                                                \
  0x100 /* parm1 = (void *)(INT_PTR)message, parm2/parm3 = parms                                   \
                                                                                                   \
note: for the WM_* constants, you can use windows.h if on Windows, and SWELL's definitions if on   \
other platforms note: for LICE_IBitmap interface, you can use LICE's definition for SWELL and                                                                                     \
LICE, see Cockos WDL -- https://www.cockos.com/wdl                                                 \
                                                                                                   \
NOTE: fx-embed documentation is now in reaper_plugin_fx_embed.h                                    \
                                                                                                   \
           messages:                                                                               \
             0 = query if editor is available/supported. returns <0 if supported but unavailable,  \
>0 if available, 0=if not supported WM_CREATE to create the editor instance WM_DESTROY to destroy                                                                             \
the editor instance (nonzero if success)                                                           \
                                                                                                   \
             WM_LBUTTON*, WM_RBUTTON*, WM_MOUSEMOVE, WM_MOUSEWHEEL -- parm2=rsvd, parm3=           \
REAPER_inline_positioninfo) these can return any combination of:                                                              \
...
*)rec->extraParms[0] return nonzero to eat the key. can return REAPER_INLINE_RETNOTIFY_INVALIDATE.                     \
                                                                                                   \
           ...
           paint messages: parm2 = LICE_IBitmap *, parm3 = (REAPER_inline_positioninfo*).          \
            ...
            Notes on Retina/HiDPI:                                                                 \
                  on macOS retina, (int)LICE_IBitmap::Extended(LICE_EXT_GET_SCALING,NULL) may      \
return 512. in this case LICE will internally render things double-sized on other platforms HiDPI,                                                                         \
if (int)LICE_IBitmap::Extended(LICE_EXT_GET_ADVISORY_SCALING,NULL) returns nonzero, then it is     \
a 24.8 scale factor which things should be scaled by.                                                                              \
                                                                                                   \
            */
...
#define REAPER_INLINEFLAG_SHOWALLTAKES                                                             \
  0x1000 // only valid as a return value flag for message=0, display in inactive take lanes
#define REAPER_INLINEFLAG_WANTOVERLAYEDCONTROLS                                                    \
  0x4000 // only valid as a return value flag for message=0, to have fades/etc still drawn over like
         // normal
...
#define PCM_SOURCE_EXT_GETEDITORSTRING                                                             \
  0x10002 // parm1=index (0 or 1), parm2=(const char**)desc, optional parm3=(int*)has_had_editor
...
#define PCM_SOURCE_EXT_ADDMIDIEVENTS                                                               \
  0x10005 // parm1=pointer to midi_realtime_write_struct_t, nch=1 for replace, =0 for overdub,
          // parm2=midi_quantize_mode_t* (optional)
...
#define PCM_SOURCE_EXT_GETMETADATA                                                                 \
  0x10007 // parm1=pointer to name string, parm2=pointer to buffer, parm3=(int)buffersizemax.
          // returns length used. defined strings are "TITLE", "ARTIST", "ALBUM", "TRACKNUMBER",
          // "YEAR", "GENRE", "COMMENT", "DESC", "BPM", "KEY", "DB_CUSTOM"
#define PCM_SOURCE_EXT_SETASSECONDARYSOURCE                                                        \
  0x10008 // parm1=optional pointer to src (same subtype as receiver), if supplied, set the receiver
          // as secondary src for parm1's editor, if not supplied, receiver has to figure out if
          // there is an appropriate editor open to attach to, parm2/3 impl defined
#define PCM_SOURCE_EXT_SHOWMIDIPREVIEW                                                             \
  0x10009 // parm1=(MIDI_eventlist*), can be NULL for all-notes-off (also to check if this source
          // supports showing preview at this moment)
#define PCM_SOURCE_EXT_SEND_EDITOR_MSG 0x1000A // impl defined parameters
#define PCM_SOURCE_EXT_SETSECONDARYSOURCELIST                                                      \
  0x1000B // parm1=(PCM_source**)sourcelist, parm2=list size, parm3=close any existing src not in
          // the list
#define PCM_SOURCE_EXT_ISOPENEDITOR                                                                \
  0x1000C // returns 1 if this source is currently open in an editor, 2 if open in a secondary
          // editor. parm1=1 to close. parm2=(int*)&flags to get extra flags (&1=editor is currently
          // hidden)
#define PCM_SOURCE_EXT_SETEDITORGRID                                                               \
  0x1000D // parm1=(double*)griddiv: 0.25=quarter note, 1.0/3.0=half note triplet, etc. parm2=int*
          // swingmode(1=swing), parm3=double*swingamt
...
#define PCM_SOURCE_EXT_GETBITRATE                                                                  \
  0x10012 // parm1=(double*)bitrate, if different from samplerate*channels*bitdepth/length
#define PCM_SOURCE_EXT_ENUMMETADATA                                                                \
  0x10013 // parm1=(int)index, parm2=(const char**)key, parm3=(const char**)value. enumerates all
          // metadata, returns 0 when no more metadata exists
#define PCM_SOURCE_EXT_GETINFOSTRING                                                               \
  0x10014 // parm1=(char*)buffer, parm2=(int)buffer length, return the data that would be displayed
...
#define PCM_SOURCE_EXT_WRITE_METADATA                                                              \
  0x20008 // parm1=char* new file name, parm2=(char**)NULL-terminated array of
          // key,value,key2,value2,... pointers, parm3=flags (&1=merge, &2=do not allow update
          // in-place). returns 1 if successfully generated a new file, 2 if original file was
          // updated
#define PCM_SOURCE_EXT_GETBPMANDINFO                                                               \
  0x40000 // parm1=pointer to double for bpm. parm2=pointer to double for snap/downbeat offset
          // (seconds).
#define PCM_SOURCE_EXT_GETNTRACKS                                                                  \
  0x80000 // for midi data, returns number of tracks that would have been available. optional
          // parm1=(int*)mask of channels available, mask&(1<<16)=metadata
#define PCM_SOURCE_EXT_GETTITLE 0x80001 // parm1=(char**)title (string persists in plugin)
#define PCM_SOURCE_EXT_ENUMTEMPOMAP                                                                \
  0x80002 // parm1=index, parm2=pointer to REAPER_tempochg, returns 0 if no tempo map or enumeration
          // complete
#define PCM_SOURCE_EXT_WANTOLDBEATSTYLE 0x80003
#define PCM_SOURCE_EXT_GETNOTATIONSETTINGS                                                         \
  0x80004 // parm1=(int)what, (what==0) => parm2=(double*)keysigmap, parm3=(int*)keysigmapsize;
          // (what==1) => parm2=(int*)display transpose semitones, (what==2) => parm2=(char*)clef1,
          // parm3=(char*)clef2
...
#define PCM_SOURCE_EXT_WANT_TRIM                                                                   \
  0x90001 // parm1=(int64*)total number of decoded samples after trimming, parm2=(int*)number of
          // samples to trim from start, parm3=(int*)number of samples to trim from end
...
#define PCM_SOURCE_EXT_EXPORTTOFILE                                                                \
  0x90004 // parm1=output filename, only currently supported by MIDI but in theory any source could
          // support this
#define PCM_SOURCE_EXT_ENUMCUES                                                                    \
  0x90005 // DEPRECATED, use PCM_SOURCE_EXT_ENUMCUES_EX instead.  parm1=(int) index of cue to get
          // (-1 to free cue), parm2=(optional)REAPER_cue **.  Returns 0 and sets parm2 to NULL when
          // out of cues. return value otherwise is how much to advance parm2 (1, or 2 usually)
#define PCM_SOURCE_EXT_ENUMCUES_EX                                                                 \
  0x90016 // parm1=(int) index of cue (source must provide persistent backing store for
          // cue->m_name), parm2=(REAPER_cue*) optional. Returns 0 when out of cues, otherwise
          // returns how much to advance index (1 or 2 usually).
// a PCM_source may be the parent of a number of beat-based slices, if so the parent should report
// length and nchannels only, handle ENUMSLICES, and be deleted after the slices are retrieved
#define PCM_SOURCE_EXT_ENUMSLICES                                                                  \
  0x90006 // parm1=(int*) index of slice to get, parm2=REAPER_slice* (pointing to caller's existing
          // slice struct), parm3=(double*)bpm. if parm2 passed in zero, returns the number of
          // slices. returns 0 if no slices or out of slices.
...
#define PCM_SOURCE_EXT_SETPREVIEWTEMPO                                                             \
  0x90008 // parm1=(double*)bpm, only meaningful for MIDI or slice-based source media; bpm==0 to
          // follow project tempo changes

enum {
  RAWMIDI_NOTESONLY = 1,
  RAWMIDI_UNFILTERED = 2,
  RAWMIDI_CHANNELFILTER = 3
}; // if RAWMIDI_CHANNELFILTER, flags>>4 is a mask of which channels to play
...
#define PCM_SOURCE_EXT_SETRESAMPLEMODE                                                             \
  0x9000A // parm1= mode to pass to resampler->Extended(RESAMPLE_EXT_SETRSMODE,mode,0,0)
...
#define PCM_SOURCE_EXT_SETSIZE                                                                     \
  0x9000C // parm1=(double*)startpos, parm2=(double*)endpos, parm3=flags. Start can be negative.
          // Receiver may adjust start/end to avoid erasing content, in which case the adjusted
          // values are returned in parm1 and parm2. parm3/flags: 1 if start/end in QN (always the
          // case now). 2=resize even pooled items
#define PCM_SOURCE_EXT_GETSOURCETEMPO                                                              \
  0x9000D // parm1=(double*)bpm, parm2=(int*)timesig_numerator<<8|timesig_denominator,
          // parm3=(double*)current preview tempo if applicable. this is for reporting purposes
          // only, does not necessarily mean the media should be adjusted (as
          // PCM_SOURCE_EXT_GETBPMANDINFO means)
#define PCM_SOURCE_EXT_ISABNORMALAUDIO                                                             \
  0x9000E // return 1 if rex, video, etc (meaning file export will just copy file directly rather
          // than trim/converting)
#define PCM_SOURCE_EXT_GETPOOLEDMIDIID                                                             \
  0x9000F // parm1=(char*)id, parm2=(int*)pool user count, parm3=(MediaItem_Take**)firstuser
...
#define PCM_SOURCE_EXT_GETIMAGE                                                                    \
  0x90012 // parm1=(LICE_IBitmap**)image. parm2 = NULL or pointer to int, which is (w<<16)|h desired
          // approx
#define PCM_SOURCE_EXT_NOAUDIO                                                                     \
  0x90013 // return 1 if video file with no audio. if parm1 is non-NULL, will (int*)parm1=1 if a
          // video file with audio and no video
#define PCM_SOURCE_EXT_HASMIDI                                                                     \
  0x90014 // returns 1 if contains any MIDI data, parm1=(double*)time offset of first event
#define PCM_SOURCE_EXT_DELETEMIDINOTES                                                             \
  0x90015 // parm1=(double*)minlen (0.125 for 1/8 notes, etc), parm2=1 if only trailing small notes
          // should be deleted, parm3=(bool*)true if any notes were deleted (return)
...
#define PCM_SOURCE_EXT_PPQCONVERT                                                                  \
  0x90020 // parm1=(double*)pos, parm2=(int)flag 0=ppq to proj time, 1=proj time to ppq, 2=start of
          // measure, 3=end of measure, 4=query ppq
...
#define PCM_SOURCE_EXT_GETSETMIDIEVT                                                               \
  0x90022 // parm1=(MIDI_eventprops*)event properties (NULL to delete); parm2=(int)event index (<0
          // to insert); parm2=(int)flag: 1=index counts notes only, 2=index counts CC only, 3=index
          // counts meta-events only
#define PCM_SOURCE_EXT_GETSUGGESTEDTEXT                                                            \
  0x90023 // parm1=char ** which will receive pointer to suggested label text, if any
#define PCM_SOURCE_EXT_GETSCALE                                                                    \
  0x90024 // parm1=unsigned int: &0xF=pitch (0=C), &0x10=root, &0x20=min2, &0x40=maj2, &0x80=min3,
          // &0xF0=maj3, &0x100=4, etc) ; parm2=(char*)name (optional), parm3=int size of name
          // buffer
#define PCM_SOURCE_EXT_SELECTCONTENT 0x90025 // parm1=1 to select, 0 to deselect
#define PCM_SOURCE_EXT_GETGRIDINFO                                                                 \
  0x90026 // parm1=(double*)snap grid size, parm2=(double*)swing strength, parm3=(double*)note
          // insert length, -1 if follows grid size
...
#define PCM_SOURCE_EXT_GETSETALLMIDI                                                               \
  0x90029 // parm1=(unsigned char*)data buffer, parm2=(int*)buffer length in bytes, parm2=(1:set,
          // 0:get). Buffer is a list of { int offset, char flag, int msglen, unsigned char msg[] }.
          // offset: MIDI ticks from previous event, flag: &1=selected &2=muted, msglen: byte length
          // of msg (usually 3), msg: the MIDI message.
#define PCM_SOURCE_EXT_DISABLESORTMIDIEVTS                                                         \
  0x90030 // disable sorting for PCM_SOURCE_EXT_GETSETMIDIEVT until PCM_SOURCE_EXT_SORTMIDIEVTS is
          // called
#define PCM_SOURCE_EXT_GETPOOLEDMIDIID2                                                            \
  0x90031 // parm1=(GUID*)id, parm2=(int*)pool user count, parm3=(MediaItem_Take**)firstuser
#define PCM_SOURCE_EXT_GETSETMIDICHANFILTER                                                        \
  0x90032 // parm1=(int*)filter: filter&(1<<n) to play channel n, parm2=(int)set: 0 to get, 1 to set
#define PCM_SOURCE_EXT_REFRESH_EDITORS 0x90033 // synchronously refresh any open editors
#define PCM_SOURCE_EXT_GET_LYRICS                                                                  \
  0x90040 // parm1=buffer, parm2= &len, parm3=(INT_PTR)flag - see GetTrackMIDILyrics
#define PCM_SOURCE_EXT_SET_LYRICS 0x90041 // parm1=nul term buffer
#define PCM_SOURCE_EXT_GETLAPPING                                                                  \
  0xC0100 // parm1 = ReaSample buffer, parm2=(INT_PTR)maxlap, returns size of lapping returned.
          // usually not supported. special purpose.
#define PCM_SOURCE_EXT_SET_PREVIEW_POS_OVERRIDE                                                    \
  0xC0101 // parm1 = (double *)&tickpos, tickpos<0 for no override
#define PCM_SOURCE_EXT_SET_PREVIEW_LOOPCNT                                                         \
  0xC0102 // parm1 = (INT64*)&decoding_loopcnt, valid only for the immediately following
          // GetSamples(), only in track preview contexts, when not using buffering source
...
typedef struct _REAPER_pcmsrc_register_t {
  ...
  PCM_source* (*CreateFromFile)(const char* filename,
                                int priority); // if priority is 5-7, and the file isn't found, open
                                               // it in an offline state anyway, thanks
  ...
  const char* (*EnumFileExtensions)(
      int i, const char** descptr); // call increasing i until returns a string, if descptr's output
                                    // is NULL, use last description
} pcmsrc_register_t;

struct REAPER_pcmsrc_igntempo_info {
  int enable_override;
  int flags; // if setting, &1 to reconform media (and if that set, &2 applies even if couldn't be
             // reconformed)
  double bpm;
  int measure_len;
  int denom;
...
**
** And you can use the PCM_Source_CreateFromSimple API to create a PCM_source from your
*ISimpleMediaDecoder derived class
*/
...
public:
  ...
  // if fullClose=0, close disk resources, but can leave decoders etc initialized (and subsequently
  // check the file date on re-open)
...
#define PCM_SINK_EXT_DONE                                                                          \
  0x80001 // parm1 = optional HWND of the render dialog in case the sink needs more user
          // interaction, parm2 = optional (char**)NULL-terminated array of
          // key,value,key2,value2,... pointers if the sink supports updating metadata after write
          // (may require the caller to have set padded metadata via SetCurrentSinkMetadata before
          // creating the sink)
#define PCM_SINK_EXT_VERIFYFMT                                                                     \
  0x80002 // parm1=int *srate, parm2= int *nch. plugin can override (and return 1!!)
...
#define PCM_SINK_EXT_SETCURBLOCKTIME                                                               \
  0x80007 // parm1 = (double *) project position -- called before each WriteDoubles etc
...
typedef struct _REAPER_pcmsink_register_t // register using "pcmsink"
{
  ...
  PCM_sink* (*CreateSink)(const char* filename, void* cfg, int cfg_l, int nch, int srate,
                          bool buildpeaks);
...
typedef struct _REAPER_pcmsink_register_ext_t // register using "pcmsink_ext"
{
  ...
  // for extended calls that refer to the generic type of sink, rather than a specific instance of a
  // sink
...
#define PCMSINKEXT_GETFORMATDATARATE                                                               \
  0x80001 // parm1=(void*)cfg, parm2=(int)cfglen, parm3 = int[] {channels, samplerate}
#define PCMSINKEXT_GETFORMATBITDEPTH                                                               \
  0x80002 // parm1=(void*)cfg, parm2=(int)cfglen, returns bit depth if supported (negative with
          // effective size if FP)
#define PCMSINKEXT_GETFORMATSTREAMDESC                                                             \
  0x80003 // parm1=(void*)cfg, parm2=(int)cfglen, parm3=(int*)streams (&1=has video, &2=has audio),
...
public:
  ...
  virtual double GetCurrentLatency() = 0; // latency in seconds buffered -- do not call between
                                          // resampleprepare and resampleout, undefined if you do...
...
#define RESAMPLE_EXT_SETFEEDMODE                                                                   \
  0x1001 // parm1 = nonzero to set ResamplePrepare's out_samples to refer to request a specific
...
public:
  ...
  virtual void set_formant_shift(
      double shift) = 0; // shift can be <0 for "only shift when in formant preserve mode", so that
                         // you can use it for effective rate changes etc in that mode
  ...
  virtual int GetSamples(int requested_output,
                         ReaSample* buffer) = 0; // returns number of samplepairs returned

  virtual void SetQualityParameter(
      int parm) = 0; // set to: (mode<<16)+(submode), or -1 for "project default" (default)
  virtual int Extended(int call, void* parm1, void* parm2, void* parm3) {
    return 0;
  } // return 0 if unsupported
};
...
public:
  ...
  virtual void ProcessSamples(ReaSample** samples, int len, int nch, int offs,
                              int spread) = 0; // in case a sink wants to build its own peaks (make
                                               // sure it was created with src=NULL)
  virtual int Run() = 0; // or let it do it automatically (created with source!=NULL)

  virtual int GetLastSecondPeaks(
      int sz, ReaSample* buf) = 0; // returns number of peaks in the last second, sz is maxsize
  virtual void GetPeakInfo(PCM_source_peaktransfer_t*
...
#define REAPER_PEAKRES_MUL_MAX                                                                     \
  1.0 // recommended for plug-ins, when 1.5pix/peak, switch to hi res source. this may be
      // configurable someday via some sneakiness

#endif // __cplusplus

/*
** accelerator_register_t allows you to register ("accelerator") a record which lets you get a place
*in the
...
typedef struct accelerator_register_t {
  ...
  // -20 (Windows only) to passed to the window, even if it is WM_SYSKEY*/VK_MENU which would
  // otherwise be dropped -666 to force it to the main window's accel table (with the exception of
  // ESC) -667 to force it to the main window's accel table, even if in a text field (5.24+ or so)
...
/*
** custom_action_register_t allows you to register ("custom_action") an action or a reascript into a
*section of the action list
** register("custom_action",ca) will return the command ID (instance-dependent but unique across all
*sections),
...
typedef struct _REAPER_custom_action_register_t {
  int uniqueSectionId; // 0/100=main/main alt, 32063=media explorer, 32060=midi editor, 32061=midi
                       // event list editor, 32062=midi inline editor, etc
  const char* idStr;   // must be unique across all sections for actions, NULL for reascripts
...
/*
** gaccel_register_t allows you to register ("gaccel") an action into the main keyboard section
*action list, and at the same time
** a default binding for it (accel.cmd is the command ID, desc is the description, and accel's other
*parameters are the
** key to bind.
** 7.07+ - can use "gaccel_global" or "gaccel_globaltext" to make the accel registered as a global
*by default
*/
...
**
** Implemented as a your-format->RPP converter, allowing you to generate directly to a
*ProjectStateContext
*/
typedef struct _REAPER_project_import_register_t // register with "projectimport"
{
  ...
  const char* (*EnumFileExtensions)(
      int i, char** descptr); // call increasing i until returns NULL. if descptr's output is NULL,
                              // use last description
  int (*LoadProject)(const char* fn,
                     ProjectStateContext*
                         genstate); // return 0=ok, Generate RPP compatible project info in genstate
...
typedef struct project_config_extension_t // register with "projectconfig"
{
  ...
  bool (*ProcessExtensionLine)(const char* line, ProjectStateContext* ctx, bool isUndo,
  ...
  // optional: called on project load/undo before any (possible) ProcessExtensionLine. NULL is OK
  // too also called on "new project" (wont be followed by ProcessExtensionLine calls in that case)
...
**
** Plug-ins may register keyboard action sections in by registering a "accel_section" to a
*KbdSectionInfo*.
*/
...
typedef struct _REAPER_preview_register_t {
  /*
  ** Note: you must initialize/deinitialize the cs/mutex (depending on OS) manually, and use it if
  *accessing most parameters while the preview is active.
  */
...
/*
** preview_register_t is not used with the normal register system, instead it's used with
*PlayPreview(), StopPreview(), PlayTrackPreview(), StopTrackPreview()
*/

#ifdef REAPER_WANT_DEPRECATED_COLORTHEMESTUFF /* no longer used -- see icontheme.h and             \
                                                 GetColorThemeStruct() */
...
/*
  ...
  program runs (stored in project etc). lParam is instance-specific parameter (i.e. "this" pointer
  etc).
*/
enum {
  SCREENSET_ACTION_GETHWND = 0, // returns HWND of screenset window. If searching for a specific
                                // window, it will be passed in actionParm
  ...
  SCREENSET_ACTION_SAVE_STATE, // save state to actionParm, max length actionParmSize (will usually
                               // be max(4096, value_returned by SCREENSET_ACTION_WANT_STATE_SIZE)),
                               // return length actually used
  SCREENSET_ACTION_WANT_STATE_SIZE, // returns desired size for SCREENSET_ACTION_SAVE_STATE, may or
                                    // may not be fulfilled!
};
typedef LRESULT (*screensetNewCallbackFunc)(int action, const char* id, void* param,
...
public:
  ...
  virtual void SendMsg(MIDI_event_t* msg,
                       int frame_offset) = 0; // frame_offset can be <0 for "instant" if supported
...
public:
  ...
  virtual void SwapBufs(unsigned int timestamp) = 0; // DEPRECATED call SwapBufsPrecise() instead //
                                                     // timestamp=process ms

  virtual void RunPreNoteTracking(int isAccum) {}

  virtual MIDI_eventlist* GetReadBuf() = 0; // note: the event list here has frame offsets that are
                                            // in units of 1/1024000 of a second, NOT sample frames

  virtual void
  SwapBufsPrecise(unsigned int coarsetimestamp,
                  double precisetimestamp) // coarse=process ms, precise=process sec, the target
                                           // will know internally which to use
  {
...
public:
  ...
  virtual const char*
  GetTypeString() = 0; // simple unique string with only A-Z, 0-9, no spaces or other chars
  virtual const char*
  GetDescString() = 0; // human readable description (can include instance specific info)
...
#define CSURF_EXT_RESET                                                                            \
  0x0001FFFF // clear all surface state and reset (harder reset than SetTrackListChange)
...
#define CSURF_EXT_SETRECMODE                                                                       \
  0x00010004 // parm1=(int*)record mode: 0=autosplit and create takes, 1=replace (tape) mode
#define CSURF_EXT_SETSENDVOLUME                                                                    \
  0x00010005 // parm1=(MediaTrack*)track, parm2=(int*)sendidx, parm3=(double*)volume
#define CSURF_EXT_SETSENDPAN                                                                       \
  0x00010006 // parm1=(MediaTrack*)track, parm2=(int*)sendidx, parm3=(double*)pan
#define CSURF_EXT_SETFXENABLED                                                                     \
  0x00010007 // parm1=(MediaTrack*)track, parm2=(int*)fxidx, parm3=0 if bypassed, !0 if enabled
...
#define CSURF_EXT_SETBPMANDPLAYRATE                                                                \
  0x00010009 // parm1=*(double*)bpm (may be NULL), parm2=*(double*)playrate (may be NULL)
#define CSURF_EXT_SETLASTTOUCHEDFX                                                                 \
  0x0001000A // parm1=(MediaTrack*)track, parm2=(int*)mediaitemidx (may be NULL), parm3=(int*)fxidx.
             // all parms NULL=clear last touched FX
#define CSURF_EXT_SETFOCUSEDFX                                                                     \
  0x0001000B // parm1=(MediaTrack*)track, parm2=(int*)mediaitemidx (may be NULL), parm3=(int*)fxidx.
             // all parms NULL=clear focused FX
...
#define CSURF_EXT_SETPAN_EX                                                                        \
  0x0001000E // parm1=(MediaTrack*)track, parm2=(double*)pan, parm3=(int*)mode 0=v1-3 balance, 3=v4+
             // balance, 5=stereo pan, 6=dual pan. for modes 5 and 6, (double*)pan points to an
             // array of two doubles.  if a csurf supports CSURF_EXT_SETPAN_EX, it should ignore
             // CSurf_SetSurfacePan.
#define CSURF_EXT_SETRECVVOLUME                                                                    \
  0x00010010 // parm1=(MediaTrack*)track, parm2=(int*)recvidx, parm3=(double*)volume
#define CSURF_EXT_SETRECVPAN                                                                       \
  0x00010011 // parm1=(MediaTrack*)track, parm2=(int*)recvidx, parm3=(double*)pan
#define CSURF_EXT_SETFXOPEN                                                                        \
  0x00010012 // parm1=(MediaTrack*)track, parm2=(int*)fxidx, parm3=0 if UI closed, !0 if open
#define CSURF_EXT_SETFXCHANGE                                                                      \
  0x00010013 // parm1=(MediaTrack*)track, whenever FX are added, deleted, or change order.
             // flags=(INT_PTR)parm2, &1=rec fx
...
#define CSURF_EXT_SETTRACKMARKERLANEVIS                                                            \
  0x00010016 // parm1=(MediaTrack*)track, parm2=(int*)lane, parm3=(int*)visible
#define CSURF_EXT_SETTRACKMARKERLANECOLOR                                                          \
  0x00010017 // parm1=(MediaTrack*)track, parm2=(int*)lane, parm3=(int*)color
...
#define UNDO_STATE_FREEZE                                                                          \
  16 // freeze state -- note that isfreeze is used independently, this is only used for the undo
     // system to serialize the already frozen state
...
#define UNDO_STATE_POOLEDENVS                                                                      \
  128 // contents of pooled envs -- not position, length, rate etc of pooled env instances, which is
      // part of envelope state
#endif
