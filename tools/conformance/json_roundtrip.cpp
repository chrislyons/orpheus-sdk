...
TEST(JsonConformance, RoundTripFixtures) {
  const char* external_command = std::getenv("ORPHEUS_JSON_ROUNDTRIP_COMMAND");
  ...
  for (const auto& fixture : fixtures) {
    ...
    ASSERT_TRUE(SessionsEqual(session, reparsed)) << "Fixture round trip mismatch: " << fixture;
    EXPECT_EQ(serialized, original) << "Fixture serialization drifted: " << fixture;
  }
}

TEST(JsonConformance, DeterministicClickFilename) {
  const SessionGraph session = LoadSessionFromFile((FixturesRoot() / "solo_click.json").string());
  const std::string filename = MakeRenderClickFilename(session.name(), "Click", 44100, 16);
  EXPECT_EQ(filename, "out/solo_click_click_44p1k_16b.wav");

  const SessionGraph loop = LoadSessionFromFile((FixturesRoot() / "loop_grid.json").string());
  const std::string loop_filename = MakeRenderClickFilename(loop.name(), "Click", 48000, 16);
...
TEST(JsonConformance, AudioFixturesAreDecodedFromText) {
  ...
  for (const auto& entry : fs::directory_iterator(audio_root)) {
    ...
    ASSERT_GE(bytes.size(), 12u) << "Decoded WAV too small: " << entry.path().string();
