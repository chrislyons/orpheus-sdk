...
struct ChannelConfig {
    ...
    /// Default constructor
    ChannelConfig()
        : name(""), group_index(0), gain_db(0.0f), pan(0.0f),
          mute(false), solo(false), color(0xFFFFFFFF) {}
};
...
struct GroupConfig {
    ...
    /// Default constructor
    GroupConfig()
        : name(""), gain_db(0.0f), mute(false), solo(false),
          output_bus(0), color(0xFFFFFFFF) {}
};
...
struct RoutingConfig {
    ...
    RoutingConfig()
        : num_channels(16), num_groups(4), num_outputs(2),
          solo_mode(SoloMode::SIP), metering_mode(MeteringMode::Peak),
          gain_smoothing_ms(10.0f), dim_amount_db(-12.0f),
...
struct AudioMeter {
    ...
    AudioMeter()
        : peak_db(-100.0f), rms_db(-100.0f), clipping(false), clip_count(0) {}
};
...
public:
    ...
    /// @return Error code
    virtual SessionGraphError configureChannel(uint8_t channel_index, const ChannelConfig& config) = 0;
    ...
    virtual SessionGraphError processRouting(
