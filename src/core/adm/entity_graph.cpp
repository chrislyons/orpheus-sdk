...
bool IsRedundantPoint(const ObjectPoint &previous, const ObjectPoint &current,
  ...
  if (current.time_seconds <= previous.time_seconds ||
      next.time_seconds <= current.time_seconds) {
    return false;
  }
...
void Object::add_point(ObjectPoint point) { points_.push_back(point); }
...
void Content::attach_object(std::size_t object_index) {
  if (std::find(objects_.begin(), objects_.end(), object_index) ==
      objects_.end()) {
    objects_.push_back(object_index);
  }
}

const std::vector<std::size_t> &Content::beds() const { return beds_; }

const std::vector<std::size_t> &Content::objects() const { return objects_; }
...
void Programme::attach_content(std::size_t content_index) {
  if (std::find(contents_.begin(), contents_.end(), content_index) ==
      contents_.end()) {
...
std::size_t EntityGraph::content_count() const { return contents_.size(); }

std::size_t EntityGraph::bed_count() const { return beds_.size(); }

std::size_t EntityGraph::object_count() const { return objects_.size(); }
...
std::vector<ObjectPoint> ThinTrajectory(
