...
SessionGraphError RoutingMatrix::configureChannel(uint8_t channel_index, const ChannelConfig& config) {
    if (!m_initialized.load(std::memory_order_acquire)) {
        return SessionGraphError::NotInitialized;
    }
...
SessionGraphError RoutingMatrix::processRouting(
    ...
    for (uint8_t ch = 0; ch < m_config.num_channels; ++ch) {
        ...
        if (m_config.enable_metering) {
            processMetering(group_buffer, num_frames,
    ...
    for (uint8_t grp = 0; grp < m_config.num_groups; ++grp) {
        ...
        if (m_config.enable_metering) {
            processMetering(group_buffer, num_frames,
                           group.peak_level, group.rms_level);
            if (detectClipping(group_buffer, num_frames)) {
    ...
    if (m_config.enable_metering) {
        ...
        processMetering(master_output[0], num_frames,
                       m_master_peak, m_master_rms);
...
void RoutingMatrix::processMetering(float* buffer, size_t num_frames,
