...
void orpheus_set_logger(orpheus_log_callback callback, void* user_data) {
  ...
  state.user_data.store(callback ? user_data : nullptr, std::memory_order_relaxed);
  state.callback.store(callback, std::memory_order_release);
}

void orpheus_set_telemetry_callback(orpheus_telemetry_callback callback, void* user_data) {
...
void EmitTelemetry(std::string_view event_name, std::string_view json_payload) {
  auto& state = TelemetryState();
  const auto callback = state.callback.load(std::memory_order_acquire);
  if (callback == nullptr) {
    return;
