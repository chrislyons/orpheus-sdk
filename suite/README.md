...
## 2026-09-10 dependency and temporal-coherence handoff

`ORP-SUITE-20260910-001` selects the published SDK source baseline
`ca949b2e0af63346d92bb6f2a51ac3c746c2745b` with governed ShmUI source
`f41fbd3fe4326a37d025a6c97a74c84cfa6ea2b6`, token contract `0.6.0`.
The SDK import was already current. The manifest now matches the actual
`boot-industries` remotes, package content hash and consumer dependency intent.
Historical snapshots and candidate/stable channel pointers are unchanged.

- FourTrack's shared four/eight-track lineage adopts the SDK pin and exact
  Swift provenance without changing generated bytes. Provenance, macOS app/core
  build and 333/333 CTest cases passed; four- and eight-track CLI record/bounce
  smoke passed with the mock driver.
- FreqFinder's existing explicit sibling-source resolution already consumed
  the selected runtime sources. Release built Standalone/AU/VST3 and passed
  CTest 1/1; Debug CTest passed 1/1. Debug VST3's manifest helper fails because
  AddressSanitizer is loaded too late; this is not a passing Debug-format gate.
- Clip Composer's update is on an isolated `suite/orp-suite-20260910-001`
  worktree under `.orpheus-suite-worktrees/ORP-SUITE-20260910-001/clip-composer`.
  Its original checkout has staged/unstaged meter work and retains its prior
  pin. Do not overwrite it or claim the isolated handoff is integrated.
  The isolated full application/test build passed; CTest recorded 747 passed,
  zero failed and one intentional CPU-performance skip in 335.56 s.
- SDK package/realtime/docs/manifest gates passed 5/5; suite quick checks passed
  5/5. ShmUI mirror, generated token/Swift consumer and registry closure passed.

All five repositories adopt the temporal-coherence release requirement in their
active guidance. The canonical contract is [Temporal Coherence](../ARCHITECTURE.md#temporal-coherence):
phase/sample integrity, responsive coherent presentation and demand-driven CPU
use are joint gates. This is policy adoption and reconnaissance, not a completed
timing implementation or measured screen/thermal acceptance.

No pushes or merges are included. Local policy/consumer branches are not yet
reachable from remote `main`, and default `status` still compares against the
historical development snapshot. That status is not rewritten to manufacture a
green release claim. Finish publication and the original Clip Composer
integration before capturing a new clean reachable development snapshot.
