...
void ClipButton::paint(juce::Graphics& g)
{
    ...
    if (m_state == State::Empty)
    {
        ...
        g.drawText(juce::String(m_buttonIndex + 1),
                   bounds,
...
void ClipButton::drawClipHUD(juce::Graphics& g, juce::Rectangle<float> bounds)
{
    ...
    // === MIDDLE: Clip Name (primary focus) ===
    {
        ...
        auto nameArea = juce::Rectangle<float>(
            contentArea.getX(),
            currentY,
    ...
    {
        auto bottomArea = juce::Rectangle<float>(
            contentArea.getX(),
            contentArea.getBottom() - 28.0f,
            contentArea.getWidth(),
            28.0f
        );
        ...
        if (m_beatOffset.isNotEmpty())
        {
            ...
            g.drawText("//" + m_beatOffset,
                       bottomArea.withSizeKeepingCentre(60.0f, 14.0f),
                       juce::Justification::centred,
                       false);
        }
        ...
        // Clip group indicator (right) - e.g., "G1", "G2"
        {
            ...
            g.drawText("G" + juce::String(m_clipGroup + 1),
                       groupBadge,
                       juce::Justification::centred,
                       false);
        }
    ...
    if (m_loopEnabled || m_fadeInEnabled || m_fadeOutEnabled || m_effectsEnabled)
    {
        auto iconArea = juce::Rectangle<float>(
            contentArea.getX(),
            contentArea.getCentreY() - 8.0f,
            contentArea.getWidth() * 0.3f,
            16.0f
        );
...
void ClipButton::drawStatusIcons(juce::Graphics& g, juce::Rectangle<float> bounds)
{
    ...
    if (m_loopEnabled)
    {
        ...
        juce::Path arrowTip;
        arrowTip.addTriangle(
            iconBounds.getRight() - 3.0f, iconBounds.getY() + 2.0f,
