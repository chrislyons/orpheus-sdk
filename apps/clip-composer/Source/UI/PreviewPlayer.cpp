...
void PreviewPlayer::jumpTo(int64_t samplePosition) {
  ...
  if (wasPlaying) {
    ...
    if (seeked) {
      m_lastPlayingState = true;
      startPositionTimer(); // Start polling position for playhead updates
      DBG("PreviewPlayer: Jogged to sample "
          << samplePosition << " (" << (useAuditionBuss ? "audition cue buss" : "main grid clip")
    ...
    if (started) {
      ...
      if (seeked) {
        DBG("PreviewPlayer: Started and seeked to sample "
            << samplePosition << " (" << (useAuditionBuss ? "audition cue buss" : "main grid clip")
            << ", click-to-start)");
      } else {
      ...
      DBG("PreviewPlayer: Failed to start clip from sample "
          << samplePosition << " (" << (useAuditionBuss ? "audition cue buss" : "main grid clip")
          << ")");
    }
  }
