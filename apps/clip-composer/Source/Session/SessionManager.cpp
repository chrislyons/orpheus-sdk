...
juce::String makeIsoUtcTimestamp() {
  ...
  auto utcMillis = now.toMilliseconds() - (static_cast<juce::int64>(now.getUTCOffsetSeconds()) * 1000);
...
bool SessionManager::relinkMissingMedia(int tabIndex, int buttonIndex,
  ...
  if (tabIndex < 0 || tabIndex >= NUM_TABS || buttonIndex < 0 ||
      buttonIndex >= BUTTONS_PER_TAB || !replacementFile.existsAsFile()) {
    return false;
  }
...
bool SessionManager::relinkMissingMediaByGlobalIndex(
    int globalClipIndex, const juce::File& replacementFile,
    MissingMediaResolution* outResolution) {
  if (globalClipIndex < 0) {
    return false;
  }
...
bool SessionManager::relinkMissingMediaFromSearchRoot(
    int tabIndex, int buttonIndex, const juce::File& searchRoot,
    MissingMediaResolution* outResolution) {
  auto clip = getClip(buttonIndex, tabIndex);
  if (!clip.isValid()) {
    return false;
...
void SessionManager::recordMissingMedia(int tabIndex, int buttonIndex,
                                        ...
                                        bool matchedByFilename, bool matchedByMetadata) {
  MissingMediaResolution resolution;
  resolution.tabIndex = tabIndex;
  resolution.buttonIndex = buttonIndex;
  ...
  auto existing = std::find_if(m_missingMediaResolutions.begin(), m_missingMediaResolutions.end(),
                               [tabIndex, buttonIndex](const MissingMediaResolution& entry) {
                                 return entry.tabIndex == tabIndex &&
...
void SessionManager::serializeClipArray(juce::DynamicObject& sessionObj,
  ...
  for (const auto& [compositeKey, clipData] : m_clips) {
    ...
    if (packageMode && clipData.mediaAvailable && sourceFile.existsAsFile()) {
      ...
      if (cacheIt != copiedMediaCache.end()) {
        ...
        while (destination.exists()) {
          destination = packageMediaDir.getChildFile(
              sourceFile.getFileNameWithoutExtension() + "_" + juce::String(suffix) +
    ...
    const bool shouldCountAsMissing = clipData.filePath.empty() || !clipData.mediaAvailable ||
                                      (packageMode && clipData.mediaAvailable &&
                                       sourceFile.existsAsFile() && !copiedMedia);
    ...
    clipObj->setProperty("trimInSamples", juce::var(static_cast<juce::int64>(clipData.trimInSamples)));
...
bool SessionManager::applySessionMetadata(const juce::DynamicObject& sessionObj,
  ...
  if (auto* lineageObj = lineageValue.getDynamicObject()) {
    ...
    m_sessionLineage.createdAtUtc = lineageObj->getProperty("createdAtUtc").toString().toStdString();
    m_sessionLineage.updatedAtUtc = lineageObj->getProperty("updatedAtUtc").toString().toStdString();
  }
  ...
  if (auto* packageObj = packageValue.getDynamicObject()) {
    ...
    m_lastPackageManifest.packagePath = packageObj->getProperty("packagePath").toString().toStdString();
    ...
    m_lastPackageManifest.packagePath = sourceFile.getParentDirectory().getFullPathName().toStdString();
...
bool SessionManager::exportSessionPackage(const juce::File& packageDirectory,
  ...
  serializeSessionMetadata(*sessionObj);
  const juce::File sessionBaseDir =
      m_currentFile.existsAsFile() ? m_currentFile.getParentDirectory()
...
bool SessionManager::importSessionPackage(const juce::File& packageDirectory) {
  ...
  if (manifestFile.existsAsFile()) {
    ...
    if (auto* manifestObj = manifestJson.getDynamicObject()) {
      ...
      m_lastPackageManifest.copiedMedia = static_cast<bool>(manifestObj->getProperty("copiedMedia"));
    }
  }
