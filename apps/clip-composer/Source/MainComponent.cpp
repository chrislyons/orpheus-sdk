...
MainComponent::MainComponent()
{
    ...
    m_clipGrid->onButtonRightClicked = [this](int buttonIndex) {
        onClipRightClicked(buttonIndex);
    };

    // Wire up left-click handler for triggering clips
    m_clipGrid->onButtonClicked = [this](int buttonIndex) {
...
void MainComponent::paint(juce::Graphics& g)
{
    ...
    g.drawText("48 Buttons | Week 3 | SDK Integration Pending",
               getWidth() - 350, 0, 330, 60,
...
int MainComponent::getButtonIndexFromKey(const juce::KeyPress& key) const
{
    ...
    if (keyCode == 'W') return 1;
    if (keyCode == 'E') return 2;
    if (keyCode == 'R') return 3;
    if (keyCode == 'T') return 4;
    if (keyCode == 'Y') return 5;

    // Row 1: A S D F G H
    if (keyCode == 'A') return 6;
    if (keyCode == 'S') return 7;
    if (keyCode == 'D') return 8;
    if (keyCode == 'F') return 9;
    if (keyCode == 'G') return 10;
    if (keyCode == 'H') return 11;

    // Row 2: Z X C V B N
    if (keyCode == 'Z') return 12;
    if (keyCode == 'X') return 13;
    if (keyCode == 'C') return 14;
    if (keyCode == 'V') return 15;
    if (keyCode == 'B') return 16;
    if (keyCode == 'N') return 17;

    // Row 3: 1 2 3 4 5 6
    if (keyCode == '1') return 18;
    if (keyCode == '2') return 19;
    if (keyCode == '3') return 20;
    if (keyCode == '4') return 21;
    if (keyCode == '5') return 22;
    if (keyCode == '6') return 23;

    // Row 4: 7 8 9 0 - =
    if (keyCode == '7') return 24;
    if (keyCode == '8') return 25;
    if (keyCode == '9') return 26;
    if (keyCode == '0') return 27;
    if (keyCode == '-') return 28;
    if (keyCode == '=') return 29;

    // Row 5: [ ] ; ' , .
    if (keyCode == '[') return 30;
    if (keyCode == ']') return 31;
    if (keyCode == ';') return 32;
    if (keyCode == '\'') return 33;
    if (keyCode == ',') return 34;
    if (keyCode == '.') return 35;

    // Row 6: F1 F2 F3 F4 F5 F6
    if (keyCode == juce::KeyPress::F1Key) return 36;
    if (keyCode == juce::KeyPress::F2Key) return 37;
    if (keyCode == juce::KeyPress::F3Key) return 38;
    if (keyCode == juce::KeyPress::F4Key) return 39;
    if (keyCode == juce::KeyPress::F5Key) return 40;
    if (keyCode == juce::KeyPress::F6Key) return 41;

    // Row 7: F7 F8 F9 F10 F11 F12
    if (keyCode == juce::KeyPress::F7Key) return 42;
    if (keyCode == juce::KeyPress::F8Key) return 43;
    if (keyCode == juce::KeyPress::F9Key) return 44;
    if (keyCode == juce::KeyPress::F10Key) return 45;
    if (keyCode == juce::KeyPress::F11Key) return 46;
    if (keyCode == juce::KeyPress::F12Key) return 47;
...
void MainComponent::updateButtonFromClip(int buttonIndex)
{
    ...
    if (m_sessionManager.hasClip(buttonIndex))
    {
        ...
        DBG("MainComponent: Updated button " << buttonIndex
            << " with clip: " << clipData.displayName
    ...
    switch (menuItemID)
    {
        ...
        case 2:  // Open Session
        {
            juce::FileChooser chooser("Open Session",
                                     juce::File::getSpecialLocation(juce::File::userDocumentsDirectory).getChildFile("Orpheus Clip Composer/Sessions"),
                                     "*.json");
