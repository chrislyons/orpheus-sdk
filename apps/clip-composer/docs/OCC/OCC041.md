...
sn_filename: "OCC041-781f6e06.txt"
prefix: occ
...
1. `**PreviewPlayer::setTrimPoints()**` (lines 77-101):
...
1. **Negative sample values** suggest arithmetic underflow when adjusting IN point near zero.
...
1. **Button onClick handlers may not be firing**
...
1. **Cue Buss handle may be invalid**
...
1. Add DBG logs to button callbacks to verify they fire
...
1. **ClipEditDialog reads loop state from metadata:**
...
1. **ClipEditDialog updates metadata when loop toggled:**
...
1. **MainComponent syncs loop state to AudioEngine on OK:**
...
1. **include/orpheus/transport_controller.h** - Add loop fields to ClipState
2. **src/core/transport/transport_controller.cpp** - Implement loop restart logic
...
1. **apps/clip-composer/Source/MainComponent.cpp** - Sync loop state on dialog OK
2. **apps/clip-composer/Source/ClipGrid/ClipButton.h** - Expose setLoop() method (if needed)
3. **apps/clip-composer/Source/ClipGrid/ClipButton.cpp** - Implement setLoop() method (if needed)
