...
sn_filename: "OCC036-3bd66d30.txt"
prefix: occ
...
1. **Right-Click Parameter Access:**
...
- Left-click = Set IN (already works)
- `Cmd/Ctrl+Click` = Set IN (add modifier check)
- Right-click = Set OUT (already works)
...
1. **Should Fade Out default to 0s or 0.1s?**
...
1. **Should we enable MicroFades by default?**
...
1. **Should we show sample numbers or time in trim editors?**
...
1. **Should we implement Floating Controls in v0.3.0 or defer?**
...
1. **OCC021 - Product Vision** (Orpheus Clip Composer)

- `apps/clip-composer/docs/OCC/OCC021 Orpheus Clip Composer - Product Vision.md`



1. **OCC023 - Component Architecture** (5-layer architecture)

- `apps/clip-composer/docs/OCC/OCC023 Component Architecture v1.0.md`



1. **OCC024 - User Interaction Flows** (8 complete workflows)

- `apps/clip-composer/docs/OCC/OCC024 User Interaction Flows v1.0.md`



1. **OCC034 - Edit Dialog v0.2.0-alpha** (Current implementation report)
