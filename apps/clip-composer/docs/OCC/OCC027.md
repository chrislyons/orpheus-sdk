...
sn_filename: "OCC027-6b6d91b5.txt"
prefix: occ
...
### 1.1 Core Interfaces (MVP Required)
 ...
 Manage clips, tracks, tempo 

 Add/remove clips, retrieve clip metadata 

 `ITransportController` 

 Playback control, timing 

 Start/stop clips, query playback position 

 `IRoutingMatrix` 

 Multi-channel routing 

 Configure group outputs, audition routing 

 `IAudioFileReader` 

 Decode audio files 

 Load WAV/AIFF/FLAC, stream samples 

 `IPerformanceMonitor` 

 Real-time diagnostics 

 Query CPU usage, latency, dropouts 
...
### 1.2 Data Structures (Shared)
 ...
 Opaque reference to a clip in SessionGraph 
 ...
 Input or output channel identifier 

 `PerformanceMetrics` 

 CPU, latency, dropout statistics 
...
### 8.2 Error Categories
 ...
 Show error dialog, offer file location 

 `UnsupportedFormat` 

 Error 

 Show error, suggest conversion 

 `OutOfMemory` 

 Critical 

 Show error, suggest closing clips 

 `InvalidParameter` 

 Bug 

 Assert/log, report bug 
...
### Appendix A: Complete Interface Summary
 ...
 UI thread for commands, callbacks on UI thread 
 ...
 UI thread for writes, const methods thread-safe 
